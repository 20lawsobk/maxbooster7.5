...
class VideoGenerationPipeline:
    def __init__(self, cfg: dict):
        ...
        # Base DiT
        self.dit = VideoDiT(cfg["dit"]).to(self.device).eval()
        self._load(self.dit, cfg.get("dit_ckpt"))
        ...
        if cfg.get("sr") and cfg.get("sr_ckpt"):
            self.sr_unet = SRUNet3D(cfg["sr"]).to(self.device).eval()
            self._load(self.sr_unet, cfg["sr_ckpt"])
