...
export default function PlaylistPitching() {
  ...
  const { data: curators = [], isLoading: curatorsLoading } = useQuery<Curator[]>({
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: string }) => {
      const response = await apiRequest('PUT', `/api/playlist-pitching/${id}`, { status });
      return response.json();
    },
    onSuccess: () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'draft':
        return <Badge variant="outline" className="bg-slate-500/10 text-slate-400 border-slate-500/20">Draft</Badge>;
      case 'submitted':
        return <Badge variant="outline" className="bg-blue-500/10 text-blue-400 border-blue-500/20">Submitted</Badge>;
      case 'under_review':
        return <Badge variant="outline" className="bg-yellow-500/10 text-yellow-400 border-yellow-500/20">Under Review</Badge>;
      case 'accepted':
        return <Badge variant="outline" className="bg-green-500/10 text-green-400 border-green-500/20">Accepted</Badge>;
      case 'rejected':
        return <Badge variant="outline" className="bg-red-500/10 text-red-400 border-red-500/20">Rejected</Badge>;
      default:
  ...
  const filteredCurators = curators.filter(c => 
    ...
    (filterGenre === 'all' || c.genre.toLowerCase().includes(filterGenre.toLowerCase()))
  );
    ...
    <AppLayout>
      <div className="p-6 max-w-7xl mx-auto space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-white tracking-tight">Playlist Pitching</h1>
            <p className="text-gray-400">Track your submissions and find the right curators for your music.</p>
          </div>
            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-500">{stats?.accepted || 0}</p>
                </div>
            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-purple-500">{(stats?.conversionRate || 0).toFixed(1)}%</p>
                </div>
                      ...
                      <tr key={pitch.id} className="hover:bg-gray-800/30 transition-colors">
                        <td className="p-4">
                          <div className="flex flex-col">
                            <span className="font-medium text-white">{pitch.trackTitle}</span>
                            <span className="text-xs text-gray-500">{pitch.artistName}</span>
                          </div>
                        </td>
                        <td className="p-4">{pitch.curatorName || 'Unknown'}</td>
                ...
                <Card key={curator.id} className="bg-gray-900 border-gray-800 hover:border-purple-500/50 transition-all group">
                  <CardHeader className="pb-2">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-sm text-gray-500 mt-1">{curator.genre}</p>
                      </div>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="flex gap-2">
                      <Button asChild variant="outline" className="flex-1 bg-gray-800 border-gray-700 h-9 text-xs">
                        <a href={curator.submissionUrl} target="_blank" rel="noopener noreferrer">
            ...
            <div className="grid grid-cols-2 gap-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <Input 
                  ...
                  onChange={(e) => setNewPitchForm({ ...newPitchForm, artistName: e.target.value })}
                />
              </div>
              <div className="space-y-2">
                ...
                <Input 
                  ...
                  onChange={(e) => setNewPitchForm({ ...newPitchForm, genre: e.target.value })}
                />
              </div>
              <div className="space-y-2">
                ...
                <Select 
                  ...
                  onValueChange={(val) => setNewPitchForm({ ...newPitchForm, status: val })}
                >
              ...
              <div className="space-y-2 col-span-2">
                ...
                <Textarea 
                  ...
                  onChange={(e) => setNewPitchForm({ ...newPitchForm, description: e.target.value })}
                />
              </div>
            </div>

            <DialogFooter>
              <Button variant="outline" onClick={() => setIsNewPitchOpen(false)} className="bg-gray-800 border-gray-700">
                Cancel
              </Button>
              <Button 
