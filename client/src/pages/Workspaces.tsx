...
import { Building2, Plus, Users, Settings, Crown, Mail, UserPlus, LogOut, Music, Briefcase, Home, Shield, Activity, Share2, Eye } from 'lucide-react';
import { 
...
export default function Workspaces() {
  ...
  const { data: activitiesData, refetch: refetchActivities } = useQuery<{ activities: ActivityItem[] }>({
  ...
  const createWorkspaceMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      workspaceOutcome.workspaceCreated(newWorkspace.name, data.workspace?.id || '');
    },
    onError: (error: Error) => {
  ...
  const updateMemberRoleMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: [`/api/workspace/${selectedWorkspace?.id}/members`] });
  ...
  const createRoleMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: [`/api/workspace/${selectedWorkspace?.id}/roles`] });
  ...
  const exportAuditMutation = useMutation({
    mutationFn: async () => {
      if (!selectedWorkspace) throw new Error('No workspace selected');
      const res = await fetch(`/api/workspace/${selectedWorkspace.id}/audit/export?format=json`, {
        credentials: 'include',
      });
...
const workspaces = workspacesData?.workspaces || [];
  ...
  const formattedMembers: WorkspaceMemberDetails[] = members.map(m => ({
    id: m.id,
    userId: m.userId,
    name: `${m.userFirstName || ''} ${m.userLastName || ''}`.trim() || m.userEmail,
    email: m.userEmail,
  ...
  const canManageMembers = selectedWorkspace?.role === 'owner' || selectedWorkspace?.role === 'admin';

  return (
    <AppLayout>
      <WorkspaceOutcomeHandler outcome={workspaceOutcome.outcome} onDismiss={workspaceOutcome.clearOutcome} />
      ...
      <div className="space-y-6">
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-xs text-muted-foreground">{workspace.memberCount} members</p>
                            </div>
                ...
                <Card>
                  <CardHeader className="border-b">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <PresenceAvatars collaborators={collaborators.filter(c => c.status !== 'offline')} maxVisible={3} size="sm" />
                        {canManageMembers && (
                          <>
                    ...
                    <Tabs value={activeTab} onValueChange={setActiveTab}>
                      <div className="border-b px-4">
                        <TabsList className="bg-transparent h-12">
                          ...
                          <TabsTrigger value="members" className="data-[state=active]:bg-transparent">
                            <Users className="h-4 w-4 mr-2" />
                            Members
                          </TabsTrigger>
                          {canManageMembers && (
                            <TabsTrigger value="roles" className="data-[state=active]:bg-transparent">
                            ...
                            <Card>
                              <CardContent className="pt-6">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    <p className="text-2xl font-bold">{formattedMembers.length}</p>
                            ...
                            <Card>
                              <CardContent className="pt-6">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    <p className="text-2xl font-bold">{formattedRoles.length}</p>
                            ...
                            <Card>
                              ...
                              <CardContent>
                                <div className="space-y-3">
                                  {formattedMembers.slice(0, 5).map(member => (
                            ...
                            {formattedMembers.length === 0 ? (
                              ...
                              formattedMembers.map(member => (
                                <MemberManagementCard
                                  ...
                                  isLoading={updateMemberRoleMutation.isPending || removeMemberMutation.isPending}
                                />
                              ))
                            )}
                          </div>
                        ...
                        {canManageMembers && (
                          <TabsContent value="roles" className="m-0">
                            <RolePermissionMatrix
                              ...
                              onCreateRole={(role) => createRoleMutation.mutate(role)}
                              onUpdateRole={(roleId, updates) => updateRoleMutation.mutate({ roleId, updates })}
                              onDeleteRole={(roleId) => deleteRoleMutation.mutate(roleId)}
                              isLoading={createRoleMutation.isPending || updateRoleMutation.isPending || deleteRoleMutation.isPending}
              ...
              <div className="space-y-2">
                ...
                <Select value={inviteRole} onValueChange={setInviteRole}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="manager">Manager - Manage releases</SelectItem>
        ...
        {selectedWorkspace && (
          <SharingDialog
            ...
            currentMembers={[]}
            currentLinks={[]}
            workspaceMembers={formattedMembers.map(m => ({ id: m.userId, name: m.name, email: m.email, avatar: m.avatar }))}
            ...
            onCreateLink={async (settings) => {
              const link = {
                ...
                expiresAt: settings.expirationDays ? new Date(Date.now() + settings.expirationDays * 86400000).toISOString() : undefined,
                password: !!settings.password,
                accessCount: 0,
                createdAt: new Date().toISOString(),
              };
              workspaceOutcome.externalLinkGenerated(link.url, link.expiresAt);
              return link;
            }}
            onRevokeLink={async () => {
