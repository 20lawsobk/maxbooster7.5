...
import {
  Loader2, Zap, Play, ChevronDown, ChevronUp, Clock, CheckCircle2, XCircle,
  AlertCircle, Settings2, Activity, TrendingUp, Calendar, BarChart3,
...
const PHASES = [
  ...
  },
  {
    key: 'release-day',
    label: 'Release Day',
    description: 'Make release day effortless — coordinated social blasts, newsletter, and fan notifications.',
  ...
  },
  {
    ...
    description: 'Protect your earnings — sales confirmations, royalty audits, sync pitches, and venue follow-ups.',
        ...
        <div key={key} className="space-y-1">
          {field.type === 'boolean' ? (
            ...
            <div className="space-y-1">
              ...
              <Select value={String(values[key] ?? field.default)} onValueChange={(v) => onChange(key, v)}>
...
function AutomationCard({ automation }: { automation: WorkflowTemplate }) {
  ...
  const toggleMutation = useMutation({
    ...
    onSuccess: (_data, enable) => {
      ...
      qc.invalidateQueries({ queryKey: ['/api/music-workflow-automations/stats'] });
      toast({
  ...
  const saveMutation = useMutation({
    mutationFn: async (config: Record<string, any>) => {
      const res = await apiRequest('PUT', `/api/music-workflow-automations/${automation.id}/config`, { config });
      return res.json();
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['/api/music-workflow-automations'] });
  ...
  const testMutation = useMutation({
    mutationFn: async () => {
      const res = await apiRequest('POST', '/api/music-workflow-automations/trigger', {
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['/api/music-workflow-automations/logs'] });
      qc.invalidateQueries({ queryKey: ['/api/music-workflow-automations/stats'] });
      toast({ title: 'Test triggered', description: `"${automation.name}" fired. Check Run History.` });
    },
    onError: () => {
  ...
  return (
    <Card className={`transition-all duration-200 ${automation.enabled ? 'border-primary/40 bg-primary/5' : 'border-border'}`}>
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3 min-w-0">
            <span className="text-2xl flex-shrink-0 mt-0.5">{automation.icon}</span>
            <div className="min-w-0">
              <CardTitle className="text-sm font-semibold leading-snug">{automation.name}</CardTitle>
              <CardDescription className="text-xs mt-1 leading-relaxed">{automation.description}</CardDescription>
...
function StatsBar({ stats, isLoading }: { stats?: AutomationStats; isLoading: boolean }) {
  const cards = [
    {
      label: 'Active Automations',
      value: isLoading ? '—' : `${stats?.enabledCount ?? 0} / ${stats?.totalTemplates ?? 0}`,
    ...
    {
      label: 'Last Run',
      value: isLoading ? '—' : stats?.lastRunAt
        ? new Date(stats.lastRunAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })
        : 'Never',
      icon: <Clock className="h-4 w-4 text-amber-500" />,
      sub: stats?.lastRunAt
        ? new Date(stats.lastRunAt).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })
...
function OverviewTab({ automations, stats }: { automations: WorkflowTemplate[]; stats?: AutomationStats }) {
  const qc = useQueryClient();
  const { toast } = useToast();
  ...
  const enableAllMutation = useMutation({
    mutationFn: async () => {
      ...
      for (const a of disabled) {
        const res = await apiRequest('POST', `/api/music-workflow-automations/${a.id}/enable`, { config: a.config });
        await res.json();
      }
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['/api/music-workflow-automations'] });
      qc.invalidateQueries({ queryKey: ['/api/music-workflow-automations/stats'] });
      toast({ title: 'All automations enabled', description: 'Your full automation suite is now active.' });
    },
  });

  const phaseStats = PHASES.map((p) => {
    const total = automations.filter((a) => a.phase === p.key).length;
    const enabled = automations.filter((a) => a.phase === p.key && a.enabled).length;
    return { ...p, total, enabled };
  });

  return (
    <div className="space-y-6">
      {automations.length > 0 && automations.filter((a) => !a.enabled).length > 0 && (
        <Card className="border-primary/30 bg-primary/5">
          <CardContent className="flex items-center justify-between gap-4 py-4">
            <div>
              <p className="font-semibold text-sm">You have {automations.filter((a) => !a.enabled).length} automations ready to activate</p>
              <p className="text-xs text-muted-foreground mt-0.5">Enable them all at once, or turn on only what you need from the Automations tab.</p>
            </div>
            <Button
              size="sm"
            ...
            >
              {enableAllMutation.isPending ? <Loader2 className="h-3 w-3 animate-spin mr-1" /> : <Zap className="h-3 w-3 mr-1" />}
              Enable All
            </Button>
          </CardContent>
        </Card>
          ...
          <Card key={p.key} className="p-4">
            ...
            <div className="mt-3 h-1.5 rounded-full bg-muted overflow-hidden">
              <div
                ...
                style={{ width: p.total > 0 ? `${(p.enabled / p.total) * 100}%` : '0%' }}
              />
            </div>
          </Card>
        ))}
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {[
            { step: '1', title: 'An event happens in Max Booster', desc: 'You upload a track, set a release date, hit a streaming milestone, or make a sale.' },
            { step: '2', title: 'Enabled automations fire instantly', desc: 'Any automation listening for that event executes its actions automatically.' },
            { step: '3', title: 'You get notified', desc: 'A push notification confirms what ran. Check Run History for full logs.' },
          ].map((item) => (
  ...
  if (logs.length === 0) {
    return (
      <div className="text-center py-16 text-muted-foreground">
        ...
        <p className="text-xs mt-1">Enable automations and use the test run button to see logs here.</p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3 text-sm text-muted-foreground">
          <span className="flex items-center gap-1"><CheckCircle2 className="h-3.5 w-3.5 text-green-500" />{successCount} succeeded</span>
          {failedCount > 0 && <span className="flex items-center gap-1"><XCircle className="h-3.5 w-3.5 text-destructive" />{failedCount} failed</span>}
...
function ScheduleTab({ automations, stats }: { automations: WorkflowTemplate[]; stats?: AutomationStats }) {
  const scheduledAutomations = automations.filter((a) => a.trigger.event.startsWith('schedule:'));
  const eventTriggered = automations.filter((a) => !a.trigger.event.startsWith('schedule:'));
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-muted-foreground mb-4">These automations run automatically on a fixed schedule — no action needed from you.</p>
        ...
        {scheduledAutomations.length === 0 ? (
          ...
          <div className="space-y-3">
            {stats?.nextScheduledRuns?.map((run) => {
              const match = scheduledAutomations.find((a) => a.name === run.name);
              return (
                <Card key={run.name} className={`p-4 ${match?.enabled ? 'border-primary/30 bg-primary/5' : 'opacity-60'}`}>
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="font-medium text-sm">{run.name}</p>
                      <p className="text-xs text-muted-foreground mt-0.5">{run.schedule}</p>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <p className="text-xs text-muted-foreground">Next run</p>
                      <p className="text-sm font-semibold">
                        {new Date(run.nextRun).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {new Date(run.nextRun).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })}
                      </p>
                    </div>
                  </div>
                  {!match?.enabled && (
                    <p className="text-xs text-amber-500 mt-2">Disabled — enable this automation to activate its schedule</p>
                  )}
                </Card>
              );
            })}
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mb-4">These fire instantly when a specific action happens in Max Booster — upload, release, sale, etc.</p>

        <div className="space-y-2">
          {eventTriggered.map((a) => (
            <div key={a.id} className={`flex items-center gap-3 p-3 rounded-lg border text-sm ${a.enabled ? '' : 'opacity-50'}`}>
              ...
              <Badge variant={a.enabled ? 'default' : 'outline'} className="text-xs py-0 h-5 flex-shrink-0">
...
export default function MusicWorkflowAutomations() {
  const { data: automationsData, isLoading: automationsLoading } = useQuery<{ automations: WorkflowTemplate[] }>({
  ...
  const automationsByPhase = (phase: string) => automations.filter((a) => a.phase === phase);
        ...
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-sm">From first recording to first stream — automate your entire music career.</p>
            </div>
          </div>
        </div>
              ...
              {PHASES.map((phase) => {
                ...
                return (
                  <section key={phase.key}>
                    <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs font-semibold mb-2 ${phase.color}`}>
                      <div className={`h-1.5 w-1.5 rounded-full ${phase.dotColor}`} />
                      {phase.label}
                      <span className="opacity-60">
                        · {phaseAutomations.filter((a) => a.enabled).length}/{phaseAutomations.length} active
                      </span>
                    </div>
