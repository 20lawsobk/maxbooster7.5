    ...
    useQuery<PendingVerificationsResponse>({
      ...
      queryFn: async () => {
        const res = await fetch(
          `/api/kyc/admin/pending?status=${statusFilter}`,
          {
            credentials: "include",
          },
        );
          ...
          <div className="space-y-4">
            {verifications.map((verification) => (
              <Collapsible
                ...
                onOpenChange={(open) =>
                  setExpandedId(open ? verification.id : null)
                }
              >
                <Card>
                  <CardHeader className="pb-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <div
                          className={`p-3 rounded-full ${verification.verificationType === "individual" ? "bg-blue-500/10" : "bg-purple-500/10"}`}
                        >
                        ...
                        <div>
                          ...
                          <CardDescription>
                            {verification.user?.email || "No email"} | Submitted{" "}
                                    ...
                                    {doc.status === "pending" && (
                                      <>
                                        <Button
                                          ...
                                          onClick={() =>
                                            window.open(
                                              `/api/kyc/admin/documents/${doc.id}/view`,
                                              "_blank",
                                            )
                                          }
                                        >
