...
interface TrainStatus {
  ...
  step: number;
  loss: number | null;
  loss_history: Array<{ session: number; loss: number; phase: number; ts: number }>;
...
function StatusBadge({ status }: { status: TrainStatus['status'] }) {
  const map: Record<TrainStatus['status'], { label: string; className: string }> = {
    idle:     { label: 'Idle',     className: 'bg-gray-500/20 text-gray-400' },
    running:  { label: 'Training', className: 'bg-green-500/20 text-green-400 animate-pulse' },
    stopping: { label: 'Stopping', className: 'bg-yellow-500/20 text-yellow-400' },
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${className}`}>
      {status === 'running' && <span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-ping" />}
      {label}
    </span>
  );
}
...
export default function TrainingDashboard() {
  ...
  const { data: status, isError: statusError } = useQuery<TrainStatus>({
    ...
    refetchInterval: (query) =>
      query.state.data?.status === 'running' || query.state.data?.status === 'stopping'
        ? 3000
        : 15000,
    retry: false,
  });
  ...
  const startMutation = useMutation({
    mutationFn: () => apiFetch('/api/training/start', {
      method: 'POST',
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              variant="ghost"
              size="sm"
              onClick={() => qc.invalidateQueries({ queryKey: ['training-status'] })}
          ...
          <Card className="bg-gray-900 border-gray-700">
            <CardContent className="pt-4 pb-3">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs text-gray-400 uppercase tracking-wide">Phase</span>
              </div>
              <div className="text-2xl font-bold text-white">
                {status?.phase ?? '—'}<span className="text-gray-500 text-sm">/4</span>
              </div>
          ...
          <Card className="bg-gray-900 border-gray-700">
            <CardContent className="pt-4 pb-3">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs text-gray-400 uppercase tracking-wide">Loss</span>
              </div>
              <div className="text-2xl font-bold text-white">
          ...
          <Card className="bg-gray-900 border-gray-700">
            <CardContent className="pt-4 pb-3">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs text-gray-400 uppercase tracking-wide">Samples</span>
              </div>
              <div className="text-2xl font-bold text-white">
        ...
        <Tabs defaultValue="control" className="space-y-4">
          <TabsList className="bg-gray-800 border border-gray-700">
            <TabsTrigger value="control" className="data-[state=active]:bg-gray-700">
              <Play className="w-3.5 h-3.5 mr-1.5" />Control
            </TabsTrigger>
            <TabsTrigger value="loss" className="data-[state=active]:bg-gray-700">
              <BarChart2 className="w-3.5 h-3.5 mr-1.5" />Loss Curve
            </TabsTrigger>
            <TabsTrigger value="datasets" className="data-[state=active]:bg-gray-700">
              <Database className="w-3.5 h-3.5 mr-1.5" />Datasets
            </TabsTrigger>
            <TabsTrigger value="schedule" className="data-[state=active]:bg-gray-700">
              <Cpu className="w-3.5 h-3.5 mr-1.5" />Curriculum
            </TabsTrigger>
          </TabsList>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Card className="bg-gray-900 border-gray-700">
                <CardHeader className="pb-3">
                  <CardTitle className="text-white text-sm font-medium">Start Training</CardTitle>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      >
                        ...
                        <SelectContent className="bg-gray-800 border-gray-600">
                          <SelectItem value="session" className="text-gray-200 text-sm">
                            Single Session
                          </SelectItem>
                          <SelectItem value="day" className="text-gray-200 text-sm">
                            Day ({nSessions} sessions)
                          </SelectItem>
                          <SelectItem value="continuous" className="text-gray-200 text-sm">
                    ...
                    <div>
                      <label className="text-xs text-gray-400 block mb-1.5">Sessions (Day mode)</label>
                      <Select
                      ...
                      >
                        ...
                        <SelectContent className="bg-gray-800 border-gray-600">
                          {[1, 2, 3, 5, 10].map(n => (
                            <SelectItem key={n} value={String(n)} className="text-gray-200 text-sm">
                              {n}
                            </SelectItem>
                          ))}
                        </SelectContent>
                  ...
                  {startMutation.isError && (
                    <p className="text-red-400 text-xs">{(startMutation.error as Error).message}</p>
                  )}
                </CardContent>
              </Card>
              ...
              <Card className="bg-gray-900 border-gray-700">
                <CardHeader className="pb-3">
                  <CardTitle className="text-white text-sm font-medium">Current Session</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between text-gray-300">
                      <span className="text-gray-400">Status</span>
                      {status ? <StatusBadge status={status.status} /> : <span className="text-gray-500">—</span>}
                    </div>
                    <div className="flex justify-between text-gray-300">
                      <span className="text-gray-400">Phase</span>
                      <span>{status?.phase ? `${status.phase} — ${status.phase_name}` : '—'}</span>
                    </div>
                    <div className="flex justify-between text-gray-300">
                    ...
                    <div className="flex justify-between text-gray-300">
                      ...
                      <span className="text-xs">
                        {status?.last_save
                          ? new Date(status.last_save * 1000).toLocaleTimeString()
                          : '—'}
                      </span>
                    </div>
                    <div className="flex justify-between text-gray-300">
              ...
              <Card className="bg-gray-900 border-gray-700">
                <CardContent className="pt-4">
                  ...
                  <p className="text-xs text-gray-500 mt-1.5">
                    Phase {schedule.current_status.current_phase}: {schedule.current_status.phase_name}
                  </p>
                </CardContent>
              </Card>
            )}
          ...
          <TabsContent value="loss">
            <Card className="bg-gray-900 border-gray-700">
              <CardHeader className="pb-3">
                ...
                <CardDescription className="text-gray-400 text-xs">
                  {status?.loss_history?.length ?? 0} data points across {status?.session_count ?? 0} sessions
                </CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <div className="mt-4 grid grid-cols-3 gap-3 text-center text-xs">
                    ...
                    <div className="bg-gray-800 rounded p-2">
                      ...
                      <div className="text-green-400 font-mono">
                        {Math.min(...status.loss_history.map(h => h.loss)).toFixed(4)}
                      </div>
                    </div>
                    <div className="bg-gray-800 rounded p-2">
                      <div className="text-gray-400 mb-1">Latest Loss</div>
                      <div className="text-indigo-400 font-mono">
                        {status.loss_history[status.loss_history.length - 1].loss.toFixed(4)}
                      </div>
                    </div>
                  </div>
                )}
          ...
          <TabsContent value="datasets">
            <div className="space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {[
                  { label: 'HMDB-51 Clips',       value: datasets?.stats?.hmdb51_clips,        icon: '🎬', color: 'text-blue-400' },
                  { label: 'UCF-101 Clips',        value: datasets?.stats?.ucf101_clips,        icon: '🎬', color: 'text-blue-400' },
                  { label: 'MusicCaps Captions',   value: datasets?.stats?.musiccaps_captions,  icon: '🎵', color: 'text-purple-400' },
                  { label: 'AudioCaps Captions',   value: datasets?.stats?.audiocaps_captions,  icon: '🔊', color: 'text-teal-400' },
                  { label: 'FMA Tracks',           value: datasets?.stats?.fma_tracks,          icon: '🎼', color: 'text-orange-400' },
                  { label: 'Total Disk Used',      value: datasets?.total_gb != null ? `${datasets.total_gb} GB` : null, icon: '💾', color: 'text-gray-300' },
              ...
              <Card className="bg-gray-900 border-gray-700">
                ...
                <CardContent>
                  {datasets?.disk_gb && Object.keys(datasets.disk_gb).length > 0 ? (
                        ...
                        .map(([name, gb]) => (
                          <div key={name} className="flex items-center gap-3">
                            ...
                            <div className="flex-1">
                              <Progress
                                value={Math.min(100, (gb / (datasets.total_gb || 1)) * 100)}
                                className="h-1.5"
                              />
                            </div>
              ...
              <Card className="bg-gray-900 border-gray-700">
                <CardContent className="pt-4">
                  <div className="flex items-center gap-2 mb-3">
                    <CheckCircle className={`w-4 h-4 ${datasets?.stats?.has_video_data ? 'text-green-400' : 'text-gray-600'}`} />
                    <span className="text-sm text-gray-300">
                      Real video frames ({datasets?.stats?.has_video_data ? 'active — 25% of training samples' : 'not yet available'})
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <CheckCircle className={`w-4 h-4 ${datasets?.stats?.has_prompt_data ? 'text-green-400' : 'text-gray-600'}`} />
                    <span className="text-sm text-gray-300">
                      Real music captions ({datasets?.stats?.has_prompt_data ? 'active — 20% of training prompts' : 'not yet available'})
                    </span>
                  </div>
                </CardContent>
              </Card>
          ...
          <TabsContent value="schedule">
            <Card className="bg-gray-900 border-gray-700">
              <CardHeader className="pb-3">
                <CardTitle className="text-white text-sm font-medium">30-Day Training Curriculum</CardTitle>
                <CardDescription className="text-gray-400 text-xs">
                ...
                {schedule?.schedule ? (
                  <div className="space-y-3">
                    {schedule.schedule.map((phase) => {
                      const isCurrent = phase.phase_id === schedule.current_status?.current_phase;
                      return (
                        <div
                          key={phase.phase_id}
                        ...
                        >
                          ...
                          <p className="text-xs text-gray-400">{phase.training_focus}</p>
                          {phase.datasets?.length > 0 && (
                            <div className="flex flex-wrap gap-1 mt-1.5">
                              {phase.datasets.slice(0, 4).map(d => (
                                <span key={d} className="text-xs bg-gray-700 text-gray-300 px-1.5 py-0.5 rounded">
                                  {d}
                                </span>
                              ))}
                              {phase.datasets.length > 4 && (
                                <span className="text-xs text-gray-500">+{phase.datasets.length - 4} more</span>
                              )}
                            </div>
                          )}
                        </div>
