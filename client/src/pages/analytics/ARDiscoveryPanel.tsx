...
const SigningPotentialBadge = memo(
  ({ potential }: { potential: "high" | "medium" | "low" }) => {
    const config = {
      high: {
...
const ArtistCard = memo(
  ({
    ...
    onSelect?: (artist: EmergingArtist) => void;
  }) => {
          ...
          <CardContent className="pt-12 pb-4">
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="text-right">
                <p
                  className={`text-3xl font-bold ${getScoreColor(artist.growthScore)}`}
                >
                  {artist.growthScore}
                </p>
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-sm text-green-600 font-medium flex items-center gap-1">
                <ArrowUp className="h-3 w-3" />+{artist.monthlyGrowth}% this
                month
              </span>
            </div>
...
const GrowthTrajectoryChart = memo(
  ({ artists }: { artists: EmergingArtist[] }) => {
...
export default function ARDiscoveryPanel({
  artists: propArtists,
  onArtistSelect,
}: ARDiscoveryProps) {
  ...
  const stats = {
    ...
    highPotential: artists.filter(
      (a: EmergingArtist) => a.signingPotential === "high",
    ).length,
    avgGrowthScore:
      artists.length > 0
        ? Math.round(
              ...
              <div className="flex items-start gap-3 p-3 bg-green-50 dark:bg-green-950/30 rounded-lg">
                ...
                <div>
                  ...
                  <p className="text-sm text-green-600">
                    Score 85+, rapid growth, strong engagement
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-3 p-3 bg-yellow-50 dark:bg-yellow-950/30 rounded-lg">
                ...
                <div>
                  ...
                  <p className="text-sm text-yellow-600">
                    Score 60-84, steady growth trajectory
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-3 p-3 bg-slate-50 dark:bg-slate-900 rounded-lg">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400">
                    Score below 60, early stage development
                  </p>
                </div>
              </div>
            </CardContent>
