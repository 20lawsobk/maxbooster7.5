...
interface StudioState {
  ...
  updatePluginParameter: (trackId: string, pluginId: string, paramId: string, value: number | boolean | string) => void;
      ...
      persist(
        (set, get) => ({
          ...
          setProject: (project) => set((state) => ({
            project: { ...state.project, ...project, modifiedAt: Date.now(), isDirty: true }
          })),
          ...
          setMixer: (mixer) => set((state) => ({
            mixer: { ...state.mixer, ...mixer }
          })),
          
          play: () => set((state) => ({
            transport: { ...state.transport, isPlaying: true, isPaused: false }
          })),
          ...
          stop: () => set((state) => ({
            transport: { ...state.transport, isPlaying: false, isPaused: false, isRecording: false, position: 0 }
          })),
          
          record: () => set((state) => ({
            transport: { ...state.transport, isPlaying: true, isRecording: true }
          })),
          ...
          setTempo: (tempo) => set((state) => ({
            transport: { ...state.transport, tempo: Math.max(20, Math.min(999, tempo)) }
          })),
          ...
          removeTrack: (trackId) => {
            ...
            set((state) => ({
              ...
              view: {
                ...
                focusedTrackId: state.view.focusedTrackId === trackId ? null : state.view.focusedTrackId
              },
              project: { ...state.project, isDirty: true, modifiedAt: Date.now() }
            }));
          ...
          duplicateTrack: (trackId) => {
            ...
            const newTrack = { ...track, id: newId, name: `${track.name} (Copy)` };
            set((state) => ({
          ...
          reorderTracks: (fromIndex, toIndex) => set((state) => {
            ...
            return { tracks, project: { ...state.project, isDirty: true, modifiedAt: Date.now() } };
          }),
          
          setTrackVolume: (trackId, volume) => set((state) => ({
            tracks: state.tracks.map(t => t.id === trackId ? { ...t, volume } : t),
            masterTrack: trackId === 'master' ? { ...state.masterTrack, volume } : state.masterTrack
          })),
          
          setTrackPan: (trackId, pan) => set((state) => ({
            tracks: state.tracks.map(t => t.id === trackId ? { ...t, pan } : t),
            masterTrack: trackId === 'master' ? { ...state.masterTrack, pan } : state.masterTrack
          })),
          
          toggleTrackMute: (trackId) => set((state) => ({
            tracks: state.tracks.map(t => t.id === trackId ? { ...t, muted: !t.muted } : t),
            masterTrack: trackId === 'master' ? { ...state.masterTrack, muted: !state.masterTrack.muted } : state.masterTrack
          })),
          ...
          toggleTrackSolo: (trackId) => {
            ...
            if (mixer.soloMode === 'exclusive') {
              set((state) => ({
                tracks: state.tracks.map(t => ({ ...t, solo: t.id === trackId ? !t.solo : false }))
              }));
            } else {
              set((state) => ({
          ...
          setTrackMeterLevel: (trackId, left, right) => set((state) => ({
            tracks: state.tracks.map(t => t.id === trackId ? { ...t, meterLevel: { left, right } } : t),
            masterTrack: trackId === 'master' ? { ...state.masterTrack, meterLevel: { left, right } } : state.masterTrack
          })),
          ...
          addPlugin: (trackId, plugin) => {
            ...
            set((state) => ({
              tracks: state.tracks.map(t => t.id === trackId ? { ...t, plugins: [...t.plugins, newPlugin] } : t),
              masterTrack: trackId === 'master' ? { ...state.masterTrack, plugins: [...state.masterTrack.plugins, newPlugin] } : state.masterTrack,
              project: { ...state.project, isDirty: true, modifiedAt: Date.now() }
            }));
          ...
          removePlugin: (trackId, pluginId) => {
            const track = get().tracks.find(t => t.id === trackId) || (trackId === 'master' ? get().masterTrack : null);
            const plugin = track?.plugins.find(p => p.id === pluginId);
            set((state) => ({
              tracks: state.tracks.map(t => t.id === trackId ? { ...t, plugins: t.plugins.filter(p => p.id !== pluginId) } : t),
              masterTrack: trackId === 'master' ? { ...state.masterTrack, plugins: state.masterTrack.plugins.filter(p => p.id !== pluginId) } : state.masterTrack,
              project: { ...state.project, isDirty: true, modifiedAt: Date.now() }
            }));
            if (plugin) get().pushHistory(`Remove plugin: ${plugin.name}`);
          },
          
          updatePluginParameter: (trackId, pluginId, paramId, value) => set((state) => ({
            ...
            masterTrack: trackId === 'master' ? {
              ...state.masterTrack,
              plugins: state.masterTrack.plugins.map(p => p.id === pluginId ? {
                ...p,
                parameters: { ...p.parameters, [paramId]: value }
              } : p)
            } : state.masterTrack
          ...
          togglePluginBypass: (trackId, pluginId) => set((state) => ({
            tracks: state.tracks.map(t => t.id === trackId ? {
              ...t,
              plugins: t.plugins.map(p => p.id === pluginId ? { ...p, bypassed: !p.bypassed } : p)
            } : t),
            masterTrack: trackId === 'master' ? {
              ...state.masterTrack,
              plugins: state.masterTrack.plugins.map(p => p.id === pluginId ? { ...p, bypassed: !p.bypassed } : p)
            } : state.masterTrack
          })),
          
          reorderPlugins: (trackId, fromIndex, toIndex) => set((state) => {
            ...
            return {
              tracks: state.tracks.map(t => t.id === trackId ? { ...t, plugins: updatePlugins(t.plugins) } : t),
              masterTrack: trackId === 'master' ? { ...state.masterTrack, plugins: updatePlugins(state.masterTrack.plugins) } : state.masterTrack
            };
          }),
          ...
          addAudioClip: (trackId, clip) => {
            ...
            set((state) => ({
              tracks: state.tracks.map(t => t.id === trackId ? { ...t, audioClips: [...t.audioClips, newClip] } : t),
              project: { ...state.project, isDirty: true, modifiedAt: Date.now() }
            }));
          ...
          removeAudioClip: (trackId, clipId) => {
            ...
            set((state) => ({
              tracks: state.tracks.map(t => t.id === trackId ? { ...t, audioClips: t.audioClips.filter(c => c.id !== clipId) } : t),
              view: { ...state.view, selectedClipIds: state.view.selectedClipIds.filter(id => id !== clipId) },
              project: { ...state.project, isDirty: true, modifiedAt: Date.now() }
            }));
          ...
          updateAudioClip: (trackId, clipId, updates) => set((state) => ({
            tracks: state.tracks.map(t => t.id === trackId ? {
              ...t,
              audioClips: t.audioClips.map(c => c.id === clipId ? { ...c, ...updates } : c)
            } : t),
            project: { ...state.project, isDirty: true, modifiedAt: Date.now() }
          })),
          ...
          moveClip: (fromTrackId, toTrackId, clipId, newStartTime) => set((state) => {
            ...
            return {
              tracks: state.tracks.map(t => {
                ...
                if (t.id === toTrackId) {
                  return { ...t, audioClips: [...t.audioClips, { ...clip, startTime: newStartTime }] };
                }
                return t;
              }),
              project: { ...state.project, isDirty: true, modifiedAt: Date.now() }
          ...
          clearSelection: () => set((state) => ({
            view: { ...state.view, selectedTrackIds: [], selectedClipIds: [] }
          })),
          ...
          undo: () => {
            ...
            if (historyIndex > 0) {
              ...
              set((state) => ({ ...prevEntry.state, historyIndex: historyIndex - 1 }));
            }
          },
          ...
          redo: () => {
            ...
            if (historyIndex < history.length - 1) {
              ...
              set((state) => ({ ...nextEntry.state, historyIndex: historyIndex + 1 }));
            }
          },
          ...
          pushHistory: (action) => {
            ...
            set((s) => ({
              history: [...s.history.slice(0, s.historyIndex + 1), entry].slice(-50),
          ...
          updatePerformanceMetrics: (cpu, latency) => set({ cpuUsage: cpu, latency }),
        }),
        {
          name: 'studio-storage',
          partialize: (state) => ({
...
export const useMasterTrack = () => useStudioStore((state) => state.masterTrack);
...
export const useTrack = (trackId: string) => useStudioStore((state) => 
  state.tracks.find(t => t.id === trackId) || (trackId === 'master' ? state.masterTrack : null)
);

export const useSelectedTracks = () => useStudioStore((state) => 
