...
export function useUnifiedStore(): UnifiedStoreState {
  ...
  useEffect(() => {
    const unsubscribe = useNewStore.subscribe(
      (state) => ({ isPlaying: state.transport.isPlaying, position: state.transport.position }),
      ({ isPlaying, position }) => {
  ...
  useEffect(() => {
    const unsubscribe = useNewStore.subscribe(
      (state) => ({ loopStart: state.transport.loopStart, loopEnd: state.transport.loopEnd }),
      ({ loopStart, loopEnd }) => {
        if (legacyStore.loopStart !== loopStart || legacyStore.loopEnd !== loopEnd) {
  ...
  useEffect(() => {
    if (legacyStore.tracks && Array.isArray(legacyStore.tracks)) {
      legacyStore.tracks.forEach(legacyTrack => {
        const existingTrack = newStore.tracks.find(t => t.id === legacyTrack.id);
        if (!existingTrack) {
