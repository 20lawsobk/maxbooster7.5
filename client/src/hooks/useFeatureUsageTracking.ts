...
export function useFeatureUsageTracking() {
  ...
  const trackFeature = useCallback((feature: string) => {
    if (currentSession.current && currentSession.current.feature !== feature) {
      currentSession.current.endTime = Date.now();
      currentSession.current.duration = currentSession.current.endTime - currentSession.current.startTime;
    }
  ...
  const trackButtonClick = useCallback((buttonId: string, context?: Record<string, any>) => {
