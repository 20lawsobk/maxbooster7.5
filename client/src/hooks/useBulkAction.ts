...
export function useBulkAction(options: UseBulkActionOptions = {}): UseBulkActionResult {
  ...
  const [pendingConfig, setPendingConfig] = useState<BulkActionConfig | null>(null);
  
  const abortControllerRef = useRef<AbortController | null>(null);

  const updateProgress = useCallback((update: Partial<BulkActionProgress>) => {
    setProgress(prev => {
  ...
  const execute = useCallback(async (config: BulkActionConfig): Promise<BulkActionResult> => {
...
export function useBulkDelete(resource: string, options: UseBulkActionOptions = {}) {
...
export function useBulkUpdate(resource: string, options: UseBulkActionOptions = {}) {
  const bulkAction = useBulkAction(options);

  const updateItems = useCallback(async (ids: string[], data: Record<string, any>) => {
    return bulkAction.execute({
      resource,
      action: 'update',
      ids,
...
export function useBulkExport(resource: string, options: UseBulkActionOptions = {}) {
  const bulkAction = useBulkAction(options);

  const exportItems = useCallback(async (ids: string[], format: string = 'csv') => {
    return bulkAction.execute({
      resource,
      action: 'export',
      ids,
...
export function useBulkStatusChange(resource: string, options: UseBulkActionOptions = {}) {
  const bulkAction = useBulkAction(options);

  const changeStatus = useCallback(async (ids: string[], status: string) => {
    return bulkAction.execute({
      resource,
