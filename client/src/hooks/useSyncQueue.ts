...
export interface UseSyncQueueOptions {
  ...
  onConflict?: (actionId: string, localData: unknown, serverData: unknown) => void;
}
...
export interface UseSyncQueueReturn {
  ...
  getConflicts: () => Promise<Array<{ actionId: string; localData: unknown; serverData: unknown; detectedAt: number }>>;
...
export function useSyncQueue(options: UseSyncQueueOptions = {}): UseSyncQueueReturn {
  const { autoSync = true, onActionAdded, onActionCompleted, onActionFailed, onConflict } = options;
  ...
  const loadStats = useCallback(async () => {
    try {
      ...
      console.error('[useSyncQueue] Failed to load stats:', error);
    }
  }, []);
  ...
  const loadActions = useCallback(async () => {
    try {
      ...
      console.error('[useSyncQueue] Failed to load actions:', error);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const unsubConflict = offlineQueue.on('conflict-detected', (event) => {
      ...
      if (event.actionId && event.conflict) {
        onConflict?.(event.actionId, event.conflict.localData, event.conflict.serverData);
      }
    });
  ...
  const getAction = useCallback(async <T,>(actionId: string): Promise<QueuedAction<T> | undefined> => {
