...
export function useHighContrast(options: HighContrastOptions = {}): HighContrastResult {
  ...
  const getContrastColors = useCallback((): ContrastColors => {
    switch (contrastMode) {
      case 'more': return MORE_CONTRAST_COLORS;
      case 'high': return HIGH_CONTRAST_COLORS;
      default: return NORMAL_COLORS;
    }
  }, [contrastMode]);
  ...
  useEffect(() => {
    ...
    root.style.setProperty('--a11y-focus-width', `${getFocusIndicatorWidth()}px`);
  ...
  }, [contrastMode, isHighContrast, getContrastColors, getFocusIndicatorWidth, getBorderWidth]);
