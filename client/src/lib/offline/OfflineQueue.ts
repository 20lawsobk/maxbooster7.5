...
export type QueueEventType = 'action-added' | 'action-updated' | 'action-removed' | 'sync-started' | 'sync-completed' | 'conflict-detected';
...
class OfflineQueue {
  ...
  async updateAction<T = unknown>(id: string, updates: Partial<QueuedAction<T>>): Promise<QueuedAction<T> | null> {
    const db = await this.ensureDb();
  ...
  async getAction<T = unknown>(id: string): Promise<QueuedAction<T> | undefined> {
    const db = await this.ensureDb();
  ...
  async getConflicts(): Promise<Array<{ actionId: string; localData: unknown; serverData: unknown; detectedAt: number }>> {
    const db = await this.ensureDb();
  ...
  async clearCompleted(): Promise<number> {
    const db = await this.ensureDb();
    const completed = await db.getAllFromIndex('actions', 'by-status', 'completed');
  ...
  async getNextBatch(batchSize = 10): Promise<QueuedAction[]> {
    ...
    for (const action of pending) {
      ...
      const dependenciesMet = !action.dependencies?.length ||
        action.dependencies.every(depId => !pending.find(p => p.id === depId));
  ...
  async getStats(): Promise<{
    ...
    const [pending, syncing, completed, failed, conflict, total] = await Promise.all([
