...
class SentryService {
  ...
  captureMessage(message: string, level: SentryLevel = 'info', context: SentryContext = {}) {
    const fullContext: SentryContext = {
      user: context.user || this.userContext || undefined,
      tags: { ...this.tags, ...context.tags },
      extra: {
  ...
  private async sendToSentry(data: { error?: SentryError; message?: string; level?: SentryLevel; context: SentryContext }) {
    if (!this.dsn) {
      return;
    }
      ...
      const payload = {
        exception: data.error ? {
          values: [{
            ...
            stacktrace: data.error.stack ? { frames: this.parseStackTrace(data.error.stack) } : undefined,
          }],
        } : undefined,
  ...
  private parseStackTrace(stack: string): Array<{ filename: string; lineno?: number; colno?: number; function?: string }> {
  ...
  withScope(callback: (scope: { setTag: (key: string, value: string) => void; setExtra: (key: string, value: unknown) => void; setUser: (user: SentryContext['user']) => void }) => void) {
    ...
    const scope = {
      setTag: (key: string, value: string) => { scopeTags[key] = value; },
      setExtra: (key: string, value: unknown) => { scopeExtra[key] = value; },
      setUser: (user: SentryContext['user']) => { scopeUser = user; },
    };
...
export function initSentry(options?: Parameters<typeof SentryService.prototype.init>[0]) {
  sentry.init(options);
}
...
export function captureMessage(message: string, level?: SentryLevel, context?: SentryContext) {
...
export function withScope(callback: Parameters<typeof SentryService.prototype.withScope>[0]) {
