...
export interface ScenePostConfig {
  ...
  colorGrading:     {
    ...
    hue: number; exposure: number; gamma: number;
    shadows: [number, number, number];
    midtones: [number, number, number];
    highlights: [number, number, number];
    temperature: number; tint: number; vibrance: number;
    filmGrain: number;
  };
...
const SCENE_PRESETS: Record<string, ScenePostConfig> = {
  concert_stage: {
    ...
    colorGrading: {
      brightness: 0.05, contrast: 1.2, saturation: 1.4, hue: 0,
      exposure: 0.3, gamma: 0.9,
      shadows: [-0.02, -0.01, 0.05], midtones: [0.02, 0.01, -0.01],
      highlights: [0.05, 0.02, -0.03],
      temperature: 2.0, tint: 0.5, vibrance: 0.3, filmGrain: 0.02,
    },
  ...
  city_nights: {
    ...
    colorGrading: {
      brightness: -0.05, contrast: 1.3, saturation: 1.2, hue: 0.02,
      exposure: 0.1, gamma: 1.0,
      shadows: [0.0, 0.02, 0.08], midtones: [0.0, 0.0, 0.03],
      highlights: [0.02, 0.0, -0.02],
      temperature: -3.0, tint: -0.5, vibrance: 0.4, filmGrain: 0.03,
    },
  ...
  studio_session: {
    ...
    colorGrading: {
      brightness: 0.02, contrast: 1.1, saturation: 0.9, hue: 0,
      exposure: 0.0, gamma: 1.0,
      shadows: [0.03, 0.02, 0.0], midtones: [0.01, 0.01, 0.0],
      highlights: [0.02, 0.01, 0.0],
      temperature: 5.0, tint: 1.0, vibrance: 0.1, filmGrain: 0.015,
    },
  ...
  golden_hour: {
    ...
    colorGrading: {
      brightness: 0.08, contrast: 1.15, saturation: 1.5, hue: 0.01,
      exposure: 0.4, gamma: 0.85,
      shadows: [0.05, 0.02, -0.03], midtones: [0.08, 0.04, -0.02],
      highlights: [0.12, 0.06, -0.04],
      temperature: 12.0, tint: 2.0, vibrance: 0.5, filmGrain: 0.01,
    },
  ...
  neon_cityscape: {
    ...
    colorGrading: {
      brightness: -0.02, contrast: 1.4, saturation: 1.8, hue: 0.03,
      exposure: 0.2, gamma: 0.95,
      shadows: [-0.03, 0.0, 0.08], midtones: [0.0, -0.02, 0.05],
      highlights: [0.05, -0.02, 0.08],
      temperature: -5.0, tint: -2.0, vibrance: 0.6, filmGrain: 0.025,
    },
  ...
  trap_aesthetic: {
    ...
    colorGrading: {
      brightness: -0.05, contrast: 1.35, saturation: 1.1, hue: 0,
      exposure: 0.15, gamma: 0.92,
      shadows: [0.0, 0.0, 0.05], midtones: [0.0, 0.0, 0.02],
      highlights: [0.02, 0.0, 0.03],
      temperature: -2.0, tint: 0.0, vibrance: 0.2, filmGrain: 0.04,
    },
  ...
  gospel_choir: {
    ...
    colorGrading: {
      brightness: 0.06, contrast: 1.1, saturation: 1.2, hue: 0.02,
      exposure: 0.35, gamma: 0.88,
      shadows: [0.04, 0.03, 0.0], midtones: [0.06, 0.04, 0.0],
      highlights: [0.10, 0.07, 0.02],
      temperature: 8.0, tint: 2.5, vibrance: 0.35, filmGrain: 0.015,
    },
    chromaticAb:  { amount: 0.001, radial: false },
  ...
  default: {
    ...
    colorGrading: {
      brightness: 0.0, contrast: 1.1, saturation: 1.15, hue: 0,
      exposure: 0.1, gamma: 1.0,
      shadows: [0.0, 0.0, 0.0], midtones: [0.0, 0.0, 0.0],
      highlights: [0.0, 0.0, 0.0],
      temperature: 0.0, tint: 0.0, vibrance: 0.1, filmGrain: 0.01,
    },
    chromaticAb:  { amount: 0.002, radial: true },
    vignette:     { intensity: 0.5, radius: 0.65, softness: 0.4 },
  },
...
export class DigitalGPUInferenceBridge {
  ...
  async init(): Promise<void> {
    try {
      ...
      const bloomProg   = this.renderer.createShaderProgram(
        'bloom', BLOOM_SHADER.vertex, BLOOM_SHADER.fragment);
      const gradeProg   = this.renderer.createShaderProgram(
        'colorGrade', COLOR_GRADING_SHADER.vertex, COLOR_GRADING_SHADER.fragment);
      const chromaProg  = this.renderer.createShaderProgram(
        'chromaAb', CHROMATIC_ABERRATION_SHADER.vertex, CHROMATIC_ABERRATION_SHADER.fragment);
      const vignetteProg = this.renderer.createShaderProgram(
        'vignette', VIGNETTE_SHADER.vertex, VIGNETTE_SHADER.fragment);

      this.renderer.createFramebuffer('pingA', this.config.width, this.config.height);
      this.renderer.createFramebuffer('pingB', this.config.width, this.config.height);
      this.renderer.createFramebuffer('bloom0', this.config.width, this.config.height);
      this.renderer.createFramebuffer('bloom1', this.config.width, this.config.height);
  ...
  async process(
    ...
    this.renderer.setUniform(gradeProg, 'u_brightness', preset.colorGrading.brightness);
    this.renderer.setUniform(gradeProg, 'u_contrast',   preset.colorGrading.contrast);
    this.renderer.setUniform(gradeProg, 'u_saturation', preset.colorGrading.saturation);
    this.renderer.setUniform(gradeProg, 'u_hue',        preset.colorGrading.hue);
    this.renderer.setUniform(gradeProg, 'u_exposure',   preset.colorGrading.exposure);
    this.renderer.setUniform(gradeProg, 'u_gamma',      preset.colorGrading.gamma);
    this.renderer.setUniform(gradeProg, 'u_shadows',    preset.colorGrading.shadows);
    this.renderer.setUniform(gradeProg, 'u_midtones',   preset.colorGrading.midtones);
    this.renderer.setUniform(gradeProg, 'u_highlights', preset.colorGrading.highlights);
    this.renderer.setUniform(gradeProg, 'u_temperature',preset.colorGrading.temperature);
    this.renderer.setUniform(gradeProg, 'u_tint',       preset.colorGrading.tint);
    this.renderer.setUniform(gradeProg, 'u_vibrance',   preset.colorGrading.vibrance);
    this.renderer.setUniform(gradeProg, 'u_lift',       [0, 0, 0]);
    this.renderer.setUniform(gradeProg, 'u_gain',       [1, 1, 1]);
    this.renderer.setUniform(gradeProg, 'u_filmGrain',  preset.colorGrading.filmGrain);
    ...
    this.renderer.setUniform(chromaProg, 'u_radial',           preset.chromaticAb.radial ? 1 : 0);
    this.renderer.setUniform(chromaProg, 'u_audioReactivity',  this.config.audioReactivity ?? 0);
    ...
    this.renderer.setUniform(vignetteProg, 'u_intensity', preset.vignette.intensity);
    this.renderer.setUniform(vignetteProg, 'u_radius',    preset.vignette.radius);
    this.renderer.setUniform(vignetteProg, 'u_softness',  preset.vignette.softness);
