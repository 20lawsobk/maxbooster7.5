...
export class AudioWorkletEngine {
      ...
      try {
        ...
        this.workletNode.port.onmessage = (event) => {
          ...
          if (type === 'metering') {
            ...
            const peakLeftDb = data.peakLeft > 0 ? 20 * Math.log10(data.peakLeft) : -Infinity;
            const peakRightDb = data.peakRight > 0 ? 20 * Math.log10(data.peakRight) : -Infinity;
            const rmsLeftDb = data.rmsLeft > 0 ? 20 * Math.log10(data.rmsLeft) : -Infinity;
            const rmsRightDb = data.rmsRight > 0 ? 20 * Math.log10(data.rmsRight) : -Infinity;
  ...
  private getTrackMeteringData(trackId: string, analyzer: AnalyserNode): MeteringData | null {
  ...
  setMasterVolume(volume: number): void {
    ...
    const linearGain = Math.pow(10, volume / 20);
    this.masterGain.gain.setTargetAtTime(linearGain, this.audioContext.currentTime, 0.01);
  }
  ...
  private scheduleClipPlayback(clip: ScheduledClip): void {
    ...
    if (currentSample + this.config.sampleRate * this.scheduleAheadTime < clip.startSample) return;
    
    const startTimeOffset = Math.max(0, (clip.startSample - currentSample) / this.config.sampleRate);
    const bufferOffset = currentSample > clip.startSample 
      ? (currentSample - clip.startSample + clip.offsetSamples) / this.config.sampleRate
      : clip.offsetSamples / this.config.sampleRate;
    
    const remainingDuration = (clipEndSample - Math.max(currentSample, clip.startSample)) / this.config.sampleRate;
    ...
    if (clip.fadeInSamples > 0 && startTimeOffset === 0) {
      ...
      clipGain.gain.setValueAtTime(0, this.audioContext.currentTime + startTimeOffset);
      clipGain.gain.linearRampToValueAtTime(clip.gain, this.audioContext.currentTime + startTimeOffset + fadeInDuration);
    }
    
    if (clip.fadeOutSamples > 0) {
      const fadeOutStart = remainingDuration - (clip.fadeOutSamples / this.config.sampleRate);
      if (fadeOutStart > 0) {
        clipGain.gain.setValueAtTime(clip.gain, this.audioContext.currentTime + startTimeOffset + fadeOutStart);
        clipGain.gain.linearRampToValueAtTime(0, this.audioContext.currentTime + startTimeOffset + remainingDuration);
      }
    }
    ...
    source.start(this.audioContext.currentTime + startTimeOffset, bufferOffset, remainingDuration);
  ...
  setPosition(sample: number): void {
    ...
    if (this.workletNode && this.workletReady) {
      this.workletNode.port.postMessage({ type: 'seek', data: { sample: this.state.currentSample } });
    }
    ...
    this.emit({ type: 'position-update', data: { 
  ...
  private startSchedulerLoop(): void {
    const scheduler = () => {
      ...
      if (!this.workletReady) {
        ...
        if (this.state.isLooping && this.state.currentSample >= this.state.loopEnd) {
      ...
      this.clips.forEach(clip => {
        ...
        const lookAheadSamples = this.config.sampleRate * this.scheduleAheadTime;
        
        if (clipStart >= this.state.currentSample && 
            clipStart < this.state.currentSample + lookAheadSamples) {
          if (!this.scheduledSources.has(clip.id) || this.scheduledSources.get(clip.id)!.length === 0) {
            this.scheduleClipPlayback(clip);
          }
        }
      });
  ...
  getLatency(): number {
    ...
    return this.audioContext.baseLatency + (this.audioContext.outputLatency || 0);
  }
