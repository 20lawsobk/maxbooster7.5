...
export function PersonalizationProvider({ children }: PersonalizationProviderProps) {
  ...
  const { data: personalization, isLoading: personalizationLoading, error: personalizationError } = useQuery<UserPersonalization>({
  ...
  const { data: recommendations = [], isLoading: recommendationsLoading } = useQuery<RecommendedAction[]>({
  ...
  const { data: dashboardLayout, isLoading: layoutLoading } = useQuery<{ widgets: DashboardWidget[] }>({
  ...
  const applyDefaultsMutation = useMutation({
    mutationFn: async ({ type, value }: { type: 'artistType' | 'genre'; value: string }) => {
      const response = await apiRequest('POST', '/api/personalization/apply-defaults', { type, value });
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/personalization/preferences'] });
  ...
  const applyScheduleMutation = useMutation({
    mutationFn: async (suggestion: ScheduleSuggestion) => {
      const response = await apiRequest('POST', '/api/personalization/apply-schedule', {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/personalization/smart-schedule'] });
    },
  });

  const saveLayoutMutation = useMutation({
    mutationFn: async (widgets: DashboardWidget[]) => {
      const response = await apiRequest('PUT', '/api/personalization/dashboard-layout', {
        name: 'Custom Layout',
        widgets,
      });
      return response.json();
  ...
  const updateWidgetMutation = useMutation({
    mutationFn: async ({ widgetId, updates }: { widgetId: string; updates: Partial<DashboardWidget> }) => {
      const response = await apiRequest('PUT', `/api/personalization/widget/${widgetId}`, updates);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/personalization/dashboard-layout'] });
  ...
  const trackFeatureMutation = useMutation({
    mutationFn: async (feature: string) => {
      const response = await apiRequest('POST', '/api/personalization/track-feature', { feature });
      return response.json();
    },
    onSuccess: () => {
  ...
  const getDefault = useCallback((category: string, key: string): SmartDefault | undefined => {
    return defaults.find(d => d.category === category && d.key === key);
  }, [defaults]);

  const getDefaultValue = useCallback(<T,>(category: string, key: string, fallback: T): T => {
  ...
  const applyArtistTypeDefaults = useCallback(async (artistType: ArtistType) => {
    await applyDefaultsMutation.mutateAsync({ type: 'artistType', value: artistType });
  }, [applyDefaultsMutation]);

  const applyGenreDefaults = useCallback(async (genre: string) => {
  ...
  const applySchedule = useCallback(async (suggestion: ScheduleSuggestion) => {
    await applyScheduleMutation.mutateAsync(suggestion);
  }, [applyScheduleMutation]);

  const updateWidgetPosition = useCallback(async (widgetId: string, position: number) => {
    await updateWidgetMutation.mutateAsync({ widgetId, updates: { position } });
  }, [updateWidgetMutation]);

  const updateWidgetVisibility = useCallback(async (widgetId: string, visible: boolean) => {
    await updateWidgetMutation.mutateAsync({ widgetId, updates: { visible } });
  }, [updateWidgetMutation]);

  const updateWidgetSize = useCallback(async (widgetId: string, size: WidgetSize) => {
  ...
  const trackInteraction = useCallback((event: InteractionEvent) => {
    setPendingInteractions(prev => [...prev, { ...event, timestamp: new Date() }]);
  }, []);
  ...
  const trackWidgetView = useCallback((widgetId: string, duration: number) => {
  ...
  const getTimeAdjustedWidgets = useCallback((): DashboardWidget[] => {
    ...
    return dashboardLayout.widgets.filter(w => w.visible).sort((a, b) => a.position - b.position);
  }, [dashboardLayout]);
  ...
  const contextValue = useMemo<PersonalizationContextValue>(() => ({
    ...
    recommendations: recommendations.filter(r => !r.completed && !r.dismissed),
...
export function useRecommendedActions() {
  const { recommendations, isLoadingRecommendations, completeAction, dismissAction } = usePersonalization();
  return { recommendations, isLoading: isLoadingRecommendations, completeAction, dismissAction };
}

export function useSmartSchedule() {
  const { scheduleSuggestions, bestPostingTime, isLoadingSchedule, applySchedule } = usePersonalization();
  return { suggestions: scheduleSuggestions, bestTime: bestPostingTime, isLoading: isLoadingSchedule, applySchedule };
}
...
export function useUserDefaults() {
  const { defaults, getDefault, getDefaultValue, applyArtistTypeDefaults, applyGenreDefaults, resetToDefaults } = usePersonalization();
  return { defaults, getDefault, getDefaultValue, applyArtistTypeDefaults, applyGenreDefaults, resetToDefaults };
}

export function useLearningInsights() {
  const { learningInsights, applyInsight, dismissInsight } = usePersonalization();
  return { insights: learningInsights, applyInsight, dismissInsight };
}

export function useInteractionTracking() {
  const { trackInteraction, trackFeatureUsage, trackWidgetView } = usePersonalization();
