...
const roomSizes: Record<
  RoomSettings["size"],
  { label: string; refDistance: number }
> = {
...
export function SpatialAudio({
  ...
  const removeObject = useCallback(
    (objectId: string) => {
      updateValue({
        objects: value.objects.filter((obj) => obj.id !== objectId),
      });
  ...
  useEffect(() => {
    ...
    value.objects.forEach((obj) => {
      ...
      const lfeLevel = obj.mute
        ? 0
        : Math.pow(10, (obj.lfeLevel + value.masterLfe) / 20);
  ...
  const TopDownView = useMemo(() => {
    ...
    return (
      <svg width={size} height={size} className="mx-auto">
        ...
        <text x={10} y={center + 3} fontSize="8" fill="rgba(255,255,255,0.4)">
          L
        </text>
        <text
          x={size - 14}
            ...
            >
              {obj.type === "bed" ? (
                <rect
                  ...
                  rx={2}
                  style={{
                    filter: `drop-shadow(0 0 ${isSelected ? 8 : 4}px ${obj.color})`,
                  }}
                />
              ) : (
                <motion.circle
  ...
  const SideView = useMemo(() => {
    ...
    return (
      <svg width={width} height={height} className="mx-auto">
        ...
        <text x={8} y={18} fontSize="7" fill="rgba(255,255,255,0.3)">
          TOP
        </text>
        <text x={8} y={height - 8} fontSize="7" fill="rgba(255,255,255,0.3)">
          BTM
        </text>
        <text x={20} y={height - 2} fontSize="7" fill="rgba(255,255,255,0.3)">
          FRONT
        </text>
        <text
          x={width - 35}
        ...
        {value.objects.map((obj) => {
          ...
          const normalizedDist = Math.min(obj.distance / 5, 1);
          const x =
            centerX - Math.cos(azimuthRad) * (maxWidth / 2) * normalizedDist;
            ...
            >
              {obj.type === "bed" ? (
                <rect
                  ...
                  rx={2}
                  style={{
                    filter: `drop-shadow(0 0 ${isSelected ? 6 : 3}px ${obj.color})`,
                  }}
                />
              ) : (
                <motion.circle
  ...
  const SpeakerLayoutView = useMemo(() => {
    ...
    const speakers =
      speakerPositions[value.speakerConfig] || speakerPositions["stereo"];
    ...
    return (
      <svg width={size} height={size}>
        ...
        {speakers.map((speaker, i) => {
          ...
          const speakerRadius =
            speaker.height === "top" ? radius * 0.7 : radius;
          ...
          return (
            <g key={i}>
              <circle
                ...
                style={{
                  filter: `drop-shadow(0 0 3px ${isHeight ? "#ffa500" : "#00ccff"})`,
                }}
              />
              <text
                x={x}
      ...
      >
        ...
        <div className="flex items-center gap-2">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  onClick={() =>
                    updateValue({
                      binauralEnabled: !value.binauralEnabled,
                      monitoringMode: value.binauralEnabled
                        ? "speakers"
                        : "binaural",
                    })
                  }
          ...
          <Select
            ...
            onValueChange={(config: SpeakerConfiguration) =>
              updateValue({ speakerConfig: config })
            }
          >
            <SelectTrigger
        ...
        <div className="flex-1 flex flex-col">
          <Tabs defaultValue="position" className="flex-1 flex flex-col">
            ...
            >
              ...
              <TabsTrigger value="room" className="text-[10px]">
                Room
              </TabsTrigger>
            </TabsList>
                    ...
                    >
                      ...
                      <div className="flex justify-around">
                        <Knob
                          ...
                          onChange={(v) =>
                            updateObject(selectedObject.id, { azimuth: v })
                          }
                          min={-180}
                          max={180}
                          defaultValue={0}
                        ...
                        <Knob
                          ...
                          onChange={(v) =>
                            updateObject(selectedObject.id, { elevation: v })
                          }
                          min={-90}
                          max={90}
                          defaultValue={0}
                      ...
                      <div>
                        ...
                        <Slider
                          ...
                          onValueChange={([v]) =>
                            updateObject(selectedObject.id, { distance: v })
                          }
                          min={0.1}
                          max={10}
                          step={0.1}
                      ...
                      <div>
                        ...
                        <Slider
                          ...
                          onValueChange={([v]) =>
                            updateObject(selectedObject.id, { width: v })
                          }
                          min={0}
                          max={180}
                          step={1}
                      ...
                      <div>
                        ...
                        <Slider
                          ...
                          onValueChange={([v]) =>
                            updateObject(selectedObject.id, { lfeLevel: v })
                          }
                          min={-60}
                          max={0}
                          step={0.5}
                ...
                <div className="flex-1 space-y-4">
                  ...
                  >
                    ...
                    <div>
                      ...
                      <Slider
                        ...
                        onValueChange={([v]) =>
                          updateValue({ room: { ...value.room, damping: v } })
                        }
                        min={0}
                        max={1}
                        step={0.01}
                  ...
                  >
                    ...
                    <p
                      ...
                      style={{ color: "var(--studio-text-muted)" }}
                    >
                      Enable HRTF-based binaural rendering for headphone
                      monitoring of spatial audio content.
                    </p>
                  </div>
                </div>
