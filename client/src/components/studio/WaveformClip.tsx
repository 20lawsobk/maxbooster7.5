...
export function WaveformClip({
  ...
  useEffect(() => {
    ...
    if (audioBuffer) {
      ...
      const samplesPerPixel = Math.max(
        1,
        Math.floor(channelData.length / renderWidth),
      );
      ...
      for (let i = 0; i < renderWidth; i++) {
        ...
        for (let j = start; j < end; j++) {
          ...
          if (channelData[j] < 0 && -channelData[j] > min)
            min = -channelData[j];
        }
      ...
      const loadAudioAndGenerateWaveform = async () => {
        try {
          const { generateWaveformPeaksPair, getSharedAudioContext } =
            await import("@/hooks/useAudioContext");
          ...
          const { maxPeaks, minPeaks } = generateWaveformPeaksPair(
            decodedBuffer,
            renderWidth,
          );
          setLocalWaveformData(maxPeaks);
          ...
          logger.warn(
            "Failed to decode audio, using placeholder waveform:",
            error,
          );
          const renderWidth = Math.max(
            Math.floor(calculatedWidth),
  ...
  useEffect(() => {
    ...
    if (showGridLines && barWidth > 0 && secondsPerBar > 0) {
      const barPhaseOffset =
        (startTime % secondsPerBar) * pixelsPerSecond * zoom;
      const firstBarX = (barWidth - barPhaseOffset) % barWidth;

      if (beatWidth > 0 && secondsPerBeat > 0) {
        const beatPhaseOffset =
          (startTime % secondsPerBeat) * pixelsPerSecond * zoom;
    ...
    if (consolidatedWaveform) {
      ...
      gradient.addColorStop(
        0.3,
        muted ? "rgba(90, 90, 90, 0.85)" : `${color}bb`,
      );
      gradient.addColorStop(
        0.5,
        muted ? "rgba(100, 100, 100, 0.75)" : `${color}99`,
      );
      gradient.addColorStop(
        0.7,
        muted ? "rgba(90, 90, 90, 0.85)" : `${color}bb`,
      ...
      gradient.addColorStop(
        0,
        muted ? "rgba(100, 100, 100, 0.8)" : `${color}cc`,
      );
      gradient.addColorStop(
        0.5,
        muted ? "rgba(100, 100, 100, 0.6)" : `${color}99`,
      );
      gradient.addColorStop(
        1,
        muted ? "rgba(100, 100, 100, 0.8)" : `${color}cc`,
    ...
    const dataPointsNeeded = Math.max(
      localWaveformData.length,
      Math.floor(renderWidth),
    );
    const resampledMax =
      localWaveformData.length < dataPointsNeeded
    ...
    const resampledMin = hasMinData
      ? localMinData.length < dataPointsNeeded
        ? resampleWaveform(localMinData, dataPointsNeeded)
        : localMinData
      : resampledMax;
    ...
    const applyFade = (amplitude: number, x: number): number => {
      ...
      if (showFades && fadeOutTime > 0) {
        const fadeOutStart =
          renderWidth - (fadeOutTime / duration) * renderWidth;
        if (x > fadeOutStart)
          a *= (renderWidth - x) / (renderWidth - fadeOutStart);
      }
      return a;
    };
  ...
  const resampleWaveform = (data: number[], targetLength: number): number[] => {
    ...
    for (let i = 0; i < targetLength; i++) {
      ...
      const interpolated =
        data[lower] * (1 - fraction) + data[upper] * fraction;
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        boxShadow: selected
          ? `0 0 12px ${color}55, inset 0 0 20px ${color}11`
    ...
    >
      ...
      >
        ...
        <span className="text-[9px] opacity-70 ml-2 flex items-center gap-1">
          ...
          <span>
            {barsSpanned} bar{barsSpanned !== 1 ? "s" : ""}
          </span>
        </span>
      </div>
