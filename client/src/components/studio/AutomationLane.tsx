...
interface AutomationLaneProps {
  trackId: string;
  parameter: 'volume' | 'pan' | 'effect-param' | 'mute' | 'send-1' | 'send-2' | 'eq-low' | 'eq-mid' | 'eq-high';
  duration: number;
...
const PARAMETER_CONFIG: Record<string, { label: string; min: number; max: number; defaultValue: number; unit: string; color: string }> = {
  ...
  mute: { label: 'Mute', min: 0, max: 1, defaultValue: 0, unit: '', color: '#ef4444' },
  'send-1': { label: 'Send 1', min: 0, max: 1, defaultValue: 0, unit: '', color: '#f59e0b' },
  'send-2': { label: 'Send 2', min: 0, max: 1, defaultValue: 0, unit: '', color: '#06b6d4' },
  'eq-low': { label: 'EQ Low', min: -12, max: 12, defaultValue: 0, unit: 'dB', color: '#8b5cf6' },
  'eq-mid': { label: 'EQ Mid', min: -12, max: 12, defaultValue: 0, unit: 'dB', color: '#ec4899' },
  'eq-high': { label: 'EQ High', min: -12, max: 12, defaultValue: 0, unit: 'dB', color: '#14b8a6' },
};

const AUTOMATION_MODE_CONFIG: Record<AutomationMode, { label: string; color: string; bgColor: string }> = {
  off: { label: 'OFF', color: '#6b7280', bgColor: 'rgba(107, 114, 128, 0.2)' },
  read: { label: 'READ', color: '#22c55e', bgColor: 'rgba(34, 197, 94, 0.2)' },
  write: { label: 'WRITE', color: '#ef4444', bgColor: 'rgba(239, 68, 68, 0.2)' },
  touch: { label: 'TOUCH', color: '#f97316', bgColor: 'rgba(249, 115, 22, 0.2)' },
  latch: { label: 'LATCH', color: '#a855f7', bgColor: 'rgba(168, 85, 247, 0.2)' },
};
...
export function AutomationLane({
  ...
  const { zoom, snapEnabled, snapResolution, automationMode, setAutomationMode, isPlaying } = useStudioStore();
  ...
  const [pencilPoints, setPencilPoints] = useState<{ time: number; value: number }[]>([]);
  const [lineStartPoint, setLineStartPoint] = useState<{ time: number; value: number } | null>(null);
  ...
  }, [currentTime, trackId, parameter, points, interpolateValue, automationMode]);
  ...
  }, [points, selectedPointId, duration, config.color, zoom, drawingTool, lineStartPoint]);
  ...
  const thinPoints = useCallback(() => {
    ...
    for (let i = 1; i < sortedPoints.length - 1; i++) {
      ...
      const expectedValue = prev.value + ((next.value - prev.value) * (curr.time - prev.time)) / (next.time - prev.time);
    ...
    if (thinnedPoints.length < points.length) {
      ...
      toast({ title: `Reduced ${points.length} points to ${thinnedPoints.length}` });
    }
  ...
  const handleCanvasClick = useCallback(
    ...
    [isAdding, duration, snapEnabled, snapResolution, points, onPointsChange, saveAutomation, drawingTool, lineStartPoint]
  );
  ...
  const drag = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (drawingTool === 'pencil' && isTouching) {
        ...
        setPencilPoints(prev => [...prev, { time, value: Math.max(0, Math.min(value, 1)) }]);
        return;
      }
    ...
    [isDragging, selectedPointId, duration, snapEnabled, snapResolution, points, onPointsChange, drawingTool, isTouching]
  );
  ...
  const endDrag = useCallback(() => {
    if (drawingTool === 'pencil' && pencilPoints.length > 1) {
      ...
      const mergedPoints = [...points, ...newPoints].sort((a, b) => a.time - b.time);
    ...
    setIsDragging(false);
    setIsTouching(false);
  }, [isDragging, points, saveAutomation, drawingTool, pencilPoints, duration, onPointsChange]);
  ...
  const getValueLabel = (normalizedValue: number) => {
    ...
    } else if (parameter === 'eq-low' || parameter === 'eq-mid' || parameter === 'eq-high') {
  ...
  const isTouchRecording = automationMode === 'touch' && isTouching && isPlaying;
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          <Circle className="h-2 w-2 flex-shrink-0" style={{ fill: config.color, color: config.color }} />
          ...
          {(isWriting || isTouchRecording) && (
            <div
              ...
              style={{
                background: isWriting ? 'rgba(239, 68, 68, 0.3)' : 'rgba(249, 115, 22, 0.3)',
                color: isWriting ? '#ef4444' : '#f97316',
              }}
            >
              <Circle className="h-1.5 w-1.5" style={{ fill: 'currentColor' }} />
              {isWriting ? 'REC' : 'TOUCH'}
            </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-1">
          <div
            ...
            style={{ background: 'var(--studio-bg-deep)', border: '1px solid var(--studio-border)' }}
          >
            {(Object.keys(AUTOMATION_MODE_CONFIG) as AutomationMode[]).map((mode) => {
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    color: isActive ? mConfig.color : 'var(--studio-text-muted)',
                  }}
          ...
          <div className="w-px h-4 mx-1" style={{ background: 'var(--studio-border)' }} />
          ...
          {isSaving && <Loader2 className="h-3 w-3 animate-spin" style={{ color: 'var(--studio-text-muted)' }} />}
        </div>
      </div>
