  ...
  const startTest = useCallback(() => {
    try {
      const ctx = new AudioContext({
        sampleRate,
        latencyHint: bufferSize / sampleRate,
      });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-[#1c1c24] border border-[#333] text-white p-0 max-w-lg w-full overflow-hidden">
        ...
        <DialogDescription className="sr-only">
          Configure audio hardware, input/output devices, sample rate, and
          buffer size.
        </DialogDescription>
        {/* Header */}
        <div className="flex items-center gap-2.5 px-5 py-4 border-b border-[#2a2a2e] bg-[#181820]">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">
              Configure audio hardware and routing
            </p>
          </div>
        </div>
        ...
        <div className="p-5 space-y-5">
          ...
          {permission === "denied" && (
            <div className="flex items-center gap-2 px-3 py-2 bg-red-900/20 border border-red-800/50 rounded-lg">
              ...
              <p className="text-xs text-red-300">
                Microphone access denied. Enable it in browser settings to list
                input devices.
              </p>
            </div>
          )}
            ...
            <div className="flex items-center justify-between gap-2 px-3 py-2 bg-amber-900/20 border border-amber-800/50 rounded-lg">
              <div className="flex items-center gap-2">
                ...
                <p className="text-xs text-amber-300">
                  Grant microphone access to enumerate all devices.
                </p>
              </div>
              <Button
                size="sm"
                onClick={requestPermission}
                className="h-6 text-xs shrink-0 bg-amber-600 hover:bg-amber-500"
              >
                Allow
              </Button>
            </div>
          ...
          <div className="flex items-center justify-between px-3 py-2 bg-[#252530] rounded-lg border border-[#333]">
            ...
            <div className="text-right">
              <p className="text-xs text-gray-500">
                {formatHz(sampleRate)} · {bufferSize} samples
              </p>
              <p className="text-xs text-gray-600 mt-0.5">
                {calculatedLatency <= 6
                  ? "Ultra-low"
                  : calculatedLatency <= 12
                    ? "Low"
                    : calculatedLatency <= 24
                      ? "Medium"
                      : "High"}{" "}
                latency
              </p>
