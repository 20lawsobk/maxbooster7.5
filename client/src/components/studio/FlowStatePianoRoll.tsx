...
export function FlowStatePianoRoll({
  ...
  const quantizeNotes = useCallback(() => {
    ...
    onNotesChange?.(newNotes);
    toast({
      title: `Quantized ${selectedNotes.size} notes to ${quantizeValue}`,
    });
    ...
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (clickedNote) {
        const newVelocity = Math.max(
          1,
          Math.min(127, Math.round((1 - y / VELOCITY_LANE_HEIGHT) * 127)),
        );
        pushUndo();
  ...
  useEffect(() => {
    ...
    notes.forEach((note) => {
      ...
      const gradient = ctx.createLinearGradient(
        ...
        x,
        VELOCITY_LANE_HEIGHT - barHeight,
      );
