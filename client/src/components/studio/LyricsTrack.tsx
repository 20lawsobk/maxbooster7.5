...
const parseLRC = (content: string): LyricLine[] => {
  ...
  while ((match = regex.exec(content)) !== null) {
    ...
    if (text) {
      ...
      const lineEndTime =
        words.length > 0 ? words[words.length - 1].endTime : startTime + 2;
...
export function LyricsTrack({
  ...
  const [resizingLine, setResizingLine] = useState<{
    id: string;
    edge: "start" | "end";
  } | null>(null);
  ...
  const handleLineClick = useCallback(
    (e: React.MouseEvent, line: LyricLine) => {
      e.stopPropagation();
  ...
  const updateLine = useCallback(
    (lineId: string, updates: Partial<LyricLine>) => {
      const newLines = lines
        .map((l) => (l.id === lineId ? { ...l, ...updates } : l))
        .sort((a, b) => a.startTime - b.startTime);
      setLines(newLines);
      onLyricsChange?.(newLines);
    },
  ...
  const handleRippleEdit = useCallback(
    (lineId: string, timeDelta: number) => {
  ...
  const handleDrag = useCallback(
    (e: React.MouseEvent) => {
      ...
      newStartTime = Math.max(
        0,
        Math.min(newStartTime, duration - lineDuration),
      );

      setPreviewPosition({
        startTime: newStartTime,
        endTime: newStartTime + lineDuration,
      });
    },
      ...
      >
        ...
        <div className="flex items-center gap-1">
          ...
          <Button
            ...
            title={
              teleprompterMode ? "Exit teleprompter mode" : "Teleprompter mode"
            }
          >
            {teleprompterMode ? (
          ...
          return (
            <ContextMenu key={line.id}>
              <ContextMenuTrigger>
                ...
                >
                  <div className="h-full flex items-center px-2 gap-1 overflow-hidden">
                    {editingLineId === line.id ? (
                      ...
                      line.words.map((word, wordIndex) => {
                        const isCurrentWord =
                          isCurrentLine && getCurrentWordIndex === wordIndex;
                        const isPastWord =
                          isCurrentLine && wordIndex < getCurrentWordIndex;
                  ...
                  <div
                    ...
                    onMouseDown={(e) =>
                      handleResizeStart(e, line.id, "start", line)
                    }
                  ...
                  <div
                    ...
                    onMouseDown={(e) =>
                      handleResizeStart(e, line.id, "end", line)
                    }
              ...
              <ContextMenuContent>
                ...
                <ContextMenuItem
                  onClick={() => onTimelineClick?.(line.startTime)}
                >
                ...
                <ContextMenuSeparator />
                <ContextMenuItem
                  onClick={() => handleRippleEdit(line.id, -0.5)}
                >
