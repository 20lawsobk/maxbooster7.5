  ...
  return (
    <>
      ...
      >
        ...
        {transport && <div className="shrink-0">{transport}</div>}
        {arrangerElement}
        ...
        {launcherPanel.visible && (
          <div className="shrink-0">{launcherElement}</div>
        )}
      </div>
    </>
  );
...
interface ArrangerTrackWrapperProps {
  ...
  onSectionResize?: (id: string, newLength: number) => void;
}

export function ArrangerTrackWrapper({
  sections = [],
  bpm = 120,
  pixelsPerBar = 100,
  scrollOffset = 0,
