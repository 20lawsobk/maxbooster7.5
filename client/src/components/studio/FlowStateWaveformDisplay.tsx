...
interface FlowStateWaveformDisplayProps {
  ...
  onClipResize?: (clipId: string, newDuration: number, edge: 'left' | 'right') => void;
...
export function FlowStateWaveformDisplay({
  ...
  const generateMockWaveform = useCallback((clipId: string, clipDuration: number): number[] => {
  ...
  const renderWaveform = useCallback((
    ...
    const waveformData = clip.waveformData || generateMockWaveform(clip.id, clip.duration);
  ...
  }, [isDragging, dragClipId, dragStartX, dragStartTime, beatsPerPixel, resizeEdge, clips, onClipMove, onClipResize]);
  ...
  const handleDoubleClick = useCallback((e: React.MouseEvent, clipId: string) => {
    const clip = clips.find(c => c.id === clipId);
    if (!clip) return;
        ...
        return (
          ...
          >
            ...
            <div className="absolute top-0 left-0 right-0 h-5 px-2 flex items-center bg-gradient-to-b from-black/40 to-transparent">
              <Waves className="h-3 w-3 mr-1" style={{ color: clip.color || trackColor }} />
            ...
            {clip.fadeOut > 0 && (
              <div 
                ...
                style={{ right: (clip.fadeOut / clip.duration) * clipWidth - 6 }}
              />
            )}
          </motion.div>
        );
