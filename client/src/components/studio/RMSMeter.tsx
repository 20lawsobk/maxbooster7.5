...
export function RMSMeter({
  ...
  const applyKWeighting = useCallback(
    (data: Float32Array, sampleRate: number): Float32Array => {
  ...
  const calculateLUFS = useCallback(
    (rmsLeft: number, rmsRight: number): number => {
  ...
  const calculateStereoCorrelation = useCallback(
    (leftData: Float32Array, rightData: Float32Array): number => {
  ...
  useEffect(() => {
    ...
    const colors = {
      background:
        computedStyle.getPropertyValue("--meter-background").trim() ||
        "#1a1a2e",
      green:
        computedStyle.getPropertyValue("--meter-green").trim() || "#22c55e",
      yellow:
        computedStyle.getPropertyValue("--meter-yellow").trim() || "#eab308",
      red: computedStyle.getPropertyValue("--meter-red").trim() || "#ef4444",
      textMuted:
        computedStyle.getPropertyValue("--studio-text-muted").trim() ||
        "#888888",
      text: computedStyle.getPropertyValue("--studio-text").trim() || "#ffffff",
      bgMedium:
        computedStyle.getPropertyValue("--studio-bg-medium").trim() ||
        "#2a2a3e",
      accent:
        computedStyle.getPropertyValue("--studio-accent").trim() || "#8b5cf6",
    };
    ...
    const draw = () => {
      ...
      if (peakLeft > peakHoldLeftRef.current) {
        ...
        peakHoldLeftRef.current = Math.max(
          peakHoldLeftRef.current - peakDecayRate,
          peakLeft,
        );
      }
      ...
      if (peakRight > peakHoldRightRef.current) {
        ...
        peakHoldRightRef.current = Math.max(
          peakHoldRightRef.current - peakDecayRate,
          peakRight,
        );
      }

      truePeakHoldLeftRef.current = Math.max(
        truePeakHoldLeftRef.current,
        truePeakLeft,
      );
      truePeakHoldRightRef.current = Math.max(
        truePeakHoldRightRef.current,
        truePeakRight,
      );
      ...
      if (clipLeft) {
        clipLeftRef.current = true;
        clipTimeLeftRef.current = currentTime;
      } else if (currentTime - clipTimeLeftRef.current > 3000)
        clipLeftRef.current = false;
      if (clipRight) {
        clipRightRef.current = true;
        clipTimeRightRef.current = currentTime;
      } else if (currentTime - clipTimeRightRef.current > 3000)
        clipRightRef.current = false;

      const lufsMomentary = calculateLUFS(
        calculateRMS(weightedLeft),
        calculateRMS(weightedRight),
      );
      shortTermHistoryRef.current.push(lufsMomentary);
      if (shortTermHistoryRef.current.length > 30)
      ...
      const dynamicRange =
        Math.max(peakLeft, peakRight) - Math.min(rmsLeft, rmsRight);
      ...
      setMeteringData({
        ...
        stereoCorrelation,
        dynamicRange,
        clipLeft: clipLeftRef.current,
        clipRight: clipRightRef.current,
      });
      ...
      drawMeterBars(
        ...
        rmsLeft,
        rmsRight,
        peakHoldLeftRef.current,
        peakHoldRightRef.current,
        colors,
      );
      drawLUFSSection(
        ctx,
      ...
      drawClipIndicators(
        ctx,
        clipLeftRef.current,
        clipRightRef.current,
        colors,
      );
      drawTruePeakIndicators(
        ctx,
        truePeakHoldLeftRef.current,
        truePeakHoldRightRef.current,
        colors,
  ...
  const drawLUFSSection = (
    ...
    lufsMeters.forEach(({ label, value, y }) => {
      ...
      const normalized = Math.max(
        0,
        (value - LUFS_MIN) / (LUFS_MAX - LUFS_MIN),
      );
      const gradient = ctx.createLinearGradient(
        startX + 12,
        0,
        startX + 12 + meterWidth,
        0,
      );
  ...
  const drawReadouts = (
    ...
    const readouts = [
      {
        label: "RMS L/R",
        value: `${data.rmsLeft.toFixed(1)} / ${data.rmsRight.toFixed(1)} dB`,
      },
      {
        label: "PEAK L/R",
        value: `${data.peakLeft.toFixed(1)} / ${data.peakRight.toFixed(1)} dB`,
      },
      {
        label: "TRUE PEAK",
        value: `${data.truePeakLeft.toFixed(1)} / ${data.truePeakRight.toFixed(1)} dBTP`,
      },
    ];
  ...
  const drawTruePeakIndicators = (
    ...
    ctx.font = "9px monospace";
    ctx.fillText(
      `${Math.max(tpLeft, tpRight).toFixed(1)} dBTP`,
      startX + 40,
      startY,
    );
  };
