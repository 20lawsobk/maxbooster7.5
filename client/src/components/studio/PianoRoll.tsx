...
export function PianoRoll({
  ...
  const [tool, setTool] = useState<
    "pencil" | "select" | "eraser" | "expression"
  >("pencil");
  ...
  const addExpressionPoint = useCallback(
    (noteId: string, type: ExpressionLaneType, time: number, value: number) => {
      ...
      const newPoint: MPEExpressionPoint = {
        time,
        value: Math.max(0, Math.min(1, value)),
      };

      const existingIndex = expressionData.findIndex(
      ...
      if (existingIndex >= 0) {
        ...
        updatedData = [...expressionData, newPoint].sort(
          (a, b) => a.time - b.time,
        );
      }
  ...
  const handleExpressionCanvasClick = (
    e: React.MouseEvent<HTMLCanvasElement>,
    laneType: ExpressionLaneType,
  ) => {
    ...
    const x = e.clientX - rect.left;
    const laneIndex =
      laneType === "pitchBend" ? 0 : laneType === "pressure" ? 1 : 2;
  ...
  useEffect(() => {
    ...
    notes.forEach((note) => {
      ...
      if (mpeEnabled && note.pitchBend && note.pitchBend.length > 1) {
        ...
        const pressure = mpeEnabled
          ? getPressureValue(note, note.startTime)
          : 1;
      ...
      if (mpeEnabled && note.slide) {
        ...
        ctx.fillRect(
          baseX + width - 4,
          baseY + height - slideIndicatorHeight,
          3,
          slideIndicatorHeight,
        );
      }
  ...
  useEffect(() => {
    ...
    const editNote = editingExpressionNote
      ? notes.find((n) => n.id === editingExpressionNote)
      : null;
    ...
    if (editNote) {
      ...
      laneTypes.forEach((laneType, laneIndex) => {
        const laneY = laneIndex * EXPRESSION_LANE_HEIGHT;
        const expressionData = editNote[laneType] as
          | MPEExpressionPoint[]
          | undefined;
        ...
        if (expressionData && expressionData.length > 0) {
          ...
          expressionData.forEach((point, i) => {
            ...
            if (laneType === "pitchBend") {
              y =
                laneY +
                EXPRESSION_LANE_HEIGHT / 2 -
                (point.value * EXPRESSION_LANE_HEIGHT) / 2;
            } else {
              y =
                laneY +
                EXPRESSION_LANE_HEIGHT -
                point.value * EXPRESSION_LANE_HEIGHT;
            }
          ...
          if (laneType === "pitchBend") {
            endY =
              laneY +
              EXPRESSION_LANE_HEIGHT / 2 -
              (lastPoint.value * EXPRESSION_LANE_HEIGHT) / 2;
          } else {
            endY =
              laneY +
              EXPRESSION_LANE_HEIGHT -
              lastPoint.value * EXPRESSION_LANE_HEIGHT;
          }
    ...
    notes.forEach((note) => {
      ...
      laneTypes.forEach((laneType, laneIndex) => {
        const laneY = laneIndex * EXPRESSION_LANE_HEIGHT;
        const expressionData = note[laneType] as
          | MPEExpressionPoint[]
          | undefined;
        const color = laneColors[laneType];
    ...
    <div className="absolute top-12 right-4 bg-zinc-900 border border-zinc-700 rounded-lg p-4 shadow-xl z-50 w-80">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setMpeZoneConfig((prev) => ({
                    ...prev,
                    lowerZone: {
                      ...prev.lowerZone,
                      masterChannel: parseInt(v),
                    },
                  }))
                }
              >
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs w-8 text-center">
                {mpeZoneConfig.lowerZone.pitchBendRange}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-zinc-700 pt-4 space-y-2">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <span className="text-xs w-8 text-center">
                {mpeZoneConfig.upperZone.pitchBendRange}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="mt-4 pt-4 border-t border-zinc-700">
        <div className="flex items-center gap-2 text-xs text-zinc-400">
          ...
          <span>
            Lower Zone: CH {mpeZoneConfig.lowerZone.masterChannel + 1}-
            {mpeZoneConfig.lowerZone.masterChannel +
              mpeZoneConfig.lowerZone.memberChannels}
          </span>
        </div>
        <div className="flex items-center gap-2 text-xs text-zinc-400 mt-1">
          ...
          <span>
            Upper Zone: CH{" "}
            {mpeZoneConfig.upperZone.masterChannel -
              mpeZoneConfig.upperZone.memberChannels}
            -{mpeZoneConfig.upperZone.masterChannel}
          </span>
        </div>
      </div>
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            />
            <Label htmlFor="mpe-toggle" className="text-xs cursor-pointer">
              MPE
            </Label>
          </div>
          ...
          {mpeEnabled && (
            <>
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <Label
                  htmlFor="expression-lanes-toggle"
          ...
          <>
            ...
            <div className="flex items-center gap-2">
              ...
              <Select
                value={
                  notes
                    .find((n) => n.id === editingExpressionNote)
                    ?.pitchBendRange?.toString() || "48"
                }
                onValueChange={(v) => {
                  if (editingExpressionNote) {
                    updateNote(editingExpressionNote, {
                      pitchBendRange: parseInt(v),
                    });
                  }
                }}
              >
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        >
          ...
          {showExpressionLanes && mpeEnabled && (
            <div
              style={{ height: totalExpressionHeight }}
            ...
            {showExpressionLanes && mpeEnabled && (
              <canvas
                ...
                onClick={(e) => {
                  ...
                  const laneTypes: ExpressionLaneType[] = [
                    "pitchBend",
                    "pressure",
                    "slide",
                  ];
                ...
                className={
                  tool === "expression" && editingExpressionNote
                    ? "cursor-crosshair"
                    : "cursor-default"
                }
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-blue-400">
              Lower (CH{mpeZoneConfig.lowerZone.masterChannel + 1}-
              {mpeZoneConfig.lowerZone.masterChannel +
                mpeZoneConfig.lowerZone.memberChannels}
              )
            </span>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-1">
              {Array.from(
                new Set(
                  notes.filter((n) => n.mpeChannel).map((n) => n.mpeChannel!),
                ),
              )
                .sort((a, b) => a - b)
                .map((ch) => (
                  <span
                    key={ch}
                    className="px-1 py-0.5 bg-zinc-800 rounded text-green-400"
                  >
                    {ch}
                  </span>
                ))}
