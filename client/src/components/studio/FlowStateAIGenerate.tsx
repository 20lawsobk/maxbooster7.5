...
const SPECIFIC_INSTRUMENTS: Record<
  string,
...
export function FlowStateAIGenerate({
  ...
  const handleGenerate = () => {
    ...
    const fullPrompt = [specificName, selectedGenre.name, prompt.trim()]
      .filter(Boolean)
      .join(" ");
          ...
          >
                  ...
                  {INSTRUMENT_GROUPS.map((inst) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        const firstSpecific =
                          SPECIFIC_INSTRUMENTS[inst.id]?.[0];
                        if (firstSpecific)
                          setSelectedSpecific(firstSpecific.id);
                      }}
                      className={cn(
            ...
            <div className="p-4 border-t border-white/5">
              ...
              <p className="text-center text-xs text-white/30 mt-3">
                AI will create a {bars}-bar{" "}
                {selectedInstrument.name.toLowerCase()} pattern in{" "}
                {selectedGenre.name} style at {tempo} BPM
              </p>
            </div>
          </motion.div>
        </motion.div>
