...
interface CollaborationOutcomesProps {
  ...
  onResolveConflict?: (conflictId: string, resolution: 'mine' | 'theirs' | 'merge') => Promise<void>;
...
export function CollaborationOutcomes({
  ...
  const handleResolveConflict = useCallback(async (resolution: 'mine' | 'theirs' | 'merge') => {
  ...
  const getSyncStatusIcon = () => {
    switch (syncState.status) {
      case 'synced': return <Cloud className="w-4 h-4 text-green-400" />;
      case 'syncing': return <RefreshCw className="w-4 h-4 text-blue-400 animate-spin" />;
      case 'offline': return <CloudOff className="w-4 h-4 text-yellow-400" />;
  ...
  const getSyncStatusText = () => {
    switch (syncState.status) {
      ...
      case 'syncing': return `Syncing${syncState.progress ? ` (${syncState.progress}%)` : '...'}`;
      case 'offline': return 'Working offline';
      case 'error': return syncState.error || 'Sync error';
    }
  };
  ...
  useEffect(() => {
    ...
    if (lastOutcome && Date.now() - lastOutcome.timestamp.getTime() < 5000) {
      if (lastOutcome.type === 'join' && lastOutcome.user?.id !== currentUserId) {
        toast({
          ...
          description: (
            <div className="flex items-center gap-2">
              <Avatar className="w-6 h-6">
                ...
                <AvatarFallback style={{ backgroundColor: lastOutcome.user?.color }}>
    ...
    <div className={cn("flex flex-col h-full bg-zinc-950 text-white", className)}>
      ...
      <div className="p-4 border-b border-zinc-800">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-xs font-medium text-zinc-500 uppercase">Active Collaborators</h3>
          <div className="flex -space-x-2">
            {onlineCollaborators.slice(0, 4).map(collab => (
              <Avatar key={collab.id} className="w-6 h-6 border-2 border-zinc-950">
                <AvatarImage src={collab.avatar} />
                <AvatarFallback style={{ backgroundColor: collab.color }} className="text-[10px]">
        ...
        <ScrollArea className="h-32">
          <div className="space-y-2">
            {collaborators.map(collab => (
              <div key={collab.id} className="flex items-center gap-2 p-2 bg-zinc-900 rounded-lg">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {collab.currentAction && (
                    <p className="text-xs text-zinc-500 truncate">{collab.currentAction}</p>
                  )}
                </div>
              </div>
            ))}
      ...
      <div className="flex-1 overflow-hidden">
        <div className="p-4">
          <h3 className="text-xs font-medium text-zinc-500 uppercase mb-2">Activity</h3>
            ...
            <div className="space-y-2">
              {outcomes.length === 0 ? (
                ...
                outcomes.slice(0, 20).map(outcome => (
                  ...
                  >
                    {outcome.type === 'join' && <UserPlus className="w-3 h-3 text-green-400 mt-0.5" />}
                    {outcome.type === 'leave' && <UserMinus className="w-3 h-3 text-red-400 mt-0.5" />}
                    {outcome.type === 'sync' && <Cloud className="w-3 h-3 text-blue-400 mt-0.5" />}
                    {outcome.type === 'conflict' && <GitBranch className="w-3 h-3 text-amber-400 mt-0.5" />}
                    {outcome.type === 'save' && <Save className="w-3 h-3 text-green-400 mt-0.5" />}
                    {outcome.type === 'lock' && <Lock className="w-3 h-3 text-purple-400 mt-0.5" />}
                    {outcome.type === 'unlock' && <Unlock className="w-3 h-3 text-blue-400 mt-0.5" />}
          ...
          {activeConflict && (
            <div className="py-4 space-y-4">
              <div className="p-3 bg-zinc-900 rounded-lg">
                <h4 className="text-sm font-medium mb-2">Conflicting Changes</h4>
                {activeConflict.users.map(user => (
                  <div key={user.id} className="flex items-center justify-between py-1">
                    <span className="text-sm text-zinc-300">{user.name}</span>
                    <Badge variant="outline" className="text-xs">{user.version}</Badge>
                  </div>
                ))}
              </div>
