...
export function FlowStateSpatialVisualizer({
  tracks,
  isPlaying,
}: FlowStateSpatialVisualizerProps) {
  ...
  const activeTracks = useMemo(
    () =>
      tracks.filter(
        (t) => !t.mute && (tracks.every((tr) => !tr.solo) || t.solo),
      ),
    [tracks],
  );
    ...
    const animate = () => {
      ...
      particlesRef.current.forEach((particle, i) => {
        ...
        const size =
          particle.size *
          (0.5 + particle.z * 0.5) *
          (isPlaying ? 1 + Math.sin(Date.now() / 200 + i) * 0.2 : 1);

        const glow = ctx.createRadialGradient(px, py, 0, px, py, size * 3);
        glow.addColorStop(
          0,
          particle.color +
            Math.floor(particle.intensity * 80)
              .toString(16)
              .padStart(2, "0"),
        );
