...
import {
  FileText, Plus, Trash2, GripHorizontal, X, Clock, ArrowUpDown,
  AlignLeft, Mic, Download, Upload, ChevronDown, Music2, LayoutList,
  Layers, Edit3, Check, Hash
} from 'lucide-react';
...
export const SECTION_META: Record<SectionType, { color: string; bg: string; label: string; accent: string }> = {
  intro:        { color: '#818cf8', bg: '#1e1b4b', label: 'Intro',       accent: '#4f46e5' },
  verse:        { color: '#60a5fa', bg: '#0c1a2e', label: 'Verse',       accent: '#2563eb' },
  'pre-chorus': { color: '#a78bfa', bg: '#1e0a3c', label: 'Pre-Chorus',  accent: '#7c3aed' },
  chorus:       { color: '#f472b6', bg: '#2d0a1e', label: 'Chorus',      accent: '#db2777' },
  bridge:       { color: '#fbbf24', bg: '#1c0a00', label: 'Bridge',      accent: '#d97706' },
  outro:        { color: '#94a3b8', bg: '#0f172a', label: 'Outro',       accent: '#475569' },
  custom:       { color: '#34d399', bg: '#022c22', label: 'Custom',      accent: '#059669' },
};

const SECTION_ORDER: SectionType[] = ['intro', 'verse', 'pre-chorus', 'chorus', 'bridge', 'outro', 'custom'];
...
function importPlainText(text: string): LyricSection[] {
  ...
  for (const block of blocks) {
    ...
    for (const t of SECTION_ORDER) {
      if (lbl.startsWith(t.replace('-', ' ').replace('-', ' '))) { type = t; break; }
    }
    ...
    const sec: LyricSection = {
      ...
      number: num,
      lines: contentLines.filter(l => l.trim()).map(l => {
    ...
    if (sec.lines.length === 0) sec.lines = [{ id: genId(), text: '', timestamp: 0 }];
    result.push(sec);
  }
...
const LyricLineRow = memo(function LyricLineRow({
  line, idx, isActive, isPlaying, showTimestamps, showChords, showPerformer,
  tapMode, sectionColor, sectionBg, fontSizeClass, onUpdate, onDelete, onAddAfter, onStamp, onSeek
}: LyricLineRowProps) {
  ...
  return (
    ...
    style={isActive && isPlaying ? { backgroundColor: sectionBg + 'cc' } : undefined}
    >
      {showChords && (
      ...
      <div className="flex items-center gap-2">
        ...
        {showTimestamps && (
          <button
            onClick={() => line.timestamp > 0 && onSeek ? onSeek(line.timestamp) : onStamp(line.id)}
            className={cn(
        ...
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === 'Enter') { e.preventDefault(); onAddAfter(line.id); }
            if (e.key === 'Backspace' && line.text === '') { e.preventDefault(); onDelete(line.id); }
          }}
          className={cn(
...
interface SectionViewProps {
  ...
  fontSizeClass: string;
  songView: boolean;
  onUpdateLine: (sId: string, lId: string, field: keyof LyricLine, val: string) => void;
...
const SectionView = memo(function SectionView({
  section, isActive, activeLineId, isPlaying, showTimestamps, showChords, showPerformer,
  tapMode, fontSizeClass, songView, onUpdateLine, onDeleteLine, onAddLine, onStamp, onSeek
}: SectionViewProps) {
    ...
    <div className={cn('mb-4', songView && !isActive && 'opacity-60')}>
      {songView && (
        <div className="flex items-center gap-2 mb-2">
          <div className="h-px flex-1" style={{ backgroundColor: meta.color + '33' }} />
          <span
      ...
      {section.lines.map((line, idx) => (
        <LyricLineRow
          ...
          onUpdate={(lId, field, val) => onUpdateLine(section.id, lId, field, val)}
...
export function LyricsPanel({
  isPlaying, playheadPosition, tempo, onSeek, defaultHeight = 300,
  ...
  const updateLine = useCallback((sectionId: string, lineId: string, field: keyof LyricLine, value: string) => {
    onSectionsChange(sections.map(s =>
      s.id !== sectionId ? s : {
        ...s,
        lines: s.lines.map(l => l.id !== lineId ? l : { ...l, [field]: value }),
      }
    ));
  }, [sections, onSectionsChange]);

  const addLine = useCallback((sectionId: string, afterLineId: string) => {
    onSectionsChange(sections.map(s => {
  ...
  const stampLine = useCallback((sectionId: string, lineId: string) => {
    onSectionsChange(sections.map(s =>
      s.id !== sectionId ? s : {
        ...s,
  ...
  const commitLabelEdit = useCallback(() => {
    ...
    onSectionsChange(sections.map(s =>
      s.id !== editingLabelId ? s : { ...s, label: editingLabelText.trim() || s.label }
    ));
  ...
  const visibleSections = songView ? sections : (activeSection ? [activeSection] : []);

  return (
    <div style={{ height }} className="flex flex-col bg-[#141418] border-t border-[#2a2a30] shrink-0 overflow-hidden">
      ...
      <div className="h-5 bg-[#0d0d12] border-b border-[#222] shrink-0 relative overflow-hidden">
        <div className="flex h-full">
          {sections.map((sec, i) => {
            ...
            return (
              <button
                key={sec.id}
                onClick={() => { onActiveSectionChange(sec.id); if (songView) setSongView(false); }}
                title={sec.label}
                ...
                style={{ backgroundColor: meta.accent + '55', borderColor: meta.color + '22' }}
              >
                <span className="text-[9px] font-bold truncate px-1 leading-none" style={{ color: meta.color }}>
                  {sec.label}
                </span>
              </button>
            );
        ...
        {/* Playhead needle */}
        {sections.length > 0 && (() => {
          const totalTs = Math.max(...allLines.filter(l => l.timestamp > 0).map(l => l.timestamp), 60);
      ...
      <div className="flex items-center gap-0.5 px-2 py-1 border-b border-[#222] bg-[#18181e] shrink-0 overflow-x-auto">
        ...
        <span className="text-xs font-semibold text-emerald-400 mr-1.5 shrink-0">Lyrics</span>
        ...
        {!songView && (
          <div className="flex items-center gap-0.5 overflow-x-auto flex-1 min-w-0">
            {sections.map((sec) => {
              ...
              return (
                <div key={sec.id} className="flex items-center shrink-0 group/tab">
                  {editingLabelId === sec.id ? (
                    ...
                    <button
                      ...
                      style={active ? { backgroundColor: meta.bg + 'dd', color: meta.color, boxShadow: `0 0 0 1px ${meta.color}33` } : {}}
                    >
                      <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: meta.color }} />
                      {sec.label}
                    </button>
                  )}
                  {active && sections.length > 1 && editingLabelId !== sec.id && (
                    <button
        ...
        <Tooltip>
          ...
          <TooltipContent>TAP mode — Enter stamps timestamp while playing</TooltipContent>
        </Tooltip>
          ...
          <TooltipTrigger asChild>
            <button
              ...
              className={cn(
                ...
                autoScroll ? 'text-emerald-400 bg-emerald-600/10' : 'text-gray-600 hover:text-gray-300 hover:bg-white/5'
              )}
            >
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              ...
              className={cn(
                'h-6 px-1.5 rounded text-[10px] flex items-center gap-1 shrink-0 transition-colors',
                showChords ? 'text-amber-400 bg-amber-600/10' : 'text-gray-600 hover:text-gray-300 hover:bg-white/5'
              )}
            >
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              ...
              className={cn(
                'h-6 px-1.5 rounded text-[10px] flex items-center gap-1 shrink-0 transition-colors',
                showPerformer ? 'text-purple-400 bg-purple-600/10' : 'text-gray-600 hover:text-gray-300 hover:bg-white/5'
              )}
            >
              <Mic className="h-2.5 w-2.5" />
              <span>Voice</span>
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              ...
              className={cn(
                'h-6 px-1.5 rounded text-[10px] flex items-center gap-1 shrink-0 transition-colors',
                showTimestamps ? 'text-blue-400 bg-blue-600/10' : 'text-gray-600 hover:text-gray-300 hover:bg-white/5'
              )}
            >
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              ...
              className={cn(
                'h-6 px-1.5 rounded text-[10px] flex items-center gap-1 shrink-0 transition-colors',
                songView ? 'text-indigo-400 bg-indigo-600/10' : 'text-gray-600 hover:text-gray-300 hover:bg-white/5'
              )}
            >
              {songView ? <Layers className="h-2.5 w-2.5" /> : <LayoutList className="h-2.5 w-2.5" />}
              <span>{songView ? 'Song' : 'Section'}</span>
            </button>
          </TooltipTrigger>
          <TooltipContent>{songView ? 'All sections stacked (Song View)' : 'One section at a time (Section View)'}</TooltipContent>
        </Tooltip>
          ...
          {(['sm', 'md', 'lg'] as const).map(s => (
            <button
              ...
              className={cn(
                ...
                fontSize === s ? 'bg-white/15 text-white' : 'text-gray-600 hover:text-gray-300'
              )}
            >
              {s === 'sm' ? 'S' : s === 'md' ? 'M' : 'L'}
            </button>
      ...
      {tapMode && (
        <div className="bg-red-900/30 border-b border-red-600/30 px-4 py-1 flex items-center gap-2 shrink-0">
          ...
          <span className="text-xs text-red-400">
            TAP mode — press <kbd className="bg-black/30 px-1 rounded text-[10px] border border-red-600/30">Enter</kbd> on a line while playing to stamp its timestamp
          </span>
        </div>
      )}
        ...
        {visibleSections.length > 0 ? (
          <>
            {!songView && activeSection && (
              <div className="flex items-center gap-2 mb-3 pb-2" style={{ borderBottom: `1px solid ${SECTION_META[activeSection.type].color}22` }}>
                {editingLabelId === activeSection.id ? (
                  <div className="flex items-center gap-1">
                    ...
                    />
                    <button onClick={commitLabelEdit} className="text-emerald-400 hover:text-emerald-300">
                  ...
                  >
                    <span
                      ...
                      style={{ color: SECTION_META[activeSection.type].color, backgroundColor: SECTION_META[activeSection.type].bg }}
                    >
                      {activeSection.label}
                    </span>
                ...
                <span className="text-[10px] text-gray-700">
                  {activeSection.lines.length} {activeSection.lines.length === 1 ? 'line' : 'lines'}
                </span>
              </div>
            )}
