...
function generatePeaksFromWaveformData(waveformData: Float32Array | number[], targetSamples: number): { min: number; max: number }[] {
...
function generatePlaceholderPeaks(duration: number, samplesPerSecond: number = 100): { min: number; max: number }[] {
  ...
  for (let i = 0; i < numSamples; i++) {
    ...
    const amplitude = Math.max(0.1, Math.min(1, (envelope * 0.6 + variation + noise + 0.3)));
...
export const PixiWaveformRenderer = forwardRef<PixiWaveformRendererRef, PixiWaveformRendererProps>(
  function PixiWaveformRenderer(
    ...
    const dragStateRef = useRef<Map<string, { isDragging: boolean; dragStartX: number; clipStartTime: number }>>(new Map());
    ...
    const drawClips = useCallback(() => {
      ...
      clips.forEach((clip) => {
        ...
        if (!container) {
          ...
          dragStateRef.current.set(clip.id, { isDragging: false, dragStartX: 0, clipStartTime: 0 });
          ...
          container.on('globalpointermove', (event: PIXI.FederatedPointerEvent) => {
            ...
            if (dragState?.isDragging && onClipDrag) {
              ...
              const newStartTime = Math.max(0, dragState.clipStartTime + deltaTime);
    ...
    }, [clips, pixelsPerSecond, height, selectedClipIds, onClipClick, drawWaveform]);
    ...
    const updatePlayhead = useCallback((position: number) => {
      ...
      playhead.stroke({ width: 2, color: hexToPixiColor(playheadColor), alpha: 1 });
