...
const DRUM_KITS: DrumKit[] = [
  { id: "standard", name: "Standard Kit", sounds: GM_DRUM_MAP },
  {
    id: "808",
    name: "TR-808",
    sounds: GM_DRUM_MAP.map((s) => ({ ...s, name: `808 ${s.name}` })),
  },
  {
    id: "909",
    name: "TR-909",
    sounds: GM_DRUM_MAP.map((s) => ({ ...s, name: `909 ${s.name}` })),
  },
  {
    id: "acoustic",
    name: "Acoustic",
    sounds: GM_DRUM_MAP.map((s) => ({ ...s, name: `Acoustic ${s.name}` })),
  },
  {
    id: "electronic",
    name: "Electronic",
    sounds: GM_DRUM_MAP.map((s) => ({ ...s, name: `Electronic ${s.name}` })),
  },
];
...
export function DrumEditor({
  ...
  const [selectedDrumIndex, setSelectedDrumIndex] = useState<number | null>(
    null,
  );
  ...
  const totalSteps = useMemo(
    () => pattern.bars * pattern.resolution,
    [pattern.bars, pattern.resolution],
  );
  ...
  const getHitAt = useCallback(
    (step: number, drumIndex: number): DrumHit | undefined => {
      return pattern.hits.find(
        (h) => h.step === step && h.drumIndex === drumIndex,
      );
    },
    [pattern.hits],
  );
  ...
  const addHit = useCallback(
    (step: number, drumIndex: number, velocity: number = 100) => {
  ...
  const updateHit = useCallback(
    (hitId: string, updates: Partial<DrumHit>) => {
      updatePattern({
  ...
  const handleGridClick = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (
        ...
        step < totalSteps &&
        drumIndex >= 0 &&
        drumIndex < selectedKit.sounds.length
      ) {
        if (velocityEditMode) {
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      const newVelocity = Math.max(
        1,
        Math.min(127, editingHit.velocity + velocityChange),
      );
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "c" && (e.ctrlKey || e.metaKey)) {
        ...
        if (selectedDrumIndex !== null) {
          const hitsToRemove = pattern.hits.filter(
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <Select
              ...
              onValueChange={(id) =>
                setSelectedKit(
                  DRUM_KITS.find((k) => k.id === id) || DRUM_KITS[0],
                )
              }
            >
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center">
              <Button
                ...
                size="sm"
                onClick={() =>
                  updatePattern({ bars: Math.max(1, pattern.bars - 1) })
                }
                className="h-8 w-8"
              ...
              <Button
                ...
                size="sm"
                onClick={() =>
                  updatePattern({ bars: Math.min(4, pattern.bars + 1) })
                }
                className="h-8 w-8"
          ...
          <div className="flex-1 overflow-auto" ref={gridRef}>
            ...
            >
              <canvas
                ...
                className={
                  velocityEditMode ? "cursor-ns-resize" : "cursor-pointer"
                }
                style={{ display: "block" }}
              />
            </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-xs text-muted-foreground">
              {selectedDrumIndex !== null
                ? selectedKit.sounds[selectedDrumIndex]?.name
                : "Select a drum"}
            </span>
          </div>
        </div>
