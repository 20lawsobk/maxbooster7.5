...
export function LayoutGrid({
  ...
  const calculateLayoutMode = useCallback(
    (width: number): LayoutMode => {
      ...
      const compactLayoutMin =
        TIMELINE_MIN_WIDTH + Math.max(inspectorWidth, browserWidth);
  ...
  const getGridColumns = useCallback(() => {
    ...
    const inspectorFraction = inspectorCollapsed
      ? "48px"
      : `clamp(180px, ${Math.round(availableWidth * 0.2)}px, 280px)`;
    const browserFraction = browserCollapsed
      ? "48px"
      : `clamp(180px, ${Math.round(availableWidth * 0.22)}px, 320px)`;
      ...
      >
        ...
        >
          <div className="flex items-center justify-between">
            <div
              className={`flex-1 overflow-hidden ${topBarCollapsed ? "h-0" : ""}`}
            >
              {topBar}
            </div>
            <Button
        ...
        <div
          ...
          style={{
            ...
            borderColor: "var(--studio-border)",
            boxShadow:
              "var(--studio-shadow-lg), inset 0 1px 0 rgba(255,255,255,0.05)",
          }}
        >
          ...
          <div
            className={`transition-all ${dockCollapsed ? "h-0 overflow-hidden" : ""}`}
          >
            {dock}
          </div>
        </div>
      ...
      >
        ...
        >
          ...
          <div className="flex-1 overflow-auto">
            {activePanel === "inspector" || activePanel === "timeline"
              ? inspector
              : browser}
          </div>
        </div>
