    ...
    return (
      <ContextMenu>
        ...
        >
          <ContextMenuItem
            onClick={onCreate}
            data-testid="context-create-pattern"
          >
          ...
          <ContextMenuItem
            onClick={onRecord}
            data-testid="context-record-pattern"
          >
  ...
  return (
    <ContextMenu>
        ...
        >
          <div className="absolute inset-0 p-2 flex flex-col justify-between">
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <div className="w-4 h-4 rounded bg-black/30 flex items-center justify-center">
                          {pattern.followAction === "next" && (
                            <SkipForward className="w-2.5 h-2.5 text-white/70" />
                          )}
                          {pattern.followAction === "stop" && (
                            <Square className="w-2.5 h-2.5 text-white/70" />
                          )}
                          {pattern.followAction === "any" && (
                            <Shuffle className="w-2.5 h-2.5 text-white/70" />
                          )}
                          {(pattern.followAction === "first" ||
                            pattern.followAction === "last" ||
                            pattern.followAction === "previous") && (
                      ...
                      <TooltipContent>
                        <p>
                          ...
                          {
                            FOLLOW_ACTIONS.find(
                              (a) => a.value === pattern.followAction,
                            )?.label
                          }
                        </p>
                      </TooltipContent>
            ...
            <div className="absolute bottom-0 left-0 right-0 h-1 flex gap-px">
              {Array.from({ length: Math.min(pattern.loopLength * 4, 16) }).map(
                (_, i) => (
                  <div
                    key={i}
      ...
      >
          ...
          >
            {FOLLOW_ACTIONS.map(({ value, label, icon: Icon }) => (
              <ContextMenuItem
                key={value}
                onClick={() =>
                  onUpdate({ followAction: value as Pattern["followAction"] })
                }
              >
          ...
          >
            {QUANTIZATION_OPTIONS.map(({ value, label }) => (
              <ContextMenuItem
                ...
                onClick={() =>
                  onUpdate({
                    launchQuantization: value as Pattern["launchQuantization"],
                  })
                }
              >
                {label}
        ...
        <ContextMenuItem
          onClick={onDuplicate}
          data-testid="context-duplicate-pattern"
        >
...
export function PatternEditor({
  ...
  const [globalQuantization, setGlobalQuantization] =
    useState<Pattern["launchQuantization"]>("1bar");
  ...
  const handleDragStart = useCallback(
    (e: React.DragEvent, pattern: Pattern) => {
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent, targetTrackId: string, targetSceneId: string) => {
      e.preventDefault();
      e.stopPropagation();
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <Select
              ...
              onValueChange={(value) =>
                setGlobalQuantization(value as Pattern["launchQuantization"])
              }
            >
              <SelectTrigger
      ...
      <ScrollArea className="flex-1">
        <div className="flex">
          ...
          <div className="flex-1 overflow-x-auto">
            <div
              className="flex"
              style={{ minWidth: `${tracks.length * 140}px` }}
            >
              {tracks.map((track) => (
                <div
                  key={track.id}
                ...
                >
                  ...
                  {sortedScenes.map((scene) => {
                    const pattern = patternsByCell.get(
                      `${track.id}-${scene.id}`,
                    );
                    const isPlaying = pattern
                      ? playingPatternIds.includes(pattern.id)
                      : false;
                    const isQueued = pattern
                      ? queuedPatternIds.includes(pattern.id)
                      : false;
                    ...
                    return (
                      ...
                      >
                        <PatternCell
                          ...
                          onUpdate={(updates) =>
                            pattern && onPatternUpdate(pattern.id, updates)
                          }
                          onDuplicate={() =>
                            pattern && onPatternDuplicate(pattern.id)
                          }
                          onDelete={() =>
                            pattern && onPatternDelete(pattern.id)
                          }
