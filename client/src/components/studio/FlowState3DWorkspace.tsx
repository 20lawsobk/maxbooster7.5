...
export function FlowState3DWorkspace({
  ...
  const [viewMode, setViewMode] = useState<'spatial' | 'circular' | 'grid'>('spatial');
  ...
  const spawnParticles = useCallback((trackNode: TrackNode, count: number) => {
    const colorIndex = Math.floor(Math.random() * colors.particleColors.length);
  ...
  const render = useCallback(() => {
    ...
    sortedNodes.forEach(node => {
      ...
      const nodeGradient = ctx.createRadialGradient(proj.x, proj.y, 0, proj.x, proj.y, size);
      nodeGradient.addColorStop(0, node.color);
      nodeGradient.addColorStop(0.7, node.color.replace(')', ', 0.6)').replace('rgb', 'rgba'));
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    trackNodesRef.current.forEach((node, id) => {
      const proj = project3D(node.x, node.y, node.z, canvas.width, canvas.height);
