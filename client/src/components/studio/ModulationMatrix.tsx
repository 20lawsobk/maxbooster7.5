...
export function ModulationMatrix({
  ...
  const updateLFO = useCallback(
    (id: string, updates: Partial<LFOConfig>) => {
  ...
  const updateEnvelope = useCallback(
    (id: string, updates: Partial<EnvelopeConfig>) => {
  ...
  const addRoute = useCallback(
    (sourceType: ModulationRoute["sourceType"], sourceId: string) => {
  ...
  const updateRoute = useCallback(
    (id: string, updates: Partial<ModulationRoute>) => {
  ...
  const getSourceName = useCallback(
    (route: ModulationRoute): string => {
      if (route.sourceType === "lfo") {
        return lfos.find((l) => l.id === route.sourceId)?.name || "Unknown LFO";
      }
      if (route.sourceType === "envelope") {
        return (
          envelopes.find((e) => e.id === route.sourceId)?.name || "Unknown Env"
        );
      }
      return (
        route.sourceType.charAt(0).toUpperCase() + route.sourceType.slice(1)
      );
    },
    [lfos, envelopes],
  );
        ...
        <div className="flex items-center gap-2">
          <Switch
            ...
            onCheckedChange={(checked) =>
              updateLFO(lfo.id, { enabled: checked })
            }
          />
          <Input
            value={lfo.name}
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="col-span-2">
          ...
          <Select
            value={lfo.waveform}
            onValueChange={(v) =>
              updateLFO(lfo.id, { waveform: v as LFOWaveform })
            }
          >
      ...
      <div className="flex items-center gap-4 mt-3">
        ...
        <div className="flex items-center gap-2">
          <Switch
            ...
            onCheckedChange={(checked) =>
              updateLFO(lfo.id, { retrigger: checked })
            }
          />
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <Switch
            checked={env.enabled}
            onCheckedChange={(checked) =>
              updateEnvelope(env.id, { enabled: checked })
            }
          />
          <Input
            value={env.name}
        ...
        <div className="flex items-center gap-1">
          <Select
            value={env.type}
            onValueChange={(v) =>
              updateEnvelope(env.id, { type: v as EnvelopeType })
            }
          >
            <SelectTrigger className="h-6 w-20 text-xs">
              <SelectValue />
        ...
        {!isPlaying && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/40">
            <span className="text-xs text-gray-400">
              Click play to preview modulation
            </span>
          </div>
        )}
      </div>
      ...
      <ScrollArea className="h-[400px]">
        <div className="p-2">
          <div className="mb-2">
            <button
              onClick={() =>
                setExpandedSection(expandedSection === "lfos" ? null : "lfos")
              }
          ...
          <div className="mb-2">
            <button
              onClick={() =>
                setExpandedSection(
                  expandedSection === "envelopes" ? null : "envelopes",
                )
              }
              className="w-full flex items-center justify-between p-2 rounded hover:bg-white/5"
            >
              <div className="flex items-center gap-2">
                ...
                >
                  <path
                    d="M2 20L6 8L10 14L14 4L18 12L22 6"
                    strokeWidth="2"
                    strokeLinecap="round"
                  />
                </svg>
                  ...
                  <div className="p-2 space-y-2">
                    {routes.length === 0 ? (
                      <div className="text-center py-4 text-xs text-gray-500">
                        No routes configured. Click the link icon on an LFO or
                        Envelope to add a route.
                      </div>
                    ) : (
                      routes.map((route) => (
                        <div
                        ...
                        >
                          ...
                          <Select
                            ...
                            onValueChange={(v) =>
                              updateRoute(route.id, { targetParameter: v })
                            }
                          >
