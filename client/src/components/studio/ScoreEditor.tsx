...
export function ScoreEditor({
  ...
  const { tempo, timeSignature, snapEnabled, snapResolution } =
    useStudioStore();
  ...
  const canvasWidth = useMemo(
    () =>
      ...
      KEY_SIG_WIDTH +
      TIME_SIG_WIDTH +
      totalMeasures * MEASURE_WIDTH * (zoom / 100) +
      50,
    [totalMeasures, zoom],
  );
  ...
  const timeToX = useCallback(
    (time: number): number => {
      const contentStart =
        LEFT_MARGIN + CLEF_WIDTH + KEY_SIG_WIDTH + TIME_SIG_WIDTH;
      return (
        contentStart + (time / beatsPerMeasure) * MEASURE_WIDTH * (zoom / 100)
      );
    },
  ...
  const xToTime = useCallback(
    (x: number): number => {
      const contentStart =
        LEFT_MARGIN + CLEF_WIDTH + KEY_SIG_WIDTH + TIME_SIG_WIDTH;
      return (
        ((x - contentStart) / (MEASURE_WIDTH * (zoom / 100))) * beatsPerMeasure
      );
    },
    [beatsPerMeasure, zoom],
  );
  ...
  const yToPitch = useCallback(
    (y: number): { pitch: number; clef: "treble" | "bass" } => {
      ...
      const position = Math.round(
        (staffY + STAFF_HEIGHT / 2 - y) / (STAFF_LINE_SPACING / 2),
      );
  ...
  const findNoteAt = useCallback(
    (x: number, y: number): MIDINote | null => {
  ...
  const drawTrebleClef = useCallback(
  ...
  const drawBassClef = useCallback(
    (ctx: CanvasRenderingContext2D, x: number, y: number) => {
      ctx.font = "40px serif";
      ctx.fillStyle = "var(--studio-text)";
      ctx.fillText("𝄢", x, y + 15);
  ...
  const drawKeySignature = useCallback(
    (ctx: CanvasRenderingContext2D, x: number, clef: "treble" | "bass") => {
      const keySig = KEY_SIGNATURES[keySignature];
      if (!keySig) return;
      ...
      if (keySig.sharps) {
        const sharpPositions =
          clef === "treble" ? [-1, 2, -2, 1, 4, 0, 3] : [1, 4, 0, 3, 6, 2, 5];
      ...
      if (keySig.flats) {
        const flatPositions =
          clef === "treble" ? [2, -1, 3, 0, 4, 1, 5] : [4, 1, 5, 2, 6, 3, 7];
  ...
  const drawTimeSignature = useCallback(
    (ctx: CanvasRenderingContext2D, x: number, staffTop: number) => {
  ...
  const drawNote = useCallback(
    (ctx: CanvasRenderingContext2D, note: MIDINote, isSelected: boolean) => {
      ...
      if (
        ...
        y > staffBottom + STAFF_LINE_SPACING / 2
      ) {
        ...
        if (y < staffTop) {
          for (
            let ly = staffTop - STAFF_LINE_SPACING;
            ly >= y - STAFF_LINE_SPACING / 2;
          ...
          for (
            let ly = staffBottom + STAFF_LINE_SPACING;
            ly <= y + STAFF_LINE_SPACING / 2;
            ly += STAFF_LINE_SPACING
          ) {
            ctx.beginPath();
      ...
      if (showDynamics && note.dynamic) {
        ...
        ctx.fillText(
          ...
          clef === "treble"
            ? TREBLE_STAFF_TOP - 10
            : BASS_STAFF_TOP + STAFF_HEIGHT + 20,
        );
      }
      ...
      if (showLyrics && note.lyric) {
        ...
        ctx.fillText(
          ...
          x,
          clef === "bass"
            ? BASS_STAFF_TOP + STAFF_HEIGHT + 25
            : TREBLE_STAFF_TOP + STAFF_HEIGHT + 65,
        );
        ctx.textAlign = "left";
      }
    },
  ...
  const handleExportPDF = useCallback(() => {
    ...
    if (printWindow) {
      ...
      toast({
        ...
        description: "Please allow pop-ups to export your score as PDF.",
      });
    }
  }, [toast, canvasRef]);
  ...
  const setDynamicForSelected = useCallback(
    (dynamic: string) => {
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <Select
              ...
              onValueChange={(v) => setNoteDuration(parseFloat(v))}
            >
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent>
              {DYNAMICS.map((d) => (
                <DropdownMenuItem
                  key={d}
                  onClick={() => setDynamicForSelected(d)}
                >
          ...
          <Tooltip>
            ...
            <TooltipContent>
              Toggle Lyrics (Double-click note to edit)
            </TooltipContent>
          </Tooltip>
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              size="sm"
              onClick={() =>
                setScrollX(Math.min(canvasWidth - 800, scrollX + 200))
              }
              className="h-6 w-6 p-0"
            >
