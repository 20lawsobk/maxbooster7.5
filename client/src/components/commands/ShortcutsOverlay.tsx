        ...
        <>
          ...
          >
                          ...
                          {categoryShortcuts.map(shortcut => (
                            ...
                            >
                              ...
                              <div className="flex items-center gap-1">
                                {formatKey(shortcut.key, shortcut.modifiers).map((k, i) => (
                                  <kbd 
                                    key={i}
            ...
            <div className="flex items-center justify-between p-3 border-t border-zinc-800 text-xs text-zinc-500">
              <p>
                Press <kbd className="px-1 py-0.5 bg-zinc-800 rounded">?</kbd> or <kbd className="px-1 py-0.5 bg-zinc-800 rounded">{platform.mod}+/</kbd> anytime to show this guide
              </p>
...
export function ShortcutHint({ shortcut, size = 'sm' }: ShortcutHintProps) {
  ...
  return (
    <span className="inline-flex items-center gap-0.5">
      {keys.map((k, i) => (
        <kbd key={i} className={cn("bg-zinc-800 border border-zinc-700 rounded", sizeClasses[size])}>
          {k}
        </kbd>
      ))}
    </span>
...
export function ShortcutTooltipContent({ description, shortcut }: { description: string; shortcut: string }) {
  return (
