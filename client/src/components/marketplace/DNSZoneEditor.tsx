...
const RECORD_TYPE_INFO: Record<
  string,
  { description: string; placeholder: string; fields: string[] }
> = {
  A: {
    description: "Points domain to an IPv4 address",
    placeholder: "192.168.1.1",
    fields: ["value"],
  },
  AAAA: {
    description: "Points domain to an IPv6 address",
    placeholder: "2001:0db8:85a3::8a2e:0370:7334",
    fields: ["value"],
  },
  CNAME: {
    description: "Alias to another domain name",
    placeholder: "example.com",
    fields: ["value"],
  },
  MX: {
    description: "Mail server for this domain",
    placeholder: "mail.example.com",
    fields: ["value", "priority"],
  },
  TXT: {
    description: "Text record (SPF, DKIM, verification)",
  ...
  SRV: {
    ...
    fields: ["value", "priority", "weight", "port"],
  },
};
...
export function DNSZoneEditor({ storefrontId, domain }: DNSZoneEditorProps) {
  ...
  const {
    ...
    queryFn: async () => {
      const resp = await apiRequest(
        "GET",
        `/api/dns/${storefrontId}/records?domain=${encodeURIComponent(domain)}`,
      );
      return resp.json();
    },
  ...
  const { data: templatesData } = useQuery({
    ...
    queryFn: async () => {
      const resp = await apiRequest(
        "GET",
        `/api/dns/${storefrontId}/templates`,
      );
      return resp.json();
    },
  });
  ...
  const addRecordMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        title: "Record Added",
        description: "DNS record has been created.",
      });
    ...
    onError: () => {
      toast({
        title: "Failed",
        description: "Could not add DNS record.",
        variant: "destructive",
      });
    },
  });
  ...
  const updateRecordMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        ...
        description: "DNS record has been updated.",
      });
    ...
    onError: () => {
      toast({
        title: "Failed",
        description: "Could not update DNS record.",
        variant: "destructive",
      });
    },
  });
  ...
  const deleteRecordMutation = useMutation({
    ...
    onError: () => {
      toast({
        title: "Failed",
        description: "Could not delete DNS record.",
        variant: "destructive",
      });
    },
  });
  ...
  const saveTemplateMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        ...
        description: "DNS template has been saved.",
      });
  ...
  const applyTemplateMutation = useMutation({
    mutationFn: async (templateId: string) => {
      const resp = await apiRequest(
        "POST",
        `/api/dns/${storefrontId}/templates/${templateId}/apply`,
        { domain },
      );
      return resp.json();
    },
    onSuccess: () => {
      toast({
        ...
        description: "DNS records from template have been applied.",
      });
  ...
  const deleteTemplateMutation = useMutation({
    mutationFn: async (templateId: string) => {
      const resp = await apiRequest(
        "DELETE",
        `/api/dns/${storefrontId}/templates/${templateId}`,
      );
      return resp.json();
    },
    onSuccess: () => {
  ...
  const templates = useMemo(
    () => (templatesData?.templates as DnsTemplate[]) || [],
    [templatesData],
  );
  ...
  const handleRefresh = useCallback(async () => {
    ...
    toast({
      ...
      description: "DNS records have been reloaded.",
    });
  }, [refetchRecords, toast]);

  const handleExportZone = useCallback(() => {
    window.open(
      `/api/dns/${storefrontId}/export?domain=${encodeURIComponent(domain)}`,
      "_blank",
    );
  }, [storefrontId, domain]);
  ...
  const RecordForm = useMemo(() => {
    const typeInfo =
      RECORD_TYPE_INFO[editingRecord.type] || RECORD_TYPE_INFO["A"];
    return (
      <div className="space-y-4">
                ...
                {RECORD_TYPES.map((t) => (
                  <SelectItem key={t} value={t}>
                    <div className="flex items-center gap-2">
                      <Badge
                        variant="outline"
                        className={`text-xs ${recordTypeColor(t)}`}
                      >
                        {t}
                      </Badge>
                      <span className="text-xs text-muted-foreground">
                        {RECORD_TYPE_INFO[t]?.description}
                      </span>
                    </div>
                  </SelectItem>
                ))}
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Use @ for the root domain, or enter a subdomain name (e.g., www,
            mail)
          </p>
        </div>
      ...
      <Card className="border-dashed">
        <CardContent className="py-12 text-center">
          ...
          <p className="text-muted-foreground">
            Set a custom domain first to manage DNS records.
          </p>
        </CardContent>
      </Card>
    );
    ...
    <div className="space-y-4">
      <Card className="border-purple-500/20 bg-purple-500/5">
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Point your domain nameservers to Max Booster to activate full DNS
            management — no third-party registrar API needed.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {MAXBOOSTER_NAMESERVERS.map((ns) => (
                <div
                  key={ns}
                  className="flex items-center justify-between p-2 rounded-md border bg-background/50 font-mono text-sm"
                >
                  <span>{ns}</span>
                  <Button
                    variant="ghost"
                    ...
                    onClick={() => {
                      ...
                      toast({
                        title: "Copied",
                        description: `${ns} copied to clipboard.`,
                      });
                    }}
                  >
            ...
            <p className="text-xs text-muted-foreground pt-1">
              Log into your domain registrar and set the nameserver to the value
              above. Changes propagate within 1-48 hours.
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex flex-wrap items-center gap-2 mb-4">
            ...
            >
              <RefreshCw
                className={`w-4 h-4 mr-1 ${recordsLoading ? "animate-spin" : ""}`}
              />
              Refresh
            </Button>
          ...
          {recordsLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground mb-2">
                No DNS records yet. Add your first record to get started.
              </p>
                    ...
                    {records.map((record, idx) => (
                      ...
                      >
                        <td className="px-4 py-3">
                          <Badge
                            variant="outline"
                            className={`text-xs font-mono ${recordTypeColor(record.type)}`}
                          >
                            {record.type}
                          </Badge>
                        </td>
      ...
      <Dialog open={showAddRecord} onOpenChange={setShowAddRecord}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Create a new DNS record for {domain}
            </DialogDescription>
          </DialogHeader>
          {RecordForm}
          <DialogFooter>
      ...
      <Dialog open={showEditRecord} onOpenChange={setShowEditRecord}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Update the DNS record for {domain}
            </DialogDescription>
          </DialogHeader>
          {RecordForm}
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowEditRecord(false)}>
              Cancel
            </Button>
            <Button
              onClick={() => {
          ...
          {selectedRecord && (
            <div className="p-3 rounded-lg border bg-muted/30 space-y-1">
              <div className="flex items-center gap-2">
                <Badge
                  variant="outline"
                  className={recordTypeColor(selectedRecord.type)}
                >
                  {selectedRecord.type}
                </Badge>
      ...
      <Dialog open={showSaveTemplate} onOpenChange={setShowSaveTemplate}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Save the current DNS configuration as a reusable template.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                placeholder="e.g., Standard Website Setup"
              />
            </div>
            <div>
            ...
            <p className="text-sm text-muted-foreground">
              This will save {records.length} record
              {records.length !== 1 ? "s" : ""} as a template.
            </p>
          </div>
          <DialogFooter>
            <Button
      ...
      <Dialog open={showApplyTemplate} onOpenChange={setShowApplyTemplate}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Apply a saved DNS configuration to {domain}. This will replace
              existing records.
            </DialogDescription>
          </DialogHeader>
                ...
                <div className="flex flex-wrap gap-1 mt-2">
                  {tmpl.records.map((r, i) => (
                    <Badge
                      key={i}
                      variant="outline"
                      className={`text-xs ${recordTypeColor(r.type)}`}
                    >
                      {r.type}: {r.name}
                    </Badge>
                  ))}
