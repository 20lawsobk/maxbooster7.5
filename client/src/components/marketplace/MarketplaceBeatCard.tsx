      ...
      <CardContent className="p-0 flex flex-col flex-1">
        ...
        >
          ...
          {(!showImage || !imageLoaded) && (
            <div className="absolute inset-0 flex flex-col items-center justify-center text-white/90 pointer-events-none">
              <Headphones className="w-16 h-16 mb-2 drop-shadow-lg" strokeWidth={1.5} />
        ...
        <div className="p-4 flex flex-col flex-1 space-y-3">
          ...
          <div className="flex items-center justify-between gap-2 text-xs text-muted-foreground">
            ...
            <div className="inline-flex items-center gap-1.5" role="group" aria-label="Rate beat">
              {[1, 2, 3, 4, 5].map((star) => (
                <button
                  key={star}
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {tier?.fileFormats && tier.fileFormats.length > 1 && (
                          <Badge variant="outline" className="text-[9px] px-1.5 py-0 h-4">
                            {tier.fileFormats.map((f) => f.toUpperCase()).join('+')}
                          </Badge>
                        )}
