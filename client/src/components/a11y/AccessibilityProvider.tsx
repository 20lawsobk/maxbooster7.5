...
export function AccessibilityProvider({
  ...
  const [fontSize, setFontSizeState] = useState<FontSize>(
    () => defaultFontSize || getStoredFontSize(),
  );
  const [colorBlindMode, setColorBlindModeState] = useState<ColorBlindMode>(
    () => defaultColorBlindMode || getStoredColorBlindMode(),
  );
  ...
  const focusMainContent = useCallback(() => {
    ...
    if (mainContent) {
      ...
      mainContent.scrollIntoView({
        behavior: reducedMotion.prefersReducedMotion ? "auto" : "smooth",
      });
  ...
  useEffect(() => {
    ...
    root.style.setProperty(
      "--a11y-font-scale",
      String(fontSizeScales[fontSize]),
    );
