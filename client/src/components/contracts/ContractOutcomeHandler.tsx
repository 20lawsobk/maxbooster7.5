...
import { CheckCircle, XCircle, AlertTriangle, Clock, FileText, Send, Users, PenTool, Ban, History } from 'lucide-react';
...
const outcomeConfig: Record<ContractOutcome, {
  title: string;
  description: string;
  ...
  template_selected: {
    ...
    description: 'Contract template has been selected. Fill in the details to continue.',
    icon: FileText,
    variant: 'default',
  },
  contract_drafted: {
    title: 'Contract Drafted',
    description: 'Your contract has been drafted with all parties. Review before sending.',
    icon: FileText,
    variant: 'success',
  },
  contract_customization_saved: {
  ...
  payments_distributed: {
    ...
    description: 'Payments have been automatically distributed according to splits.',
    icon: CheckCircle,
    variant: 'success',
  },
  roles_credits_defined: {
...
export function ContractOutcomeHandler({ outcome, details, onAcknowledge }: ContractOutcomeHandlerProps) {
  ...
  useEffect(() => {
    if (outcome && outcome !== shown) {
      ...
      if (details?.signedCount !== undefined && details?.totalCount !== undefined) {
      ...
      if (details?.splitTotal !== undefined && outcome === 'split_percentages_defined') {
...
export function getOutcomeVariant(outcome: ContractOutcome): 'default' | 'success' | 'warning' | 'destructive' {
