...
import { AlertCircle, CheckCircle, AlertTriangle, Plus, Trash2, Eye } from 'lucide-react';
...
export function ContractBuilder({ 
    ...
    mutationFn: async () => {
      ...
      const res = await fetch('/api/contracts/validate', {
        ...
        body: JSON.stringify({ 
          ...
          variables: template.variables.includes('splits') ? { ...variables, splits } : variables 
        }),
      });
      return res.json();
    },
  ...
  const addSplit = () => {
    setSplits(prev => [...prev, { name: '', percentage: 0, role: 'Contributor' }]);
  };
  ...
  const handleSubmit = () => {
    ...
    onSubmit(template.variables.includes('splits') ? { ...variables, splits } : variables);
  };
  ...
  const getVariableType = (variable: string): 'text' | 'number' | 'textarea' | 'date' => {
    if (variable.includes('Price') || variable.includes('Fee') || variable.includes('Amount') || 
        variable.includes('Rate') || variable.includes('Percentage') || variable.includes('Limit') ||
    ...
    if (variable.includes('description') || variable.includes('terms') || variable.includes('Terms')) {
      return 'textarea';
    }
    return 'text';
  };
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="space-y-4">
          ...
          <ScrollArea className="h-[400px] pr-4">
            <div className="space-y-4">
              {template.variables.filter(v => v !== 'splits').map((variable) => {
        ...
        <div className="space-y-4">
          ...
          <Card className={cn(
            "p-4",
            validation?.valid === true && "border-green-500/50 bg-green-50/50 dark:bg-green-950/20",
            validation?.valid === false && "border-red-500/50 bg-red-50/50 dark:bg-red-950/20",
