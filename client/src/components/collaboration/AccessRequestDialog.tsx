...
interface AccessRequestDialogProps {
  ...
  onSubmitRequest?: (accessLevel: AccessLevel, message?: string) => Promise<void>;
...
const ACCESS_LEVELS: { value: AccessLevel; label: string; icon: typeof Eye; description: string }[] = [
  {
    value: 'view',
    label: 'View Only',
    icon: Eye,
...
export function AccessRequestDialog({
  ...
  }, [onSubmitRequest, selectedAccess, requestMessage, projectId, onOutcome, onOpenChange, toast]);
  ...
  const handleApprove = useCallback(async (request: AccessRequest) => {
    ...
    try {
      ...
      const outcomeType = request.requestedAccess === 'view'
        ? 'view_only_access_granted'
      ...
      toast({
        ...
        description: (
          <div className="flex items-center gap-2">
            ...
            <span>{request.requesterName} now has {request.requestedAccess} access</span>
          </div>
        ),
      });
  ...
  const handleUpgrade = useCallback(async (userId: string, userName: string, newLevel: AccessLevel) => {
    if (!onUpgrade) return;
      ...
      toast({
        ...
        description: (
          <div className="flex items-center gap-2">
            ...
            <span>{userName}'s access upgraded to {newLevel}</span>
          </div>
        ),
      });
  ...
  const handleDowngrade = useCallback(async (userId: string, userName: string, newLevel: AccessLevel) => {
      ...
      toast({
        ...
        description: (
          <div className="flex items-center gap-2">
            ...
            <span>{userName}'s access downgraded to {newLevel}</span>
          </div>
        ),
      });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={cn("max-w-lg bg-zinc-950 border-zinc-800", className)}>
        <DialogHeader>
          ...
          <div className="space-y-6">
            {currentAccess && (
              <div className="p-3 bg-zinc-900 rounded-lg">
                <p className="text-xs text-zinc-500 mb-1">Current Access Level</p>
                <div className="flex items-center gap-2">
                  {ACCESS_LEVELS.find(l => l.value === currentAccess)?.icon && (
                  ...
                  <span className="text-sm font-medium capitalize">{currentAccess}</span>
                </div>
              </div>
            )}
            ...
            <div className="space-y-3">
              ...
              >
                {ACCESS_LEVELS.map((level) => {
                  ...
                  const isDowngrade = currentAccess === 'edit' && (level.value === 'view' || level.value === 'comment');
                  ...
                  return (
                    <div
                      ...
                      onClick={() => !isCurrentLevel && setSelectedAccess(level.value)}
                    >
                      <RadioGroupItem
                        value={level.value}
                        id={level.value}
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          <Label
                            htmlFor={level.value}
                            className={cn("cursor-pointer", isCurrentLevel && "cursor-not-allowed")}
                          >
                            {level.label}
                          </Label>
                          {isCurrentLevel && (
              ...
              {pendingRequests.length === 0 ? (
                ...
                <AnimatePresence mode="popLayout">
                  {pendingRequests.map((request) => {
                    ...
                    const RequestIcon = ACCESS_LEVELS.find(l => l.value === request.requestedAccess)?.icon || Eye;
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between">
                              <div>
                                ...
                                <p className="text-xs text-zinc-500">{request.requesterEmail}</p>
                              </div>
