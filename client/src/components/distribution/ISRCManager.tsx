...
export function ISRCManager({ releaseId, onCodeAssigned }: ISRCManagerProps) {
  ...
  const generateCodeMutation = useMutation({
    mutationFn: async (request: CodeGenerationRequest) => {
      const response = await apiRequest(
        "POST",
        "/api/distribution/codes/generate",
        request,
      );
  ...
  useMutation({
    mutationFn: async ({
      ...
      const response = await apiRequest(
        "POST",
        "/api/distribution/codes/validate",
        { code, type },
      );
      return response.json();
    },
  ...
  useMutation({
    mutationFn: async ({
      ...
      const response = await apiRequest(
        "POST",
        `/api/distribution/codes/${codeId}/assign`,
        {
          releaseId,
          trackId,
        },
  ...
  const revokeCodeMutation = useMutation({
    mutationFn: async (codeId: string) => {
      const response = await apiRequest(
        "POST",
        `/api/distribution/codes/${codeId}/revoke`,
      );
      return response.json();
    },
    onSuccess: () => {
  ...
  const validateCode = (code: string) => {
    ...
    if (type === "isrc") {
      ...
      if (upcPattern.test(code)) {
        ...
        setValidationResult({
          valid: false,
          message: "Invalid UPC format. Expected: 12-13 digits",
        });
      }
    }
  };
                ...
                <div className="space-y-4 py-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      value={generateCount}
                      onChange={(e) =>
                        setGenerateCount(parseInt(e.target.value) || 1)
                      }
                    />
                    <p className="text-xs text-muted-foreground">
                      Available:{" "}
                      {generateType === "isrc"
                        ? dormantStats.availableISRC
                        : dormantStats.availableUPC}
                    </p>
                  </div>
                </div>
                <DialogFooter>
                      ...
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => {
                          if (checked) {
                            ...
                            setSelectedCodes(
                              selectedCodes.filter((id) => id !== code.id),
                            );
                          }
                        }}
                      />
