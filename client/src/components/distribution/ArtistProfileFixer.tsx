...
export default function ArtistProfileFixer({ open, onOpenChange, profile, onSubmitted }: Props) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        ...
        description: 'Your release mapping will be corrected on the next distribution update.',
      });
      setTargetUri('');
      setNotes('');
      onSubmitted();
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            If your music is appearing under the wrong artist page on Spotify, use this tool to request a re-mapping.
          </DialogDescription>
        </DialogHeader>
        ...
        {profile.fixerPending && (
          <Alert>
            ...
            <AlertDescription>
              A fixer request is already pending for this profile. Status: <strong>{profile.fixerStatus}</strong>.
              Submitting a new one will replace the existing request.
            </AlertDescription>
          </Alert>
        )}
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{profile.artistName}</p>
            {profile.spotifyArtistUri && (
              <p className="text-xs text-muted-foreground font-mono mt-0.5">Current: {profile.spotifyArtistUri}</p>
            )}
          </div>
          ...
          <Alert variant="destructive" className="bg-orange-50 border-orange-200 text-orange-800">
            ...
            <AlertDescription className="text-orange-800">
              Only use this if your music is incorrectly grouped with another artist. The fixer request will update
              your artist URI and apply to all future releases. Past releases may require manual DSP support tickets.
            </AlertDescription>
          </Alert>

          <div className="space-y-2">
            <Label>
              Correct Spotify Artist URI <span className="text-destructive">*</span>
            </Label>
            <Input
            ...
            <p className="text-xs text-muted-foreground">
              Find your correct artist URI on Spotify: right-click your artist page → Share → Copy Spotify URI.
            </p>
          </div>
