...
export interface BatchEditField {
  key: string;
  label: string;
  type: 'text' | 'number' | 'select' | 'textarea' | 'checkbox' | 'date' | 'tags' | 'multiselect';
...
export function BatchEditDialog({
  ...
  const [applyResult, setApplyResult] = useState<{ success: string[]; failed: Array<{ id: string; error: string }> } | null>(null);
    ...
    if (open) {
      setChanges(
        fields.reduce((acc, field) => {
          acc[field.key] = { enabled: false, value: field.defaultValue ?? getDefaultValue(field) };
          return acc;
  ...
  const handleApply = useCallback(async () => {
    ...
    try {
      const result = await onApply(enabledChanges, selectedItems.map(item => item.id));
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[85vh]">
        <DialogHeader>
          ...
          <DialogDescription>
            {description || `Edit ${selectedItems.length} selected item${selectedItems.length > 1 ? 's' : ''}. Only enabled fields will be updated.`}
          </DialogDescription>
        </DialogHeader>
        ...
        {applyResult ? (
              ...
              {applyResult.failed.length === 0 ? (
                <div className="text-center">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {applyResult.success.length} item{applyResult.success.length !== 1 ? 's' : ''} updated
                  </p>
                </div>
              ) : (
                <div className="w-full space-y-4">
                  <div className="flex items-center justify-center gap-8">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-green-500">{applyResult.success.length}</div>
                      <div className="text-xs text-muted-foreground">Succeeded</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-destructive">{applyResult.failed.length}</div>
                  ...
                  <ScrollArea className="h-40 border rounded-lg">
                    <div className="p-2 space-y-2">
                      {applyResult.failed.map((failure, i) => (
                        <div key={i} className="flex items-start gap-2 p-2 bg-destructive/5 rounded text-sm">
          ...
          <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'edit' | 'preview')}>
            <TabsList className="grid w-full grid-cols-2">
              ...
              <TabsTrigger value="preview" className="gap-2" disabled={enabledChangesCount === 0}>
              ...
              <ScrollArea className="h-[400px] pr-4">
                <div className="space-y-3">
                  {previews.map((preview) => (
                    <div key={preview.id} className="border rounded-lg p-3 space-y-2">
        ...
        {isApplying && (
          <div className="space-y-2">
            ...
            <p className="text-sm text-center text-muted-foreground">
              Applying changes to {applyProgress.current} of {applyProgress.total} items...
            </p>
          </div>
        )}
        ...
        <DialogFooter className="flex items-center justify-between sm:justify-between">
          <div className="text-sm text-muted-foreground">
            {enabledChangesCount} field{enabledChangesCount !== 1 ? 's' : ''} selected for {selectedItems.length} item{selectedItems.length !== 1 ? 's' : ''}
          </div>
          <div className="flex gap-2">
            {!applyResult && (
              <>
                ...
                <Button
                  onClick={handleApply}
                  disabled={isApplying || enabledChangesCount === 0 || Object.keys(errors).length > 0}
                >
                  {isApplying ? (
                    <>
...
export const releaseEditFields: BatchEditField[] = [
  ...
  { key: 'genre', label: 'Primary Genre', type: 'select', group: 'Metadata', options: [
    { value: 'pop', label: 'Pop' },
  ...
  { key: 'explicit', label: 'Explicit Content', type: 'checkbox', group: 'Metadata' },
  { key: 'label', label: 'Label Name', type: 'text', placeholder: 'Enter label name', group: 'Metadata' },
  { key: 'releaseDate', label: 'Release Date', type: 'date', group: 'Scheduling' },
  { key: 'territories', label: 'Territories', type: 'select', group: 'Distribution', options: [
...
export const trackEditFields: BatchEditField[] = [
  ...
  { key: 'tags', label: 'Tags', type: 'tags', placeholder: 'chill, summer, vibes' },
...
export const postEditFields: BatchEditField[] = [
  ...
  { key: 'hashtags', label: 'Hashtags', type: 'tags', placeholder: '#music #newrelease' },
...
export const fileEditFields: BatchEditField[] = [
  { key: 'folder', label: 'Move to Folder', type: 'select', options: [
    { value: '/', label: 'Root' },
    { value: '/audio', label: 'Audio' },
    { value: '/images', label: 'Images' },
    { value: '/documents', label: 'Documents' },
  ]},
  { key: 'tags', label: 'Tags', type: 'tags', placeholder: 'project, final, v2' },
