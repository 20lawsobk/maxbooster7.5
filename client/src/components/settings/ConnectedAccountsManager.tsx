...
const providerCategories: Record<
  string,
...
export function ConnectedAccountsManager() {
  ...
  const refreshMutation = useMutation({
    mutationFn: async (accountId: string) => {
      const res = await apiRequest(
        "POST",
        `/api/auth/connected-accounts/${accountId}/refresh`,
      );
      return res.json();
    },
    onSuccess: () => {
    ...
    onError: () => {
      toast({
        ...
        description:
          "Could not refresh the connection. You may need to reconnect.",
        variant: "destructive",
      });
    },
  });
  ...
  const updatePermissionsMutation = useMutation({
    mutationFn: async ({
      ...
      const res = await apiRequest(
        "PUT",
        `/api/auth/connected-accounts/${accountId}/permissions`,
        permissions,
      );
      return res.json();
    },
  ...
  const metaConnected =
    connectedProviders.has("facebook") || connectedProviders.has("instagram");
    ...
    <div className="space-y-6">
      {expiredAccounts.length > 0 && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            {expiredAccounts.length} connected account
            {expiredAccounts.length > 1 ? "s need" : " needs"} attention. Please
            refresh or reconnect them.
          </AlertDescription>
        </Alert>
      )}
          ...
          {accounts.length === 0 ? (
            <div className="text-center py-8 bg-muted/10 rounded-lg border border-dashed">
              ...
              <p className="text-muted-foreground mb-4">
                Connect your streaming and social media accounts to sync your
                music.
              </p>
            </div>
          ) : (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-left">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {connectingProvider === provider
                              ? "Connecting..."
                              : providerCategories[provider]?.label ||
                                "Connect"}
                          </p>
                        </div>
                      </div>
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {selectedAccount && providerNames[selectedAccount.provider]}{" "}
              Permissions
            </DialogTitle>
            <DialogDescription>
      ...
      >
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() =>
                selectedAccount && disconnectMutation.mutate(selectedAccount.id)
              }
