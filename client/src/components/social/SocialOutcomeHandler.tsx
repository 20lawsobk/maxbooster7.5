  ...
  if (showInline) {
    return (
      <Alert className={`${config.bg} border-0`}>
        <StatusIcon
          className={`h-4 w-4 ${config.color} ${outcome.status === "loading" ? "animate-spin" : ""}`}
        />
        ...
        <div className="flex gap-2 mt-3">
          ...
          {outcome.secondaryActionLabel && outcome.onSecondaryAction && (
            <Button
              size="sm"
              variant="outline"
              onClick={outcome.onSecondaryAction}
            >
    ...
    <Card className={`${config.bg} border-0 shadow-lg`}>
      <CardContent className="p-4">
        <div className="flex items-start gap-3">
          ...
          >
            <StatusIcon
              className={`w-5 h-5 ${config.color} ${outcome.status === "loading" ? "animate-spin" : ""}`}
            />
          </div>
          <div className="flex-1 min-w-0">
            ...
            {countdown !== null && (
              <div className="mt-2">
                <Progress
                  value={(countdown / (outcome.retryAfter || 60)) * 100}
                  className="h-1"
                />
...
export function useOutcomeHandler() {
  ...
  const handleOAuthDenied = useCallback(
    (platform: string) => {
      showOutcome({
        status: "warning",
        category: "oauth",
  ...
  const handleOAuthExpired = useCallback(
    (platform: string, onReauth: () => void) => {
      showOutcome({
  ...
  const handlePlatformUnavailable = useCallback(
    (platform: string, retryAfter?: number) => {
      showOutcome({
        status: "error",
        category: "oauth",
  ...
  const handlePostScheduled = useCallback(
    (scheduledTime: string, platforms: string[]) => {
      showOutcome({
        status: "success",
        category: "post",
  ...
  const handlePostPublished = useCallback(
    (platforms: string[], postUrls?: Record<string, string>) => {
      showOutcome({
        status: "success",
        category: "post",
  ...
  const handlePostFailed = useCallback(
    (platform: string, errorMessage: string, errorCode?: string) => {
      showOutcome({
        status: "error",
        category: "post",
        title: "Post Failed",
  ...
  const handleMediaUploadFailed = useCallback(
    (reason: string, maxSize?: string, supportedFormats?: string[]) => {
  ...
  const handleContentGenerated = useCallback(
    (variationsCount: number, hasHashtags: boolean, optimalTime?: string) => {
  ...
  const handleContentGenerationFailed = useCallback(
    (reason: string, onUseFallback?: () => void) => {
      showOutcome({
        status: "error",
        category: "content",
  ...
  const handleAnalyticsLoaded = useCallback(
    (metricsCount: number, period: string) => {
      showOutcome({
        status: "success",
        category: "analytics",
  ...
  const handleNoAnalyticsData = useCallback(
    (platform: string) => {
      showOutcome({
        status: "warning",
        category: "analytics",
  ...
  const handleMessageAssigned = useCallback(
    (assigneeName: string) => {
      showOutcome({
        status: "success",
        category: "inbox",
