...
export function OAuthStatusIndicator({
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <div
              ...
              onClick={platform.status === 'disconnected' ? handleConnect : undefined}
            >
              <PlatformIcon className="w-6 h-6" style={{ color: platformColor }} />
              <div className={`absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full ${platform.status === 'connected' ? 'bg-green-500' : platform.status === 'disconnected' ? 'bg-gray-400' : 'bg-orange-500'} border-2 border-white dark:border-gray-900 flex items-center justify-center`}>
          ...
          <TooltipContent>
            <div className="text-sm">
              ...
              {platform.username && <p className="text-muted-foreground">@{platform.username}</p>}
            </div>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
  ...
  return (
    <Card className={`${statusConfig.bgColor} border ${statusConfig.borderColor} transition-all hover:shadow-md`}>
      <CardContent className="p-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                >
                  <StatusIcon className={`w-3 h-3 mr-1 ${platform.status === 'connecting' ? 'animate-spin' : ''}`} />
                  {statusConfig.label}
                </Badge>
                {platform.followerCount !== undefined && platform.followerCount > 0 && (
            ...
            {(platform.status === 'connected' || platform.status === 'error') && (
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  {platform.profileUrl && (
                    <DropdownMenuItem onClick={() => window.open(platform.profileUrl, '_blank')}>
        ...
        {showDetails && platform.status !== 'disconnected' && (
          <div className="mt-4 pt-4 border-t border-border/50">
            ...
            {platform.retryAfter && (
              <div className="mt-2">
                <Progress value={(platform.retryAfter / 60) * 100} className="h-1" />
...
export function OAuthStatusGrid({
  ...
  const hasIssues = platforms.some(p => ['expired', 'error', 'scope_changed'].includes(p.status));
