...
async function checkAudioFile(file: File): Promise<{ duration: number } | null> {
...
export class FileValidator {
  static validate(file: File, options: ValidationOptions = {}): ValidationResult {
    const errors: string[] = [];
    ...
    const {
      ...
      allowedExtensions = ['.wav', '.mp3', '.flac', '.aiff', '.aif', '.ogg', '.webm', '.jpg', '.jpeg', '.png', '.gif', '.webp'],
    } = options;

    if (file.size > maxSize) {
      errors.push(`File size (${formatBytes(file.size)}) exceeds maximum allowed (${formatBytes(maxSize)})`);
      details.push({
        check: 'File Size',
        status: 'fail',
      ...
      errors.push(`File size (${formatBytes(file.size)}) is below minimum required (${formatBytes(minSize)})`);
      details.push({
        check: 'File Size',
        status: 'fail',
  ...
  static async validateAsync(file: File, options: ValidationOptions = {}): Promise<ValidationResult> {
    const result = this.validate(file, options);

    if (options.checkCorruption !== false) {
      const integrityIndex = result.details.findIndex(d => d.check === 'File Integrity');
      try {
        ...
        if (isValid) {
          ...
          result.errors.push('File integrity check failed - file may be corrupted');
          result.valid = false;
        }
      } catch {
        result.details[integrityIndex] = {
    ...
    if (file.type.startsWith('image/') && (options.imageMinWidth || options.imageMinHeight || options.imageMaxWidth || options.imageMaxHeight)) {
      ...
      if (dimensions) {
        ...
        if (options.imageMinWidth && dimensions.width < options.imageMinWidth) {
          issues.push(`width ${dimensions.width}px < ${options.imageMinWidth}px min`);
          dimensionOk = false;
        }
        if (options.imageMinHeight && dimensions.height < options.imageMinHeight) {
          issues.push(`height ${dimensions.height}px < ${options.imageMinHeight}px min`);
          dimensionOk = false;
        }
        if (options.imageMaxWidth && dimensions.width > options.imageMaxWidth) {
          issues.push(`width ${dimensions.width}px > ${options.imageMaxWidth}px max`);
          dimensionOk = false;
        }
        if (options.imageMaxHeight && dimensions.height > options.imageMaxHeight) {
          issues.push(`height ${dimensions.height}px > ${options.imageMaxHeight}px max`);
          dimensionOk = false;
        }
        ...
        result.details.push({
          ...
          message: dimensionOk ? 'Dimensions are acceptable' : issues.join(', '),
  ...
  static getAcceptString(options: ValidationOptions = {}): string {
    const types = options.allowedTypes || [...DEFAULT_AUDIO_FORMATS, ...DEFAULT_IMAGE_FORMATS];
    const extensions = options.allowedExtensions || ['.wav', '.mp3', '.flac', '.aiff', '.aif', '.ogg'];
...
export function FileValidationDisplay({ result, className, compact = false }: FileValidationDisplayProps) {
  const passCount = result.details.filter(d => d.status === 'pass').length;
  const failCount = result.details.filter(d => d.status === 'fail').length;
  const warningCount = result.details.filter(d => d.status === 'warning').length;
  const pendingCount = result.details.filter(d => d.status === 'pending').length;
          ...
          {result.details.map((detail, index) => (
            <div key={index} className="flex items-start gap-3 py-1.5">
              ...
              <div className="flex-1 min-w-0">
                ...
                {detail.value && (
                  <p className="text-xs text-muted-foreground/70 mt-0.5">{detail.value}</p>
                )}
              </div>
            </div>
          ))}
