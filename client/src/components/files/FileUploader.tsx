...
export interface UploadFile {
  ...
  speed: number;
  status: 'pending' | 'validating' | 'uploading' | 'paused' | 'processing' | 'success' | 'error' | 'cancelled';
  error?: string;
  previewUrl?: string;
  chunks?: ChunkInfo[];
  currentChunk?: number;
...
export function FileUploader({
  ...
  maxFiles = 10,
  acceptedTypes = ['audio/*', 'image/*', '.wav', '.mp3', '.flac', '.aiff', '.ogg'],
  ...
  const startUpload = useCallback(async (uploadFile: UploadFile) => {
    ...
    try {
      ...
      const useChunked = enableChunkedUpload && uploadFile.file.size > chunkSize;
      ...
      if (useChunked) {
        ...
        for (let i = 0; i < chunks.length; i++) {
          ...
          const elapsed = (Date.now() - (uploadStartTimeRef.current.get(uploadFile.id) || Date.now())) / 1000;
        ...
        await uploadWhole(uploadFile, abortController, (loaded) => {
          ...
          updateFile(uploadFile.id, { progress, uploadedBytes: loaded, speed });
        });
      }
  ...
  }, [enableChunkedUpload, chunkSize, category, uploadEndpoint, files, updateFile, toast, onUploadComplete, onError]);
  ...
  const cancelUpload = useCallback((id: string) => {
    ...
    toast({ title: 'Upload Cancelled', description: 'Upload has been cancelled and cleaned up' });
  }, [updateFile, toast]);

  const retryUpload = useCallback((id: string) => {
    const file = files.find(f => f.id === id);
    if (file && file.retryCount < MAX_RETRIES) {
      updateFile(id, { status: 'pending', error: undefined, progress: 0, uploadedBytes: 0 });
      startUpload({ ...file, status: 'pending', error: undefined, progress: 0, uploadedBytes: 0 });
    } else {
      toast({
  ...
  const processFiles = useCallback(async (fileList: FileList | File[]) => {
    ...
    for (const file of fileArray) {
      ...
      const previewUrl = showPreview ? await createPreviewUrl(file) : undefined;
  ...
  }, [files.length, maxFiles, maxFileSize, acceptedTypes, showPreview, toast, onFileAdded, startUpload]);
  ...
  const activeUploads = files.filter(f => ['uploading', 'pending', 'validating'].includes(f.status));
        ...
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Max {formatBytes(maxFileSize)}</p>
            </div>
          </div>
        </div>
    ...
    <div className={cn('space-y-4', className)}>
      <div
        ...
        className={cn(
          ...
          isDragging ? 'border-primary bg-primary/10 scale-[1.02]' : 'border-muted-foreground/25 hover:border-primary/50 hover:bg-muted/30',
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center text-center gap-4">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 text-xs text-muted-foreground">
            ...
            <span className="text-muted-foreground/60">Max {formatBytes(maxFileSize)}</span>
          </div>
            ...
            <div className="flex items-center justify-between">
              <div className="text-sm font-medium">
                {activeUploads.length > 0 ? (
                  ...
                  <span className="text-green-600">{completedUploads.length} file{completedUploads.length > 1 ? 's' : ''} uploaded</span>
                ) : null}
              </div>
              {files.length > 0 && (
                <Button
...
function FileUploadItem({
  ...
  const statusConfig = {
    pending: { icon: <Loader2 className="h-4 w-4 animate-pulse text-muted-foreground" />, text: 'Queued', color: 'bg-muted' },
    validating: { icon: <Loader2 className="h-4 w-4 animate-spin text-amber-500" />, text: 'Validating...', color: 'bg-amber-500/10' },
    uploading: { icon: <Loader2 className="h-4 w-4 animate-spin text-primary" />, text: `${file.progress}%`, color: 'bg-primary/10' },
    paused: { icon: <Pause className="h-4 w-4 text-muted-foreground" />, text: 'Paused', color: 'bg-muted' },
    processing: { icon: <Loader2 className="h-4 w-4 animate-spin text-amber-500" />, text: 'Processing...', color: 'bg-amber-500/10' },
    success: { icon: <CheckCircle2 className="h-4 w-4 text-green-500" />, text: 'Complete', color: 'bg-green-500/10' },
    error: { icon: <AlertCircle className="h-4 w-4 text-destructive" />, text: 'Failed', color: 'bg-destructive/10' },
    cancelled: { icon: <X className="h-4 w-4 text-muted-foreground" />, text: 'Cancelled', color: 'bg-muted' },
  };
  ...
  if (compact) {
    return (
      <div className={cn('flex items-center gap-2 p-2 rounded-md text-sm', config.color)}>
        {config.icon}
        <span className="flex-1 truncate text-xs font-medium">{file.file.name}</span>
        {file.status === 'uploading' && (
          <span className="text-xs text-muted-foreground tabular-nums">{file.progress}%</span>
        )}
        {file.status === 'uploading' && (
          <Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => onPause(file.id)}>
            <Pause className="h-3 w-3" />
          </Button>
        )}
        {file.status === 'paused' && (
          <Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => onResume(file.id)}>
            <Play className="h-3 w-3" />
          </Button>
        )}
        {file.status === 'error' && (
          <Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => onRetry(file.id)}>
            <RotateCcw className="h-3 w-3" />
          </Button>
        )}
        {['pending', 'uploading', 'paused', 'error'].includes(file.status) && (
          <Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => onCancel(file.id)}>
            <X className="h-3 w-3" />
          </Button>
        )}
      </div>
  ...
  return (
    <div className={cn(
      ...
      <div className="flex-1 min-w-0 space-y-1">
        ...
        {(file.status === 'uploading' || file.status === 'paused') && (
          <>
            ...
            <div className="flex justify-between text-xs text-muted-foreground">
              <span>{formatBytes(file.uploadedBytes)} / {formatBytes(file.file.size)}</span>
        ...
        {file.status === 'pending' && (
          <p className="text-xs text-muted-foreground">{formatBytes(file.file.size)} - Waiting to upload</p>
        )}
        ...
        {file.status === 'success' && (
          <p className="text-xs text-green-600">{formatBytes(file.file.size)} uploaded successfully</p>
        )}
      ...
      <div className="flex items-center gap-1 flex-shrink-0">
        ...
        {file.status === 'uploading' && (
          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => onPause(file.id)} title="Pause">
            <Pause className="h-4 w-4" />
          </Button>
        )}
        {file.status === 'paused' && (
          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => onResume(file.id)} title="Resume">
            <Play className="h-4 w-4" />
          </Button>
        )}
        {file.status === 'error' && (
          <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-primary/10" onClick={() => onRetry(file.id)} title="Retry">
            <RotateCcw className="h-4 w-4" />
          </Button>
        )}
        {['pending', 'uploading', 'paused', 'error', 'cancelled'].includes(file.status) && (
          <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-destructive/10 hover:text-destructive" onClick={() => onCancel(file.id)} title="Cancel">
            <X className="h-4 w-4" />
          </Button>
        )}
        {file.status === 'success' && (
          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => onRemove(file.id)} title="Remove">
            <X className="h-4 w-4" />
          </Button>
        )}
      </div>
