...
const OUTCOME_CONFIG: Record<UploadOutcome, {
  label: string;
...
export function UploadProgressTracker({
  ...
  const activeUploads = uploads.filter(u => ['uploading', 'validating', 'processing', 'pending'].includes(u.outcome));
  ...
  const remainingBytes = activeUploads.reduce((sum, u) => sum + (u.file.size - u.uploadedBytes), 0);

  if (uploads.length === 0) return null;

  return (
    <Card className={cn('fixed bottom-4 right-4 w-96 shadow-lg z-50', className)}>
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            {showNetworkStatus && (
              <div className={cn('p-1', isOnline ? 'text-green-500' : 'text-destructive')}>
        ...
        <CardContent className="pt-2 pb-3">
          <div className="flex items-center gap-1 mb-2">
            ...
            {failedUploads.length > 0 && onRetryFailed && (
              <Button variant="ghost" size="sm" onClick={onRetryFailed} className="h-7 text-xs">
...
function UploadItem({
  ...
  const isActive = ['uploading', 'validating', 'processing', 'pending'].includes(upload.outcome);

  return (
    <div className={cn(
      'p-2 rounded-lg border transition-all',
      isError ? 'border-destructive/30 bg-destructive/5' :
      upload.outcome === 'success' ? 'border-green-500/30 bg-green-500/5' :
      'border-border'
    )}>
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          {(upload.outcome === 'uploading' || upload.outcome === 'processing') && (
          ...
          {upload.outcome === 'error_invalid_type' && upload.errorDetails?.allowedTypes && (
            <p className="text-xs text-muted-foreground mt-0.5">
              Allowed: {upload.errorDetails.allowedTypes.slice(0, 3).join(', ')}...
            </p>
          )}

          {upload.outcome === 'error_quota_exceeded' && upload.errorDetails && (
            <p className="text-xs text-muted-foreground mt-0.5">
              {formatBytes(upload.errorDetails.quotaUsed || 0)} / {formatBytes(upload.errorDetails.quotaLimit || 0)} used
            </p>
          )}
        </div>
        ...
        <div className="flex items-center gap-0.5 flex-shrink-0">
          {upload.outcome === 'uploading' && onPause && (
            <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => onPause(upload.id)}>
              <Pause className="h-3 w-3" />
            </Button>
          )}
          {upload.outcome === 'pending' && onResume && (
            <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => onResume(upload.id)}>
              <Play className="h-3 w-3" />
            </Button>
          )}
          {isError && upload.canRetry && onRetry && (
            <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => onRetry(upload.id)}>
          ...
          {(upload.outcome === 'success' || upload.outcome === 'cancelled') && onRemove && (
            <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => onRemove(upload.id)}>
              <X className="h-3 w-3" />
            </Button>
          )}
        </div>
