...
export type BulkOperationType = 'delete' | 'move' | 'download' | 'duplicate' | 'share' | 'restore';

export type BulkOperationStatus = 'idle' | 'confirming' | 'processing' | 'complete' | 'error';
...
interface BulkFileManagerProps {
  ...
  onMove?: (ids: string[], targetFolder: string) => Promise<BulkOperationResult>;
...
export function BulkFileManager({
  ...
  const [operationProgress, setOperationProgress] = useState({ current: 0, total: 0, currentFile: '' });
  ...
  const executeOperation = async () => {
    ...
    setOperationProgress({ current: 0, total: selectedCount, currentFile: selectedFiles[0]?.name || '' });
      ...
      switch (operationType) {
        ...
        case 'move':
          if (onMove) {
            ...
            result = { success: ids, failed: [], totalRequested: ids.length, totalSucceeded: ids.length, totalFailed: 0 };
          }
          break;
        case 'download':
          if (onDownload) {
  ...
  const getOperationSuccessTitle = (type: BulkOperationType) => {
    switch (type) {
      case 'delete': return 'Files Deleted';
      case 'move': return 'Files Moved';
      case 'download': return 'Download Started';
      case 'duplicate': return 'Files Duplicated';
      case 'restore': return 'Files Restored';
      default: return 'Operation Complete';
    }
  };
  ...
  const getOperationIcon = (type: BulkOperationType) => {
    switch (type) {
      ...
      case 'move': return <FolderInput className="h-4 w-4" />;
      case 'download': return <Download className="h-4 w-4" />;
      case 'duplicate': return <Copy className="h-4 w-4" />;
      case 'restore': return <Undo2 className="h-4 w-4" />;
      default: return null;
    }
  };
        ...
        {selectedCount > 0 && (
          <div className="flex items-center gap-2">
            {!showTrashActions ? (
              <>
                {onDownload && (
                  <Button variant="outline" size="sm" onClick={() => startOperation('download')}>
                    <Download className="h-4 w-4 mr-1" />
                    Download
                  </Button>
                )}
                {folders.length > 0 && (
                  <Button variant="outline" size="sm" onClick={() => startOperation('move')}>
                    <FolderInput className="h-4 w-4 mr-1" />
                    Move
                  </Button>
                )}
                {onDuplicate && (
                  <Button variant="outline" size="sm" onClick={() => startOperation('duplicate')}>
              ...
              <>
                <Button variant="outline" size="sm" onClick={() => startOperation('restore')}>
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              className={cn(
                operationType === 'delete' && showTrashActions && 'bg-destructive text-destructive-foreground'
              )}
            >
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="p-3 rounded-lg bg-muted">
                  <p className="text-2xl font-bold">{operationResult.totalRequested}</p>
                  <p className="text-xs text-muted-foreground">Requested</p>
                </div>
                <div className="p-3 rounded-lg bg-green-500/10">
                  <p className="text-2xl font-bold text-green-500">{operationResult.totalSucceeded}</p>
                  <p className="text-xs text-muted-foreground">Succeeded</p>
                </div>
                <div className="p-3 rounded-lg bg-destructive/10">
                  <p className="text-2xl font-bold text-destructive">{operationResult.totalFailed}</p>
                  ...
                  <ScrollArea className="h-32">
                    <div className="space-y-1">
                      {operationResult.failed.map(({ id, error }) => (
                        <div key={id} className="text-xs p-2 rounded bg-destructive/5">
    ...
    <div className={cn('p-4 border rounded-lg space-y-3', className)}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium capitalize">{operation}ing files...</span>
        </div>
        {onCancel && (
