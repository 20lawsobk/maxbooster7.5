...
function MetricCard({
  ...
  return (
    ...
    >
      <Card
        className={`bg-gradient-to-br ${colorClasses[color as keyof typeof colorClasses]} ${compact ? "p-2" : ""}`}
      >
          ...
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <div className="flex items-center gap-2">
                {icon && (
                  <span
                    className={`${textColors[color as keyof typeof textColors]} opacity-60`}
                  >
                    {icon}
                  </span>
                )}
              ...
              <p
                className={`${compact ? "text-lg" : "text-2xl"} font-bold mt-1`}
              >
                {value}
              </p>
              {change !== undefined && (
  ...
  const {
    ...
    queryFn: async () => {
      ...
      return {
        ...
        followerChange: 0,
        playChange: 0,
        platforms: (streams.byPlatform || []).map(
        ...
        triggerCities: (streams.byCity || []).map(
          (c: Record<string, unknown>) => ({
            city: c.city || "Unknown",
        ...
        revenueBySource:
          raw.revenue?.revenueBySource || raw.revenueBySource || [],
        streamHistory: (streams.daily || []).map(
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Comprehensive career metrics and insights
          </p>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
        <MetricCard
          ...
          color="blue"
          sparkline={(data.streamHistory || []).map(
            (d: Record<string, unknown>) => d.streams || 0,
          )}
          compact
        />
        <MetricCard
          ...
          color="green"
          sparkline={(data.streamHistory || []).map(
            (d: Record<string, unknown>) => d.revenue || 0,
          )}
          compact
        />
        <MetricCard
        ...
        <Card className="bg-gradient-to-br from-amber-500/10 to-amber-600/5 border-amber-500/20">
          <CardContent className="p-3">
            ...
            <div className="flex items-center gap-2">
              <span
                className={`text-2xl font-bold ${getScoreColor(data.artistScore || 0)}`}
              >
                {data.artistScore || 0}
              </span>
          ...
          <CardContent>
            {(data.platforms || []).length === 0 ? (
              ...
              <div className="space-y-3">
                {(data.platforms || []).map(
                  (platform: Record<string, unknown>, index: number) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          className={`text-xs w-14 text-right ${platform.change > 0 ? "text-green-500" : "text-red-500"}`}
                        >
                          {platform.change > 0 ? "+" : ""}
                          {platform.change}%
                        </div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
          ...
          <CardContent>
            <ScrollArea className="h-[220px]">
              <div className="space-y-2">
                {(data.geoData || []).map(
                  (geo: Record<string, unknown>, index: number) => (
                    <motion.div
                      key={geo.code}
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                    ...
                    >
                      ...
                      <div className="text-right">
                        <p className="text-sm font-medium">
                          {formatNumber(geo.streams)}
                        </p>
                        <p
                          className={`text-xs ${geo.growth > 0 ? "text-green-500" : "text-red-500"}`}
                        >
                          {geo.growth > 0 ? "+" : ""}
                          {geo.growth}%
                        </p>
        ...
        <Card>
                ...
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p className="max-w-xs text-xs">
                      Cities where your music is growing fastest and could
                      "trigger" wider regional success
                    </p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
          ...
          <CardContent>
            <div className="space-y-3">
              {(data.triggerCities || []).map(
                (city: Record<string, unknown>, index: number) => (
                  <motion.div
                    key={city.city}
                    initial={{ opacity: 0, x: 20 }}
                    animate={{ opacity: 1, x: 0 }}
                  ...
                  >
                    ...
                    <div className="text-right">
                      <p className="text-sm font-medium">
                        {formatNumber(city.listeners)} listeners
                      </p>
