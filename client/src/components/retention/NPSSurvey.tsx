...
export function NPSSurvey({ triggerContext = '30_day', onDismiss, onSubmit }: NPSSurveyProps) {
  const [step, setStep] = useState<'score' | 'comment' | 'thanks'>('score');
  const [score, setScore] = useState<number | null>(null);
  const [comment, setComment] = useState('');

  const mutation = useMutation({
    mutationFn: (data: { score: number; comment?: string; triggerContext: string }) =>
  ...
  const handleSubmit = () => {
    ...
    mutation.mutate({ score, comment: comment.trim() || undefined, triggerContext });
  };
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="flex items-center justify-between px-5 pt-4 pb-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-gray-800 dark:text-gray-100">Quick question</span>
          </div>
          <button
            onClick={onDismiss}
        ...
        <div className="px-5 pb-5">
          ...
          {step === 'comment' && score !== null && (
            <>
              <p className="text-sm text-gray-700 dark:text-gray-300 mb-1">
                You selected <span className="font-bold text-purple-600">{score}/10</span> — {scoreLabel(score)}.
              </p>
          ...
          {step === 'thanks' && (
            <div className="text-center py-2">
              ...
              <p className="text-sm font-semibold text-gray-800 dark:text-gray-100">Thank you for your feedback!</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Your response helps us build a better platform for artists.</p>
            </div>
          )}
        </div>
      </div>
