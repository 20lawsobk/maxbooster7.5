...
export function StemsManager({ listingId, isOwner }: StemsManagerProps) {
  ...
  const purchaseStemMutation = useMutation({
    mutationFn: async (stemId: string) => {
      const response = await apiRequest(
        "POST",
        `/api/marketplace/stems/${stemId}/purchase`,
      );
      return response.json();
    },
                    ...
                    <TableRow key={stem.id}>
                      <TableCell className="font-medium">
                        <div className="flex items-center gap-2">
                          <Button
                            ...
                            onClick={() =>
                              handlePlayPause(stem.id, stem.fileUrl)
                            }
                          >
                            {playingStem === stem.id ? (
                      ...
                      <TableCell>
                        <Badge
                          variant="outline"
                          className={
                            STEM_TYPE_COLORS[stem.stemType] ||
                            STEM_TYPE_COLORS.other
                          }
                        >
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete this stem? This action cannot be
              undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
