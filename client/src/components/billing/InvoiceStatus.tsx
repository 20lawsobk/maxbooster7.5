...
const getStatusBadgeVariant = (color: string): 'default' | 'secondary' | 'destructive' | 'outline' => {
  switch (color) {
    case 'green':
      return 'default';
    case 'yellow':
...
export default function InvoiceStatus({ 
  ...
  const { data, isLoading, error, refetch, isRefetching } = useQuery<{ invoices: Invoice[] }>({
    queryKey: ['/api/billing/invoices', statusFilter !== 'all' ? statusFilter : undefined],
  });
  ...
  const pendingCount = invoices.filter(inv => inv.status === 'open' && !inv.isOverdue).length;
          ...
          <div className="flex gap-3">
            {overdueCount > 0 && (
              <Alert variant="destructive" className="flex-1">
                ...
                <AlertDescription>
                  {overdueCount} overdue invoice{overdueCount > 1 ? 's' : ''} requiring attention
                </AlertDescription>
              </Alert>
            )}
            {pendingCount > 0 && (
            ...
            <Table>
              ...
              <TableBody>
                {displayedInvoices.map((invoice) => (
                  <TableRow 
                    key={invoice.id}
                    className={invoice.isOverdue ? 'bg-red-50/50 dark:bg-red-950/20' : ''}
                  >
                    <TableCell>
                      <div className="flex items-center gap-2">
                    ...
                    <TableCell>
                      <div>
                        ...
                        {invoice.dueDate && invoice.status === 'open' && (
                          <p className={`text-xs ${invoice.isOverdue ? 'text-red-600' : 'text-muted-foreground'}`}>
                      ...
                      <div>
                        ...
                        {invoice.amountRemaining > 0 && invoice.amountRemaining < invoice.amount && (
                          <p className="text-xs text-muted-foreground">
                            {formatCurrency(invoice.amountRemaining, invoice.currency)} remaining
                          </p>
                        )}
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex flex-col gap-1">
                        <Badge variant={getStatusBadgeVariant(invoice.statusColor)}>
                          {invoice.statusDisplay}
                        </Badge>
                        {invoice.attemptCount > 0 && invoice.status === 'open' && (
                          <span className="text-xs text-muted-foreground">
                            {invoice.attemptCount} attempt{invoice.attemptCount > 1 ? 's' : ''}
                          </span>
                        )}
