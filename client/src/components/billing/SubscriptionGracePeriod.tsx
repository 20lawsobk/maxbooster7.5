...
export default function SubscriptionGracePeriod({
  ...
  const { gracePeriodDaysRemaining, gracePeriodEndsAt, payment, actions } = gracePeriod;
  const totalGraceDays = 7;
  const progressPercent = ((totalGraceDays - gracePeriodDaysRemaining) / totalGraceDays) * 100;
  ...
  if (variant === 'inline') {
    return (
      <div className={`flex items-center gap-2 p-2 rounded-md ${styles.bg} ${styles.text}`}>
        ...
        {actions?.canRetryPayment && onRetryPayment && (
          <Button size="sm" variant="ghost" className="h-6 px-2" onClick={onRetryPayment}>
      ...
      <CardContent className="pt-6">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className={`font-semibold ${styles.text}`}>Grace Period Active</h3>
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Grace period progress</span>
          ...
          {payment && (
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div className="flex items-center gap-2">
                ...
                <span>
                  Retry attempts: {payment.retryAttempts}/{payment.maxRetryAttempts}
                </span>
              </div>
              {payment.nextRetryAt && (
                <div className="flex items-center gap-2">
          ...
          <div className="flex gap-2 pt-2">
            ...
            {actions?.canUpdatePaymentMethod && onUpdatePaymentMethod && (
              <Button onClick={onUpdatePaymentMethod} variant="outline" className="flex-1">
