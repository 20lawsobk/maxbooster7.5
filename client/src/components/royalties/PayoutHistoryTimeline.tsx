    ...
    <Card className="glassmorphism" data-testid="payout-timeline">
      ...
      <CardContent className="space-y-6">
        {showInstantPayoutInfo && (
          <div className="p-4 rounded-lg bg-purple-500/10 border border-purple-500/20" data-testid="instant-payout-info">
            <div className="flex items-start gap-3">
              <Zap className="w-5 h-5 text-purple-500 mt-0.5" />
              <div>
                <p className="font-medium text-purple-500">Instant Payouts Available</p>
                <p className="text-sm text-muted-foreground">
                  Get your earnings immediately for a {instantPayoutFeePercentage}% fee.
                  Standard payouts (free) arrive in 3-5 business days.
                </p>
              </div>
            </div>
          </div>
        ...
        {groupedPayouts.map(([monthYear, monthPayouts]) => (
          <div key={monthYear} data-testid={`payout-group-${monthYear}`}>
            <h3 className="text-sm font-semibold text-muted-foreground mb-4">{monthYear}</h3>
                  ...
                  <div className="p-4 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        ...
                        {payout.fee !== undefined && payout.fee > 0 && (
                          <div className="flex items-center gap-2 text-sm text-muted-foreground mt-1">
                            <span>Fee: {formatCurrency(payout.fee, payout.currency)}</span>
                            <ArrowRight className="w-3 h-3" />
                            <span className="font-medium text-foreground">
                              Net: {formatCurrency(payout.netAmount || payout.amount - payout.fee, payout.currency)}
                            </span>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="text-right text-sm">
                        <p className="text-muted-foreground">{formatDate(payout.requestedAt)}</p>
                        {payout.estimatedArrival && payout.status !== 'completed' && (
                          <p className="text-xs text-muted-foreground mt-1">
                            Est. arrival: {formatDate(payout.estimatedArrival)}
                          </p>
                        )}
                      </div>
                    </div>
