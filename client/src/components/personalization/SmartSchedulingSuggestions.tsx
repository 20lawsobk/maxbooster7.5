...
import { useSmartScheduling, DayOfWeek, ScheduleSuggestion } from '@/hooks/useSmartScheduling';
import {
  Clock,
  Calendar,
  TrendingUp,
...
export function SmartSchedulingSuggestions({
  ...
  const {
    ...
    applySchedule,
    isApplying,
  } = useSmartScheduling(selectedPlatform, contentType);

  const handleSelectTime = async (suggestion: ScheduleSuggestion) => {
    await applySchedule(suggestion);
    onSelectTime?.(suggestion);
  };

  const displayedSuggestions = topSuggestions.slice(0, maxSuggestions);
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-6">
          ...
          <div className="p-4 rounded-lg bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-950/30 dark:to-blue-950/30 border border-purple-200 dark:border-purple-800">
            <div className="flex items-center gap-2 mb-2">
              ...
              <Badge variant="secondary" className="text-xs ml-auto">
                {Math.round(bestTime.confidence * 100)}% confidence
              </Badge>
            </div>
            <div className="flex items-center justify-between">
              <div>
                <span className="text-2xl font-bold">
                  {fullDayLabels[bestTime.dayOfWeek]}, {bestTime.specificTime}
                </span>
                <p className="text-sm text-muted-foreground mt-1">
                  {bestTime.reasoning}
                </p>
              </div>
              <Button onClick={() => handleSelectTime(bestTime)} disabled={isApplying}>
        ...
        {showHeatmap && !compact && (
          <div>
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-medium">Weekly Engagement Heatmap</span>
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger>
            ...
            <div className="grid grid-cols-7 gap-2">
              {daysOfWeek.map((day) => {
                const activity = weeklyHeatmap[day] || 0;
                const opacity = 0.2 + (activity * 0.8);
                const suggestion = scheduleData.suggestions.find(s => s.dayOfWeek === day);
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <div
                          ...
                          onClick={() => suggestion && handleSelectTime(suggestion)}
                        >
                      ...
                      <TooltipContent>
                        ...
                        {suggestion && (
                          <p className="text-xs">Best time: {suggestion.specificTime}</p>
                        )}
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    index === 0 && 'border-purple-200 dark:border-purple-800 bg-purple-50/50 dark:bg-purple-950/20'
                  )}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-sm">
                        {fullDayLabels[suggestion.dayOfWeek]} at {suggestion.specificTime}
                      </span>
                      {index === 0 && (
        ...
        {showAnalytics && audienceTimezones.length > 0 && !compact && (
          <div className="pt-4 border-t">
            ...
            <div className="space-y-2">
              {audienceTimezones.slice(0, 4).map((tz) => (
        ...
        <div className="pt-4 border-t text-xs text-muted-foreground flex items-center gap-2">
          ...
          Last updated: {scheduleData.lastUpdated ? new Date(scheduleData.lastUpdated).toLocaleDateString() : 'Recently'}
        </div>
      </CardContent>
    </Card>
  );
