...
export function SmartScheduleSuggestion({
  ...
  const { data: scheduleData, isLoading, error } = useQuery<SmartScheduleData>({
    queryKey: ['/api/personalization/smart-schedule', selectedPlatform, contentType],
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
          ...
          <div className="p-4 rounded-lg bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-950/30 dark:to-blue-950/30 border border-purple-200 dark:border-purple-800">
            <div className="flex items-center gap-2 mb-2">
              ...
              <Badge variant="secondary" className="text-xs ml-auto">
                {Math.round(scheduleData.bestOverallTime.confidence * 100)}% confidence
              </Badge>
            </div>
            <div className="flex items-center justify-between">
              <div>
                <span className="text-2xl font-bold">
                  {fullDayLabels[scheduleData.bestOverallTime.dayOfWeek]}, {scheduleData.bestOverallTime.specificTime}
                </span>
                <p className="text-sm text-muted-foreground mt-1">
                  {scheduleData.bestOverallTime.reasoning}
                </p>
              </div>
              <Button onClick={() => handleSelectTime(scheduleData.bestOverallTime!)}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      <span className="text-xs text-muted-foreground flex items-center gap-1">
                        ...
                        {Math.round(suggestion.audienceActivity * 100)}% audience active
                      </span>
                    </div>
                  </div>
        ...
        <div className="pt-4 border-t text-xs text-muted-foreground flex items-center gap-2">
          ...
          Last updated: {new Date(scheduleData.lastUpdated).toLocaleDateString()}
        </div>
      </CardContent>
    </Card>
  );
