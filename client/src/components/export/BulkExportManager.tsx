    ...
    mutationFn: async () => {
      ...
      const job: BulkExportJob = {
        ...
        items: selectedItems.map((item) => ({
          ...
          progress: 0,
          format: formatConfig.useUnifiedFormat
            ? formatConfig.format
            : item.format,
        })),
    ...
    const interval = setInterval(async () => {
      ...
      if (pollCount > MAX_POLLS) {
        ...
        toast({
          ...
          description: "Export job timed out. Please try again.",
        });
        return;
      }
      ...
      try {
        ...
        const isDone = ["complete", "failed", "cancelled", "partial"].includes(
          serverStatus,
        );
          ...
          const updatedItems = prev.items.map((item, idx) => {
            ...
            if (idx === completedItems && !isDone) {
              return {
                ...
                progress:
                  (progress % (100 / Math.max(totalItems, 1))) * totalItems,
              };
            }
            return item;
          });
          ...
          const finalStatus: BulkExportJob["status"] = isDone
            ? serverStatus === "complete"
              ...
              : serverStatus === "cancelled"
                ? "cancelled"
                : failedItems > 0 && failedItems < totalItems
                  ? "partial"
                  : "failed"
          ...
          const job: BulkExportJob = {
            ...
            ...(isDone
              ? {
                  ...
                  zipUrl:
                    serverJob.downloadUrl ||
                    `/api/export/download/zip/${jobId}`,
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription className="mt-1">
              {selectedIds.size} of {items.length} items selected · Est.{" "}
              {estimatedSize}
            </CardDescription>
          </div>
          <div className="flex items-center gap-2">
            ...
            {formatConfig.useUnifiedFormat && (
              <>
                ...
                <QualitySelector
                  ...
                  isLossless={
                    AUDIO_FORMATS.find((f) => f.value === formatConfig.format)
                      ?.lossless || false
                  }
                />
              </>
