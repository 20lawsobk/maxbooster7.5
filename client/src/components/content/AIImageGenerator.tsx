...
export type ImageStyle = 'modern' | 'cinematic' | 'minimal' | 'bold' | 'vintage' | 'neon' | 'luxury';

export type ImageTone = 'energetic' | 'calm' | 'professional' | 'playful' | 'dramatic';
...
const STYLES: Array<{ id: ImageStyle; name: string; description: string; color: string }> = [
  { id: 'modern', name: 'Modern', description: 'Clean contemporary', color: '#6366f1' },
  { id: 'cinematic', name: 'Cinematic', description: 'Film-quality dramatic', color: '#e94560' },
  { id: 'minimal', name: 'Minimal', description: 'Clean & focused', color: '#6b7280' },
  { id: 'bold', name: 'Bold', description: 'High contrast impact', color: '#f59e0b' },
  { id: 'vintage', name: 'Vintage', description: 'Retro aesthetic', color: '#92400e' },
  { id: 'neon', name: 'Neon', description: 'Vibrant electric', color: '#ff6ec7' },
  { id: 'luxury', name: 'Luxury', description: 'Premium gold/black', color: '#d4af37' },
];
...
export function AIImageGenerator({
  ...
  return (
    <Card className={`border-2 border-indigo-200 bg-gradient-to-r from-indigo-50 to-violet-50 dark:from-indigo-950/20 dark:to-violet-950/20 ${className}`}>
      <CardHeader>
      ...
      <CardContent className="space-y-5">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-7 gap-2">
            {STYLES.map((s) => (
              ...
              >
                ...
                <span className="font-medium leading-tight text-center">{s.name}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-1">
            <Label>Tone</Label>
            <Select value={selectedTone} onValueChange={(v) => setSelectedTone(v as ImageTone)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
            ...
            {result.url ? (
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Badge variant="secondary">{result.processing_time_ms}ms</Badge>
                </div>
                <div className="flex gap-2">
                  ...
                  <Button size="sm" onClick={handleGenerate} variant="outline" className="flex-1">
                ...
                <div className="p-4 rounded-lg bg-muted/50 border space-y-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {Object.entries(result.visual_spec.color_scheme).map(([key, hex]) => (
                  ...
                  <div className="flex gap-2 flex-wrap">
                    <Badge variant="outline">
                      {result.visual_spec.dimensions.width}×{result.visual_spec.dimensions.height}
                    </Badge>
                    <Badge variant="outline">{result.visual_spec.format?.toUpperCase()}</Badge>
                    <Badge variant="outline">{result.visual_spec.processing_time_ms}ms</Badge>
                  </div>
                </div>

                <Button size="sm" onClick={handleGenerate} variant="outline" className="w-full">
                  <RefreshCw className="w-4 h-4 mr-2" />
                  Regenerate
                </Button>
              </div>
