...
const PALETTE_OPTIONS = Object.entries(DEFAULT_PALETTES).map(
  ([key, value]) => ({
    id: key,
...
export function VideoContentGenerator({
  ...
  const checkCapabilities = useCallback(async () => {
    ...
    try {
      ...
      if (!caps.overall.supportsExport) {
        setErrorMessage(
          "Your browser does not support video export. Please use a modern browser like Chrome or Firefox.",
        );
        setPhase("error");
        return false;
      }
    ...
    switch (selectedTemplate) {
      ...
      case "lyric":
        templateOptions = {
          ...
          artistName,
          quotationStyle:
            selectedTemplate === "lyric" ? "typewriter" : "minimal",
  ...
  const initializePreview = useCallback(
    async (result: CompilationResult) => {
            ...
            {phase === "exporting" && exportProgress && (
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between text-xs text-muted-foreground">
                  <span>
                    Frame {exportProgress.currentFrame} /{" "}
