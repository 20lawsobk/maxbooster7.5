...
export function PendingChangesIndicator({
  ...
  const { isOnline, isOffline, pendingCount, failedCount, conflictCount, syncStatus, lastSyncAt } = useOfflineStatus();
  ...
  const content = (
    <div className={cn('flex items-center gap-1.5', className)}>
      ...
      {(variant === 'badge' || variant === 'button') && totalPending > 0 && (
        <Badge variant={getBadgeVariant()} className="px-1.5 py-0 text-xs min-w-[1.25rem] text-center">
          {totalPending}
        </Badge>
      )}
      {variant === 'button' && (
        <span className="text-sm">
          {isSyncing ? 'Syncing...' : isOffline ? 'Offline' : totalPending > 0 ? 'Pending' : 'Synced'}
        </span>
      )}
      {showDropdown && <ChevronDown className="h-3 w-3 text-muted-foreground" />}
    </div>
  );
  ...
  return (
    <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="sm" className={cn('gap-1 h-8', className)}>
          {content}
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-72">
        <div className="px-2 py-1.5">
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            ...
            <div className={cn('p-2 rounded', failedCount > 0 ? 'bg-destructive/10' : 'bg-muted/50')}>
              <div className={cn('font-medium text-lg', failedCount > 0 && 'text-destructive')}>
                {failedCount}
              </div>
              <div className="text-muted-foreground">Failed</div>
            </div>
            <div className={cn('p-2 rounded', conflictCount > 0 ? 'bg-yellow-100 dark:bg-yellow-900/20' : 'bg-muted/50')}>
              <div className={cn('font-medium text-lg', conflictCount > 0 && 'text-yellow-600')}>
                {conflictCount}
              </div>
        ...
        {recentActions.length > 0 && (
          <>
            ...
            {recentActions.map((action) => (
              <DropdownMenuItem key={action.id} className="flex items-center gap-2 text-xs">
