import type { NodeId, ChunkId, PocketPolicy, FabricStorageNode } from '../types.js';
...
export class PlacementStrategy {
  ...
  async placeChunk(chunkId: ChunkId, sizeBytes: number, policy: PocketPolicy): Promise<PlacementDecision> {
    ...
    if (policy.regionAffinity && policy.regionAffinity.length > 0) {
      const regional = candidates.filter(n => policy.regionAffinity!.includes(n.region));
    ...
    if (candidates.length === 0) {
      throw new Error('No nodes with sufficient capacity satisfy the placement policy');
    }

    candidates.sort((a, b) =>
      (a.usedBytes / a.capacityBytes) - (b.usedBytes / b.capacityBytes)
    );
  ...
  async findRebalanceCandidates(highWatermark = 0.8): Promise<{ hot: FabricStorageNode[]; cold: FabricStorageNode[] }> {
    const nodes = await this.nodeRegistry.listHealthyNodes();
    const hot = nodes.filter(n => n.usedBytes / n.capacityBytes > highWatermark);
    const cold = nodes.filter(n => n.usedBytes / n.capacityBytes < highWatermark * 0.6);
    return { hot, cold };
  }
}
