...
const MOCK_TRENDING: TrendingTopic[] = [
  { id: '1', topic: '#NewMusicFriday', volume: 125000, volumeChange: 45, sentiment: 82, relevanceScore: 95, platforms: ['twitter', 'instagram'] },
  { id: '2', topic: 'Music NFTs', volume: 45000, volumeChange: 120, sentiment: 58, relevanceScore: 75, platforms: ['twitter', 'youtube'] },
  { id: '3', topic: '#IndieArtist', volume: 89000, volumeChange: 12, sentiment: 78, relevanceScore: 90, platforms: ['instagram', 'tiktok'] },
  { id: '4', topic: 'Spotify Algorithm', volume: 34000, volumeChange: -5, sentiment: 45, relevanceScore: 85, platforms: ['twitter', 'youtube'] },
  { id: '5', topic: '#MusicProduction', volume: 156000, volumeChange: 8, sentiment: 71, relevanceScore: 80, platforms: ['tiktok', 'youtube'] },
];
...
const MOCK_INFLUENCERS: Influencer[] = [
  {
    ...
    mentionedYou: 5,
    sentiment: 'positive',
    topics: ['production', 'beats', 'tutorials'],
    verified: true,
  },
  {
    id: '2',
    name: 'Indie Music Blog',
    username: 'indiemusicblog',
    platform: 'twitter',
    followers: 850000,
    engagement: 3.2,
    mentionedYou: 12,
    sentiment: 'positive',
    topics: ['reviews', 'new releases', 'interviews'],
    verified: true,
  },
  {
    id: '3',
    name: 'Beat Maker Daily',
    username: 'beatmakerdaily',
    platform: 'tiktok',
    followers: 1800000,
    engagement: 8.5,
    mentionedYou: 3,
    sentiment: 'neutral',
    topics: ['tutorials', 'beat battles', 'gear reviews'],
    verified: false,
  },
];
...
const MOCK_ALERTS: CrisisAlert[] = [
  {
    ...
    description: 'Detected 15% increase in negative mentions related to "payment issues"',
    mentions: 45,
    sentiment: -28,
    keywords: ['payment', 'refund', 'issue'],
  ...
  },
  {
    ...
    description: 'A positive post mentioning your brand is gaining traction',
    mentions: 2500,
    sentiment: 85,
...
export function SocialListening() {
  ...
  const { data: keywordsData, isLoading: keywordsLoading } = useQuery({
    ...
    placeholderData: { keywords: MOCK_KEYWORDS },
  });

  const { data: trendingData, isLoading: trendingLoading } = useQuery({
    queryKey: ['/api/social/listening/trending'],
    placeholderData: { topics: MOCK_TRENDING },
  });

  const { data: influencersData, isLoading: influencersLoading } = useQuery({
    queryKey: ['/api/social/listening/influencers'],
    placeholderData: { influencers: MOCK_INFLUENCERS },
  });

  const { data: alertsData, isLoading: alertsLoading } = useQuery({
    queryKey: ['/api/social/listening/alerts'],
    placeholderData: { alerts: MOCK_ALERTS },
  });

  const keywords = keywordsData?.keywords || MOCK_KEYWORDS;
  const trending = trendingData?.topics || MOCK_TRENDING;
  const influencers = influencersData?.influencers || MOCK_INFLUENCERS;
  const alerts = alertsData?.alerts || MOCK_ALERTS;
