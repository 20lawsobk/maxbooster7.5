...
- **Knobs wired end-to-end:** posting-time (`optimalHours`), content (`variantCount`/`visualPriority`), hashtag/caption/CTA, and posting-format/engagement-targeting all affect real output. Wire guidance once at the shared `advancedSocialAIService.generateAdvancedContent` chokepoint so autopilot AND manual/scheduled posts are covered; it stays idempotent for autopilot (which already passes explicit values). A deterministic post-process guard backs every MaxCore hint so a knob can never silently no-op.
...
- **typecheck OOM/starvation:** `tsc --noEmit` (the `typecheck` workflow) can consume ~4GB and take >2min, starving tsx and crashing the app at boot. Kill any stray `tsc` before restarting the app; don't run a full-project typecheck alongside the running app.
