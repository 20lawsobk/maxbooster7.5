...
- **Video Generation Engine**: Three-tier cascade for Stage 6 (Video Assembly):
  1. **PyTorch Latent Video Diffusion** (`video_diffusion/`, port 8010 FastAPI) — music-conditioned 3D VAE + DiT backbone + DDIM sampler. Accepts BPM, energy, style, beat position, emotional goal, and style-blend weight directly from CreativeContext. Two-stage cascade: base 256×256/16f → SR UNet 512×512/32f. Start with `scripts/launch_api_server.sh`.
  2. **FFmpeg videoGeneratorService** — pure FFmpeg `geq` animated gradient renderer with 13 visual styles from KeyframeStyleSelector.
  3. **Placeholder** — fallback path ID when both services are unavailable.
- **video_diffusion/ Module** (`video_diffusion/`): Full PyTorch 2.x custom video diffusion repo. Layout: `models/` (autoencoder_3d, dit_video, conditioning, diffusion, sr_unet), `data/` (datasets, transforms), `train/` (train_autoencoder, train_diffusion), `infer/` (sampler, pipeline, api_server), `utils/` (distributed, logging, checkpoint, schedule), `configs/` (YAML), `scripts/` (shell launchers). MusicConditioner encodes BPM/energy/style/beat/emotional_goal into a conditioning token for every diffusion step.
- **MaxCore DigitalGPU v2** — Three-tier system, fully integrated into the video diffusion pipeline:
  - **`server/services/digitalgpu.py`** — Base GPU context (`GPUContext`): detects CUDA/MPS/CPU, wraps matmul/conv2d/softmax/silu with PyTorch dispatch, exposes `get_gpu()` singleton. Satisfies the `from digitalgpu import get_gpu` import in `layers.py`.
  - **`video_diffusion/utils/digital_gpu.py`** — `DigitalGPUManager` singleton: `DeviceCapabilities` (TF32, BF16, FlashAttn detection), `VRAMBudget` tracker, `CUDAStreamManager` (4 async streams), `PinnedMemoryPool`, `torch.compile()` wrapper. Used by pipeline.py and training loops.
  - **`video_diffusion/infer/gpu_postprocess.py`** — `DigitalGPUPostProcessor`: 13-style scene presets (mirrors `DigitalGPUInferenceBridge.ts` exactly), 7-pass PyTorch CUDA chain: color grading → bloom (separable Gaussian) → film grain → chromatic aberration → vignette → BPM flash → temporal consistency.
  - **`client/src/components/video/DigitalGPUVideoPlayer.tsx`** — React component: consumes frames_b64 + scene_name from API, drives `DigitalGPUInferenceBridge` WebGL2 chain (client-side GPU), real-time audio reactivity via bass/mid/treble props, frame player with RAF loop.
  - **API additions**: `POST /generate` now accepts `use_digital_gpu` + `temporal_smooth` flags; response includes `scene_name`, `gpu_applied`, `scene_metadata` (preset summary for client WebGL init). `GET /gpu/status` returns full device capabilities + VRAM stats + available scenes. Server post-processing runs automatically (default `use_digital_gpu=true`).
  - **Pipeline**: DiT compiled with `torch.compile(mode="reduce-overhead")`, SR-UNet with `max-autotune`. All inference runs inside `DigitalGPUManager.autocast()` for BF16/FP16 mixed precision on Ampere+.
...
- **Root cause (layer 1 — CJS bundle ordering)**: In the esbuild CJS bundle, `socialMedia`'s module-level `log` helper is declared via `var` (hoisted `undefined`) but only assigned inside a lazy init. `safeLoadRoute` calls it before that init fires — `TypeError` propagates silently, router never mounted.
- **Fix (layer 1)**: `server/routes.ts` registers the socialMedia router via an eager `await import()` before the `routeModules` lazy-load array. Old lazy-load entry removed to prevent double-registration.
- **Root cause (layer 2 — TensorFlow browser package)**: After the eager import was added, production logged `[routes] socialMedia direct load FAILED: Cannot find module '@tensorflow/tfjs-backend-webgl'`. The socialMedia router imports `contentQualityPipeline` → `unifiedAIController` → `SocialAutopilotEngine` et al. in `shared/ml/models/`, all of which do `import * as tf from '@tensorflow/tfjs'` (browser build). `@tensorflow/tfjs` auto-requires `@tensorflow/tfjs-backend-webgl` which isn't installed in the production container.
- **Fix (layer 2a — use Node.js TF)**: All 20 files in `shared/ml/models/`, `shared/ml/audio/`, and `shared/ml/utils/` had their TF import changed from `@tensorflow/tfjs` to `@tensorflow/tfjs-node`.
- **Root cause (layer 2b — missing libtensorflow.so.2)**: `@tensorflow/tfjs-node` ships `libtensorflow.so.2.9.1` inside its npm package but the `.node` binding's SONAME looks for `libtensorflow.so.2` — a versioned symlink that npm's install script doesn't always create in NixOS/Replit.
- **Fix (layer 2b — symlinks + LD_LIBRARY_PATH)**: `start.sh` now (step 4): creates `libtensorflow.so.2` → `libtensorflow.so.2.9.1` and `libtensorflow_framework.so.2` → `libtensorflow_framework.so.2.9.1` symlinks inside `node_modules/@tensorflow/tfjs-node/deps/lib/` if missing, then exports `LD_LIBRARY_PATH` to include that directory.
- **Verification**: Production bundle (`node dist/index.cjs`) with `LD_LIBRARY_PATH` set — all 6 endpoints (`platform-status`, `weekly-stats`, `ai-insights`, `posts`, `calendar`, `activity`) return **401** (auth required) instead of 404. Log shows `[routes] socialMedia router registered directly at /api/social`.
