...
-   **Startup Parallelization** (`server/index.ts` + `server/safety/databaseIndexes.ts`):
    ...
    - **Net result**: From "serving on port 5000" to "all middleware and routes registered" dropped from ~24 s to ~17 s (measured in dev with all 51 indexes pre-existing).
