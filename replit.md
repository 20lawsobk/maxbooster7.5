...
-   **LuaExecutor tuning** (`server/lib/luaExecutor.ts`): `MAX_CONCURRENT_WORKERS` reduced from 3 → 2 (less PDIM HTTP concurrency) and the per-script execution timeout reduced from 60 s → 20 s (stale wasmoon Workers fail 3× faster, freeing semaphore slots sooner). Each redis.call() through PDIM has a 500 ms commandTimeout + 1 retry ≈ 1 s max, so BullMQ scripts doing 10–20 calls worst-case need at most 20 s. Old 60 s kept 3 slots locked for a full minute during PDIM load, saturating the Node.js event loop and causing 30–40 s response times site-wide (including the Settings page and auth/me endpoint). Boot-burst LuaExecutor timeouts are handled automatically by ChainFixer semaphore resets.
