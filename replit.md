...
- **9-Phase Optimization Pass (2026-03)**: Dead code removed (splitsContract.ts, splitService.ts, royaltiesForecastingService.ts); releaseWorkflow.ts confirmed live (imported by distribution.ts); careerCoach.ts `previousAnalytics` and `previousRevenue` queries fixed (both now correctly query the 30–60 day window using `lte` — growth rate and revenue trend calculations are now accurate); all careerCoach 500 error responses normalized from `{message:}` to `{error:}` to match codebase convention; marketplace.ts hardened (PUT /license-templates uses partial schema safeParse, PATCH /contracts uses schema safeParse); songwriting.ts enhanced (55+ word rhyme dict, 10 new genre progressions, 4 mood overrides, /stats endpoint with caching, search/filter support); shows.ts rewritten (safe date validation via refine, upcoming/past filter on list, enriched stats with upcomingCount/pastCount/pastRevenue, error messages normalized); autoPostingService.ts and autoPostingServiceV2.ts deduplicated — shared detectHookPattern extracted to server/services/postingUtils.ts; apiKeys.ts rate-limited (10 ops/hour per user, 20-key cap).
