...
-   **Python AI**: Python 3.11 module installed; packages (`numpy`, `Pillow`) live in `.venv/` (created by `npm run setup:python`, which is prepended to all build scripts). Server auto-creates the venv on first startup if missing (`ensureVenv()` in `server/index.ts`). All `spawn('python3', ...)` calls now use the `PYTHON` constant from `server/services/pythonPath.ts`, which resolves to `.venv/bin/python3`. `requirements.txt` in the project root tracks the pip dependencies.