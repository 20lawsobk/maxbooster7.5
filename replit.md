...
-   **Deployment builds**: Use `npm run build:prod` for deployment — runs a full fresh TypeScript/Vite compile followed by `npm prune --production`. `security-fix.ts` is only used in `npm run build` (dev), never in `build:prod`. Key `.dockerignore` exclusions: `.git` (2.2 GiB), `node_modules/` (reinstalled fresh), source dirs not needed at runtime (`attached_assets`, `docs`, `tests`, etc.). Always use "Clear build cache" in the Deployments pane when redeploying after environment changes (Python module removal, etc.).
