...
-   **Deployment builds**: Use `npm run build:prod` for deployment. The build command checks for pre-built `dist/index.cjs` + `dist/public/index.html` and skips the TypeScript/Vite compile if both exist (pre-build strategy). Only `npm prune --production` runs in the container. `security-fix.ts` is only used in `npm run build` (dev) — never in `build:prod`. Key `.dockerignore` exclusions: `client/src`, `server`, `script` (source not needed since dist/ is pre-built), `.git` (2.2 GiB), `node_modules/` (reinstalled fresh). Before redeploying, always run `npm run build:deploy` locally to refresh `dist/` artifacts.
