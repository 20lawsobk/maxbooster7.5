...
- **`@tensorflow/tfjs-node` native libs preserved**: `build.sh` previously deleted `node_modules/@tensorflow/tfjs-node/deps/` (containing `libtensorflow.so.2.9.1`, ~242 MB) before packing the PDIM capsule. `start.sh` adds `deps/lib` to `LD_LIBRARY_PATH` for native TF acceleration — removing it caused the `WARNING: @tensorflow/tfjs-node deps/lib not found` startup warning and degraded all ML inference. The folder is now preserved and travels in the `node_modules.pdim` capsule.
- **`boosterstate` sidecar compiled and committed**: Rust `rust-stable` module added to `.replit`. The boosterstate Rust sidecar (key-value store with WAL, axum HTTP layer, CORS) is pre-compiled to `bin/boosterstate` (1.7 MB ELF binary) and committed to the repo via `!bin/boosterstate` exception in `.gitignore`. `start.sh` finds it at `./bin/boosterstate` and starts it on `BOOSTERSTATE_SIDECAR_PORT` (default 9877) with bearer-token auth. `build.sh` also recompiles from source when cargo is available.
