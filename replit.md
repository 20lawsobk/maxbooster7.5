...
- **Root cause (deep)**: In the esbuild CJS bundle, `socialMedia`'s module-level `log` helper (`Jt`) is declared via `var` (hoisted as `undefined`) but only assigned inside a lazy module initializer (`xae()`). In certain production bundle initialization orderings, `ZRe()` (safeLoadRoute) calls `Jt()` before `xae()` has run, causing `TypeError: Jt is not a function`. The catch block also calls `Jt()`, so it throws too — the error propagates silently and the router is never mounted. This is a CJS bundle lazy-init ordering hazard that only surfaces in production (dev runs TypeScript directly, so ordering is different).
- **Fix (v2 — definitive)**: `server/routes.ts` now registers the socialMedia router via an **eager `await import()`** immediately before the `routeModules` lazy-load array. This bypasses the lazy-init ordering entirely. The old lazy-load entry for `socialMedia` is removed from the `routeModules` array to prevent double-registration. Both `dist/index.cjs` and `dist/cluster.cjs` rebuilt and committed. `console.log` confirmation messages are added for production log visibility.
- **Verification**: Production bundle tested locally — all `/api/social/platform-status`, `/weekly-stats`, `/ai-insights`, `/posts` now return **401** (auth required) instead of 404.
