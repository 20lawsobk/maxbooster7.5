...
- **Read replica query failure**: `dbRead` pointed to a Neon read replica (`DATABASE_REPLICA_URLS`) that was failing with `Error: Failed query: select ... from "users"`. **Fix:** Removed the deprecated `neonConfig.fetchConnectionCache = true` (now always-on in the Neon driver). Added `verifyReadReplica()` in `db.ts` — called once at server startup — that runs a live `SELECT 1` against the replica. If it fails for any reason (bad URL, Neon proxy rejection, cold-start timeout), `dbRead` is permanently re-pointed to the primary `db` with a loud error log. No per-query try/catch needed; `storage.ts` auth methods are clean. Startup logs now clearly indicate whether the replica is active or fallen back.
