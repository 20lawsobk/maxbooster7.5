...
- **Long-running jobs must use workflows, not `nohup`:** backgrounded shell procs (`nohup … &`) are reaped when a tool call returns, so a cold full `tsc` (>120s) cannot run via `nohup &`+poll — it dies before the next poll and leaves an EMPTY output file (which greps as a FALSE "0 errors"). Run via the `tccap` workflow (Replit-managed, persistent) + poll `/tmp/tc_done`; copy `/tmp/tc_*.txt` to `.local/` before any restart wipes `/tmp`. See `.agents/memory/background-procs-dont-survive.md`.
