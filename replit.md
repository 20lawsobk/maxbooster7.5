...
- **Professional Artist Audit Fixes (2026-04)**: Comprehensive platform audit fixed: (1) Dashboard `socialReach` growth now computed from real DB analytics (30-60 day comparison); dashboard `recentActivity` now built from real projects/releases/shows sorted by timestamp. (2) Shows `PATCH /api/shows/:id` route added (client sent PATCH, server only had PUT); `status` field added to `createShowSchema`. (3) Fan Hub `/api/fan-hub/message` broadcast now actually sends emails via SendGrid to all subscribers in batches of 50 (fire-and-forget, respects circuit breaker). `emailService.send()` public method added. (4) Analytics dashboard: `streams.weekly` and `streams.monthly` now computed from daily DB data (no longer empty arrays); `streams.byTrack` populated from user releases with proportional stream distribution; `revenue.monthlyRevenue` and `revenue.yearlyRevenue` now from real DB queries. (5) Playlist Pitching `conversionRate` now counts both `accepted` AND `placed` statuses as successful conversions. (6) PressKit "Download Technical Rider" button now functional — downloads rider content as `.txt` file. (7) Social Media Content Calendar: added `POST /api/social/calendar` (create post), `PUT /api/social/calendar/:postId` (update post), `DELETE /api/social/calendar/:postId` (delete post) — entire calendar tab was broken without these. (8) Royalties payout balance: `instantPayoutService.calculateAvailableBalance` now sums both marketplace order earnings AND `royalty_transactions` table — streaming royalties are now included in the withdrawable balance. (9) Royalties download statement: `handleDownloadStatement` in `Royalties.tsx` now calls correct endpoint `/api/payouts/statements/:id/download` (was calling non-existent `/api/royalties/download-statement/:id`). (10) Studio recording: `useAudioRecorder.uploadRecording` now posts to correct `/api/studio/record/upload` endpoint (was pointing to non-existent `/api/studio/projects/:id/tracks/:id/clips/upload`) and passes `projectId`/`trackId` in body as the server expects. (11) Distribution ReleaseWizard: wizard now does proper two-step submission — first POSTs JSON metadata to `POST /api/distribution/releases`, then uploads each audio file individually to `POST /api/distribution/releases/:id/tracks` with correct `audio` field name (was sending all tracks as `audio_0`/`audio_1` to a JSON-only endpoint). (12) Advertising image upload: `POST /api/advertising/upload-image` replaced 501 stub with real multer handler that stores images via the upload service. (13) Advertising lookalike audiences: added `POST /api/advertising/lookalike-audiences` and `PATCH /api/advertising/lookalike-audiences/:id` routes (frontend mutations were calling these but they didn't exist on the server).
