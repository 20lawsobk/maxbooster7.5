...
- **Performance Hardening**: Pagination, Redis query caching, composite DB indexes, Neon PostgreSQL, and request correlation IDs. All Redis middleware operations (rate limiters, admission control, session store) have 400–500ms timeouts with automatic fallback to in-process or PostgreSQL stores — preventing PDIM congestion from stalling API requests. Steady-state response times: 200–400ms; startup congestion: 500–2000ms.
