...
- **UI Bug Fixes (2026-03, post-optimization)**: Comprehensive end-to-end UI testing identified and fixed 3 crash/access issues: (1) `Workspaces.tsx` lines 347+536 — `role.charAt(0)` and `selectedWorkspace.type.charAt(0)` crashed when API returned undefined values; fixed with `|| 'member'` / `|| 'project'` null guards; (2) 8 collaboration components (AccessRequestDialog, CommentThread ×2, ConflictResolutionDialog, LiveEditingBanner ×2, UserPresenceIndicator, VersionHistory) had bare `.charAt(0)` on potentially-undefined string properties — all guarded with `|| '?'` fallbacks; (3) `KYCReview.tsx` used non-existent `user?.isAdmin` property (always `undefined`) — replaced with `user?.role === 'admin'` in both the query `enabled` guard and the render-guard. Client bundle rebuilt via Vite after each fix batch.
