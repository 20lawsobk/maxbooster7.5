...
-   **Performance Hardening**: Pagination, Redis query caching, composite DB indexes, Neon PostgreSQL, and request correlation IDs are implemented. New production-scale indexes added: `social_accounts(is_active, token_expires_at)` to eliminate the recurring slow query (250-280ms/minute) from the token refresh monitor; `social_accounts(user_id)`, `social_accounts(platform, user_id)`, `autopilot_preferences(user_id)`, `autopilot_preferences(is_active)` for autopilot scheduler performance. Additional indexes for marketplace commerce: `orders(seller_id)`, `orders(seller_id, status)` (composite for getProducers N+1 fix), `orders(user_id)` (buyer), `orders(status)`, `storefront_follows(storefront_id)`, `storefront_follows(user_id)`, `storefront_ratings(storefront_id)`, `storefronts(user_id)`. Index DDL switched from `CREATE INDEX CONCURRENTLY` to `CREATE INDEX IF NOT EXISTS` to avoid Neon serverless transaction-block restrictions. `getAllEnabledAutopilotConfigs()` now uses a targeted SQL WHERE filter (`preferences->>'advertisingAutopilotConfig' enabled = true`) instead of a full users table scan. Social engagement analytics refresh cron added (8-hour interval) in server/index.ts, wiring `advertisingDispatchService.collectAllActiveEngagement()`.
...
- **`POST /api/audio/transcribe`** — MIDI transcription via basic-pitch. Accepts `{ filePath: string }`, returns `{ midi_path, note_count, notes[], processing_time_ms }`. MIDI files saved to `uploads/midi/`. Python handler: `POST /analyze/transcribe` on port 9878.
...
- Python AI service exposes `/analyze/audio`, `/analyze/transcribe`, and `/analyze/audio-features` endpoints on port 9878.
