...
-   **MaxBooster Veo Music**: A Veo-style music video campaign generation pipeline in `maxbooster_veo_music/`. Uses HyperGPU (512 lanes, 8 tensor cores) to generate platform-specific video assets from BoostSheets + audio. Architecture: AudioEncoder → TextEncoder → BoostSheetEncoder → VideoLatentVAE → VideoGenerator → PlatformHeads → CampaignGenerator. Supports YouTube (full_video), TikTok (hook_clip), Spotify Canvas (loop_visualizer), Instagram Reels, Shorts, and more. Generates multi-platform campaigns in ~2s with 7K+ GPU ops. Includes synthetic dataset generation for training.
