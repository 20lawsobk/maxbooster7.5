...
**Bug 4 (Admin login 500 — deployment restarts + read replica failure):**
Two root causes compounded each other:
- **Deployment health check loop**: Replit's health check always hits `/`. The React app catch-all (`serveStatic`) is registered AFTER a ~2-minute async init window, so `/` returned 500 during that window. Replit detected the 500 and restarted the deployment every 2–3 minutes — the server never stabilized long enough for anyone to log in. **Fix:** Registered an `app.get('/')` in `startup-probes.ts` (before async init) that immediately reads and serves `dist/public/index.html` — the real React app HTML — so `/` returns 200 from the first millisecond the process starts.
- **Read replica query failure + auth routing**: `dbRead` (Neon read replica) was failing with `Error: Failed query: select ... from "users"` on every login. Root cause had two layers:
  1. `verifyReadReplica()` was never actually running — it was called via `await import('./db.js')` (a dynamic import in `index.ts`). In the esbuild CJS bundle, this tries to find `dist/db.js` at runtime, which doesn't exist (everything is inlined into `dist/index.cjs`). So the verification silently failed and `dbRead` stayed broken.
  2. Even if fixed, auth queries should NOT use the read replica — authentication requires the latest committed data and cannot tolerate replica lag. The read replica is for analytics/dashboard reads.
  **Fix:** (a) `verifyReadReplica()` is now statically imported in `index.ts` (`import { verifyReadReplica } from "./db.ts"`) so esbuild inlines it correctly and it actually runs. (b) Added `import { sql } from 'drizzle-orm'` as a static import in `db.ts`. (c) `getUser`, `getUserByEmail`, `getUserByUsername`, `getUserByPasswordResetToken` in `storage.ts` now use the primary `db` directly — correct database architecture. The read replica (`dbRead`) remains active for all other queries (analytics, platform data, etc.).
