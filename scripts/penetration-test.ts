...
async function checkAuthGuards() {
  ...
  for (const route of protectedRoutes) {
    try {
      ...
      check(
        `${route} requires auth (got ${r.status})`,
...
async function checkSecurityHeaders() {
  ...
  try {
    ...
    check(
      ...
      !!sts || process.env.NODE_ENV !== "production",
    );
...
async function checkErrorLeakage() {
  ...
  try {
    ...
    check(
      "Login error does not contain stack trace",
      !text.includes("at ") || !text.includes(".ts:"),
    );
    check(
      "Login error does not expose DB query",
      !text.toLowerCase().includes("select ") &&
        !text.toLowerCase().includes("pg:"),
    );
...
async function checkSQLInjection() {
  ...
  for (const payload of sqlPayloads) {
    try {
      ...
      check(
        `SQL payload rejected or handled safely: ${payload.slice(0, 25)}... (${r.status})`,
        r.status !== 200,
      );
    } catch {
      check(
        `SQL payload attempt — server reachable for: ${payload.slice(0, 25)}...`,
        false,
      );
    }
  }
...
async function checkXSSResistance() {
  ...
  try {
    ...
    const api = await post("/api/auth/login", {
      email: "xss-test@nowhere.invalid",
      password: "x",
    });
...
async function checkRateLimitHeaders() {
  ...
  try {
    const r = await post("/api/auth/login", {
      email: "ratetest@test.invalid",
      password: "wrong",
    });
    // A single failed login must NOT instantly hit the rate limit
    check(
      "Single failed login does not immediately trigger 429",
      r.status !== 429,
    );
    // Must be a 4xx (wrong creds) not a server error
    check(
      "Login returns 4xx for wrong credentials (not 5xx)",
...
async function checkMetricsProtected() {
  ...
  try {
    ...
    check(
      `GET /metrics requires auth (got ${r.status})`,
  ...
  try {
    ...
    check(
      `GET /api/audit-logs requires auth (got ${r.status})`,
      [401, 403, 404].includes(r.status),
    );
  } catch {
...
async function checkAdminRoutes() {
  ...
  for (const route of adminRoutes) {
    try {
      const r = await get(route);
      check(
        `${route} requires admin (got ${r.status})`,
        [401, 403, 404].includes(r.status),
      );
    } catch {
      check(`${route} — reachable`, false);
...
async function checkPublicEndpoints() {
  ...
  for (const route of publicRoutes) {
    try {
      const r = await get(route);
      check(
        `${route} is publicly accessible (got ${r.status})`,
        r.status === 200,
      );
    } catch {
      check(`${route} — reachable`, false);
...
async function checkPathTraversal() {
  ...
  for (const path of payloads) {
    try {
      ...
      check(
        `Path traversal blocked: ${path.slice(0, 40)} (${r.status})`,
        r.status !== 200 || !(await r.clone().text()).includes("root:"),
      );
    } catch {
      // Connection error is fine — means the request was refused
      check(
        `Path traversal blocked (connection refused): ${path.slice(0, 40)}`,
        true,
      );
    }
  }
