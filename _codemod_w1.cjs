...
if (process.env.ONLY_FILES) {
  ...
  for (const k of [...errsByFile.keys()])
    if (!only.has(k)) errsByFile.delete(k);
}
...
function hasDecorators(node) {
  const mods = ts.canHaveDecorators?.(node)
...
for (const [file, errs] of errsByFile) {
  ...
  for (const e of errs) {
    let pos;
    try {
      pos = sf.getPositionOfLineAndCharacter(e.line - 1, e.col - 1);
    } catch {
      bump("bad_pos");
      continue;
    }
    ...
    let n = node;
    // ---------- imports ----------
    const imp = (function up(x) {
      while (x) {
        if (ts.isImportDeclaration(x)) return x;
        x = x.parent;
      }
      return null;
    })(n);
    ...
    const vd = (function up(x) {
      while (x) {
        if (ts.isVariableDeclaration(x)) return x;
        if (ts.isFunctionLike(x) || ts.isClassLike(x) || ts.isStatement(x))
          return null;
        x = x.parent;
      }
      return null;
    ...
    if (
      ...
      (ts.isIdentifier(vd.name) ||
        e.code === "6198" ||
        ts.isObjectBindingPattern(vd.name) ||
        ts.isArrayBindingPattern(vd.name))
    ) {
      ...
      if (ts.isVariableStatement(stmt) && list.declarations.length === 1) {
        ...
        if (!isConst) {
          bump("skip_nonconst_local");
          continue;
        } // let/var may be write-only
        // case: identifier name, OR all-unused destructure (6198), OR object/array pattern node itself flagged
        const nameIsIdent =
          ts.isIdentifier(vd.name) && vd.name.getStart(sf) === pos;
        const isWholeDecl =
          nameIsIdent || (e.code === "6198" && vd.name.getStart(sf) === pos);
        if (isWholeDecl) {
          if (!vd.initializer) {
        ...
        if (ts.isObjectBindingPattern(vd.name)) {
          const be = (function up(x) {
            while (x) {
              if (ts.isBindingElement(x)) return x;
              x = x.parent;
            }
            return null;
          })(n);
          if (be && be.parent === vd.name) {
            ...
            edits.push({
              ...
              text: "",
              tag: "obj_binding_remove",
            });
            continue;
          }
        }
      }
      bump("skip_var_complex");
      continue;
    }
    // ClassDeclaration
    const cls = (function up(x) {
      while (x) {
        if (ts.isClassDeclaration(x)) return x;
        x = x.parent;
      }
      return null;
    })(n);
    if (cls && cls.name && cls.name.getStart(sf) === pos) {
      if (hasDecorators(cls)) {
        bump("skip_class_decorated");
        continue;
      }
    ...
    // FunctionDeclaration
    const fn = (function up(x) {
      while (x) {
        if (ts.isFunctionDeclaration(x)) return x;
        x = x.parent;
      }
      return null;
    })(n);
    ...
    if (ta && ta.name && ta.name.getStart(sf) === pos) {
      ...
      edits.push({
        ...
        text: "",
        tag: ts.isEnumDeclaration(ta) ? "enum_remove" : "type_remove",
        ctx: _ctx,
      });
      continue;
    }
    // TS6138 class member (private property/method)
    const mem = (function up(x) {
      while (x) {
        if (ts.isPropertyDeclaration(x) || ts.isMethodDeclaration(x)) return x;
        x = x.parent;
      }
      return null;
    })(n);
    if (mem && mem.name && mem.name.getStart(sf) === pos) {
      if (!ts.isIdentifier(mem.name)) {
        bump("skip_member_nonident");
        continue;
      }
      if (
        ts.isPropertyDeclaration(mem) &&
        mem.initializer &&
        hasSideEffects(mem.initializer)
      ) {
        bump("skip_member_sideeffect");
        continue;
      }
      if (hasDecorators(mem)) {
        bump("skip_member_decorated");
        continue;
      }
      ...
      if (
        new RegExp("(?:this|super)\\s*\\??\\.\\s*" + esc + "\\b").test(text) ||
        new RegExp("['\"`]" + esc + "['\"`]").test(text)
      ) {
        bump("skip_member_referenced");
        continue;
      }
      ...
      edits.push({
        ...
        text: "",
        tag: ts.isMethodDeclaration(mem) ? "method_remove" : "property_remove",
        ctx: _ctx,
      });
      continue;
    }
    ...
    if (e.code === "6133") {
      const param = (function up(x) {
        while (x) {
          if (ts.isParameter(x)) return x;
          if (ts.isFunctionLike(x)) break;
          x = x.parent;
        }
        return null;
      })(n);
      if (
        ...
        param.name.getStart(sf) === pos
      ) {
        // skip parameter properties (constructor private/public/readonly) and decorated params
        if (param.modifiers && param.modifiers.length) {
          bump("skip_param_property");
          continue;
        }
        if (hasDecorators(param)) {
          bump("skip_param_decorated");
          continue;
        }
        const pname = param.name.getText(sf);
        if (pname.startsWith("_")) {
          bump("skip_param_already_underscore");
          continue;
        }
        ...
        const paramText = fnNode.parameters
          .map((p) => p.name.getText(sf))
          .join(",");
        if (new RegExp("\\b" + escN + "\\b").test(paramText)) {
          bump("skip_param_collision");
          continue;
        }
        edits.push({
          start: param.name.getStart(sf),
          end: param.name.getEnd(),
          text: newName,
          tag: "param_underscore",
        });
        continue;
      }
    }
  ...
  for (const [imp, names] of importGroups) {
    ...
    if (!ic) {
      bump("skip_import_sideeffect");
      continue;
    }
    let keepDefault =
      ic.name && !names.has(ic.name.getText(sf)) ? ic.name.getText(sf) : null;
    let namedParts = [];
    let namespacePart = null;
    if (ic.namedBindings) {
      if (ts.isNamespaceImport(ic.namedBindings)) {
        if (!names.has(ic.namedBindings.name.getText(sf)))
          namespacePart = ic.namedBindings.getText(sf);
      } else {
    ...
    if (clauseBits.length === 0) {
      ...
      const repl =
        "import " + typeOnly + clauseBits.join(", ") + " from " + mod + ";";
      edits.push({
  ...
  if (sf2.parseDiagnostics && sf2.parseDiagnostics.length) {
    filesReverted++;
    bump("REVERTED_parse");
    continue;
  }
