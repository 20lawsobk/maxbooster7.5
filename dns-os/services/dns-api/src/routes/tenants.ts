...
export default async function tenantsRoutes(app: FastifyInstance) {
  app.post<{ Body: { name: string } }>('/', {
    schema: {
      body: { type: 'object', required: ['name'], properties: { name: { type: 'string', minLength: 1, maxLength: 255 } } },
    },
  }, async (req, reply) => {
    const { name } = req.body;
    const { rows } = await pool.query<{ id: string; name: string; api_key: string; created_at: string }>(
