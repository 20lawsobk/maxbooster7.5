    ...
    try {
      ...
      return (data.results || [])
        .map((r) => {
          ...
          return { id, ...cls, title: `[Search] ${r.title}`, description: r.content.slice(0, 500), detectedAt: new Date(), sourceUrl: r.url, feedSource: "Tavily" } satisfies LiveIndustrySignal;
        })
        .filter((x): x is LiveIndustrySignal => x !== null);
    } finally {
      clearTimeout(timeout);
    }
  }
    ...
    try {
      ...
      return (data.results || [])
        .map((r) => {
          ...
          return { id, ...cls, title: `[Search] ${r.title}`, description: text.slice(0, 500), detectedAt: new Date(), sourceUrl: r.url, feedSource: "Exa" } satisfies LiveIndustrySignal;
        })
        .filter((x): x is LiveIndustrySignal => x !== null);
  } finally {
      clearTimeout(timeout);
