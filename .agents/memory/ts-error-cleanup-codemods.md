...
- `grep -c DOMPurify file` counts _lines_, not occurrences — a one-lined corruption shows `1` while having 16,382 tokens. Use `grep -oE DOMPurify | wc -l`.
...
**How to apply:** find the innermost = argument of the _last_ `DOMPurify.sanitize(` (deepest opened); replace the whole `__html` value with it.
...
- **Unused classes/functions/interfaces** → deletion is the proper fix but is a _significant change_; flagged-unused is reliable only within TS program visibility (`allowJs:false` hides any JS-side consumers).
...
## Library getter removal leaves TWO error shapes (zod 4 `.errors`→`.issues`)

When a library removes a member (zod 4 dropped the `ZodError.errors` getter; `.issues` is the canonical `ZodIssue[]`), the old usages split into two populations:

- **Typed sites** → emit the targetable diagnostic (here TS2339 `Property 'errors' does not exist on type 'ZodError<...>'`). A position-driven codemod can swap the token. `errors`→`issues` is the same length, so replacing by exact `(line,col)` has ZERO column drift even with multiple per line — verify the 6 chars read `errors` and are preceded by `.` first.
- **Untyped catch sites** → `catch (error)` makes `error` `unknown`, so `error.errors` is TS18046 (or silent if `any`), NOT the TS2339 the codemod sweeps. These compile-pass but **silently return `undefined` at runtime** (latent bug). Fix by narrowing: `error.name === "ZodError"` → `error instanceof z.ZodError`, then `error.issues` (also clears the TS18046 on `error.name`).
  **Why:** the TS2339 sweep alone leaves the silent runtime breakage in catch blocks. **How to apply:** after any getter-removal codemod, also `rg` the old member name in catch/`unknown`/`any` contexts and convert the name-string check to `instanceof`. `.issues` is behavior-identical to the old `.errors` (same array); `flatten()`/`format()` are unaffected.

## AST ancestor-climb must respect scope boundaries (the regression that bit us)

When mapping a tsc diagnostic (line:col → AST node) up to the _declaration_ it belongs to, the ancestor walk MUST stop at the first `ts.isFunctionLike(x) || ts.isClassLike(x) || ts.isStatement(x)` boundary. Otherwise an unused identifier **nested inside a callback within an initializer** (e.g. an unused param in `const m = useMutation({ onError: (e)=>... })`) climbs PAST the arrow function to the outer `const` and the codemod deletes/side-effect-converts a _still-used_ declaration → TS2304 regressions.
