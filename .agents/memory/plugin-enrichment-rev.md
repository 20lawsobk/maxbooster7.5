...
**Known limitation:** `plugin_presets` has no unique constraint on `(plugin_id, name)` for factory rows. With multiple cluster workers, the first boot of a new `MANIFEST_REV` has a small window where two workers can both pass the existence check and double-insert. After that boot the rev-gate makes seeding a no-op. If duplicates become a problem, add a partial unique index: `CREATE UNIQUE INDEX plugin_presets_factory_uniq ON plugin_presets (plugin_id, name) WHERE is_factory AND user_id IS NULL;` and add `onConflictDoNothing()` to the insert path.
