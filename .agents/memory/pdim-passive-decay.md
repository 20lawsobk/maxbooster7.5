...
3. **The fast-fail path _prevents_ the successes that would decay the gap.** When estimated chain wait exceeds `_MAX_DIRECT_WAIT_MS`, callers fall back to PG/in-memory and **never make the HTTP request** that would call `_pdimAdaptSuccess()`. Vicious cycle: high gap → high wait → fast-fail → no success → gap stays high.
...
Doing so re-introduces the sawtooth the existing comment in `_pdimAdaptSuccess` warns about: under sustained load, a large additive step collapses the gap straight back to floor, triggering another 429 within a handful of requests, infinitely. The passive decay solves the _idle-recovery_ case without touching the _under-load_ case.
