...
describe("AI Analytics & Fanbase Insights — Real Data Endpoints", () => {
    ...
    if (res.status === 200) {
      ...
      expect(
        "totalFans" in body ||
          body.fans !== undefined ||
          body.data !== undefined ||
          body.fanbase !== undefined ||
          Array.isArray(body.topPlatforms),
      ).toBe(true);
    }
  });
  ...
  it("GET /api/analytics/milestones → 200 with milestones array", async () => {
    ...
    if (res.status === 200) {
      ...
      expect(
        Array.isArray(body) ||
          Array.isArray((body as Record<string, unknown>).milestones) ||
          body !== null,
      ).toBe(true);
    }
  });
