...
async function runTests(): Promise<TestResult[]> {
  ...
  results.push({
    ...
    passed:
      systemResult.status === 200 &&
      (systemResult.data?.status === "ok" ||
        systemResult.data?.status === "operational" ||
        systemResult.data?.healthy === true),
  ...
  results.push({
    ...
    passed:
      circuitResult.status === 200 &&
      circuitHealthy === circuitTotal &&
      circuitTotal >= 12,
    responseTime: circuitResult.responseTime,
    details: circuitResult.data?.circuits
      ? `Circuits: ${Object.keys(circuitResult.data.circuits).join(", ")}`
      : undefined,
  });

  // Test 4: Distribution Platforms
  console.log("Testing P0-04: Distribution Platforms...");
  const platformResult = await makeRequest(
    "GET",
    "/api/distribution/platforms",
  );
  const platformCount = Array.isArray(platformResult.data)
    ? platformResult.data.length
    : platformResult.data?.platforms
      ? platformResult.data.platforms.length
      : 0;
  results.push({
    id: "P0-04",
  ...
  const templateCount = Array.isArray(contractResult.data)
    ? contractResult.data.length
    : contractResult.data?.templates
      ? contractResult.data.templates.length
      : 0;
  results.push({
    id: "P0-05",
    ...
    details: Array.isArray(contractResult.data)
      ? `Templates: ${contractResult.data
          .slice(0, 3)
          .map((t: any) => t.name || t.type || t)
          .join(", ")}...`
      : undefined,
  });
  ...
  const hasHtml =
    typeof frontendResult.data === "string" &&
    (frontendResult.data.includes("<!DOCTYPE") ||
      frontendResult.data.includes("<html") ||
      frontendResult.data.includes("<div"));
  results.push({
    id: "P0-07",
    name: "Frontend Loads",
    endpoint: "GET /",
  ...
  const billingExists =
    billingResult.status === 200 || billingResult.status === 401;
  results.push({
    id: "P0-08",
  ...
  results.push({
    ...
    passed:
      ...
      execResult.status === 401 ||
      execResult.status === 403,
  ...
  results.push({
    ...
    passed:
      secResult.status === 200 ||
      secResult.status === 401 ||
  ...
  const presetExists =
    presetResult.status === 200 || presetResult.status === 401;
  const presetCount = presetResult.data?.presets
    ? presetResult.data.presets.length
    : 0;
  results.push({
    id: "P0-12",
