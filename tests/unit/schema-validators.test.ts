...
describe('insertUserSchema', () => {
  ...
  it('allows missing firstName and lastName (optional)', () => {
    const result = insertUserSchema.safeParse({ email: validUser.email, password: validUser.password });
    expect(result.success).toBe(true);
  });

  it('does not accept extra fields in strict mode', () => {
    const strict = insertUserSchema.strict();
    const result = strict.safeParse({ ...validUser, role: 'admin', isAdmin: true });
    expect(result.success).toBe(false);
  });
});
