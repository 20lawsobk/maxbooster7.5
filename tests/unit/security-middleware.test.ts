...
describe('Mandatory middleware stack', () => {
  ...
  it('applies rate limiting', () => {
    const hasLimit = src.includes('rateLimit') || src.includes('rateLimiter') || src.includes('RateLimit');
...
describe('Session security', () => {
  ...
  it('production check validates SESSION_SECRET is set', () => {
    expect(defaultsSrc).toMatch(/SESSION_SECRET[\s\S]{0,200}production|production[\s\S]{0,200}SESSION_SECRET/);
  });
});
