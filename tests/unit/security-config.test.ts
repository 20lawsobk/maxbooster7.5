...
describe("Auth guards", () => {
  it("requireAuth is used on protected routes", () => {
    ...
    const authChecks = (
      mainRoutes.match(/Not authenticated|isAuthenticated\(\)|requireAuth/g) ||
      []
    ).length;
  ...
  it("JWT secret has no dev fallback in jwtAuthService", () => {
    ...
    const hasFallback =
      src.includes("|| 'dev-secret'") || src.includes('|| "dev-secret"');
...
describe("Audit logging", () => {
  ...
  it("audit logger is imported in request/error middleware", () => {
    ...
    const hasAudit =
      requestLogger.includes("auditLogger") ||
      errorHandler.includes("auditLogger");
