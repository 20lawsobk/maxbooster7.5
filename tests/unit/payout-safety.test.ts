...
describe('Instant payout service — safety patterns', () => {
  ...
  it('payout code uses an idempotency or lock mechanism', () => {
    for (const file of files) {
      ...
      if (!src) continue;
      const hasLock = src.includes('lock') || src.includes('idempotency') ||
                      src.includes('processing') || src.includes('mutex');
  ...
  it('payout routes validate the amount is positive', () => {
    const src = readSrc('server/routes/payouts.ts');
    if (src) {
      const hasValidation = src.includes('amount') && (
        src.includes('positive') || src.includes('> 0') || src.includes('>= 0') || src.includes('min(')
      );
...
describe('Payout amount validation', () => {
  function validatePayoutAmount(amount: unknown): { valid: boolean; error?: string } {
    ...
    if (amount > 50_000) return { valid: false, error: 'Amount exceeds single-payout limit' };
    return { valid: true };
  }
