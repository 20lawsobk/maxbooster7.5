...
vi.mock('../../server/services/musicIndustryContextFilter.js', () => ({
  musicIndustryContextFilter: { getContextForMode: vi.fn().mockResolvedValue(null) },
}));
...
vi.mock('../../shared/ml/nlp/ContentGenerator.js', () => ({ ContentGenerator: class {} }));
vi.mock('../../shared/ml/nlp/SentimentAnalyzer.js', () => ({ SentimentAnalyzer: class {} }));
vi.mock('../../shared/ml/models/RecommendationEngine.js', () => ({ RecommendationEngine: class {} }));
vi.mock('../../shared/ml/models/AdOptimizationEngine.js', () => ({
  AdOptimizationEngine: class { initialize = vi.fn(); },
}));
vi.mock('../../shared/ml/models/SocialAutopilotEngine.js', () => ({
  SocialAutopilotEngine: class { initialize = vi.fn(); },
}));
...
describe('applyPostingOptimization — content-type/objective bias (unified controller)', () => {
  ...
  it('maps carousel → engagement when unpinned, and respects a pinned contentType', async () => {
    ...
    expect(svc.applyPostingOptimization('tiktok', undefined).contentType).toBe('engagement');

    // Pinned → caller choice wins.
    expect(svc.applyPostingOptimization('tiktok', 'promotional').contentType).toBe('promotional');
  });
