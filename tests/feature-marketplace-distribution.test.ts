...
describe('Feature: Beat Marketplace & Music Distribution', () => {
  ...
  describe('Beat Marketplace — Public Browsing', () => {
    it('GET /api/marketplace/beats returns beat listings (public)', async () => {
      const r = await fetch(`${BASE}/api/marketplace/beats`, { signal: AbortSignal.timeout(10000) });
      expect(r.status).toBe(200);
    ...
    it('GET /api/marketplace/beats supports genre filter', async () => {
      const r = await fetch(`${BASE}/api/marketplace/beats?genre=hip-hop`, { signal: AbortSignal.timeout(10000) });
      expect(r.status).toBe(200);
    });

    it('GET /api/marketplace/beats supports BPM range filter', async () => {
      const r = await fetch(`${BASE}/api/marketplace/beats?bpmMin=80&bpmMax=120`, { signal: AbortSignal.timeout(10000) });
      expect(r.status).toBe(200);
    });

    it('GET /api/marketplace/beats supports pagination', async () => {
      const r = await fetch(`${BASE}/api/marketplace/beats?page=1&limit=10`, { signal: AbortSignal.timeout(10000) });
      expect(r.status).toBe(200);
    });

    it('GET /api/marketplace/producers returns producer list (public)', async () => {
      const r = await fetch(`${BASE}/api/marketplace/producers`, { signal: AbortSignal.timeout(10000) });
      expect(r.status).toBe(200);
    });
  ...
  describe('Marketplace auth guards', () => {
    it('GET /api/marketplace/my-beats without auth → 401/403', async () => {
      const r = await fetch(`${BASE}/api/marketplace/my-beats`, { signal: AbortSignal.timeout(8000) });
      expect([401, 403]).toContain(r.status);
    });
    it('GET /api/marketplace/purchases without auth → 401/403', async () => {
      const r = await fetch(`${BASE}/api/marketplace/purchases`, { signal: AbortSignal.timeout(8000) });
      expect([401, 403]).toContain(r.status);
    });
  });
  ...
  describe('Music Distribution', () => {
    ...
    it('GET /api/distribution/policies returns DSP submission policies', async () => {
      const r = await fetch(`${BASE}/api/distribution/policies`, { signal: AbortSignal.timeout(10000) });
      expect(r.status).toBe(200);
    });
    ...
    it('POST /api/distribution/releases creates a release', async () => {
      const r = await api('POST', '/api/distribution/releases', {
        ...
        releaseDate: new Date(Date.now() + 60 * 24 * 3600 * 1000).toISOString().split('T')[0],
        label: 'Independent',
        language: 'en',
      });
    ...
    it('PATCH /api/distribution/releases/:id updates release metadata', async () => {
      if (!releaseId) return;
      const r = await api('PATCH', `/api/distribution/releases/${releaseId}`, { label: 'Test Label LLC' });
      expect([200, 204]).toContain(r.status);
    });

    it('GET /api/distribution/releases/:id/status gets release status', async () => {
      if (!releaseId) return;
      const r = await api('GET', `/api/distribution/releases/${releaseId}/status`);
    ...
    it('GET /api/distribution/releases without auth → 401', async () => {
      const r = await fetch(`${BASE}/api/distribution/releases`, { signal: AbortSignal.timeout(8000) });
      expect([401, 403]).toContain(r.status);
    });
  });
  ...
  describe('Royalties', () => {
    ...
    it('GET /api/royalties without auth → 401', async () => {
      const r = await fetch(`${BASE}/api/royalties`, { signal: AbortSignal.timeout(8000) });
      expect([401, 403]).toContain(r.status);
    });
  });
});
