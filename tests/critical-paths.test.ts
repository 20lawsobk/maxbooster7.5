...
async function api(method: string, path: string, body?: any, useCookies = true) {
...
describe('Critical Path Tests - Production Readiness', () => {
  ...
  describe('1. User Registration & Authentication', () => {
    ...
    it('should reject invalid email format', async () => {
      const res = await api('POST', '/api/auth/register', { ...testUser, email: 'bademail' });
      expect(res.status).toBe(400);
    });

    it('should reject short password', async () => {
      const res = await api('POST', '/api/auth/register', {
        ...testUser, email: 'short@test.com', password: '12345'
      });
      expect(res.status).toBe(400);
    });
  ...
  describe('4. Social Media Management', () => {
    ...
    it('should reject publishing non-existent post', async () => {
      const res = await api('POST', '/api/social/calendar/nonexistent-id/publish');
  ...
  describe('11. Security', () => {
    ...
    it('should handle SQL injection attempt safely', async () => {
      const res = await api('GET', "/api/search/unified?q='; DROP TABLE users; --");
