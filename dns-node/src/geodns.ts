...
try {
  ...
  console.warn(
    "[GeoDNS] Invalid REGION_MAP JSON — using single IP for all regions",
  );
}
...
async function getReader(): Promise<any> {
  ...
  if (!fs.existsSync(GEODB_PATH)) {
    console.info(
      `[GeoDNS] Database not found at ${GEODB_PATH}. GeoDNS disabled.`,
    );
    return null;
  }
...
export function isGeoDnsEnabled(): boolean {
  return GEODNS_ENABLED;
}
