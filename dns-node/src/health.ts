...
function setAnycastRoute(up: boolean): void {
  ...
  try {
    ...
    bgpRouteActive = up;
    console.log(
      `[BGP] anycast0 ${up ? "UP" : "DOWN"} — route ${up ? "advertised" : "withdrawn"}`,
    );
  } catch {
...
function checkBgpHealth(healthy: boolean): void {
  ...
  if (healthy) {
    ...
    if (consecutiveFailures >= BGP_FAIL_THRESH && bgpRouteActive) {
      console.warn(
        `[BGP] ${BGP_FAIL_THRESH} consecutive failures — withdrawing anycast route`,
      );
  ...
  if (url.pathname === "/health" && method === "GET") {
    ...
    respond(
      ...
      JSON.stringify({
        ...
        version: VERSION,
        bgp: BGP_ENABLED
          ? { active: bgpRouteActive, failures: consecutiveFailures }
          : undefined,
      }),
    );
    return;
  ...
  if (url.pathname === "/reload" && method === "POST") {
    try {
      ...
      respond(
        res,
        200,
        JSON.stringify({ ok: true, message: "Zone reloaded from disk" }),
      );
    } catch (err: any) {
  ...
  if (url.pathname === "/ds-record" && method === "GET") {
    ...
    if (!ksk || !ds) {
      respond(
        res,
        503,
        JSON.stringify({ ok: false, error: "DNSSEC not initialized" }),
      );
      return;
    }
...
export function startHealthServer(): void {
  ...
  server.listen(HEALTH_PORT, HEALTH_HOST, () => {
    console.log(
      `[Health] HTTP server on ${HEALTH_HOST}:${HEALTH_PORT} (role=${ROLE})`,
    );
  });
  server.on("error", (err) => {
