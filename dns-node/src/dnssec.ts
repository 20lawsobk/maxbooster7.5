...
const KEY_DIR =
  process.env.DNSSEC_KEY_DIR || path.join(process.cwd(), "dns-node", "keys");
...
function pubKeyToRaw(pubPem: string): Buffer {
  ...
  if (raw[0] !== 0x04)
    throw new Error("[DNSSEC] Unexpected P-256 public key format");
...
function saveKeys(zone: string): void {
  ...
  for (const [prefix, kp] of [
    ["ksk", ksk!],
    ["zsk", zsk!],
  ] as const) {
    fs.writeFileSync(
      path.join(KEY_DIR, `${prefix}.priv.pem`),
      kp.privateKeyPem,
    );
    fs.writeFileSync(path.join(KEY_DIR, `${prefix}.pub.pem`), kp.publicKeyPem);
  }
  fs.writeFileSync(
    path.join(KEY_DIR, "key-meta.json"),
    JSON.stringify(
      {
        zone,
        kskTag: ksk!.keyTag,
  ...
  try {
    const makeKP = (priv: string, pub: string, flags: number): DnsKeyPair => {
      ...
      const meta = JSON.parse(
        fs.readFileSync(path.join(KEY_DIR, "key-meta.json"), "utf8"),
      );
      return {
        isKsk: flags === KSK_FLAGS,
        flags,
...
export function initDnssec(zone: string): void {
  ...
  if (!loadKeys()) {
    ...
    console.log(
      `[DNSSEC] Keys loaded — KSK tag=${ksk!.keyTag}  ZSK tag=${zsk!.keyTag}`,
    );
  }
  dnssecReady = true;
}

export function isDnssecReady(): boolean {
  return dnssecReady;
}
export function getKsk(): DnsKeyPair | null {
  return ksk;
}
export function getZsk(): DnsKeyPair | null {
  return zsk;
}
...
export function signRRset(
  ...
  try {
    ...
    sigDer = crypto.sign("SHA256", toSign, {
      key: privKey,
      dsaEncoding: "der",
    });
  } catch {
...
export function getDSRecord(zone: string): DnsRecord | null {
  ...
  const digest = crypto
    .createHash("sha256")
    .update(Buffer.concat([ownerWire, rdata]))
    .digest();
...
export function nsec3Hash(
  ...
  const wire = canonicalName(
    nameLower.endsWith(".") ? nameLower : `${nameLower}.`,
  );
...
export function nsec3HashedOwner(
  name: string,
  zone: string,
  salt: Buffer,
): string {
...
export function buildTypeBitmap(types: number[]): Buffer {
  ...
  const parts: Buffer[] = [];
  for (const [win, bits] of [...windows.entries()].sort(
    (a, b) => a[0] - b[0],
  )) {
...
export function buildNsec3Rdata(
  ...
  b[p++] = NSEC3_ALGORITHM;
  b[p++] = 0; // flags
  b.writeUInt16BE(NSEC3_ITERATIONS, p);
  p += 2;
  b[p++] = salt.length;
  salt.copy(b, p);
  p += salt.length;
