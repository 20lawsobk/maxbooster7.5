...
import { notifyWakeFromSleep } from "./python-server.js";
...
async function runHeartbeat(): Promise<void> {
  ...
  if (gap > SLEEP_GAP_MS && _heartbeatCount > 1) {
    ...
    // Notify the health monitor to hold off killing Python for 90 s so it
    // can resume from GC pause before the hung-detection threshold fires.
    notifyWakeFromSleep();
