...
import { startAutoResolver, stopAutoResolver } from "./services/autoResolver.js";
...
if (cluster.isPrimary) {
  ...
  // Keepalive: warm all endpoints so the AI server never idles.
  // Skip in proxy-only mode — the primary api-server instance already runs
  // the sweep; a second loop from the dashboard proxy doubles the load on
  // Python without adding value.
  if (process.env.DISABLE_PYTHON_SPAWN !== "1") {
    startKeepalive();
  }

  // Auto-resolver: monitors the live production URL and triggers a Replit
  // redeploy when it stays dark for FAILURE_THRESHOLD consecutive polls.
  // Skips itself when NODE_ENV=production so the production VM never tries
  // to redeploy itself.
  startAutoResolver();
