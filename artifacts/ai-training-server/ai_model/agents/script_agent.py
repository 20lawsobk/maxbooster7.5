...
_INSTRUCTION_PREFIX_RE = re.compile(
    ...
    r"write|line\s+\d|step\s+\d|part\s+\d|"
    r"\d+\s*[.)]\s*\S|"              # numbered list items: "1. Hook:", "2) Body:"
    r"(?:hook|cta|body|headline|caption|intro|outro|section)\s*:)",  # section labels
    re.IGNORECASE,
)

# Detects numbered/section markers anywhere in a line — used by _is_content_signal
# to catch prefaced schemas like "For launch day: 1. Hook: grab. 2. Body: describe."
_INLINE_SCHEMA_MARKER_RE = re.compile(
    r"(?<!\w)\d+\s*[.)]\s*[A-Za-z]"               # "1. Hook" / "1) Body"
    r"|(?:line|step|part)\s+\d"                    # "Line 1", "Step 2"
    r"|(?:hook|cta|body|headline|caption|intro|outro|section)\s*:",  # "Hook:", etc.
    re.IGNORECASE,
)


def _is_content_signal(text: str) -> bool:
    """Return True only when the text is genuine creative content, not an instruction.

    Rejects lines that:
    - Begin with an imperative instruction prefix (write/always/never/line N/hook:…)
    - Contain 2+ numbered/section schema markers anywhere (prefaced instruction
      schemas like "For launch day: 1. Hook: grab. 2. Body: describe." that do not
      start with a recognised prefix but are unambiguously instruction documents).
    """
    t = text.strip()
    if _INSTRUCTION_PREFIX_RE.match(t):
        return False
    if len(_INLINE_SCHEMA_MARKER_RE.findall(t)) >= 2:
        return False
    return True
