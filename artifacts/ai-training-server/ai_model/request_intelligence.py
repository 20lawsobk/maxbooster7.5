...
def _narrative_clause(narrative: str, limit: int = 140) -> str:
    ...
    if not s:
        return ""
    # ── Instruction-schema guards ─────────────────────────────────────────────
    # Two tiers, both applied before AND after imperative-lead-in stripping.
    #
    # Tier 1 — start-anchored: catches schemas where the very first token is
    #   a numbered/section marker (e.g. "Line 1:", "1. Hook:", "Hook:").
    # Tier 2 — content-wide: catches prefaced schemas where a short preamble
    #   precedes the instruction list (e.g. "For launch day: 1. Hook… 2. Body…").
    #   Two or more schema markers anywhere in the text is an unambiguous signal
    #   that the narrative blob is an instruction document, not creative prose.
    #
    # Patterns caught (all case-insensitive):
    #   "Line 1:", "Step 2:", "Part 3:" — explicit numbered-line directives
    #   "1. Hook:", "2) Body:"          — numbered-list section headers
    #   "Hook:", "CTA:", "Body:",       — bare section-label directives
    #   "Headline:", "Caption:", etc.
    #   "For launch day: 1. Hook… 2. Body… 3. CTA…" — prefaced schemas
    _INSTRUCTION_SCHEMA_START_RE = re.compile(
        r"""^(?:
            (?:line|step|part)\s+\d                         # Line 1 / Step 2 / Part 3
          | \d+\s*[.)]\s*\S                                 # 1. or 1) numbered item
          | (?:hook|cta|body|headline|caption|intro|outro
               |section|verse|chorus|bridge)\s*:            # section-label directives
        )""",
        re.IGNORECASE | re.VERBOSE,
    )
    # Tier-2 marker: list item OR "Label:" anywhere (word-boundary protected
    # so "1 million streams" doesn't false-positive on "1 ").
    _INSTRUCTION_SCHEMA_MARKER_RE = re.compile(
        r"(?<!\w)\d+\s*[.)]\s*[A-Za-z]"               # "1. Hook" / "1) Body"
        r"|(?:line|step|part)\s+\d"                    # "Line 1", "Step 2"
        r"|(?:hook|cta|body|headline|caption|intro|outro"
        r"|section|verse|chorus|bridge)\s*:",           # "Hook:", "CTA:", etc.
        re.IGNORECASE,
    )

    def _is_instruction_schema(text: str) -> bool:
        t = text.strip()
        if _INSTRUCTION_SCHEMA_START_RE.match(t):
            return True
        # Tier-2: 2+ schema markers anywhere → instruction document, not prose
        return len(_INSTRUCTION_SCHEMA_MARKER_RE.findall(t)) >= 2

    if _is_instruction_schema(s):
        return ""
    ...
    # Re-apply the guard: the remainder after stripping might itself be a
    # numbered/section schema (e.g. "Write 3 lines. Line 1: hook...").
    if _is_instruction_schema(s):
        return ""
    if len(s) > limit:
        cut = s[:limit]
