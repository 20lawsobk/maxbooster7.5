  ...
  generateSimilarSynthParams(style: StyleProfile): SynthParams {
    const oscType =
      style.brightness > 0.6
        ? "sawtooth"
        : style.brightness > 0.3
          ? "square"
          : "triangle";

    return {
      type: "lead",
      oscillators: [{ type: oscType as any, frequency: 1, detune: 0 }],
      filter: {
        type: "lowpass",
...
export class AIAudioGenerator {
  ...
  async generateFromText(
    ...
    console.log(
      `[AIAudioGenerator] input.bars=${input.bars} parsed.patternLength=${parsed.patternLength} → bars=${bars}, tempo=${resolvedTempo}`,
    );
    ...
    const config: GenerationConfig = {
      ...
      swing:
        parsed.params.genre === "trap" || parsed.params.genre === "hiphop"
          ? 0.2
          : 0,
    };
    ...
    switch (parsed.intent.category) {
      ...
      case "arp": {
        ...
        const instrumentParams = {
          ...
          sustain: 0.6,
          instrumentName:
            parsed.intent.instrument || parsed.intent.subType || "synth_lead",
        };
    ...
    console.log(
      `[AIAudioGenerator] audioData.length=${audioData.length} sampleRate=${this.sampleRate} → duration=${duration.toFixed(2)}s (${(duration / (60 / config.tempo) / 4).toFixed(1)} bars at ${config.tempo}bpm)`,
    );
  ...
  async generateFromReference(
    ...
    const styleProfile = this.styleTransfer.analyzeReference(
  ...
  floatToInt16(floatData: Float32Array): Int16Array {
    ...
    for (let i = 0; i < floatData.length; i++) {
      int16Data[i] = Math.max(
        -32768,
        Math.min(32767, Math.floor(floatData[i] * 32767)),
      );
    }
    return int16Data;
  }
