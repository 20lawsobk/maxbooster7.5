...
export class IntelligentMasteringEngine {
  ...
  public suggestSettings(
    ...
    reasoning.push(
      `EQ optimized based on frequency balance: sub=${analysis.frequencyBalance.sub.toFixed(2)}, bass=${analysis.frequencyBalance.bass.toFixed(2)}`,
    );

    const optimizedMultiband = this.optimizeMultibandForAnalysis(
      analysis,
      preset.multibandSettings,
    );
    reasoning.push(
      `Multiband compression adjusted for dynamic range: ${analysis.dynamicRange.toFixed(1)} dB`,
    );

    const stereoSettings = this.optimizeStereoSettings(
      analysis,
      preset.stereoWidth,
    );
    reasoning.push(
      `Stereo width set to ${stereoSettings.width.toFixed(2)} based on current width: ${analysis.stereoWidth.toFixed(2)}`,
    );

    const loudnessSettings = this.calculateLoudnessSettings(
      analysis,
      preset.targetLUFS,
      preset.truePeak,
    );
    reasoning.push(
      `Targeting ${preset.targetLUFS} LUFS (current: ${analysis.currentLUFS.toFixed(1)} LUFS)`,
    );
  ...
  private detectGenre(analysis: MasteringAnalysis): MasteringGenre {
    ...
    if (
      analysis.frequencyBalance.sub > 0.1 &&
      analysis.frequencyBalance.bass > 0.25
    ) {
      scores["hip-hop"] += 3;
      scores["electronic"] += 2;
      scores["r&b"] += 2;
    ...
    if (
      analysis.frequencyBalance.mid > 0.3 &&
      analysis.frequencyBalance.highMid > 0.15
    ) {
      scores["rock"] += 2;
      scores["metal"] += 1;
    }
  ...
  private optimizeEQForAnalysis(
    ...
    for (const band of optimized) {
      if (band.frequency < 60) {
        ...
        const diff =
          idealBalance.brilliance - analysis.frequencyBalance.brilliance;
        band.gain += diff * 10;
      }
  ...
  private optimizeStereoSettings(
    analysis: MasteringAnalysis,
    presetWidth: number,
  ): StereoSettings {
  ...
  private calculateConfidence(
    ...
    const tempoMatch =
      Math.abs(analysis.rhythm.tempo - (preset.name === "hip-hop" ? 90 : 120)) <
      20;
