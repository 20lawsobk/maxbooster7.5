...
export const GENRE_AUDIO_PROFILES: Record<
  string,
  {
...
export class GenreClassificationModel extends BaseModel {
  ...
  public async trainOnMFCC(
    ...
    const oneHotLabels = trainingLabels.map((genre) =>
      this.genreToOneHot(genre),
    );
  ...
  public async classifyGenre(
    ...
    const inputTensor = tf.tensor4d(
      [[normalized]],
      [1, this.mfccHeight, this.mfccWidth, 1],
    );
    ...
    const shifted = mfcc.map((row) => {
      ...
      if (shiftAmount > 0) {
        return [
          ...
          ...newRow.slice(0, -shiftAmount),
        ];
      } else {
        return [
          ...newRow.slice(-shiftAmount),
          ...new Array(-shiftAmount).fill(0),
        ];
      }
    });
  ...
  protected postprocessOutput(output: tf.Tensor): Genre {
    ...
    const genreIndex = Array.from(probabilities).indexOf(
      Math.max(...Array.from(probabilities)),
    );
  ...
  public async evaluateAccuracy(
    ...
    const genreCounts: Record<Genre, { correct: number; total: number }> =
      {} as any;
