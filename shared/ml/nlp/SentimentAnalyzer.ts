...
export class SentimentAnalyzer {
  ...
  public analyzeSentiment(text: string): SentimentResult {
    ...
    const normalizedScore =
      wordCount > 0 ? totalScore / Math.sqrt(wordCount) : 0;
  ...
  private calculateSentimentConfidence(
    ...
    const coverage = Math.min(
      1,
      sentimentWordCount / Math.max(5, totalTokens * 0.3),
    );
  ...
  public detectEmotions(text: string): EmotionResult {
    ...
    const normalizedScores: Record<Emotion, number> = {} as Record<
      Emotion,
      number
    >;
    ...
    const maxEmotion = (
      Object.entries(normalizedScores) as [Emotion, number][]
  ...
  public detectToxicity(text: string): ToxicityResult {
    ...
    for (const [phrase, category] of Array.from(
      this.toxicityPatterns.entries(),
    )) {
  ...
  public addCustomTerm(
    term: string,
    score: number,
    category?: "sentiment" | "emotion" | "toxicity",
    emotion?: Emotion,
  ): void {
