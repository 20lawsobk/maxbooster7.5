...
export default function FanHub() {
  ...
  const addTagMutation = useMutation({
    mutationFn: async ({ id, tags }: { id: string; tags: string[] }) => {
      const res = await apiRequest(
        "PUT",
        `/api/fan-hub/subscribers/${id}/tag`,
        { tags },
      );
      return res.json();
    },
  ...
  const updateFanMutation = useMutation({
    mutationFn: async ({
      ...
      id: string;
      data: Partial<FanSubscriber>;
    }) => {
      const res = await apiRequest(
        "PUT",
        `/api/fan-hub/subscribers/${id}`,
        data,
      );
      return res.json();
    },
  ...
  const handleExport = () => {
    ...
    toast({
      ...
      description: `${subscribers.length} fans exported to CSV.`,
    });
  };
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                {stats?.totalFans
                  ? Math.round((stats.vipCount / stats.totalFans) * 100)
                  : 0}
                % of your fan base
              </p>
            </CardContent>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                $
                {stats?.avgSpend?.toLocaleString(undefined, {
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-card p-4 rounded-lg border shadow-sm">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
                ...
                <form onSubmit={handleAddFan} className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      id="tags"
                      name="tags"
                      placeholder="tour, merch-buyer, early-adopter"
                    />
                  </div>
                  <div className="space-y-2">
            ...
            <Dialog
              open={isComposingMessage}
              onOpenChange={setIsComposingMessage}
            >
              <DialogTrigger asChild>
                ...
                <form onSubmit={handleSendMessage} className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      id="subject"
                      name="subject"
                      placeholder="New release out now!"
                      required
                    />
                  </div>
                  <div className="space-y-2">
                          ...
                          <div className="py-12 px-6">
                            <div className="max-w-xl mx-auto text-center mb-8">
                              ...
                              <p className="text-muted-foreground text-sm">
                                You don't have any fans yet. Here are some ways
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-2xl mx-auto">
                              <div className="rounded-lg border bg-card p-4 text-center">
                                ...
                                <p className="text-xs text-muted-foreground">
                                  Distribute on all platforms to reach new
                                  listeners and convert them into fans.
                                </p>
                              </div>
                              <div className="rounded-lg border bg-card p-4 text-center">
                                ...
                                <p className="text-xs text-muted-foreground">
                                  Send your press kit link to blogs, playlists,
                                  and venues to build awareness.
                                </p>
                              </div>
                              <div className="rounded-lg border bg-card p-4 text-center">
                                ...
                                <p className="text-xs text-muted-foreground">
                                  Already have fans elsewhere? Import them via
                                  CSV from the Import tab above.
                                </p>
                              </div>
                            </div>
                    ...
                    subscribers.map((fan) => (
                      ...
                      >
                        ...
                        <TableCell className="font-medium">
                          $
                          {fan.totalSpent.toLocaleString(undefined, {
                            minimumFractionDigits: 2,
                          })}
                        </TableCell>
                        <TableCell
                    ...
                    vipFans.map((fan) => (
                      ...
                      >
                        ...
                        <TableCell>
                          <Badge
                            variant="outline"
                            className="capitalize border-amber-500/50 text-amber-600"
                          >
                            VIP
                          </Badge>
                        </TableCell>
                ...
                <Card className="p-12 text-center">
                  <div className="flex flex-col items-center justify-center text-muted-foreground">
                    ...
                    <p className="max-w-sm mx-auto mt-1 mb-6">
                      Start engaging with your fans by sending your first
                      broadcast message.
                    </p>
                      ...
                      <div className="grid grid-cols-3 gap-4 sm:gap-8 mb-6">
                        ...
                        <div>
                          ...
                          <p className="text-2xl font-bold">
                            {msg.recipientCount > 0
                              ? Math.round(
                                  (msg.openCount / msg.recipientCount) * 100,
                                )
                              : 0}
                            %
                          </p>
                        ...
                        <div>
                          ...
                          <p className="text-2xl font-bold">
                            {msg.openCount > 0
                              ? Math.round(
                                  (msg.clickCount / msg.openCount) * 100,
                                )
                              : 0}
                            %
                          </p>
            ...
            <Card className="p-10 max-w-2xl mx-auto">
              <div className="flex flex-col items-center text-center">
                ...
                <p className="text-muted-foreground mb-8">
                  Already have a list of fans from Mailchimp, Bandcamp, or your
                  website? Upload a CSV file to import them all at once.
                </p>
                <div className="w-full max-w-sm space-y-4">
                  <div
                    className={`relative border-2 border-dashed rounded-xl p-8 transition-colors ${importFile ? "border-primary/50 bg-primary/5" : "border-muted-foreground/20 hover:border-primary/30"}`}
                  >
                    <input
                      type="file"
                      accept=".csv"
        ...
        <Sheet
          open={!!selectedFan}
          onOpenChange={(open) => !open && setSelectedFan(null)}
        >
                ...
                <SheetHeader className="pb-6 border-b">
                  <div className="flex justify-between items-start">
                    <div className="h-16 w-16 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-2xl mb-4">
                      {selectedFan.name?.charAt(0) ||
                        selectedFan.email.charAt(0).toUpperCase()}
                    </div>
                    <Badge
                      variant={selectedFan.isVip ? "default" : "outline"}
                      className={
                        selectedFan.isVip
                  ...
                  <SheetTitle className="text-2xl">
                    {selectedFan.name || "Anonymous Fan"}
                  </SheetTitle>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 bg-muted/50 rounded-lg">
                      ...
                      <p className="font-medium">
                        {format(new Date(selectedFan.joinedAt), "MMM d, yyyy")}
                      </p>
                    </div>
                    <div className="p-4 bg-muted/50 rounded-lg">
                      <p className="text-xs text-muted-foreground uppercase font-semibold mb-1">
                      ...
                      <p className="font-medium">
                        ${selectedFan.totalSpent.toLocaleString()}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          setIsTaggingFan(selectedFan);
                          setTagInput((selectedFan?.tags || []).join(", "));
                        }}
                      >
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        setEditForm({
                          ...
                          phone: selectedFan.phone || "",
                          notes: selectedFan.notes || "",
                          isVip: selectedFan.isVip,
                        });
                      }}
                    >
                      Edit Profile
                    ...
                    <Button
                      ...
                      onClick={() =>
                        updateFanMutation.mutate({
                          id: selectedFan.id,
                          data: { isVip: !selectedFan.isVip },
                        })
                      }
                      disabled={updateFanMutation.isPending}
                    >
                      <Star
                        className={`h-4 w-4 mr-2 ${selectedFan.isVip ? "fill-amber-500 text-amber-500" : ""}`}
                      />
        ...
        <Dialog
          open={!!isEditingFan}
          onOpenChange={(open) => !open && setIsEditingFan(null)}
        >
          <DialogContent className="max-w-sm">
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setIsEditingFan(null)}>
                Cancel
              </Button>
              <Button
                onClick={() => {
        ...
        <Dialog
          open={!!isTaggingFan}
          onOpenChange={(open) => !open && setIsTaggingFan(null)}
        >
          <DialogContent className="max-w-sm">
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                {isTaggingFan?.name || isTaggingFan?.email}
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-3 py-2">
              ...
              />
              <p className="text-xs text-muted-foreground">
                Current: {isTaggingFan?.tags?.join(", ") || "none"}
              </p>
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setIsTaggingFan(null)}>
                Cancel
              </Button>
              <Button
                onClick={() => {
        ...
        <AlertDialog
          open={!!pendingDeleteFanId}
          onOpenChange={(open) => !open && setPendingDeleteFanId(null)}
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Remove Fan</AlertDialogTitle>
              <AlertDialogDescription>
                Are you sure you want to remove this fan from your list? This
              ...
              <AlertDialogAction
                ...
                onClick={() => {
                  if (pendingDeleteFanId) {
                    ...
                    if (selectedFan?.id === pendingDeleteFanId)
                      setSelectedFan(null);
                  }
                }}
              >
