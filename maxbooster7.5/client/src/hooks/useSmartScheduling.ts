...
export function useSmartScheduling(platform: string = 'all', contentType: string = 'post') {
  const queryClient = useQueryClient();

  const { data: scheduleData, isLoading, error, refetch } = useQuery<SmartScheduleData>({
  ...
  const getSuggestionsForDay = useCallback((day: DayOfWeek): ScheduleSuggestion[] => {
    if (!scheduleData?.suggestions) return [];
    return scheduleData.suggestions.filter(s => s.dayOfWeek === day);
  }, [scheduleData]);

  const getSuggestionsForPlatform = useCallback((platformName: string): ScheduleSuggestion[] => {
    if (!scheduleData?.suggestions) return [];
    return scheduleData.suggestions.filter(s => s.platforms.includes(platformName));
  }, [scheduleData]);

  const getBestTimeForPlatform = useCallback((platformName: string): ScheduleSuggestion | null => {
  ...
  const getEngagementScore = useCallback((day: DayOfWeek, hour: number): number => {
    if (!engagementPatterns) return 0.5;
    const patterns = engagementPatterns.filter(p =>
      p.dayOfWeek === day && p.hour === hour
    );
    if (!patterns.length) return 0.5;
    return patterns.reduce((sum, p) => sum + p.avgEngagement, 0) / patterns.length;
  }, [engagementPatterns]);

  const getOptimalTimeRange = useCallback((day: DayOfWeek): { start: string; end: string } | null => {
    const daySuggestions = getSuggestionsForDay(day);
    if (!daySuggestions.length) return null;
    
    const times = daySuggestions.map(s => parseInt(s.specificTime.split(':')[0]));
...
export function useAudienceTimezones() {
  const { audienceTimezones, primaryTimezone, isLoading } = useSmartScheduling();
