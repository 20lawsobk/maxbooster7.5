...
export function usePrecisionTransport(): [PrecisionTransportState, PrecisionTransportControls] {
  ...
  const [metronomeEnabled, setMetronomeEnabled] = useState(transport.metronomeEnabled);
  ...
  const nudgeForward = useCallback((amount = 1) => {
    seek(localPosition + amount);
  }, [localPosition, seek]);
  
  const nudgeBackward = useCallback((amount = 1) => {
