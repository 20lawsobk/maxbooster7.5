...
import { errorService, ErrorCategory, ErrorSeverity } from '@/lib/errorService';
...
const DEFAULT_RETRYABLE_CATEGORIES: ErrorCategory[] = ['network', 'timeout', 'system'];
...
function categorizeError(error: Error): ErrorCategory {
  ...
  if (message.includes('audio') || message.includes('media') || message.includes('video')) {
    return 'media';
  }
...
function getUserFriendlyMessage(category: ErrorCategory): string {
  const messages: Record<ErrorCategory, string> = {
    ...
    system: "Our servers are temporarily unavailable. We're working to fix this.",
...
export function useApiError(options: ApiErrorOptions = {}): UseApiErrorResult {
  ...
  const execute = useCallback(async <T,>(fn: () => Promise<T>): Promise<T | null> => {
    ...
    try {
      ...
      if (showToast) {
        const toastVariant = category === 'auth' ? 'destructive' : 
                            category === 'validation' ? 'warning' : 'destructive';
        toast({
  ...
  }, [state.isOffline, state.retryCount, maxRetries, retryDelay, showToast, onError, onRetry, onSuccess, retryableCategories]);
...
export function useApiMutation<TData, TVariables = void>(
  ...
  const mutate = useCallback(async (variables: TVariables): Promise<TData | null> => {
    setIsLoading(true);
    try {
      const result = await apiError.execute(() => options.mutationFn(variables));
      if (result !== null) {
        setData(result);
...
export function useCancelableRequest<T>() {
  ...
  const execute = useCallback(async (
    requestFn: (signal: AbortSignal, onProgress?: (progress: number) => void) => Promise<T>
  ): Promise<T | null> => {
    if (abortControllerRef.current) {
      abortControllerRef.current.abort();
    }
    ...
    try {
      const result = await requestFn(abortControllerRef.current.signal, setProgress);
      setProgress(100);
      return result;
    } catch (error) {
