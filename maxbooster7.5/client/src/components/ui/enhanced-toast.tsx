...
>(
  (
    { className, variant, showProgress, duration = 5000, children, ...props },
    ref,
  ) => {
...
function ToastUndoButton({
  ...
  return (
    <Button
      ...
      size="sm"
      onClick={onClick}
      className="min-h-[32px] font-medium"
    >
      {label}
    </Button>
  );
...
export function useEnhancedToast() {
  ...
  const manageQueue = React.useCallback((id: string) => {
    ...
    if (queue.size > maxToasts) {
      const oldestId = Array.from(queue.entries()).sort(
        ([, a], [, b]) => a - b,
      )[0]?.[0];
      if (oldestId) {
