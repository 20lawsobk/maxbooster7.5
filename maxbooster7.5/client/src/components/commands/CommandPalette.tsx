...
import { getPlatformModifiers, formatShortcutKeys } from '@/lib/shortcuts/types';
...
export function CommandPalette({ className }: CommandPaletteProps) {
  const { isOpen, close, search, execute, recentCommands } = useCommandPalette();
  ...
  const handleSelect = useCallback(async (command: Command) => {
    close();
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    switch (e.key) {
      case 'ArrowDown':
        e.preventDefault();
        setSelectedIndex(i => Math.min(i + 1, displayedCommands.length - 1));
        break;
      case 'ArrowUp':
        e.preventDefault();
  ...
  const getCommandIcon = (command: Command) => {
    const IconComponent = COMMAND_ICONS[command.id] || CATEGORY_ICONS[command.category] || CommandIcon;
          ...
          >
                ...
                {query.trim() ? (
                  ...
                  Object.entries(groupedCommands).map(([category, commands]) => (
                    <div key={category} className="mb-4">
                      ...
                      {commands.map((cmd) => {
                        const idx = displayedCommands.findIndex(c => c.id === cmd.id);
                        return (
                          <CommandItem
                            key={cmd.id}
                            command={cmd}
            ...
            <div className="flex items-center justify-between p-3 border-t border-zinc-800 text-xs text-zinc-500">
              ...
              <div className="flex items-center gap-1">
                <kbd className="px-1.5 py-0.5 bg-zinc-800 rounded">{platform.mod}</kbd>
...
function CommandItem({ command, index, isSelected, onSelect, icon: Icon }: CommandItemProps) {
  return (
    <button
      data-index={index}
      ...
      className={cn(
        ...
        isSelected ? "bg-amber-600/20 text-amber-400" : "hover:bg-zinc-800/50 text-zinc-300"
      )}
    >
      ...
      {command.shortcut && (
        <div className="flex items-center gap-1 flex-shrink-0">
          {command.shortcut.modifiers?.map(mod => (
            <kbd key={mod} className="px-1.5 py-0.5 bg-zinc-800 border border-zinc-700 rounded text-xs">
              {mod === 'cmd' ? getPlatformModifiers().mod : mod === 'shift' ? '⇧' : mod === 'alt' ? getPlatformModifiers().alt : mod}
            </kbd>
          ))}
      ...
      {isSelected && <ArrowRight className="w-4 h-4 text-amber-400 flex-shrink-0" />}
    </button>
  );
}
