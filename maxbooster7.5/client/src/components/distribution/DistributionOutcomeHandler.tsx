...
type OutcomeCategory = 'release' | 'submission' | 'contentId' | 'codes' | 'takedown' | 'analytics';
...
interface SubmissionOutcome {
  type: 'submission_started' | 'platform_processing' | 'validation_error' | 'partial_success' | 'all_success' | 'queue_update';
  status: 'success' | 'warning' | 'error' | 'in_progress';
  message: string;
  platforms?: Array<{
    name: string;
...
interface ContentIdOutcome {
  type: 'fingerprint_generated' | 'content_detected' | 'registration_confirmed' | 'conflict_found';
  status: 'success' | 'warning' | 'error' | 'in_progress';
  message: string;
  trackTitle?: string;
...
interface CodeOutcome {
  type: 'isrc_generated' | 'upc_generated' | 'validation_passed' | 'validation_failed' | 'code_in_use';
...
export function DistributionOutcomeHandler({ releaseId, onAction }: DistributionOutcomeHandlerProps) {
  ...
  const [activeCategory, setActiveCategory] = useState<OutcomeCategory>('release');

  const { data: outcomes, isLoading, refetch } = useQuery<{
  ...
  const retryMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: 'Retry initiated', description: 'The operation is being retried.' });
      queryClient.invalidateQueries({ queryKey: [`/api/distribution/releases/${releaseId}/outcomes`] });
    },
    onError: (error: Error) => {
  ...
  const getCategoryIcon = (category: OutcomeCategory) => {
    switch (category) {
      case 'release': return <Music className="h-4 w-4" />;
      case 'submission': return <Globe className="h-4 w-4" />;
      case 'contentId': return <Shield className="h-4 w-4" />;
      case 'codes': return <Hash className="h-4 w-4" />;
      case 'takedown': return <Trash2 className="h-4 w-4" />;
    ...
    <div key={index} className="flex items-start gap-3 p-3 bg-muted/30 rounded-lg hover:bg-muted/50 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        ...
        {outcome.details && (
          <div className="mt-1 text-xs text-muted-foreground">
            {typeof outcome.details === 'string' ? outcome.details : JSON.stringify(outcome.details)}
          </div>
        )}
      ...
      {outcome.status === 'error' && (
        <Button
          size="sm"
          variant="ghost"
          onClick={() => retryMutation.mutate({ type: outcome.type, data: outcome })}
      ...
      return (
        <div className="text-center py-8">
          ...
          <p className="text-sm text-muted-foreground">
            Analytics will appear here once your release is live and generating streams.
          </p>
        </div>
      );
    }
          ...
          <div key={index} className="space-y-3">
            ...
            {outcome.type === 'geographic' && outcome.data && (
              <div className="grid grid-cols-2 gap-2 pl-7">
                {Object.entries(outcome.data).slice(0, 6).map(([country, streams]: [string, any]) => (
                  <div key={country} className="flex items-center justify-between p-2 bg-muted/20 rounded">
                    <span className="flex items-center gap-2 text-sm">
                      <MapPin className="h-3 w-3" />
                      {country}
                    </span>
                    <span className="text-sm font-medium">{streams.toLocaleString()}</span>
                  </div>
                ))}
              </div>
            )}
            {outcome.type === 'platform_comparison' && outcome.data && (
              <div className="space-y-2 pl-7">
                {Object.entries(outcome.data).map(([platform, stats]: [string, any]) => (
                  <div key={platform} className="flex items-center gap-3">
                    <span className="text-sm w-24">{platform}</span>
                    <div className="flex-1">
                      <Progress value={(stats.streams / stats.total) * 100} className="h-2" />
                    </div>
                    <span className="text-sm font-medium w-20 text-right">{stats.streams.toLocaleString()}</span>
                  </div>
                ))}
              </div>
            )}
    ...
    <div className="space-y-6">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Track all distribution activities and their outcomes in one place.
              </CardDescription>
            </div>
        ...
        <CardContent>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
            <div className="text-center p-3 bg-green-500/10 rounded-lg">
              <p className="text-xl font-bold text-green-500">{summary.successes}</p>
              <p className="text-xs text-muted-foreground">Successful</p>
            </div>
            <div className="text-center p-3 bg-blue-500/10 rounded-lg">
              <p className="text-xl font-bold text-blue-500">{summary.inProgress}</p>
          ...
          <Tabs value={activeCategory} onValueChange={(v) => setActiveCategory(v as OutcomeCategory)}>
            <TabsList className="grid w-full grid-cols-6">
              {(['release', 'submission', 'contentId', 'codes', 'takedown', 'analytics'] as OutcomeCategory[]).map((category) => (
                <TabsTrigger key={category} value={category} className="flex items-center gap-1 text-xs">
                  ...
                  {getCategoryCount(category) > 0 && (
                    <Badge variant="secondary" className="ml-1 h-5 px-1.5">{getCategoryCount(category)}</Badge>
                  )}
                </TabsTrigger>
              ))}
            </TabsList>
            ...
            <ScrollArea className="h-[400px] mt-4">
              <TabsContent value="release" className="space-y-3 m-0">
                {outcomes?.release?.length ? (
                  outcomes.release.map((outcome, index) => renderOutcomeItem(outcome, index))
                ) : (
              ...
              <TabsContent value="submission" className="space-y-3 m-0">
                {outcomes?.submission?.length ? (
                  outcomes.submission.map((outcome, index) => renderOutcomeItem(outcome, index))
                ) : (
                  <div className="text-center py-8 text-muted-foreground">
              ...
              <TabsContent value="contentId" className="space-y-3 m-0">
                {outcomes?.contentId?.length ? (
                  outcomes.contentId.map((outcome, index) => renderOutcomeItem(outcome, index))
                ) : (
                  <div className="text-center py-8 text-muted-foreground">
              ...
              <TabsContent value="codes" className="space-y-3 m-0">
                {outcomes?.codes?.length ? (
                  outcomes.codes.map((outcome, index) => renderOutcomeItem(outcome, index))
                ) : (
                  <div className="text-center py-8 text-muted-foreground">
              ...
              <TabsContent value="takedown" className="space-y-3 m-0">
                {outcomes?.takedown?.length ? (
                  outcomes.takedown.map((outcome, index) => renderOutcomeItem(outcome, index))
                ) : (
                  <div className="text-center py-8 text-muted-foreground">
      ...
      {summary.errors > 0 && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            {summary.errors} error(s) require your attention. Review the outcomes above and retry failed operations.
          </AlertDescription>
        </Alert>
      )}
    </div>
