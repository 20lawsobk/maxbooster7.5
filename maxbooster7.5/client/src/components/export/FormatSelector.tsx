...
export const AUDIO_FORMATS: FormatOption<AudioFormat>[] = [
  { value: 'wav', label: 'WAV', description: 'Uncompressed audio (lossless)', icon: FileAudio, lossless: true, recommended: true },
  { value: 'mp3', label: 'MP3', description: 'Compressed audio (lossy)', icon: FileAudio, lossless: false },
  { value: 'flac', label: 'FLAC', description: 'Lossless compression', icon: FileAudio, lossless: true },
  { value: 'aiff', label: 'AIFF', description: 'Apple lossless format', icon: FileAudio, lossless: true },
  { value: 'ogg', label: 'OGG', description: 'Open format (lossy)', icon: FileAudio, lossless: false },
  { value: 'aac', label: 'AAC', description: 'Advanced Audio Coding', icon: FileAudio, lossless: false },
];

export const DATA_FORMATS: FormatOption<DataFormat>[] = [
  { value: 'csv', label: 'CSV', description: 'Spreadsheet compatible', icon: FileSpreadsheet, recommended: true },
  { value: 'xlsx', label: 'Excel', description: 'Full Excel workbook', icon: FileSpreadsheet },
  ...
  { value: 'png', label: 'PNG', description: 'Chart image export', icon: Image },
  { value: 'svg', label: 'SVG', description: 'Vector chart export', icon: Image },
];

export const REPORT_FORMATS: FormatOption<ReportFormat>[] = [
  { value: 'pdf', label: 'PDF', description: 'Print-ready document', icon: FileText, recommended: true },
  { value: 'xlsx', label: 'Excel', description: 'Editable spreadsheet', icon: FileSpreadsheet },
  { value: 'csv', label: 'CSV', description: 'Data only (no formatting)', icon: FileSpreadsheet },
];
              ...
              {compact ? (
                ...
                <>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    {option.lossless && (
                      <Badge variant="secondary" className="text-[10px] px-1.5 py-0 bg-green-900/30 text-green-400">
                        Lossless
                      </Badge>
                    )}
                  </div>
...
export const FormatSelector = memo(FormatSelectorInner) as typeof FormatSelectorInner;
...
export const ChartFormatSelector = memo(function ChartFormatSelector({
  ...
  const formatOptions: FormatOption<'png' | 'svg' | 'pdf'>[] = [
    { value: 'png', label: 'PNG', description: 'Raster image', icon: Image, recommended: true },
          ...
          <div className="flex gap-2">
            {resolutionOptions.map((opt) => (
              <button
                key={opt.value}
                type="button"
                onClick={() => onChange({ ...value, resolution: opt.value as ChartExportFormat['resolution'] })}
                className={cn(
          ...
          <div className="flex gap-2">
            {backgroundOptions.map((opt) => (
              <button
                key={opt.value}
                type="button"
                onClick={() => onChange({ ...value, background: opt.value as ChartExportFormat['background'] })}
                className={cn(
    ...
    <div className={cn("space-y-4", className)}>
      <div className="flex items-center justify-between p-3 bg-zinc-900 rounded-lg border border-zinc-800">
        <div>
          ...
          <p className="text-xs text-zinc-500">Export all {itemCount} items in the same format</p>
        </div>
        <button
          type="button"
          onClick={() => onChange({ ...value, useUnifiedFormat: !value.useUnifiedFormat })}
          className={cn(
      ...
      <div className="flex items-center justify-between p-3 bg-zinc-900 rounded-lg border border-zinc-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-500">Bundle all exports in a single download</p>
          </div>
        </div>
        <button
          type="button"
          onClick={() => onChange({ ...value, zipCompression: !value.zipCompression })}
          className={cn(
            "relative inline-flex h-6 w-11 items-center rounded-full transition-colors",
