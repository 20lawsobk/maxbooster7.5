...
export function LyricDisplay({
  ...
  const [displayMode, setDisplayMode] = useState<'scroll' | 'karaoke' | 'teleprompter'>('scroll');
  ...
  useEffect(() => {
    if (autoScroll && activeLineRef.current && containerRef.current && isPlaying) {
      ...
      const scrollTarget = activeLine.offsetTop - containerRect.height / 2 + lineRect.height / 2;
  ...
  const handleChangeType = useCallback((id: string, type: LyricLine['type']) => {
    const updated = lyrics.map((l) =>
      l.id === id ? { ...l, type } : l
    );
    onLyricsChange?.(updated);
    ...
    >
      <div className="flex items-center justify-between p-3 border-b border-zinc-800 bg-zinc-900/80 backdrop-blur">
        ...
        <div className="flex items-center gap-1">
          ...
          >
            {alignment === 'left' ? <AlignLeft className="w-3.5 h-3.5" /> : <AlignCenter className="w-3.5 h-3.5" />}
          </Button>
          ...
          <Button
            ...
            className={cn('h-7 w-7', autoScroll ? 'text-purple-400' : 'text-zinc-400')}
          >
          ...
          <Button
            ...
            className={cn('h-7 w-7', showSettings ? 'text-purple-400' : 'text-zinc-400')}
          >
          ...
          <Button
            ...
            className={cn('h-7 w-7', isEditing ? 'text-purple-400' : 'text-zinc-400')}
          >
      ...
      >
        {lyrics.length === 0 ? (
          <div className="h-full flex items-center justify-center">
            <div className="text-center space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-zinc-600">Add lyrics to get started</p>
              </div>
              {isEditing && (
          ...
          <div className={cn('space-y-4', displayMode === 'karaoke' && 'space-y-8')}>
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  <div className={cn(
                    ...
                    lyrics[currentLineIndex]?.type && `bg-gradient-to-r ${SECTION_COLORS[lyrics[currentLineIndex].type!]} bg-clip-text text-transparent`
                  )}>
            ...
            {displayMode !== 'karaoke' && lyrics.map((line, index) => {
              const isActive = index === currentLineIndex;
              const isPast = currentLineIndex >= 0 && index < currentLineIndex;
              const isUpcoming = currentLineIndex >= 0 && index === nextLineIndex;
                        ...
                        isActive
                          ...
                          : isUpcoming
                            ...
                            : isPast
                              ? 'text-zinc-600'
                              : 'text-zinc-400',
                        displayMode === 'teleprompter' && isActive && 'text-4xl'
                      )}
                  ...
                  {isEditing && !editingLineId && (
                    <div className="absolute -right-24 top-1/2 -translate-y-1/2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <select
                        ...
                        onChange={(e) => handleChangeType(line.id, e.target.value as LyricLine['type'])}
      ...
      {displayMode === 'teleprompter' && isPlaying && (
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-4 bg-zinc-900/90 backdrop-blur px-4 py-2 rounded-full border border-zinc-700">
          <div className="text-xs text-zinc-500">Line {currentLineIndex + 1} of {lyrics.length}</div>
          <div className="w-32 h-1 bg-zinc-700 rounded-full overflow-hidden">
            <motion.div
              className="h-full bg-purple-500"
              style={{
                width: currentLineIndex >= 0
