...
export type OutcomeType = 
  | 'generation_progress' | 'generation_complete' | 'generation_failed' | 'style_transfer' | 'ai_suggestions'
  | 'mix_applied' | 'master_applied' | 'processing_progress' | 'processing_failed'
  | 'track_added' | 'track_deleted' | 'track_duplicated' | 'track_bounced'
  | 'plugin_loaded' | 'plugin_error' | 'preset_applied' | 'preset_saved'
  | 'collaborator_joined' | 'sync_progress' | 'conflict_resolution' | 'session_saved'
  | 'export_progress' | 'export_complete' | 'export_failed'
  | 'undo' | 'redo';
...
const StudioOutcomeContext = createContext<StudioOutcomeContextType | null>(null);

export function useStudioOutcome() {
  const context = useContext(StudioOutcomeContext);
  if (!context) {
    throw new Error('useStudioOutcome must be used within StudioOutcomeProvider');
  }
  return context;
}
...
export function StudioOutcomeProvider({ children }: StudioOutcomeProviderProps) {
  const [outcomes, setOutcomes] = useState<StudioOutcome[]>([]);

  const addOutcome = useCallback((outcome: Omit<StudioOutcome, 'id' | 'timestamp'>): string => {
  ...
  const updateOutcome = useCallback((id: string, updates: Partial<StudioOutcome>) => {
  ...
  return (
    <StudioOutcomeContext.Provider value={{ outcomes, addOutcome, updateOutcome, removeOutcome, clearOutcomes }}>
      {children}
...
const getOutcomeColor = (type: OutcomeType) => {
  if (type.includes('failed') || type === 'plugin_error') return 'from-red-500 to-red-600';
  if (type.includes('progress') || type === 'sync_progress') return 'from-blue-500 to-cyan-500';
  if (type.includes('complete') || type.includes('applied') || type.includes('saved') || type === 'plugin_loaded') return 'from-green-500 to-emerald-500';
  if (type === 'generation_progress' || type === 'style_transfer' || type === 'ai_suggestions') return 'from-purple-500 to-pink-500';
  if (type.includes('track')) return 'from-amber-500 to-orange-500';
  if (type.includes('collaborator') || type === 'conflict_resolution') return 'from-violet-500 to-purple-500';
...
function OutcomeCard({ outcome, onDismiss }: OutcomeCardProps) {
  ...
  const isProgress = outcome.type.includes('progress') || outcome.progress !== undefined;
    ...
    >
      <div className="flex items-start gap-3">
        <div className={cn(
          ...
          {isProgress && outcome.progress !== undefined && outcome.progress < 100 ? (
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            <h4 className="font-semibold text-white text-sm truncate">{outcome.title}</h4>
            <button 
              onClick={onDismiss}
          ...
          {outcome.data?.waveform && (
            <div className="mt-2 h-8 bg-zinc-800 rounded overflow-hidden">
              <div className="h-full flex items-center justify-center gap-px px-2">
                {(outcome.data.waveform as number[]).slice(0, 50).map((val, i) => (
                  <div 
                    key={i}
...
export function useStudioOutcomeHelpers() {
  ...
  const showGenerationProgress = useCallback((title: string, progress: number) => {
    return addOutcome({
  ...
  const showGenerationComplete = useCallback((title: string, data?: Record<string, any>) => {
    return addOutcome({
  ...
  const showMixApplied = useCallback((title: string, data?: { lufs?: number; truePeak?: number }) => {
    return addOutcome({
      type: 'mix_applied',
      title,
  ...
  const showMasterApplied = useCallback((lufs: number, truePeak: number) => {
    return addOutcome({
      type: 'master_applied',
  ...
  const showTrackAdded = useCallback((trackName: string) => {
    return addOutcome({
      type: 'track_added',
      title: 'Track Added',
  ...
  const showTrackDeleted = useCallback((trackName: string, onUndo?: () => void) => {
    return addOutcome({
      type: 'track_deleted',
      title: 'Track Deleted',
  ...
  const showPluginLoaded = useCallback((pluginName: string) => {
    return addOutcome({
      type: 'plugin_loaded',
      title: 'Plugin Loaded',
  ...
  const showPluginError = useCallback((pluginName: string, error: string) => {
    return addOutcome({
      type: 'plugin_error',
      title: 'Plugin Error',
  ...
  const showCollaboratorJoined = useCallback((name: string) => {
    return addOutcome({
  ...
  const showExportProgress = useCallback((filename: string, progress: number, format: string) => {
    return addOutcome({
  ...
  const showExportComplete = useCallback((filename: string, downloadUrl: string) => {
    return addOutcome({
  ...
  const showUndoAction = useCallback((actionName: string) => {
    return addOutcome({
      type: 'undo',
      title: 'Undo',
  ...
  const showRedoAction = useCallback((actionName: string) => {
    return addOutcome({
      type: 'redo',
      title: 'Redo',
