...
import { Volume2, Headphones, Mic, Music, Radio, Sliders, Plus, X, Trash2 } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="font-medium text-white text-sm truncate">{data.name}</span>
      </div>
      ...
      <div className="flex items-center gap-1">
        ...
        <span className="text-[10px] text-zinc-400 w-8 text-right">
          {data.volume > 0 ? `+${data.volume.toFixed(1)}` : data.volume.toFixed(1)} dB
        </span>
      </div>
      ...
      <div className="flex items-center gap-1 mt-2">
        <button
          className={cn(
            ...
            data.muted ? 'bg-amber-500 text-black' : 'bg-zinc-700 text-zinc-400 hover:bg-zinc-600'
          )}
        >
          M
        </button>
        <button
          className={cn(
            'px-2 py-0.5 text-[10px] font-bold rounded transition-colors',
            data.solo ? 'bg-yellow-400 text-black' : 'bg-zinc-700 text-zinc-400 hover:bg-zinc-600'
          )}
        >
          S
        </button>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mt-2">
        ...
        />
        <span className="text-[10px] text-zinc-400">{data.volume.toFixed(1)}</span>
      </div>
        ...
        <div className="flex flex-col items-center">
          <div className="w-4 h-16 bg-zinc-800 rounded-full relative overflow-hidden">
            <div
              ...
              style={{ height: `${Math.max(0, (60 + data.volume) / 66 * 100)}%` }}
            />
          </div>
...
export function RoutingFlowPanel({ projectId, onClose }: RoutingFlowPanelProps) {
  ...
  const initialNodes = useMemo<Node<RoutingNode>[]>(() => {
    const trackNodes: Node<RoutingNode>[] = tracks.map((track, index) => ({
      id: track.id,
      type: 'track',
      position: { x: 100 + (index % 6) * 180, y: 50 + Math.floor(index / 6) * 150 },
      data: {
        id: track.id,
        type: 'track',
        name: track.name,
  ...
  const initialEdges = useMemo<Edge[]>(() => {
    ...
    tracks.forEach((track) => {
      const targetBus = track.type === 'audio' ? 'bus-instruments' : 
                        track.name.toLowerCase().includes('drum') ? 'bus-drums' :
                        track.name.toLowerCase().includes('vocal') ? 'bus-vocals' : 'bus-instruments';
    ...
    ['bus-drums', 'bus-vocals', 'bus-instruments'].forEach((busId) => {
      edges.push({
        ...
        markerEnd: { type: MarkerType.ArrowClosed, color: NODE_TYPE_COLORS.bus },
      });
    });
