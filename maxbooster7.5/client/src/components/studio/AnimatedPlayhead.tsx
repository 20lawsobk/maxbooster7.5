...
function formatTime(seconds: number, format: 'seconds' | 'bars' | 'timecode', bpm: number): string {
  if (format === 'seconds') {
...
export const LiveMeter = memo(function LiveMeter({
  ...
  return (
    ...
    >
      ...
      {clipping && (
        <div 
          className="absolute top-0 left-0 right-0 h-2 bg-red-500 animate-pulse"
        />
      )}
...
export const StereoMeter = memo(function StereoMeter({
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      {label && (
        <span className="text-[9px] text-white/50 truncate" style={{ maxWidth: width }}>
          {label}
        </span>
      )}
    </div>
