...
const DEFAULT_PHASES: GenerationPhase[] = [
  {
    id: "1",
    name: "Analyzing",
    description: "Understanding your parameters",
    progress: 0,
    status: "pending",
  },
  {
    id: "2",
    name: "Composing",
    description: "Creating musical elements",
    progress: 0,
    status: "pending",
  },
  {
    id: "3",
    name: "Arranging",
    description: "Structuring the composition",
    progress: 0,
    status: "pending",
  },
  {
    ...
    if (generatedAudio?.url && !audioRef.current) {
      ...
      audioRef.current.addEventListener("timeupdate", () => {
        if (audioRef.current && generatedAudio.duration) {
          setPlaybackProgress(
            (audioRef.current.currentTime / generatedAudio.duration) * 100,
          );
        }
      });
    }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-500">
              ...
              {state === "complete" &&
                generatedAudio &&
                `${generatedAudio.duration.toFixed(1)}s generated`}
              {state === "error" && "An error occurred"}
              {state === "style_transfer" &&
                styleTransfer &&
                `${styleTransfer.similarity}% similarity`}
            </p>
          </div>
        </div>
        {onClose && (
        ...
        {state === "complete" && generatedAudio && (
          <>
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex flex-wrap gap-1">
                  <Badge variant="outline" className="text-xs">
                    {generatedAudio.parameters.genre}
                  </Badge>
                  <Badge variant="outline" className="text-xs">
                    {generatedAudio.parameters.tempo} BPM
                  </Badge>
                  <Badge variant="outline" className="text-xs">
                    {generatedAudio.parameters.key}
                  </Badge>
                </div>
              </div>
            ...
            <div className="flex gap-2">
              ...
              <Button
                variant="outline"
                onClick={() => window.open(generatedAudio.url, "_blank")}
              >
            ...
            <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
              <div className="flex items-center justify-between mb-3">
                ...
                <Badge className="bg-blue-600">
                  {styleTransfer.similarity}% Match
                </Badge>
              </div>
