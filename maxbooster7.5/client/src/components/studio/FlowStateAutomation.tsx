...
export function FlowStateAutomation({
  ...
  const pushUndo = useCallback(() => {
    setUndoStack(prev => [...prev.slice(-30), JSON.parse(JSON.stringify(lanes))]);
    setRedoStack([]);
  }, [lanes]);
  ...
  const interpolateValue = useCallback((lane: AutomationLane, time: number): number => {
  ...
  const addPoint = useCallback((laneId: string, time: number, value: number) => {
    pushUndo();
  ...
  const updatePoint = useCallback((laneId: string, pointId: string, updates: Partial<AutomationPoint>) => {
    const newLanes = lanes.map(lane => {
      if (lane.id !== laneId) return lane;
      return {
        ...lane,
        points: lane.points.map(p => p.id === pointId ? { ...p, ...updates } : p),
      };
    });
    onLanesChange(newLanes);
  }, [lanes, onLanesChange]);

  const deletePoint = useCallback((laneId: string, pointId: string) => {
    pushUndo();
    const newLanes = lanes.map(lane => {
      if (lane.id !== laneId) return lane;
      return {
  ...
  const toggleLaneLock = useCallback((laneId: string) => {
    const newLanes = lanes.map(lane => 
  ...
  const handleCanvasMouseDown = useCallback((laneId: string, e: React.MouseEvent<HTMLCanvasElement>) => {
  ...
  const handleCanvasMouseMove = useCallback((laneId: string, e: React.MouseEvent<HTMLCanvasElement>) => {
    const canvas = canvasRefs.current.get(laneId);
    if (!canvas) return;
  ...
  }, [beatsPerPixel, isDrawing, isDragging, selectedPointId, selectedLaneId, drawingTool, updatePoint, snapToGrid]);

  const handleCanvasMouseUp = useCallback((laneId: string) => {
  ...
  }, [lanes, beatsPerPixel, canvasWidth, selectedPointId, currentTime, isDrawing, pencilPoints, selectedLaneId, interpolateValue]);
  ...
  return (
    <div className="flex flex-col bg-slate-950 border border-slate-800 rounded-lg overflow-hidden">
          ...
          <div className="flex items-center gap-0.5 bg-slate-800 rounded-md p-0.5">
            {AUTOMATION_MODES.map(mode => (
              <button
                ...
                style={{
                  backgroundColor: automationMode === mode.id ? `${mode.color}33` : 'transparent',
            ...
            <div key={lane.id} className="border-b border-slate-800 last:border-b-0">
              <div className="h-8 px-3 flex items-center justify-between bg-slate-900/30">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span className="text-xs font-medium text-white">{lane.label}</span>
              ...
              <canvas
                ...
                className={cn(
                  "w-full",
                  lane.locked ? "cursor-not-allowed opacity-50" : "cursor-crosshair"
                )}
      ...
      <div className="h-8 px-3 flex items-center justify-between border-t border-slate-800 bg-slate-900/30">
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-white/40">{lanes.length} lanes</span>
          <span className="text-[10px] text-white/40">
