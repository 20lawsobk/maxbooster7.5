...
interface PurchaseConfirmationFlowProps {
  beat: Beat;
  license: License;
  onConfirm: () => Promise<{ success: boolean; downloadUrl?: string; licenseUrl?: string; error?: string }>;
  onCancel: () => void;
  useEscrow?: boolean;
  className?: string;
}
...
export function PurchaseConfirmationFlow({
  ...
  const [result, setResult] = useState<{ downloadUrl?: string; licenseUrl?: string; error?: string } | null>(null);
    ...
    if (currentStep === 'review') {
      ...
      try {
        ...
        if (response.success) {
          ...
          setResult({ error: response.error || 'Payment failed. Please try again.' });
      ...
      <div className="bg-gradient-to-r from-purple-600 to-indigo-600 p-4">
        <div className="flex items-center justify-between mb-4">
          {steps.map((step, index) => {
            const isActive = steps.findIndex((s) => s.id === currentStep || (currentStep === 'processing' && s.id === 'payment') || (currentStep === 'failed' && s.id === 'complete')) >= index;
          ...
          <div className="space-y-6">
            <div className="flex items-start gap-4 p-4 bg-muted rounded-lg">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground">by {beat.artist}</p>
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-2">
                ...
                <Label htmlFor="terms" className="text-sm leading-tight cursor-pointer">
                  I agree to the Terms of Service and understand that all sales are final
                </Label>
              </div>
              <div className="flex items-start gap-2">
                <Checkbox
          ...
          <div className="space-y-6">
            <div className="text-center">
              ...
              <p className="text-muted-foreground mt-2">
                Total: <span className="font-bold text-foreground">${total.toFixed(2)}</span>
              </p>
            </div>
          ...
          <div className="py-8 space-y-6">
            ...
            <Alert className="bg-blue-50 dark:bg-blue-950/30 border-blue-200">
              <AlertDescription className="text-sm">
                A confirmation email has been sent to your email address with download links and license details.
              </AlertDescription>
            </Alert>
          </div>
        )}
      ...
      {(currentStep === 'review' || currentStep === 'payment') && (
        <CardFooter className="flex justify-between p-6 pt-0">
          ...
          <Button
            ...
            disabled={currentStep === 'review' && (!agreedToTerms || !agreedToLicense)}
