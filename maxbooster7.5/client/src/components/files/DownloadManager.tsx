...
const OUTCOME_CONFIG: Record<DownloadOutcome, {
  label: string;
...
export function DownloadManager({
  ...
  const activeDownloads = downloads.filter(d => ['downloading', 'preparing', 'pending'].includes(d.outcome));
  const completedDownloads = downloads.filter(d => d.outcome === 'complete');
  const failedDownloads = downloads.filter(d => d.outcome.startsWith('error_'));
  const pausedDownloads = downloads.filter(d => d.outcome === 'paused');

  const totalSize = downloads.reduce((sum, d) => sum + d.fileSize, 0);
  const downloadedSize = downloads.reduce((sum, d) => sum + d.downloadedBytes, 0);
  ...
  const remainingBytes = activeDownloads.reduce((sum, d) => sum + (d.fileSize - d.downloadedBytes), 0);

  useEffect(() => {
    const newlyCompleted = downloads.filter(d => 
      d.outcome === 'complete' && 
      d.completedAt && 
  ...
  return (
    <Card className={cn('fixed bottom-4 left-4 w-96 shadow-lg z-50', className)}>
        ...
        <CardContent className="pt-2 pb-3">
          <div className="flex items-center gap-1 mb-2">
            ...
            {completedDownloads.length > 0 && onClearCompleted && (
              <Button variant="ghost" size="sm" onClick={onClearCompleted} className="h-7 text-xs ml-auto">
                Clear Complete
              </Button>
            )}
          </div>
...
function DownloadItem({
  ...
  const isActive = ['downloading', 'preparing', 'pending'].includes(download.outcome);

  return (
    <div className={cn(
      'p-2 rounded-lg border transition-all',
      isError ? 'border-destructive/30 bg-destructive/5' :
      download.outcome === 'complete' ? 'border-green-500/30 bg-green-500/5' :
      'border-border'
    )}>
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground mt-0.5">
            <span>{formatBytes(download.downloadedBytes)} / {formatBytes(download.fileSize)}</span>
          ...
          {(download.outcome === 'downloading' || download.outcome === 'paused') && (
        ...
        <div className="flex items-center gap-0.5 flex-shrink-0">
          {download.outcome === 'downloading' && onPause && (
            <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => onPause(download.id)}>
              <Pause className="h-3 w-3" />
            </Button>
          )}
          {download.outcome === 'paused' && onResume && (
            <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => onResume(download.id)}>
              <Play className="h-3 w-3" />
            </Button>
          )}
          {isError && download.canRetry && onRetry && (
            <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => onRetry(download.id)}>
              <RotateCcw className="h-3 w-3" />
            </Button>
          )}
          {download.outcome === 'complete' && onOpenFolder && (
            <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => onOpenFolder(download.id)}>
              <FolderOpen className="h-3 w-3" />
            </Button>
          )}
          {(isActive || isError || download.outcome === 'paused') && onCancel && (
            <Button 
              variant="ghost" 
              size="icon" 
          ...
          {(download.outcome === 'complete' || download.outcome === 'cancelled') && onRemove && (
            <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => onRemove(download.id)}>
              <X className="h-3 w-3" />
            </Button>
          )}
        </div>
...
export function useDownloadManager() {
  ...
  const startDownload = useCallback(async (fileId: string, fileName: string, fileSize: number, fileType: string) => {
    ...
    try {
      ...
      setDownloads(prev => prev.map(d => 
        d.id === downloadId 
          ? { ...d, outcome: 'downloading' as const, downloadUrl: data.downloadUrl }
          : d
      ));
  ...
  const pauseDownload = useCallback((id: string) => {
    setDownloads(prev => prev.map(d => 
      d.id === id ? { ...d, outcome: 'paused' as const } : d
    ));
  }, []);
  ...
  const retryDownload = useCallback((id: string) => {
    ...
    if (download) {
      startDownload(download.fileId, download.fileName, download.fileSize, download.fileType);
...
export function DownloadOutcomeBadge({ outcome }: { outcome: DownloadOutcome }) {
