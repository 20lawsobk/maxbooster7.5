...
export interface QueuedFile {
  ...
  speed: number;
  status: 'queued' | 'uploading' | 'paused' | 'processing' | 'success' | 'error' | 'cancelled';
  error?: string;
  priority: number;
  addedAt: number;
  startedAt?: number;
...
export function UploadQueue({
  ...
  const queuedFiles = files.filter(f => f.status === 'queued').sort((a, b) => b.priority - a.priority);
  ...
  }, [files, maxConcurrent, isPaused, activeUploads.length, queuedFiles, onUpload]);

  const handlePauseAll = useCallback(() => {
    setIsPaused(true);
    activeUploads.forEach(f => onPause(f.id));
    toast({ title: 'Queue Paused', description: 'All uploads have been paused' });
  ...
  const handleCancelAll = useCallback(() => {
    files.filter(f => ['queued', 'uploading', 'paused'].includes(f.status))
      .forEach(f => onCancel(f.id));
    toast({ title: 'Queue Cancelled', description: 'All pending uploads have been cancelled' });
  }, [files, onCancel, toast]);

  const handleRetryAll = useCallback(() => {
    failedFiles.forEach(f => onRetry(f.id));
    toast({ title: 'Retrying Failed Uploads', description: `Retrying ${failedFiles.length} failed upload${failedFiles.length > 1 ? 's' : ''}` });
  }, [failedFiles, onRetry, toast]);

  const handleClearCompleted = useCallback(() => {
    completedFiles.forEach(f => onRemove(f.id));
    toast({ title: 'Cleared', description: 'Completed uploads have been cleared' });
          ...
          <div className="flex items-center justify-between text-xs text-muted-foreground">
            ...
            {averageSpeed > 0 && (
              <span className="flex items-center gap-1">
                ...
                {estimatedTime > 0 && ` · ~${formatDuration(estimatedTime * 1000)} remaining`}
              </span>
            )}
          </div>
        </div>
        ...
        <CardContent className="pt-2">
          <div className="flex items-center gap-2 mb-3">
            {isPaused || pausedFiles.length > 0 ? (
              <Button variant="outline" size="sm" onClick={handleResumeAll} className="flex-1">
                <Play className="h-4 w-4 mr-1" />
                Resume All
              </Button>
            ) : (
              <Button variant="outline" size="sm" onClick={handlePauseAll} className="flex-1" disabled={activeUploads.length === 0}>
                <Pause className="h-4 w-4 mr-1" />
                Pause All
              </Button>
            )}
            
            <Button variant="outline" size="sm" onClick={handleCancelAll} className="flex-1" disabled={queuedFiles.length === 0 && activeUploads.length === 0}>
...
function QueueItem({
  ...
  const statusConfig = {
    queued: { icon: <ListOrdered className="h-4 w-4 text-muted-foreground" />, color: 'bg-muted' },
    uploading: { icon: <Loader2 className="h-4 w-4 animate-spin text-primary" />, color: 'bg-primary/10' },
    paused: { icon: <Pause className="h-4 w-4 text-muted-foreground" />, color: 'bg-muted' },
    processing: { icon: <Loader2 className="h-4 w-4 animate-spin text-amber-500" />, color: 'bg-amber-500/10' },
    success: { icon: <CheckCircle2 className="h-4 w-4 text-green-500" />, color: 'bg-green-500/10' },
    error: { icon: <AlertCircle className="h-4 w-4 text-destructive" />, color: 'bg-destructive/10' },
    cancelled: { icon: <X className="h-4 w-4 text-muted-foreground" />, color: 'bg-muted' },
  };
    ...
    <div className={cn(
      ...
      {position && (
        <div className="flex flex-col items-center gap-0.5">
          {onPriorityChange && canMoveUp && (
            <Button variant="ghost" size="icon" className="h-4 w-4" onClick={() => onPriorityChange(file.id, 'up')}>
              <ChevronUp className="h-3 w-3" />
            </Button>
          )}
          <span className="text-xs text-muted-foreground font-medium w-4 text-center">{position}</span>
          {onPriorityChange && canMoveDown && (
            <Button variant="ghost" size="icon" className="h-4 w-4" onClick={() => onPriorityChange(file.id, 'down')}>
      ...
      <div className="flex items-center gap-1 flex-shrink-0">
        ...
        {file.status === 'uploading' && (
          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => onPause(file.id)}>
            <Pause className="h-3 w-3" />
          </Button>
        )}
        {file.status === 'paused' && (
          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => onResume(file.id)}>
            <Play className="h-3 w-3" />
          </Button>
        )}
        {file.status === 'error' && (
          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => onRetry(file.id)}>
            <RotateCcw className="h-3 w-3" />
          </Button>
        )}
        {['queued', 'uploading', 'paused', 'error'].includes(file.status) && (
          <Button variant="ghost" size="icon" className="h-7 w-7 hover:bg-destructive/10 hover:text-destructive" onClick={() => onCancel(file.id)}>
            <X className="h-3 w-3" />
          </Button>
        )}
        {file.status === 'success' && (
