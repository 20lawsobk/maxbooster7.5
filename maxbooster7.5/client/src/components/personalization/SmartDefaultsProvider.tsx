...
const SmartDefaultsContext = createContext<SmartDefaultsContextValue | undefined>(undefined);

const artistTypeDefaultWidgets: Record<ArtistType, string[]> = {
...
const genrePresets: Record<string, Partial<GenrePreset>> = {
  'hip-hop': { defaultBPM: 90, suggestedPlatforms: ['spotify', 'tiktok', 'youtube'], postingFrequency: 'high' },
  'electronic': { defaultBPM: 128, suggestedPlatforms: ['spotify', 'soundcloud', 'beatport'], postingFrequency: 'medium' },
  'pop': { defaultBPM: 120, suggestedPlatforms: ['spotify', 'tiktok', 'instagram'], postingFrequency: 'high' },
  'rock': { defaultBPM: 140, suggestedPlatforms: ['spotify', 'youtube', 'bandcamp'], postingFrequency: 'medium' },
  'r&b': { defaultBPM: 85, suggestedPlatforms: ['spotify', 'apple-music', 'instagram'], postingFrequency: 'medium' },
  'indie': { defaultBPM: 110, suggestedPlatforms: ['bandcamp', 'spotify', 'youtube'], postingFrequency: 'low' },
  'classical': { defaultBPM: 60, suggestedPlatforms: ['spotify', 'apple-music', 'youtube'], postingFrequency: 'low' },
  'jazz': { defaultBPM: 120, suggestedPlatforms: ['spotify', 'bandcamp', 'tidal'], postingFrequency: 'low' },
};
...
export function SmartDefaultsProvider({ children, userId }: SmartDefaultsProviderProps) {
  const queryClient = useQueryClient();
  const currentTimeLayout = getCurrentTimeOfDay();

  const { data: preferences, isLoading: prefsLoading, error: prefsError } = useQuery<PersonalizationPreferences>({
    queryKey: ['/api/personalization/preferences'],
    staleTime: 10 * 60 * 1000,
  });

  const { data: defaults = [], isLoading: defaultsLoading } = useQuery<SmartDefault[]>({
  ...
  const trackInteractionMutation = useMutation({
    mutationFn: async (event: InteractionEvent) => {
      const response = await apiRequest('POST', '/api/personalization/track-interaction', {
        ...event,
  ...
  const applyGenrePreset = useCallback(async (genre: string) => {
...
export function useSmartDefaultsContext() {
  ...
  if (context === undefined) {
    throw new Error('useSmartDefaultsContext must be used within a SmartDefaultsProvider');
  }
  return context;
}
