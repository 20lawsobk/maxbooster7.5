(function () {
  ...
  window.addEventListener("error", function (e) {
    if (
      ...
      d.style.cssText =
        "position:fixed;top:0;left:0;right:0;bottom:0;background:#0f172a;color:#e2e8f0;display:flex;align-items:center;justify-content:center;font-family:Inter,system-ui,sans-serif;padding:20px;z-index:99999;text-align:center;";
      ...
      reloadBtn.style.cssText =
        "background:#3b82f6;color:white;border:none;padding:10px 24px;border-radius:8px;cursor:pointer;font-size:14px";
  ...
  var observer = new MutationObserver(function () {
    ...
    if (
      ...
      root.firstElementChild.id !== "mb-initial-loader"
    ) {
      gone = true;
      observer.disconnect();
      ...
      if ("serviceWorker" in navigator && navigator.serviceWorker.controller) {
        var scripts = Array.from(document.querySelectorAll("script[src]"))
          ...
          .filter(function (s) {
            return (
              s.includes("/assets/") &&
              (s.includes("vendor-react") ||
                s.includes("/index-") ||
                s.includes("vendor-state") ||
                s.includes("vendor-ui"))
            );
          });
