...
describe("Unauthenticated route guards", () => {
  ...
  it("GET /api/auth/me → 200 (public, returns null)", async () =>
    expect(await unauthGet("/api/auth/me")).toBe(200));
  it("GET /api/auth/notifications → 401", async () =>
    expect(await unauthGet("/api/auth/notifications")).toBe(401));
  it("GET /api/auth/sessions → 401", async () =>
    expect(await unauthGet("/api/auth/sessions")).toBe(401));
  it("GET /api/auth/login-history → 401", async () =>
    expect(await unauthGet("/api/auth/login-history")).toBe(401));
  ...
  // --- Marketplace ---
  it("GET /api/marketplace/my-beats → 401", async () =>
    expect(await unauthGet("/api/marketplace/my-beats")).toBe(401));
  ...
  it("GET /api/distribution/releases → 401", async () =>
    expect(await unauthGet("/api/distribution/releases")).toBe(401));
...
describe("Public routes are accessible", () => {
  it("GET /api/health is public", async () =>
    expect(await unauthGet("/api/health")).toBe(200));
  it("GET /health is public", async () =>
    expect(await unauthGet("/health")).toBe(200));
  it("GET /ready is public", async () => {
    const s = await unauthGet("/ready");
    expect([200, 503]).toContain(s);
  });
  it("GET /api/ping is public", async () =>
    expect(await unauthGet("/api/ping")).toBe(200));
