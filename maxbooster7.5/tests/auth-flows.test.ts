...
describe("OAuth Callback Structure", () => {
  ...
  it("23. GET /api/auth/google/callback with valid state reaches a different redirect than invalid_state", async () => {
    ...
    if (initLocation.includes("google_not_configured")) {
      ...
      console.warn(
        "[AuthTest] GOOGLE_CLIENT_ID not configured — skipping valid-state callback test",
      );
      return;
    }
    ...
    try {
      ...
      console.warn(
        "[AuthTest] Could not parse Google redirect URL — skipping valid-state callback test",
      );
      return;
    }
