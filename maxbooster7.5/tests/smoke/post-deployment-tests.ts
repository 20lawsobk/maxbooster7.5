...
class PostDeploymentSmokeTests {
    ...
    this.tests = [
      ...
      },
      {
        ...
        test: async () => {
          ...
          return {
            ok: data.status === "ok" && !!data.timestamp,
      ...
      },
      {
        ...
        test: async () => {
          ...
          return {
            ok: data.ok === true && typeof data.uptime === "number",
            detail: `uptime=${data.uptime}s`,
          };
        },
      },
      ...
      // ── Critical: Security middleware active ─────────────────────────────────
      {
        ...
        test: async () => {
          ...
          return {
            ok: xcto === "nosniff",
            detail: `x-content-type-options=${xcto}`,
          };
        },
      },
      {
      ...
      },
      {
        ...
        critical: true,
        test: async () => {
          const r = await fetch(`${BASE}/api/auth/notifications`, {
            signal: AbortSignal.timeout(8000),
          });
  ...
  async runAllTests(): Promise<{
    ...
    for (const test of this.tests) {
      try {
        ...
        console.log(
          `${icon} ${test.name}${tag}${detail ? ` — ${detail}` : ""}`,
        );
      } catch (error) {
  ...
  private generateReport(): {
    ...
    console.log(
      `Critical Failures:   ${criticalFailures} ${criticalFailures > 0 ? "🚨" : "✅"}`,
    );
    console.log("═".repeat(70));
    ...
    if (criticalFailures === 0 && passedTests === totalTests) {
      ...
      console.log(
        "     Critical tests passed; some non-critical tests need review.",
      );
    } else {
      console.log("  ❌ DEPLOYMENT: FAILED");
      console.log(
        "     Critical smoke tests failed. Investigate before serving traffic.",
      );
    }
    console.log("═".repeat(70) + "\n");
