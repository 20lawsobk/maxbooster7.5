...
export const DEFAULT_CONTENT_GUIDELINES: ContentGuideline[] = [
  ...
  },
  {
    ...
    validator: (content) => {
      const ctaPatterns =
        /\b(listen|watch|click|link|bio|check out|swipe|tap|follow|share|subscribe|comment|tag|dm|stream)\b/i;
  ...
  },
  {
    ...
    type: "recommended",
    description:
      "Content should be between 50-200 characters for optimal engagement",
  ...
  },
  {
    ...
    validator: (content) => {
      const spamPatterns =
        /(.)\1{4,}|!{3,}|\?{3,}|\${3,}|free money|click here now/i;
...
export const DEFAULT_SCHEDULING_RULES: SchedulingRule[] = [
  ...
  },
  {
    ...
    condition: (ctx) => {
      ...
      return (
        ctx.scheduledTime.getTime() - ctx.lastPostTime.getTime() < minGapMs
      );
    },
    action: "delay",
    delayMinutes: 60,
  ...
  },
  {
    ...
    delayMinutes: 240,
    reason:
      "High audience fatigue detected. Delaying to prevent disengagement.",
  },
  {
    id: "ad_coordination",
...
export class SocialMediaRuleEngine {
  private contentGuidelines: ContentGuideline[] = [
    ...DEFAULT_CONTENT_GUIDELINES,
  ];
  private schedulingRules: SchedulingRule[] = [...DEFAULT_SCHEDULING_RULES];
  private customPlatformLimits: Map<string, Partial<PlatformLimits>> =
    new Map();

  constructor() {}

  public addContentGuideline(guideline: ContentGuideline): void {
    const existingIdx = this.contentGuidelines.findIndex(
      (g) => g.id === guideline.id,
    );
    if (existingIdx >= 0) {
  ...
  public setCustomPlatformLimits(
    platform: string,
  ...
  public getPlatformLimits(platform: string): PlatformLimits {
    const base =
      PLATFORM_LIMITS[platform.toLowerCase()] || PLATFORM_LIMITS.twitter;
  ...
  public evaluateScheduling(context: SchedulingContext): RuleEvaluationResult {
    ...
    if (maxDelay > 0) {
      adjustedTime = new Date(
        context.scheduledTime.getTime() + maxDelay * 60 * 1000,
      );
    }
    ...
    if (hour < 6 || hour > 23) {
      recommendations.push(
        "Consider scheduling between 6 AM and 11 PM for better engagement",
      );
    }
  ...
  public evaluatePost(
    ...
    return {
      ...
      recommendations: [
        ...
        ...schedulingResult.recommendations,
      ],
