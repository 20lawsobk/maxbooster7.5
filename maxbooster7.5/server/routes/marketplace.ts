...
import { eq, and, gte, sql, desc, asc, inArray } from 'drizzle-orm';
...
// GET /api/marketplace/filters/available - Get all available filter options
router.get('/filters/available', async (req: Request, res: Response) => {
  try {
    // Get all unique filter values from database
    const genres = await db.selectDistinct({ genre: listings.genre })
      .from(listings)
      .where(eq(listings.status, 'active'))
      .orderBy(asc(listings.genre));
    
    const moods = await db.selectDistinct({ mood: listings.mood })
      .from(listings)
      .where(eq(listings.status, 'active'))
      .orderBy(asc(listings.mood));
    
    const keys = await db.selectDistinct({ key: listings.key })
      .from(listings)
      .where(eq(listings.status, 'active'))
      .orderBy(asc(listings.key));
    
    // Get BPM range
    const [bpmRange] = await db.select({
      min: sql<number>`MIN(${listings.bpm})`,
      max: sql<number>`MAX(${listings.bpm})`,
    ...
    res.json({
      ...
      keys: keys.map(k => k.key).filter(Boolean),
      bpmRange: {
        min: Number(bpmRange?.min) || 60,
        max: Number(bpmRange?.max) || 180,
      },
      priceRange: {
        min: Number(priceRange?.min) || 0,
        max: Number(priceRange?.max) || 500,
      },
    });
  } catch (error: any) {
...
// GET /api/marketplace/beats/trending - Get trending beats
router.get('/beats/trending', async (req: Request, res: Response) => {
  try {
    const { limit = 12, timeframe = '7d' } = req.query;
    ...
    .orderBy(desc(listings.plays), desc(listings.views))
    .limit(Number(limit));
    
    res.json({ beats: trendingBeats });
  } catch (error: any) {
    logger.error('Error fetching trending beats:', error);
    res.status(500).json({ error: 'Failed to fetch trending beats' });
  }
});

// POST /api/marketplace/beats/batch-add-to-cart - Add multiple beats to cart
router.post('/beats/batch-add-to-cart', requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id;
    const { beatIds } = req.body;
    
    if (!Array.isArray(beatIds) || beatIds.length === 0) {
      return res.status(400).json({ error: 'beatIds must be a non-empty array' });
    }
    
    // Fetch all beats using secure parameterized query
    const beats = await db.select()
      .from(listings)
      .where(and(
        inArray(listings.id, beatIds),
        eq(listings.status, 'active')
      ));
    
    if (beats.length !== beatIds.length) {
      return res.status(404).json({ error: 'Some beats not found or unavailable' });
    }
    
    // Add to cart (implement cart logic here)
    const cartItems = beats.map(beat => ({
      id: beat.id,
      title: beat.title,
      price: beat.price,
      addedAt: new Date(),
    ...
    logger.error('Error adding beats to cart:', error);
    res.status(500).json({ error: 'Failed to add beats to cart' });
  }
});
...
// GET /api/marketplace/beats/:id/similar - Get similar beats
router.get('/beats/:id/similar', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { limit = 8 } = req.query;

    const beat = await db.select().from(listings).where(eq(listings.id, id)).limit(1);
    
    if (beat.length === 0) {
      return res.status(404).json({ error: 'Beat not found' });
    }

    const sourceBeat = beat[0];

    // Find similar beats based on genre, BPM, and key
    const similarBeats = await db.select()
      .from(listings)
      .where(and(
        eq(listings.status, 'active'),
        sql`${listings.id} != ${id}`,
        or(
          eq(listings.genre, sourceBeat.genre),
          sql`ABS(${listings.bpm} - ${sourceBeat.bpm}) < 10`,
          eq(listings.key, sourceBeat.key)
        )
      ))
      .orderBy(desc(listings.plays))
      .limit(Number(limit));

    res.json({ beats: similarBeats });
  } catch (error: any) {
    logger.error('Error fetching similar beats:', error);
    res.status(500).json({ error: 'Failed to fetch similar beats' });
  }
});

// POST /api/marketplace/beats/:id/favorite - Add beat to favorites
router.post('/beats/:id/favorite', requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id;
    const { id } = req.params;
    ...
    // Add to favorites (implement favorites table logic here)
    logger.info(`Beat ${id} favorited by user ${userId}`);
    ...
    logger.error('Error favoriting beat:', error);
    res.status(500).json({ error: 'Failed to favorite beat' });
  }
});

// DELETE /api/marketplace/beats/:id/favorite - Remove beat from favorites
router.delete('/beats/:id/favorite', requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id;
    const { id } = req.params;

    // Remove from favorites (implement favorites table logic here)
    logger.info(`Beat ${id} unfavorited by user ${userId}`);
    ...
    logger.error('Error unfavoriting beat:', error);
    res.status(500).json({ error: 'Failed to unfavorite beat' });
  }
});

// GET /api/marketplace/purchases/history - Get purchase history
router.get('/purchases/history', requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id;
...
// GET /api/marketplace/downloads/:purchaseId - Get download link for purchase
router.get('/downloads/:purchaseId', requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id;
    ...
    // Generate temporary download link (implement signed URL logic)
    const downloadUrl = beat[0].audioFileUrl || beat[0].audioPreviewUrl;
    const expiresAt = new Date(Date.now() + 24 * 60 * 60 * 1000); // 24 hours

    res.json({
      downloadUrl,
      expiresAt,
      beatTitle: beat[0].title,
...
// POST /api/marketplace/licensing/custom-request - Request custom licensing
router.post('/licensing/custom-request', requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id;
    const { beatId, producerId, requestDetails, intendedUse, budget } = req.body;

    if (!beatId || !producerId || !requestDetails) {
      return res.status(400).json({ 
        error: 'beatId, producerId, and requestDetails are required' 
      });
    }

    // Create custom licensing request
    const request = {
    ...
    logger.info(`Custom licensing request created: ${request.id}`);
    ...
    res.json({
      ...
      message: 'Custom licensing request submitted. The producer will respond within 48 hours.',
    });
  } catch (error: any) {
    logger.error('Error creating custom licensing request:', error);
    res.status(500).json({ error: 'Failed to create licensing request' });
  }
});

// GET /api/marketplace/producer/:id/profile - Get producer profile
router.get('/producer/:id/profile', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;

    // Get producer's listings
    const producerBeats = await db.select()
      .from(listings)
      .where(and(
        eq(listings.producerId, id),
        eq(listings.status, 'active')
      ))
    ...
    const totalSales = await db.select({ count: sql<number>`COUNT(*)` })
      .from(orders)
      .where(sql`${orders.listingId} IN (SELECT id FROM ${listings} WHERE ${listings.producerId} = ${id})`);
    ...
    res.json({
      ...
      recentBeats: producerBeats,
    });
  } catch (error: any) {
    logger.error('Error fetching producer profile:', error);
    res.status(500).json({ error: 'Failed to fetch producer profile' });
  }
});

// POST /api/marketplace/beats/:id/preview-count - Increment preview count
router.post('/beats/:id/preview-count', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    await db.update(listings)
      .set({ 
        plays: sql`${listings.plays} + 1`,
      })
      .where(eq(listings.id, id));

    res.json({ success: true });
  } catch (error: any) {
    logger.error('Error incrementing preview count:', error);
