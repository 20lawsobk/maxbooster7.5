...
const optimizeCampaignSchema = z.object({
  ...
  performance: z.object({
    ...
    platform: z.enum(['instagram', 'facebook', 'twitter', 'tiktok', 'youtube', 'spotify']).optional(),
    objective: z.enum(['awareness', 'engagement', 'conversions', 'traffic', 'leads']).optional(),
...
const generateContentSchema = z.object({
  campaignId: z.string().min(1).max(100),
  contentType: z.enum(['promotional', 'educational', 'engagement', 'announcement']).optional(),
  platform: z.enum(['instagram', 'facebook', 'twitter', 'tiktok', 'youtube', 'linkedin']).optional(),
  topic: z.string().min(1).max(500).optional(),
  tone: z.enum(['professional', 'casual', 'energetic', 'emotional', 'humorous', 'inspirational']).optional(),
});
...
router.post('/optimize-campaign', aiRateLimiter, requireAuth, async (req, res) => {
  try {
    // Validate input with Zod schema
    const validated = optimizeCampaignSchema.parse(req.body);
    const { campaignId, performance } = validated;
...
router.post('/generate-content', aiRateLimiter, requireAuth, async (req, res) => {
  try {
    // Validate input with Zod schema
