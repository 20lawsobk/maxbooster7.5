...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const data = hyperFollowSchema.parse(rawData);
...
router.patch(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const data = hyperFollowSchema.partial().parse(rawData);
  ...
  try {
    ...
    if (req.body.data) {
      ...
      try {
        ...
        data = duplicateCheckSchema.parse(rawData);
      } catch (parseError) {
        return res.status(400).json({ 
...
// POST /api/distribution/releases/:id/takedown - Request release takedown
router.post('/releases/:id/takedown', requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = (req.user as AuthenticatedUser).id;
    const { id } = req.params;
    const { platforms, reason, immediate = false } = req.body;
    ...
    logger.info(`Takedown requested for release ${id}`, { platforms, reason, immediate });

    // Update release metadata with takedown request
    const metadata = release.metadata as any;
    metadata.takedownRequests = metadata.takedownRequests || [];
    metadata.takedownRequests.push({
      platforms,
      reason,
      immediate,
    ...
    await storage.updateDistroRelease(id, { metadata });

    res.json({
      success: true,
      message: `Takedown request submitted for ${platforms.length} platform(s)`,
      platforms,
      estimatedCompletionDays: immediate ? 3 : 7,
    });
  } catch (error: unknown) {
    logger.error('Error requesting takedown:', error);
    res.status(500).json({ error: 'Failed to request takedown' });
...
// GET /api/distribution/releases/:id/earnings - Get release earnings breakdown
router.get('/releases/:id/earnings', requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = (req.user as AuthenticatedUser).id;
    const { id } = req.params;
    ...
    // Get earnings from analytics table (if integrated)
    // For now, return mock structure
    const earnings = {
      totalEarnings: 0,
      currency: 'USD',
      platformBreakdown: [] as any[],
      trackBreakdown: [] as any[],
      dateRange: {
    ...
    res.status(500).json({ error: 'Failed to fetch release earnings' });
  }
});

// GET /api/distribution/releases/:id/platforms - Get release status per platform
router.get('/releases/:id/platforms', requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = (req.user as AuthenticatedUser).id;
    const { id } = req.params;
    ...
    const platformStatuses = selectedPlatforms.map((platform: string) => ({
      platform,
      status: metadata?.dispatchStatus?.[platform] || 'pending',
      liveUrl: metadata?.liveUrls?.[platform] || null,
      goLiveDate: release.releaseDate,
      errorMessage: metadata?.platformErrors?.[platform] || null,
    }));
    ...
    res.json({
      ...
      livePlatforms: platformStatuses.filter((p: any) => p.status === 'live').length,
    });
  } catch (error: unknown) {
    logger.error('Error fetching platform statuses:', error);
    res.status(500).json({ error: 'Failed to fetch platform statuses' });
  }
});

// PATCH /api/distribution/releases/:id/artwork - Update release artwork
router.patch('/releases/:id/artwork', requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = (req.user as AuthenticatedUser).id;
    const { id } = req.params;
    ...
    if (!artworkUrl) {
      return res.status(400).json({ error: 'artworkUrl is required' });
    }
    ...
    if (release.status === 'live') {
      return res.status(400).json({ 
        error: 'Cannot update artwork for live release. Consider creating a new version.' 
      });
    }

    await storage.updateDistroRelease(id, { artworkUrl });

    logger.info(`Artwork updated for release ${id}`);
...
// POST /api/distribution/splits/calculate - Calculate revenue splits
router.post('/splits/calculate', requireAuth, async (req: Request, res: Response) => {
  try {
    const { totalRevenue, splits } = req.body;

    if (!totalRevenue || !Array.isArray(splits)) {
      return res.status(400).json({ error: 'totalRevenue and splits array required' });
    }
    ...
    const calculatedSplits = splits.map((split: any) => ({
      ...split,
    ...
    logger.error('Error calculating splits:', error);
    res.status(500).json({ error: 'Failed to calculate splits' });
  }
});

// GET /api/distribution/royalties/breakdown - Get detailed royalties breakdown
router.get('/royalties/breakdown', requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = (req.user as AuthenticatedUser).id;
    const { dateRange = '30d', releaseId } = req.query;

    const days = parseInt(String(dateRange).replace('d', '')) || 30;
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - days);

    // Get royalties breakdown (integrate with analytics/payments when ready)
    const breakdown = {
      totalRoyalties: 0,
      currency: 'USD',
      byRelease: [] as any[],
    ...
    logger.error('Error fetching royalties breakdown:', error);
    res.status(500).json({ error: 'Failed to fetch royalties breakdown' });
  }
});
...
// In-memory upload session tracking (replace with Redis in production)
...
// GET /api/distribution/upload/:sessionId/status - Get upload session status
router.get('/upload/:sessionId/status', requireAuth, async (req: Request, res: Response) => {
  try {
...
// GET /api/distribution/upload/:sessionId/progress - Get upload progress
router.get('/upload/:sessionId/progress', requireAuth, async (req: Request, res: Response) => {
  try {
    const { sessionId } = req.params;
    const userId = (req.user as AuthenticatedUser).id;
...
// GET /api/distribution/releases/:id/status - Get detailed release status
router.get('/releases/:id/status', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const userId = (req.user as AuthenticatedUser).id;
    ...
    // Check LabelGrid status if integrated
    let labelGridStatus = null;
    if (metadata?.labelGridReleaseId) {
      try {
        labelGridStatus = await labelGridService.getReleaseStatus(metadata.labelGridReleaseId);
    ...
    res.json({
      ...
      platforms: metadata?.selectedPlatforms || [],
      labelGridStatus,
      dispatchStatus: metadata?.dispatchStatus || [],
      liveUrls: metadata?.liveUrls || {},
      submittedAt: release.submittedAt,
      approvedAt: release.approvedAt,
      liveDate: release.releaseDate,
    });
  } catch (error: unknown) {
    logger.error('Error fetching release status:', error);
    res.status(500).json({ error: 'Failed to fetch release status' });
