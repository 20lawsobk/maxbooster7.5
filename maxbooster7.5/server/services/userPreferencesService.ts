...
const DEFAULT_WIDGETS: DashboardWidget[] = [
  { id: 'overview', name: 'Overview Stats', visible: true, order: 0, size: 'large', column: 0 },
  { id: 'revenue', name: 'Revenue', visible: true, order: 1, size: 'medium', column: 0 },
  { id: 'analytics', name: 'Analytics', visible: true, order: 2, size: 'medium', column: 1 },
  { id: 'releases', name: 'Upcoming Releases', visible: true, order: 3, size: 'small', column: 1 },
  { id: 'social', name: 'Social Media', visible: true, order: 4, size: 'medium', column: 0 },
  { id: 'ai-coach', name: 'AI Career Coach', visible: true, order: 5, size: 'medium', column: 1 },
  { id: 'collaborators', name: 'Collaborators', visible: true, order: 6, size: 'small', column: 0 },
  { id: 'achievements', name: 'Achievements', visible: true, order: 7, size: 'small', column: 1 },
  { id: 'quick-actions', name: 'Quick Actions', visible: true, order: 8, size: 'small', column: 0 },
  { id: 'tips', name: 'Tips & Guidance', visible: true, order: 9, size: 'small', column: 1 },
];
...
const CAREER_STAGE_MODIFIERS: Record<CareerStage, Partial<UserPreferences>> = {
  ...
  established: {
    primaryGoals: ['maximize_revenue', 'expand_audience', 'strategic_partnerships'],
    aiAssistantLevel: 'moderate',
    dashboardLayout: { preset: 'standard', widgets: DEFAULT_WIDGETS },
  },
  professional: {
...
class UserPreferencesService {
  ...
  async getUserPreferences(userId: string): Promise<UserPreferences | null> {
    try {
      ...
      if (redis) {
        await redis.setEx(`${this.CACHE_PREFIX}${userId}`, this.CACHE_TTL, JSON.stringify(preferences));
      }
  ...
  async updateUserPreferences(userId: string, updates: Partial<UserPreferences>): Promise<UserPreferences> {
    try {
      const current = await this.getUserPreferences(userId) || this.getDefaultPreferences('solo', 'emerging');
      ...
      const redis = await getRedisClient();
      if (redis) {
        await redis.setEx(`${this.CACHE_PREFIX}${userId}`, this.CACHE_TTL, JSON.stringify(updated));
      }
  ...
  getDefaultPreferences(artistType: ArtistType, careerStage: CareerStage): UserPreferences {
    ...
    return this.deepMerge(this.deepMerge(baseDefaults, artistDefaults), stageModifiers);
  }

  async recordBehaviorEvent(userId: string, event: BehaviorEvent): Promise<void> {
    try {
      const redis = await getRedisClient();
      if (!redis) return;
      ...
      logger.error('Error recording behavior event:', error);
    }
  }
  ...
  async learnFromBehavior(userId: string): Promise<PreferenceRecommendation[]> {
    try {
      ...
      if (featureUsage.studioUsage > 0.7 && preferences.studioPreferences.autoSave === false) {
        recommendations.push({
          category: 'studio',
          recommendation: 'Enable auto-save for your studio projects',
          reason: 'You use the studio frequently - auto-save will protect your work',
          priority: 'high',
          actionable: true,
      ...
      if (timePatterns.peakHour && !preferences.contentPreferences.preferredPostingTimes.includes(`${timePatterns.peakHour}:00`)) {
        recommendations.push({
          category: 'content',
          ...
          priority: 'medium',
          actionable: true,
          suggestedValue: { preferredPostingTimes: [`${timePatterns.peakHour}:00`, ...preferences.contentPreferences.preferredPostingTimes.slice(0, 2)] },
        });
      }

      if (contentPatterns.topContentType && !preferences.contentPreferences.contentTypes.includes(contentPatterns.topContentType)) {
        recommendations.push({
          category: 'content',
          ...
          priority: 'medium',
          actionable: true,
          suggestedValue: { contentTypes: [...preferences.contentPreferences.contentTypes, contentPatterns.topContentType] },
        });
      }

      if (featureUsage.aiUsage < 0.2 && preferences.aiAssistantLevel !== 'minimal') {
        recommendations.push({
          category: 'ai',
          recommendation: 'Consider trying our AI assistant features',
          reason: "You haven't fully explored AI tools that could save you time",
          priority: 'low',
          actionable: false,
        });
      }
      ...
      logger.error('Error learning from behavior:', error);
      return [];
    }
  }

  async getPreferenceRecommendations(userId: string): Promise<PreferenceRecommendation[]> {
    ...
    if (preferences.dashboardLayout.widgets.filter(w => w.visible).length < 3) {
      generalRecommendations.push({
        category: 'dashboard',
