...
class CareerCoachService {
  ...
  /**
   * Process AI-powered chat conversation with career coach
   */
  async processChat(userId: string, message: string, context?: any): Promise<string> {
    try {
      logger.info(`Processing career coach chat for user ${userId}`);
      ...
      const systemContext = {
        ...
        lastRelease: snapshot.lastReleaseDate,
        topPlatform: snapshot.topPlatform,
        socialAccounts: snapshot.socialAccounts.length,
        careerGaps: gaps.slice(0, 3),
        activeGoals: activeGoals.filter(g => g.status === 'active').length,
      };

      const prompt = `You are an AI career coach for musicians and artists. Help guide this artist's career growth.

Artist Context:
- Total Streams: ${systemContext.totalStreams.toLocaleString()}
- Followers: ${systemContext.totalFollowers.toLocaleString()}
- Releases: ${systemContext.releaseCount}
- Last Release: ${systemContext.lastRelease ? new Date(systemContext.lastRelease).toLocaleDateString() : 'Never'}
- Top Platform: ${systemContext.topPlatform || 'None'}
- Connected Social Accounts: ${systemContext.socialAccounts}
- Active Goals: ${systemContext.activeGoals}

${gaps.length > 0 ? `Career Gaps:\n${gaps.slice(0, 3).map(g => `- ${g.area}: ${g.description}`).join('\n')}` : ''}

User Message: ${message}

Provide practical, actionable advice tailored to their specific situation. Be encouraging but realistic. Keep responses concise (2-3 paragraphs max).`;

      const aiResponse = await this.callAIService(prompt);

      return aiResponse || 'I\'m here to help you grow your music career! What specific area would you like to focus on - releases, promotion, or analytics?';
    } catch (error) {
      logger.error('Error processing career coach chat:', error);
      throw error;
    }
  }

  /**
   * Call AI service for chat responses
   * Note: This is a placeholder that should be connected to an actual AI service (OpenAI, etc.)
   */
  private async callAIService(prompt: string): Promise<string> {
    logger.warn('AI service not configured - returning fallback response');
    
    const fallbackResponses = [
      'Based on your analytics, I recommend focusing on consistent release scheduling. Artists who release regularly see 3x better algorithmic placement.',
      'Your engagement metrics show room for growth. Consider deepening fan connections through social media interaction and exclusive content.',
      'I notice you haven\'t connected all your social accounts yet. This limits cross-platform promotion opportunities. Would you like help setting those up?',
      'Your streaming numbers are solid! To take them to the next level, focus on playlist placement and collaborative releases with other artists.',
      'Consider setting a SMART goal around your weakest metric. What specific area would you like to improve in the next 30 days?',
    ];
