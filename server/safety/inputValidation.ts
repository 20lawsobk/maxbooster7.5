...
export function validate<T extends ZodSchema>(
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const _validated = await schema?.parseAsync(data);
      ...
      if (error instanceof ZodError) {
        const _errors = error?.issues.map((e) => ({
          field: e?.path.join("."),
          message: e?.message,
        }));
...
export function sanitizationMiddleware(
  ...
  if (req?.query && typeof req?.query === "object") {
    const _sanitized = sanitizeObject(req?.query as Record<string, any>);
 ...
 */
const _validationErrorCounts = new Map<
  string,
...
export function trackValidationError(ip: string): boolean {
  const _now = new Date();
  const _entry = validationErrorCounts?.get(ip);
