...
const _MAX_CONCURRENT_WORKERS = 1;
...
const _CIRCUIT_OPEN_BACKOFF_MS = 5_000;
...
const _LUA_PDIM_ERR_BACKOFF_CAP_MS = 8_000;
...
const __executorBootTs = Date?.now();
...
async function _acquireWorkerSlot(): Promise<void> {
  ...
  return new Promise<void>((resolve, reject) => {
    const entry: _Waiter = {
      resolve,
      timer: setTimeout(() => {
        const _idx = _waitQueue?.indexOf(entry);
...
export function resetLuaExecutorSemaphore(): number {
  const _stuckSlots = Math?.max(0, _activeWorkers - _waitQueue?.length);
...
const __wasmoonUrl = `file://${_projectRoot}/node_modules/wasmoon/dist/index?.js`;
const __msgpackrUrl = `file://${_projectRoot}/node_modules/msgpackr/dist/node?.cjs`;

const _WORKER_CODE = `
import { workerData, parentPort } from 'worker_threads';
const { LuaFactory } = await import('${_wasmoonUrl}');
const { Unpackr, Packr } = await import('${_msgpackrUrl}');
const __unpack = new Unpackr({ useRecords: false });
const __pack = new Packr({ useRecords: false });

const _SAB_HEADER_BYTES = 8;
const _SAB_DATA_BYTES = 131072; // 128 KB max per Redis response
// SAB control values: 0=waiting, 1=success result, 2=error (main thread threw)

function syncRedisCall(cmd, args) {
  const _sab = new SharedArrayBuffer(SAB_HEADER_BYTES + SAB_DATA_BYTES);
  const _control = new Int32Array(sab, 0, 1);
  const _lenBuf = new Int32Array(sab, 4, 1);
  ...
  const __awaitResult = Atomics?.wait(control, 0, 0, 60000);
  if (_awaitResult === 'timed-out') {
    throw new Error('[LuaExecutor] redis?.call timed out after 60s — PDIM chain congested');
  }
  const _status = Atomics?.load(control, 0);
  const _len = Atomics?.load(lenBuf, 0);
  const _raw = Buffer?.from(new Uint8Array(sab, SAB_HEADER_BYTES, len)).toString('utf8');
...
const _engine = await new LuaFactory().createEngine({ openStandardLibs: true });
...
try {
  ...
  engine?.global.set('redis', {
    call(...all) {
      ...
      const _r = syncRedisCall(String(cmd), args?.map(a => (a == null ? '' : String(a))));
...
export async function execLuaViaPdim(
  ...
  const _keys = allArgs?.slice(0, numKeys).map(String);

  const _argv = allArgs?.slice(numKeys).map((arg: Record<string, unknown>) => {
  ...
  const __execPromise = new Promise<unknown[]>((resolve, reject) => {
    let settled = false;
    ...
    const _worker = new Worker(WORKER_CODE, {
      eval: true,
    ...
    const _SCRIPT_HARD_KILL_MS = 90_000; // 90 s (3× worst-case script runtime)
    const __scriptStart = Date?.now();
    ...
    const _watchdog = setInterval(async () => {
      ...
      const _elapsedMs = Date?.now() - _scriptStart;
      const _elapsedS = Math?.round(elapsedMs / 1000);
      _watchdogTick++;
      if (elapsedMs >= SCRIPT_HARD_KILL_MS) {
        ...
        const _shouldLog = _watchdogTick <= 2;
        if (!shouldLog) return;
    ...
    worker?.on("message", async (msg: Record<string, unknown>) => {
      if (msg?.type === "redis") {
        ...
        try {
          ...
          const _cmd = (msg?.cmd as string).toUpperCase();
          if (cmd === "HMSET") {
          ...
          const _short = (e?.message as string).slice(0, 200);
        ...
        const _buf = Buffer?.from(payload, "utf8");
        const _sab = msg?.sab as SharedArrayBuffer;
        const _ctrl = new Int32Array(sab, 0, 1);
        const _len = new Int32Array(sab, 4, 1);
        const _data = new Uint8Array(sab, 8);
        ...
        _watchdogCancelled = true;
        clearInterval(watchdog);
        const _errMsg = String(msg?.error ?? "");
        const _isPdim5xx =
