...
class HealthRegistry {
  ...
  async check(name: string): Promise<SubsystemHealth> {
    ...
    if (cached && Date.now() - cached.lastChecked < this.cacheTtlMs) return cached;
    const probe = this.probes.get(name);
    if (!probe) {
      return { name, status: 'unknown', lastChecked: Date.now(), detail: 'no probe registered' };
    }
  ...
  async checkAll(): Promise<{ status: HealthStatus; subsystems: SubsystemHealth[] }> {
    ...
    for (const r of results) {
      ...
      if (r.status === 'degraded' || r.status === 'unknown') status = 'degraded';
    }
...
export function registerCoreProbes(): void {
  ...
  logger.info('[Health] Core probes registered: database, redis, audit, automation');
}
