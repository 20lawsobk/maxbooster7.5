...
export function validateScaleConfig(): ValidationResult {
  ...
  if (!process.env.REDIS_URL && !process.env.REDIS_CLUSTER_URLS) {
    errors.push('[ScaleConfig] Neither REDIS_URL nor REDIS_CLUSTER_URLS is set. Redis is required in production.');
  }
  ...
  if (!process.env.WEBHOOK_SECRET) {
    errors.push('[ScaleConfig] WEBHOOK_SECRET is not set in production. Webhook signature verification will fail.');
  }

  // ── Admission control ─────────────────────────────────────────────────────
  const maxConcurrent = parseInt(process.env.MAX_CONCURRENT_REQUESTS ?? '5000', 10);
