...
export function cbRecordFailure(): void {
  ...
  if (nowInGrace && _state === 'CLOSED') {
    ...
    if (_failures === 5 || _failures === 10 || _failures % 20 === 0) {
      const phaseMsg = _firstSuccessAt > 0
      ...
      logger.warn(`[PDIM] ${_failures} startup failures suppressed — ${phaseMsg}`);
    }
    return;
  }
  ...
  if (_state === 'CLOSED' && _inSlowLane()) {
    _failures++;
    const slowRemaining = Math.ceil((POST_GRACE_SLOW_MS - (Date.now() - _capExpiredAt)) / 1000);
    ...
    logger.warn(`[PDIM] Slow-lane failure cap reached (${POST_GRACE_FAILURE_CAP}) — applying normal threshold`);
  }
