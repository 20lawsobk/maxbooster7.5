...
export function getRedisClient(): RedisClient {
  ...
  if (!_redis) {
    logger.info('[Redis] PDIM active — routing all Redis operations through PDIM');
...
export function newBullMQRedisConnection(): RedisClient {
  if (!isPdimConfigured()) {
    throw new Error('[Redis/BullMQ] PDIM is not configured — PDIM_HTTP_EXEC_URL must be set');
  }
