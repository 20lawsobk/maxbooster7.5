    ...
    if (existingContractTemplates?.length === 0) {
      const _defaultContractTemplates = [
        ...
        },
        {
          ...
          description:
            "Free promotional use license with attribution requirements",
    ...
    const _defaults = [
      {
        ...
        isActive: true,
        metadata: {
          description: "Triggers when API error rate exceeds 5% in 5 minutes",
        },
      },
      {
        ...
        isActive: true,
        metadata: {
          description: "Triggers when server memory usage exceeds 90%",
          unit: "percent",
        },
      },
      {
        name: "Database Latency High",
        condition: "db_latency > threshold",
        threshold: 500,
        severity: "warning",
        channels: ["email"],
        isActive: true,
        metadata: {
          description: "Triggers when database query latency exceeds 500ms",
          unit: "ms",
        },
      },
      {
        ...
        metadata: {
          description:
            "Triggers when failed login attempts exceed 10 in 15 minutes",
        },
      },
      {
        name: "Revenue Drop",
        condition: "revenue_change < threshold",
        threshold: -20,
        severity: "warning",
        channels: ["email"],
        isActive: true,
        metadata: {
          description:
            "Triggers when daily revenue drops more than 20% vs prior day",
          unit: "percent",
        },
      },
      {
        name: "Stream Count Spike",
        condition: "stream_count > threshold",
        threshold: 10000,
        severity: "info",
        channels: ["push"],
        isActive: true,
        metadata: {
          description: "Notification when daily streams exceed 10,000",
        },
      },
      {
        name: "Payout Processing Failed",
        condition: "payout_status == failed",
        threshold: 1,
        severity: "critical",
        channels: ["email", "push"],
        isActive: true,
        metadata: { description: "Triggers when any payout processing fails" },
      },
      {
        ...
        metadata: {
          description:
            "Triggers when rate limit is hit more than 100 times in an hour",
        },
      },
      {
      ...
      },
      {
        ...
        condition: "release_streams > threshold",
        threshold: 1000,
        severity: "info",
        channels: ["push", "email"],
        isActive: true,
        metadata: {
          description: "Notification when a release reaches 1,000 streams",
        },
      },
      {
        name: "Social Engagement Spike",
        ...
        isActive: true,
        metadata: {
          description: "Notification when social engagement rate exceeds 10%",
          unit: "percent",
        },
      },
      {
        name: "Distribution Status Change",
        condition: "release_status_changed",
        threshold: 1,
        severity: "info",
        channels: ["push", "email"],
        isActive: true,
        metadata: {
          description:
            "Notification when a release distribution status changes",
        },
      },
    ];
