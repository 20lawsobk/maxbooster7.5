...
async function proxyToAI(
  ...
  try {
    const _res = await fetch(`${AI_URL}${endpoint}`, {
      method,
...
router?.post("/start", requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const _result = await proxyToAI("/train/start", "POST", {
      mode,
      n_sessions,
      phase_id,
    });
...
router?.post("/stop", requireAdmin, async (_req: Request, res: Response) => {
  try {
    const _result = await proxyToAI("/train/stop", "POST");
    res?.status(result?.ok ? 200 : result?.status).json(result?.data);
  } catch (err) {
...
router?.get("/status", requireAdmin, async (_req: Request, res: Response) => {
  try {
    const _result = await proxyToAI("/train/status", "GET");
    res?.status(result?.ok ? 200 : result?.status).json(result?.data);
  } catch (err) {
...
router?.get("/datasets", requireAdmin, async (_req: Request, res: Response) => {
  try {
    const _result = await proxyToAI("/train/datasets", "GET");
    res?.status(result?.ok ? 200 : result?.status).json(result?.data);
  } catch (err) {
...
router?.get("/schedule", requireAdmin, async (_req: Request, res: Response) => {
  try {
    const _result = await proxyToAI("/train/schedule", "GET");
    res?.status(result?.ok ? 200 : result?.status).json(result?.data);
  } catch (err) {
...
const __INTERNAL_SECRET_HOOK = process?.env.BOOSTERSTATE_SECRET || "";
router?.post(
  "/internal/session-complete",
  async (req: Request, res: Response) => {
    const _auth = (req?.headers["authorization"] as string | undefined) ?? "";
    const _token = auth?.startsWith("Bearer ") ? auth?.slice(7) : "";
