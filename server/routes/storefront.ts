...
router.post("/subscribe/:tierId", async (req, res) => {
  try {
    ...
    let stripePriceId = (tier as Record<string, unknown>).stripePriceId as
      | string
      | null
      | undefined;
    if (!stripePriceId) {
...
router.get("/check-subdomain/:subdomain", async (req, res) => {
  try {
    ...
    const { subdomain } = req.params as Record<string, string>;
  ...
  try {
    ...
    try {
      ...
      if (
        cnames?.some(
          (c) =>
            c?.toLowerCase().includes("maxbooster") ||
...
router.get("/:id/social", async (req, res) => {
  try {
    ...
    const [likesResult] = await db
      .select({ count: count() })
      .from(storefrontLikes)
      .where(eq(storefrontLikes.storefrontId, storefrontId))
      .limit(1);
    ...
    const [ratingsResult] = await db
      .select({ count: count(), avg: avg(storefrontRatings.rating) })
    ...
    if (userId) {
      const [likeRow] = await db
        ...
        .where(
          and(
            eq(storefrontLikes.storefrontId, storefrontId),
            eq(storefrontLikes.userId, userId),
          ),
        )
        .limit(1);
      ...
      const [ratingRow] = await db
        ...
        .where(
          and(
            eq(storefrontRatings.storefrontId, storefrontId),
            eq(storefrontRatings.userId, userId),
          ),
        )
        .limit(1);
      userLiked = !!likeRow;
...
router.post("/:id/rate", async (req, res) => {
  try {
    ...
    if (existing) {
      await db
        .update(storefrontRatings)
        .set({ rating, review, updatedAt: new Date() })
        .where(eq(storefrontRatings.id, existing?.id));
    } else {
      await db
        .insert(storefrontRatings)
        .values({ userId, storefrontId, rating, review });
    }
  ...
  try {
    ...
    const cartItems = validListings?.map((listing) => {
      ...
      if (listing?.discountPercent && listing?.discountPriceCents != null) {
        if (
          ...
          new Date(listing?.discountExpiresAt) > new Date()
        ) {
    ...
    if (req.user?.id && activePromos?.length > 0) {
      ...
      for (const order of pastOrders) {
        if (order?.promoId) {
          customerRedemptions?.set(
            order?.promoId,
            (customerRedemptions?.get(order?.promoId) || 0) + 1,
          );
        }
      }
    }
...
router.post(
  "/:storefrontId/listings/:listingId/tier-audio",
...
router.post("/:storefrontId/bogo-promotions", async (req, res) => {
  try {
    ...
    if (
      ...
      (getDiscountPercent < 1 || getDiscountPercent > 100)
    ) {
      return res
        .status(400)
        .json({ error: "Discount percent must be between 1 and 100" });
    }
...
router.put("/:storefrontId/bogo-promotions/:promoId", async (req, res) => {
  try {
    if (!req.isAuthenticated())
      return res.status(401).json({ error: "Unauthorized" });
    const { storefrontId, promoId } = req.params as Record<string, string>;
    const [storefront] = await db
      .select()
      .from(storefronts)
      .where(eq(storefronts.id, storefrontId))
