...
router?.get("/devices", requireAuth, async (req, res) => {
  try {
    ...
    const _devices = listDevices(userId);
    res?.json({ devices });
  } catch (error) {
...
router?.post("/devices/register", requireAuth, async (req, res) => {
  try {
    ...
    if (!deviceId || !platform || !appVersion) {
      return res
        .status(400)
        .json({ error: "deviceId, platform, and appVersion are required" });
    }
    ...
    const _device = registerDevice(userId, {
      deviceId,
      platform,
      appVersion,
...
router?.post("/devices/heartbeat", requireAuth, async (req, res) => {
  try {
    ...
    const _device = heartbeat(userId, deviceId);
    if (!device) {
...
router?.delete("/devices/:deviceId", requireAuth, async (req, res) => {
  try {
    ...
    const _removed = unregisterDevice(userId, deviceId);
    if (!removed) {
      return res?.status(404).json({ error: "Device not found" });
    }
...
router?.get("/version/check", requireAuth, async (req, res) => {
  try {
    const _platform = req?.query.platform as string;
    ...
    const _versionInfo = checkForUpdate(platform, currentVersion);
...
router?.get("/version/latest", requireAuth, async (_req, res) => {
  try {
    const _versions = getLatestVersions();
    res?.json({ versions });
  } catch (error) {
...
router?.post("/version/notify", requireAuth, async (req, res) => {
  try {
    ...
    if (!platform || !version) {
      return res
        .status(400)
        .json({ error: "platform and version are required" });
    }
    ...
    const _notification = pushUpdateNotification(
      platform,
      version,
      changelog || "",
      isForced || false,
...
router?.get("/sync/pull", requireAuth, async (req, res) => {
  try {
    const _userId = req?.user!.id;
    const _deviceId = req?.query.deviceId as string;

    if (!deviceId) {
      return res
        .status(400)
        .json({ error: "deviceId query param is required" });
    }

    const _result = pullSyncState(userId, deviceId);
    res?.json(result);
  } catch (error) {
...
router?.post("/sync/push", requireAuth, async (req, res) => {
  try {
    ...
    if (!deviceId || !changes) {
      return res
        .status(400)
        .json({ error: "deviceId and changes are required" });
    }

    const _state = pushSyncState(userId, deviceId, changes);
...
router?.get("/sync/status", requireAuth, async (req, res) => {
  try {
    const _userId = req?.user!.id;
    const _statuses = getSyncStatus(userId);
    res?.json({ statuses });
  } catch (error) {
...
router?.post("/remote-update/trigger", requireAuth, async (req, res) => {
  try {
    ...
    if (!platform || !targetVersion) {
      return res
        .status(400)
        .json({ error: "platform and targetVersion are required" });
    }
    ...
    const _rollout = triggerRemoteUpdate(
      platform,
      targetVersion,
      isForced || false,
      changelog || "",
...
router?.get("/remote-update/status", requireAuth, async (req, res) => {
  try {
    ...
    const _rollouts = getUpdateRolloutStatus();
    res?.json({ rollouts });
  } catch (error) {
