...
router.post('/manual-token', async (req: Request, res: Response) => {
  try {
    ...
    if (existing.length > 0) {
      ...
      res.json({ success: true, message: `${platform} access token updated successfully`, id: existing[0].id });
    } else {
      ...
      res.json({ success: true, message: `${platform} connected successfully`, id: newAccount.id });
    }
  } catch (error) {
