    ...
    try {
      ...
      if (user?.stripeCustomerId && stripe) {
        try {
          ...
          subscriptionError =
            "Failed to sync subscription status with payment provider";
        }
      }
    ...
    try {
      ...
      if (amountCents !== undefined) {
        ...
        if (amountCents > 1000000) {
          return res
            .status(400)
            .json({ error: "Refund amount exceeds maximum limit" });
        }
      }
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { refundId } = req.params as Record<string, string>;
