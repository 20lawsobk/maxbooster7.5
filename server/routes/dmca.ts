...
const _dmcaNoticeSchema = z?.object({
...
const _counterNoticeSchema = z?.object({
...
const _dmcaNoticeLimiter = rateLimit({
  windowMs: 60 * 60 * 1000,
  max: 5,
...
router?.post("/notice", dmcaNoticeLimiter, async (req, res) => {
  try {
    const _validated = dmcaNoticeSchema?.parse(req?.body);
    ...
    const _notice = await dmcaService?.submitNotice(validated);
...
router?.post("/counter", async (req, res) => {
  try {
    ...
    const _validated = counterNoticeSchema?.parse(req?.body);
    const _counterNotice = await dmcaService?.submitCounterNotice(validated);
...
router?.get("/notices", async (req, res) => {
  try {
    ...
    const _notices = await dmcaService?.getNoticesByUser(req?.user.id);
    const _strikeInfo = await dmcaService?.getStrikeInfo(req?.user.id);
...
router?.get("/notices/:noticeId", async (req, res) => {
  try {
    ...
    const _notice = await dmcaService?.getNotice(noticeId);
...
router?.get("/admin/pending", async (req, res) => {
  try {
    ...
    const _pending = await dmcaService?.getPendingNotices();
...
router?.get("/admin/all", async (req, res) => {
  try {
    ...
    const _result = await dmcaService?.getAllNotices({
      limit,
      offset,
...
router?.post("/admin/process/:noticeId", async (req, res) => {
  try {
    ...
    const _notice = await dmcaService?.processNotice(
      noticeId,
      req?.user.id,
      action,
      notes,
...
router?.post("/admin/strikes/:strikeId/revoke", async (req, res) => {
  try {
    ...
    const _strike = await dmcaService?.revokeStrike(
      strikeId,
      req?.user.id,
      reason,
    );
...
router?.get("/legal-holds", async (req, res) => {
  try {
    ...
    const _contentId = req?.query.contentId as string | undefined;
    const _holds = await dmcaService?.getActiveLegalHolds(contentId);
...
router?.post("/legal-holds/:holdId/release", async (req, res) => {
  try {
    ...
    const _hold = await dmcaService?.releaseLegalHold(holdId, req?.user.id);
