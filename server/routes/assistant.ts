...
const _anonChatLimiter = rateLimit({
  windowMs: 60 * 1000,
  max: 10,
...
router?.get("/history", async (req: Request, res: Response) => {
  try {
    ...
    const _beforeId = req?.query.before as string | undefined;
    ...
    const _convId = conversation[0].id;
    ...
    const _total = Number(countResult[0]?.count ?? 0);
    ...
    if (beforeId) {
      ...
      const _cursorDate = cursorMsg[0].createdAt;
    ...
    const _hasMore = fetchedRows?.length > PAGE_SIZE;
    const _pageRows = hasMore ? fetchedRows?.slice(0, PAGE_SIZE) : fetchedRows;

    // Reverse so oldest-first order for display
    const _messages = pageRows?.reverse();
...
router?.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const _trimmed = message?.trim().slice(0, 2000);
      const _user = req?.user;
      ...
      const _aiResponse = generateMaxResponse(trimmed, history);
...
router?.delete("/history", async (req: Request, res: Response) => {
  try {
    ...
    if (conversations?.length > 0) {
      const _convIds = conversations?.map((c) => c?.id);
      await db
