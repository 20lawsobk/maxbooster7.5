...
router.put("/individual", async (req, res) => {
  try {
    ...
    const verification = await kycService.updateIndividualInfo(
      verificationId,
      validated,
      req.user.id,
    );
...
router.put("/business", async (req, res) => {
  try {
    ...
    const verification = await kycService.updateBusinessInfo(
      verificationId,
      validated,
      req.user.id,
    );
...
router.get("/documents", async (req, res) => {
  try {
    ...
    const documents = await kycService.getVerificationDocuments(
      verification.id,
    );
...
router.post("/submit", async (req, res) => {
  try {
    ...
    const verification = await kycService.submitForReview(
      verificationId,
      req.user.id,
    );
...
router.get("/payout-eligibility", async (req, res) => {
  try {
    ...
    const eligibility = await kycService.checkPayoutEligibility(
      req.user.id,
      amount,
    );
...
router.post("/upgrade", async (req, res) => {
  try {
    ...
    const verification = await kycService.upgradeVerificationLevel(
      verificationId,
      newLevel,
      req.user.id,
    );
