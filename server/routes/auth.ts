...
router.post('/extend-session', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.id;
    const sessionId = req.session?.id;
...
router.get('/sessions', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const formattedSessions = userSessions.map(session => {
      ...
      else if (userAgent.includes('Android')) { device = 'Android Device'; os = 'Android'; }
      else if (userAgent.includes('Windows')) { device = 'Windows PC'; os = 'Windows'; }
      else if (userAgent.includes('Macintosh')) { device = 'Mac'; os = 'macOS'; }
      else if (userAgent.includes('Linux')) { device = 'Linux PC'; os = 'Linux'; }
...
router.delete('/sessions/other', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.id;
    const currentSessionId = req.session?.id;

    const otherSessions = await db.select({ id: sessions.id })
      .from(sessions)
      .where(
        and(
          eq(sessions.userId, userId),
...
router.get('/session-status', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.id;
    const currentSessionId = req.session?.id;
...
router.get('/social-token-status', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.id;
    ...
    const platformStatus = accounts.map(account => {
      ...
      const expiresInSeconds = tokenExpiresAt 
        ? Math.floor((new Date(tokenExpiresAt).getTime() - now.getTime()) / 1000)
        : null;
      ...
      return {
        ...
        outcome: status === 'connected' ? 'token_valid' : 
                 status === 'expired' ? 'token_expired' : 
                 status === 'expiring_soon' ? 'token_expiring_soon' : 'needs_connection'
      };
    });
...
router.post('/social/:platform/refresh', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.id;
...
router.get('/security-alerts', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.id;
...
router.post('/security-alerts/:alertId/dismiss', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.id;
