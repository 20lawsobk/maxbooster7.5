...
function classifyAsset(asset: {
  ...
  if (
    ...
    return { platform: "windows", type: "installer" } as const;
  if (name?.endsWith(".exe"))
    return { platform: "windows", type: "portable" } as const;
  if (name?.endsWith(".msi"))
    return { platform: "windows", type: "installer" } as const;
  if (name?.endsWith(".dmg")) return { platform: "mac", type: "dmg" } as const;
  if (name?.endsWith(".zip") && name?.includes("mac"))
    return { platform: "mac", type: "zip" } as const;
  ...
  try {
    ...
    if (
      ...
      desktopDownloads?.push({
        ...
        fallbackUrl:
          fallback !== primary ? fallback!.browser_download_url : undefined,
    ...
    if (
      release?.platforms.linux?.appimage ||
      ...
      desktopDownloads?.push({
        ...
        extras: [
          ...
          ...(tarball
            ...
            : []),
          ...(release?.platforms.linux?.appimage &&
          primary !== release?.platforms.linux?.appimage
            ? [
                {
                  label: "AppImage",
