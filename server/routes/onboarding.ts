...
router.post('/complete-welcome', requireAuth, async (req, res) => {
  try {
    ...
    const { displayName, bio, avatarUrl, genres, interests, artistType } = req.body;
...
router.post('/track-tutorial', requireAuth, async (req, res) => {
  try {
    ...
    if (!tutorialId || !stepId) {
      return res.status(400).json({ error: 'tutorialId and stepId are required' });
    }
    ...
    logger.info(`Tutorial ${tutorialId} step ${stepId} tracked for user ${userId}`);
    ...
    await db
      ...
      .set({
        onboardingData: {
          ...
          remindAt: remindLater ? new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString() : null,
        },
      })
      .where(eq(users.id, userId));
  ...
  try {
    ...
    const stats = {
      ...
      byCategory: {
        ...
        collaboration: achievements.filter(a => a.category === 'collaboration'),
