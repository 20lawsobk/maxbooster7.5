...
import { PLATFORM_SPECS, ALL_PLATFORMS, type SupportedPlatform } from '../services/contentPipeline/platformFormatters.js';
...
function validateInput(body: unknown): { valid: boolean; error?: string; input?: UnifiedContentInput } {
  ...
  const requestedPlatforms = Array.isArray(b.platforms) ? b.platforms as string[] : undefined;
  if (requestedPlatforms) {
    const invalid = requestedPlatforms.filter(p => !validPlatforms.has(p as SupportedPlatform));
    if (invalid.length > 0) {
      return { valid: false, error: `Invalid platforms: ${invalid.join(', ')}. Supported: ${ALL_PLATFORMS.join(', ')}` };
    }
  }

  const inputType = (b.type as string) === 'boostsheet' ? 'boostsheet' : 'artist_context';
  const commonFields = {
    ...
    brandVoice: (b.brandVoice as ArtistContextInput['brandVoice']) ?? 'energetic',
    colorPalette: Array.isArray(b.colorPalette) ? b.colorPalette as string[] : undefined,
  ...
  const input: UnifiedContentInput = inputType === 'boostsheet'
    ? { ...commonFields, type: 'boostsheet', sheetId: b.sheetId as string | undefined } as BoostSheetInput
...
router.post('/artist-only', requireAuth, async (req: Request, res: Response) => {
  const { valid, error, input } = validateInput(req.body);
  if (!valid || !input) {
    res.status(400).json({ error });
    return;
...
router.post('/maxbooster-only', requireAuth, async (req: Request, res: Response) => {
  const { valid, error, input } = validateInput(req.body);
  if (!valid || !input) {
    res.status(400).json({ error });
    return;
...
router.post('/platform/:platform', requireAuth, async (req: Request, res: Response) => {
