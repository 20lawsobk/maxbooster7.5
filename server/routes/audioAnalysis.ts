    ...
    try {
      ...
      res?.json({
        ...
        recommendations: {
          spotify:
            estimatedLufs > -14
              ? "Track may be turned down by Spotify"
              : "Track will be normalized up by Spotify",
          appleMusic:
            estimatedLufs > -16
              ? "Track may be turned down by Apple Music"
              : "Track will be normalized up by Apple Music",
          youtube:
            estimatedLufs > -14
              ? "Track may be turned down by YouTube"
              : "Track will be normalized up by YouTube",
          truePeak:
            estimatedTruePeak > -1
              ? "Consider reducing peaks to avoid inter-sample clipping"
              : "True peak level is acceptable",
        },
      });
    } catch (error) {
...
router?.get("/loudness-targets", (_req: Request, res: Response) => {
  res?.json({
    ...
    recommendations: {
      ...
      dynamic:
        "Preserve dynamics - loudness normalization will bring quiet tracks up",
      truePeak:
        "Keep true peak below -1 dBTP to avoid inter-sample clipping on lossy codecs",
    },
  });
});
