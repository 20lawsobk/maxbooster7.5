...
router.post('/nps', requireAuth, async (req: any, res) => {
  try {
    ...
    if (!parsed.success) {
      return res.status(400).json({ error: 'Invalid NPS data', details: parsed.error.flatten() });
    }
    ...
    logger.info(`[Retention] NPS submitted: user=${userId} score=${score} category=${category}`);

    return res.json({ success: true, category });
  } catch (err) {
    logger.error('[Retention] NPS submission failed:', err);
...
router.post('/cancellation-feedback', requireAuth, async (req: any, res) => {
  try {
    ...
    logger.error('[Retention] Cancellation feedback failed:', err);
...
router.post('/feature-event', requireAuth, async (req: any, res) => {
  try {
    ...
    logger.error('[Retention] Feature event tracking failed:', err);
...
router.get('/health-score', requireAuth, async (req: any, res) => {
  try {
    ...
    logger.error('[Retention] Health score retrieval failed:', err);
...
router.get('/admin/at-risk', requireAuth, async (req: any, res) => {
  try {
    ...
    logger.error('[Retention] Admin at-risk query failed:', err);
