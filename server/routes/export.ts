...
interface ExportJob {
  ...
  format: string;
  status: 'queued' | 'preparing' | 'processing' | 'encoding' | 'uploading' | 'complete' | 'failed' | 'cancelled';
  progress: number;
  stage?: string;
  startTime?: Date;
...
router.post('/audio/:projectId', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const estimatedSeconds = options.exportType === 'stems' 
    ...
    logger.error('Error starting audio export:', error);
    res.status(500).json({ 
      success: false, 
...
router.post('/data', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error starting data export:', error);
    res.status(500).json({ 
      success: false, 
      error: 'Failed to start export' 
    });
...
router.delete('/history/:id', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error deleting export history item:', error);
...
router.post('/batch', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error starting batch export:', error);
...
const reportTypes = ['royalties', 'analytics', 'invoice', 'contract', 'tax'] as const;
...
router.post('/report/:type', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const estimatedSeconds = type === 'tax' ? 60 : type === 'contract' ? 45 : 30;
...
const chartExportSchema = z.object({
  chartType: z.enum(['streams', 'revenue', 'audience', 'geographic', 'demographics', 'comparison']),
...
router.post('/chart', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const job: ExportJob = {
      ...
      settings: {
        ...options,
        effectiveWidth: (options.dimensions?.width || 1200) * resolutionMultipliers[options.resolution],
        effectiveHeight: (options.dimensions?.height || 800) * resolutionMultipliers[options.resolution],
      },
    };
...
const masteredExportSchema = z.object({
  ...
  preset: z.enum(['streaming', 'cd', 'vinyl', 'broadcast', 'archival']).optional(),
  processing: z.object({
    ...
    ditherType: z.enum(['none', 'triangular', 'noise-shaped']).default('none'),
  }).optional(),
  quality: z.object({
...
router.post('/audio/:projectId/mastered', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const presetSettings: Record<string, Partial<typeof options.processing>> = {
      ...
      cd: { normalize: true, normalizeTarget: -0.3, dither: true, ditherType: 'noise-shaped' },
      vinyl: { normalize: true, normalizeTarget: -3, limiter: true, limiterCeiling: -0.5 },
      broadcast: { normalize: true, normalizeTarget: -23, limiter: true, limiterCeiling: -1 },
    ...
    logger.error('Error starting mastered export:', error);
...
const stemsExportSchema = z.object({
  ...
  namingConvention: z.enum(['track-name', 'numbered', 'custom']).default('track-name'),
  quality: z.object({
    sampleRate: z.number().default(48000),
    bitDepth: z.number().default(24),
  }).optional(),
...
router.post('/audio/:projectId/stems', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error starting stems export:', error);
