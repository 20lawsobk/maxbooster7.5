...
router.post('/analyze', (req, res) => {
  try {
    ...
    const hintedClass =
      ...
      /sql|injection|union.*select/i.test(textLower) ? 'sql_injection' :
      /xss|<script|javascript:/i.test(textLower) ? 'xss_attempt' :
      /\.\.\/|path.*travers/i.test(textLower) ? 'path_traversal' :
      null;
