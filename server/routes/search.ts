...
import { users, projects, beats, releases, studioProjects, storefronts, analytics, socialCampaigns, searchHistory, filterPresets } from '../../shared/schema.js';
  ...
  try {
    ...
    if (userId && q) {
      await db.insert(searchHistory).values({ userId, query: q, resultCount: totalCount })
        .catch((err: unknown) => logger.warn({ err: err }, 'Search history insert failed (non-critical):'));
    }
