...
function validatePreferences(preferences: Partial<AccessibilityPreferences>): { 
  ...
  if (preferences.reducedMotion !== undefined) {
    if (preferences.reducedMotion !== null && typeof preferences.reducedMotion !== 'boolean') {
  ...
  if (preferences.colorBlindMode !== undefined) {
    ...
    if (!validModes.includes(preferences.colorBlindMode)) {
      errors.push('colorBlindMode must be none, protanopia, deuteranopia, tritanopia, or achromatopsia');
    } else {
  ...
  if (preferences.focusIndicatorWidth !== undefined) {
    if (typeof preferences.focusIndicatorWidth !== 'number' || 
  ...
  if (preferences.keyboardNavigationEnabled !== undefined) {
    if (typeof preferences.keyboardNavigationEnabled !== 'boolean') {
      ...
      sanitized.keyboardNavigationEnabled = preferences.keyboardNavigationEnabled;
    }
  }
...
router.get('/accessibility-preferences', requireAuth, async (req: Request, res: Response) => {
  try {
...
router.put('/accessibility-preferences', requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
...
router.delete('/accessibility-preferences', requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
...
router.get('/accessibility-preferences/defaults', (_req: Request, res: Response) => {
