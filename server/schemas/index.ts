...
export const _paginationSchema = z?.object({
...
export const _uuidParamSchema = z?.object({
  id: z?.string().uuid("Invalid UUID format"),
});

export const _searchQuerySchema = z?.object({
  q: z
    .string()
...
export const _dateRangeSchema = z?.object({
...
export const _resetPasswordSchema = z
  .object({
...
export const _createCheckoutSessionSchema = z?.object({
  ...
  birthdate: z?.string().refine(
    (date) => {
      ...
      const _age = today?.getFullYear() - birthDate?.getFullYear();
      const _monthDiff = today?.getMonth() - birthDate?.getMonth();
      const _actualAge =
        monthDiff < 0 ||
...
export const _registerAfterPaymentSchema = z?.object({
...
export const _updateProfileSchema = z
  .object({
    firstName: z?.string().min(1).max(255).optional(),
    lastName: z?.string().min(1).max(255).optional(),
...
export const _updateUserPreferencesSchema = z
  .object({
...
export const _updateNotificationPreferencesSchema = z
  .object({
...
export const _ALLOWED_AUDIO_MIMES = [
  "audio/mpeg",
  "audio/mp3",
  "audio/wav",
  "audio/wave",
...
export const _ALLOWED_AUDIO_EXTENSIONS = [
  ".mp3",
  ".wav",
  ".ogg",
  ".aac",
...
export const _ALLOWED_IMAGE_MIMES = [
  "image/jpeg",
  "image/jpg",
  "image/png",
  "image/webp",
...
export const _ALLOWED_IMAGE_EXTENSIONS = [
  ".jpg",
  ".jpeg",
  ".png",
  ".webp",
...
export const _MAX_AUDIO_SIZE = 100 * 1024 * 1024; // 100MB
export const _MAX_IMAGE_SIZE = 10 * 1024 * 1024; // 10MB
export const _MAX_AVATAR_SIZE = 5 * 1024 * 1024; // 5MB

export const _fileUploadMetadataSchema = z?.object({
...
export const _audioUploadSchema = z?.object({
...
export const _avatarUploadSchema = z?.object({
...
export const _updateProjectSchema = z
  .object({
    title: z?.string().min(1).max(255).optional(),
    description: z?.string().max(2000).optional(),
    genre: z?.string().max(100).optional(),
...
export const _updateContentFlagSchema = z
  .object({
    status: z
...
export const _manualDeleteAccountSchema = z
  .object({
...
export const _schedulePostSchema = z?.object({
  platform: z?.enum([
    "facebook",
    "instagram",
    "twitter",
...
export const _generateContentSchema = z?.object({
  platform: z?.enum([
    "facebook",
    "instagram",
    "twitter",
...
export const _createListingSchema = z?.object({
  title: z?.string().min(1, "Title is required").max(255),
...
export const _purchaseBeatSchema = z?.object({
...
export const _createRoyaltySplitSchema = z
  .object({
  ...
  .refine(
    (data) => {
      const _totalPercentage = data?.splits.reduce(
...
export const _requestPayoutSchema = z?.object({
  amount: z
    .number()
...
>;
export type RegisterAfterPaymentInput = z?.infer<
  typeof registerAfterPaymentSchema
>;
