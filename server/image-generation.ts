...
export class SocialMediaContentGenerator {
  ...
  async generateContentFromURL(
    ...
    try {
      ...
      const _extractedData = await this?.extractContentFromURL(url);

      // Generate AI content based on extracted data
      const _aiContent = await this?.generateAIContentFromExtractedData(
        extractedData,
        platform,
        targetAudience,
      );
      ...
      const _result = await this?.generateSocialMediaContent(
        platform,
        extractedData,
        targetAudience,
        "all",
  ...
  async generateSocialMediaImage(
    ...
    try {
      ...
      const _svgContent = await this?.createSharpOptimizedDesign(
        platform,
        musicData,
        targetAudience,
        dimensions,
      ...
      const _buffer = await sharp(Buffer?.from(svgContent))
        .png({ quality: 90 })
        .toBuffer();

      // Save image
      const _filename = `social-${platform}-${Date?.now()}.png`;
      const _filepath = join(this?.imageDir, filename);
  ...
  private async createSharpOptimizedDesign(
    ...
    const { width, height } = dimensions;
    const _data = (musicData as Record<string, unknown>) || {};
    const _title = data?.title || data?.name || "New Release";
    const _artist = data?.artist || data?.artistName || "B-Lawz Music";

    // B-Lawz Music brand colors
    const _brandGold = "#FFD700";
    const _brandPurple = "#9B59B6";
    const _brandDark = "#1A1A2E";
  ...
  async generateSocialMediaVideo(
    ...
    try {
      const _dimensions = this?.getPlatformVideoDimensions(platform);
      const _data = (musicData as Record<string, unknown>) || {};
      const _title = data?.title || data?.name || "New Release";
      const _artist = data?.artist || data?.artistName || "B-Lawz Music";
      ...
      for (let i = 0; i < frameCount; i++) {
        ...
        const _frameBuffer = await sharp(Buffer?.from(frameSvg))
      ...
      const _gifFilename = `social-${platform}-${timestamp}.gif`;
      const _gifFilepath = join(this?.videoDir, gifFilename);

      const _combinedBuffer = await sharp(frames[0])
        .composite(
      ...
      const _webpFilename = `social-${platform}-${timestamp}.webp`;
      const _webpFilepath = join(this?.videoDir, webpFilename);
  ...
  async generateSocialMediaAudio(
    ...
    try {
      const _filename = `social-${platform}-${Date?.now()}.mp3`;
      const _filepath = join(this?.audioDir, filename);

      // AI-powered audio generation
      const _audioContent = await this?.createAIAudioContent(
        platform,
        musicData,
        targetAudience,
      );
  ...
  private async createAIAudioContent(
    ...
    try {
      const _genre = data?.genre || "pop";
      const _mood = data?.mood || "energetic";
      const _textPrompt = `${mood} ${genre} beat at ${data?.bpm || 120} bpm`;

      const _generationResult = await this?.audioGenerator.generateFromText({
        text: textPrompt,
        duration: audioConfig?.length,
        bars: Math?.ceil(audioConfig?.length / 2),
      });

      const _audioData = generationResult?.audioData;
      const _sampleRate = generationResult?.sampleRate;
  ...
  private float32ToWavBuffer(
    ...
    const _numSamples = audioData?.length;
    const _dataSize = numSamples * numChannels * (bitsPerSample / 8);
    const _fileSize = 44 + dataSize;

    const _buffer = Buffer?.alloc(fileSize);
    ...
    for (let i = 0; i < numSamples; i++) {
      const _sample = Math?.max(-1, Math?.min(1, audioData[i]));
      const _intSample = Math?.floor(sample * 32767);
  ...
  private generateFallbackAudio(durationSeconds: number): Buffer {
    ...
    const _numSamples = sampleRate * durationSeconds;
    const _dataSize = numSamples * 2;
    const _fileSize = 44 + dataSize;
  ...
  private async generateAIContentFromExtractedData(
    ...
    const _data = (extractedData as Record<string, unknown>) || {};
    const _audience = (targetAudience as Record<string, unknown>) || {};
    ...
    try {
      ...
      const _result = await this?.contentGenerator.generateCaption({
...
export const _contentGenerator = new SocialMediaContentGenerator();
