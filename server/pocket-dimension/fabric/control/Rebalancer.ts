...
export class Rebalancer {
  ...
  private readonly HIGH_WATERMARK = 0.80;
  ...
  async rebalance(): Promise<{ moved: number; errors: number }> {
    ...
    try {
      const { hot, cold } = await this.placement.findRebalanceCandidates(this.HIGH_WATERMARK);
      ...
      for (const hotNode of hot) {
        const overageBytes = hotNode.usedBytes - hotNode.capacityBytes * this.HIGH_WATERMARK;
        let migratedBytes = 0;

        const candidateCold = cold.filter(n =>
        ...
        logger.info(`[FabricRebalancer] Moving chunks from ${hotNode.id} (${(hotNode.usedBytes / hotNode.capacityBytes * 100).toFixed(1)}% full) to ${targetNode.id}`);
    ...
    logger.info(`[FabricRebalancer] Rebalance complete: moved=${moved} errors=${errors}`);
    return { moved, errors };
  }

  async migrateChunk(chunkId: ChunkId, fromNodeId: NodeId, toNodeId: NodeId): Promise<void> {
    ...
    if (loc) {
      const newNodeIds = [...loc.nodeIds.filter(id => id !== fromNodeId), toNodeId];
      await this.chunkIndex.putChunkLocation({ ...loc, nodeIds: newNodeIds });
    }

    await fromStore.deleteChunk(chunkId);
    await this.nodeRegistry.updateNode(fromNodeId, { usedBytes: Math.max(0, (await this.nodeRegistry.getNode(fromNodeId))!.usedBytes - (loc?.sizeBytes ?? 0)) });
    await this.nodeRegistry.updateNode(toNodeId, { usedBytes: (await this.nodeRegistry.getNode(toNodeId))!.usedBytes + (loc?.sizeBytes ?? 0) });
  }
}
