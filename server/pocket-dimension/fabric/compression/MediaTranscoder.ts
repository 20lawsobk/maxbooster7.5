...
const VIDEO_MIMES = new Set(['video/mp4', 'video/webm', 'video/avi', 'video/mov', 'video/quicktime', 'video/x-matroska']);
const AUDIO_MIMES = new Set(['audio/mpeg', 'audio/mp3', 'audio/ogg', 'audio/wav', 'audio/flac', 'audio/aac', 'audio/opus']);
const IMAGE_MIMES = new Set(['image/jpeg', 'image/png', 'image/gif', 'image/tiff', 'image/bmp', 'image/webp']);

export function classifyContentType(contentType: string, name: string): ContentClass {
  const ct = contentType.toLowerCase();
  const ext = path.extname(name).toLowerCase();

  if (VIDEO_MIMES.has(ct) || ['.mp4', '.webm', '.avi', '.mov', '.mkv'].includes(ext)) return 'video';
  if (AUDIO_MIMES.has(ct) || ['.mp3', '.ogg', '.wav', '.flac', '.aac', '.opus'].includes(ext)) return 'audio';
  if (IMAGE_MIMES.has(ct) || ['.jpg', '.jpeg', '.png', '.gif', '.tiff', '.webp', '.avif'].includes(ext)) return 'image';
  if (ct === 'application/json' || ext === '.json') return 'json';
  if (ct.startsWith('text/') || ['.txt', '.md', '.csv', '.xml', '.yaml', '.yml'].includes(ext)) return 'text';
  if (['.log', '.out', '.err'].includes(ext) || name.includes('.log')) return 'log';
...
export class MediaTranscoder {
  ...
  async transcodeVideo(data: Buffer, inputExt = '.mp4'): Promise<TranscodeResult> {
  ...
  async transcodeAudio(data: Buffer, inputExt = '.wav'): Promise<TranscodeResult> {
    await this.checkFfmpeg();
    const tmp = path.join(os.tmpdir(), randomBytes(8).toString('hex'));
    const inFile = `${tmp}${inputExt}`;
  ...
  async transcode(data: Buffer, contentClass: ContentClass, originalName: string): Promise<TranscodeResult | null> {
    const ext = path.extname(originalName).toLowerCase() || this.classToExt(contentClass);
