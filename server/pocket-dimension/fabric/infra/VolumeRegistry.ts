...
export class VolumeRegistry {
  async createVolume(pocketId: PocketId, name: string, type: VolumeType): Promise<FabricVolume> {
    const id = randomUUID();
    const now = new Date();
    await db.insert(fabricVolumes).values({ id, pocketId, name, type, createdAt: now });
    return { id, pocketId, name, type, createdAt: now };
  }

  async getVolume(id: VolumeId): Promise<FabricVolume | null> {
    const rows = await db.select().from(fabricVolumes).where(eq(fabricVolumes.id, id));
    return rows[0] ? this.rowToVolume(rows[0]) : null;
  }

  async listVolumes(pocketId: PocketId): Promise<FabricVolume[]> {
    const rows = await db.select().from(fabricVolumes).where(eq(fabricVolumes.pocketId, pocketId));
