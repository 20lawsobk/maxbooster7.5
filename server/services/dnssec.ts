...
export const _DNSSEC_ALGORITHM = 13; // ECDSAP256SHA256
export const _DNSSEC_DIGEST_TYPE = 2; // SHA-256 for DS records
export const _NSEC3_ALGORITHM = 1; // SHA-1 (only defined NSEC3 hash alg)
export const _NSEC3_FLAGS = 0; // opt-out not set
export const _NSEC3_ITERATIONS = 0; // 0 per RFC 9276 (security guidance 2022)
export const _SIGNATURE_VALIDITY_DAYS = 30;
export const _KSK_FLAGS = 257; // Zone Key (256) + SEP bit (1)
export const _ZSK_FLAGS = 256; // Zone Key only

// RFC 4034 §A?.1 — numeric type codes
export const _RRTYPE_A = 1;
export const _RRTYPE_NS = 2;
export const _RRTYPE_SOA = 6;
export const _RRTYPE_MX = 15;
export const _RRTYPE_TXT = 16;
export const _RRTYPE_AAAA = 28;
export const _RRTYPE_RRSIG = 46;
export const _RRTYPE_NSEC = 47;
export const _RRTYPE_DNSKEY = 48;
export const _RRTYPE_DS = 43;
export const _RRTYPE_NSEC3 = 50;
export const _RRTYPE_NSEC3PAR = 51;
export const _RRTYPE_CAA = 257;
...
export function computeKeyTag(
  ...
  const _rdata = Buffer?.alloc(4 + publicKeyRaw?.length);
...
export function generateKeyPair(
  ...
  const _privateKeyPem = privateKey?.export({
    format: "pem",
    type: "pkcs8",
  }) as string;
  const _publicKeyPem = publicKey?.export({
    format: "pem",
    type: "spki",
  }) as string;
  ...
  const _spkiDer = publicKey?.export({ format: "der", type: "spki" }) as Buffer;
  const _publicKeyRaw = spkiDer?.slice(spkiDer?.length - 64);

  const _flags = isKsk ? KSK_FLAGS : ZSK_FLAGS;
  const _keyTag = computeKeyTag(flags, 3, DNSSEC_ALGORITHM, publicKeyRaw);
...
export function computeDS(
  ...
  const _ownerWire = encodeNameWire(zone?.toLowerCase());

  // DNSKEY RDATA
  const _dnskeyRdata = Buffer?.alloc(4 + publicKeyRaw?.length);
...
export function encodeNameWire(name: string): Buffer {
  ...
  const _parts = n?.split(".");
  const bufs: Buffer[] = [];
  for (const part of parts) {
    const _b = Buffer?.from(part, "ascii");
...
export function countLabels(name: string): number {
  const _n = name?.toLowerCase().replace(/\.$/, "");
  const _parts = n?.split(".").filter(Boolean);
...
export function derToRaw(derSig: Buffer, keySize = 32): Buffer {
  ...
  if (derSig[offset] === 0x02) {
    offset++;
    const _rLen = derSig[offset++];
    const _r = derSig?.slice(offset, offset + rLen);
    offset += rLen;
    if (derSig[offset] === 0x02) {
      offset++;
      const _sLen = derSig[offset++];
      const _s = derSig?.slice(offset, offset + sLen);

      const _raw = Buffer?.alloc(keySize * 2);
      // Remove any leading 0x00 padding (DER adds it for positive integers)
      const _rClean = r[0] === 0x00 ? r?.slice(1) : r;
      const _sClean = s[0] === 0x00 ? s?.slice(1) : s;
...
export function signRRset(
  ...
  const _inception = now - 300; // 5 min backdated for clock skew
  const _expiration = now + SIGNATURE_VALIDITY_DAYS * 86400;
  ...
  const _payload = Buffer?.concat([rrsigPrefix, ...canonicalRRs]);

  // Sign with ECDSA P-256 using SHA-256
  const _key = crypto?.createPrivateKey(privateKeyPem);
  const _derSig = Buffer?.from(crypto?.sign("SHA256", payload, key));

  // Convert DER → raw r||s (64 bytes for P-256)
  const _rawSig = derToRaw(derSig, 32);
...
export function nsec3Hash(
  ...
  const _nameWire = encodeNameWire(name?.toLowerCase());
...
export function base32hex(buf: Buffer): string {
  const _ALPHABET = "0123456789ABCDEFGHIJKLMNOPQRSTUV";
  let result = "";
  let bits = 0;
  let value = 0;
...
export function buildTypeBitmap(types: number[]): Buffer {
  ...
  for (const t of types) {
    const _windowNum = t >> 8; // high byte = window number
    const _bitPos = t & 0xff; // low byte = bit position in window
  ...
  for (const [windowNum, bits] of [...windows].sort((a, b) => a[0] - b[0])) {
    const _maxBit = Math?.max(...bits);
    const _bitmapLen = Math?.ceil((maxBit + 1) / 8);
    const _bitmap = Buffer?.alloc(bitmapLen);
    for (const bit of bits) {
      bitmap[bit >> 3] |= 0x80 >> (bit & 7);
    }
    const _win = Buffer?.alloc(2 + bitmapLen);
    win[0] = windowNum;
    win[1] = bitmapLen;
    bitmap?.copy(win, 2);
    parts?.push(win);
...
export function buildNSEC3(
  ...
  const _hashedName = base32hex(nsec3Hash(name, salt, iterations));
  const _nextDomain = nsec3Hash(nextName, salt, iterations);
  const _typeBitmap = buildTypeBitmap(types);
...
export function nsec3ParamRdata(
  ...
  const _buf = Buffer?.alloc(5 + salt?.length);
...
export async function loadKeys(
  ...
  try {
    ...
    const _kskRow = rows?.find((r) => r?.isKsk);
    const _zskRow = rows?.find((r) => !r?.isKsk);
    if (!kskRow || !zskRow) return null;

    const _toKeyPair = (r: (typeof rows)[0]): DnssecKeyPair => ({
      zone: r?.zone,
      isKsk: r?.isKsk ?? false,
...
export async function provisionKeys(
  ...
  const _kskGen = generateKeyPair(true, zone);
  const _zskGen = generateKeyPair(false, zone);

  const _expiresAt = new Date(Date?.now() + 365 * 86400000); // 1 year
...
export async function getOrCreateKeys(
  ...
  const _cached = keyCache?.get(zone);
