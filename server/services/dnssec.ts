 ...
 */
export function computeKeyTag(flags: number, protocol: number, algorithm: number, publicKeyRaw: Buffer): number {
 ...
 */
export function generateKeyPair(isKsk: boolean, zone: string): {
  privateKeyPem: string;
  publicKeyPem:  string;
  publicKeyRaw:  Buffer;
  flags:         number;
 ...
 * RFC 4034 §5.1.4
 */
export function computeDS(zone: string, flags: number, protocol: number, algorithm: number, publicKeyRaw: Buffer, keyTag: number): DsRecord {
 ...
 */
function canonicalRR(ownerWire: Buffer, type: number, classIN: number, ttl: number, rdata: Buffer): Buffer {
...
export function signRRset(
  ...
  let offset = 0;
  rrsigPrefix.writeUInt16BE(typeNum, offset);     offset += 2;
  rrsigPrefix.writeUInt8(DNSSEC_ALGORITHM, offset); offset += 1;
  rrsigPrefix.writeUInt8(labels, offset);         offset += 1;
  rrsigPrefix.writeUInt32BE(ttl, offset);         offset += 4;
  rrsigPrefix.writeUInt32BE(expiration, offset);  offset += 4;
  rrsigPrefix.writeUInt32BE(inception, offset);   offset += 4;
  rrsigPrefix.writeUInt16BE(keyTag, offset);      offset += 2;
  ...
  const derSig = Buffer.from(
    crypto.sign('SHA256', payload, key)
  );
 ...
 */
export function nsec3ParamRdata(salt: Buffer, iterations = NSEC3_ITERATIONS): Buffer {
...
export function nsec3Rdata(
  ...
  const buf = Buffer.alloc(5 + salt.length + 1 + nextDomain.length + typeBitmap.length);
  let off = 0;
  buf[off++] = NSEC3_ALGORITHM;
  buf[off++] = NSEC3_FLAGS;
  buf.writeUInt16BE(iterations, off); off += 2;
  buf[off++] = salt.length;
  salt.copy(buf, off); off += salt.length;
  buf[off++] = nextDomain.length;
  nextDomain.copy(buf, off); off += nextDomain.length;
...
export async function loadKeys(zone: string): Promise<{ ksk: DnssecKeyPair; zsk: DnssecKeyPair } | null> {
  try {
    const rows = await db
      .select()
      .from(dnssecKeys)
...
export async function provisionKeys(zone: string): Promise<{ ksk: DnssecKeyPair; zsk: DnssecKeyPair }> {
  ...
  const toInsert = (gen: ReturnType<typeof generateKeyPair>, isKsk: boolean) => ({
    zone,
    isKsk,
    algorithm:     DNSSEC_ALGORITHM,
    flags:         gen.flags,
  ...
  await db.insert(dnssecKeys).values([
    toInsert(kskGen, true),
    toInsert(zskGen, false),
  ]);

  logger.info({ zone, kskTag: kskGen.keyTag, zskTag: zskGen.keyTag }, '[DNSSEC] Keys provisioned');

  const makeKP = (gen: ReturnType<typeof generateKeyPair>, isKsk: boolean): DnssecKeyPair => ({
    zone, isKsk, algorithm: DNSSEC_ALGORITHM, flags: gen.flags,
    keyTag: gen.keyTag, privateKeyPem: gen.privateKeyPem,
    publicKeyPem: gen.publicKeyPem, publicKeyRaw: gen.publicKeyRaw, expiresAt,
  });
...
const keyCache = new Map<string, { keys: { ksk: DnssecKeyPair; zsk: DnssecKeyPair }; cachedAt: number }>();

export async function getOrCreateKeys(zone: string): Promise<{ ksk: DnssecKeyPair; zsk: DnssecKeyPair } | null> {
...
export function makeDS(kp: DnssecKeyPair): DsRecord {
  return computeDS(kp.zone, kp.flags, 3, kp.algorithm, kp.publicKeyRaw, kp.keyTag);
}
