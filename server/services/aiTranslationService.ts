...
const LANGUAGE_DATA: Record<string, {
  ...
  en: {
    name: 'English',
    nativeName: 'English',
    culturalNotes: ['Use contractions for casual tone', 'Abbreviations widely understood'],
    commonPhrases: {},
...
const MUSIC_TERMS: Record<string, Record<string, string>> = {
  beat: { es: 'ritmo', fr: 'rythme', de: 'Beat', pt: 'batida', it: 'ritmo', ja: 'ビート', ko: '비트', zh: '节拍', ar: 'إيقاع', hi: 'बीट' },
  track: { es: 'canción', fr: 'morceau', de: 'Track', pt: 'faixa', it: 'traccia', ja: '曲', ko: '트랙', zh: '曲目', ar: 'أغنية', hi: 'ट्रैक' },
  album: { es: 'álbum', fr: 'album', de: 'Album', pt: 'álbum', it: 'album', ja: 'アルバム', ko: '앨범', zh: '专辑', ar: 'ألبوم', hi: 'एल्बम' },
  lyrics: { es: 'letras', fr: 'paroles', de: 'Texte', pt: 'letras', it: 'testi', ja: '歌詞', ko: '가사', zh: '歌词', ar: 'كلمات', hi: 'गाने के बोल' },
  vibe: { es: 'vibra', fr: 'ambiance', de: 'Stimmung', pt: 'vibe', it: 'atmosfera', ja: '雰囲気', ko: '분위기', zh: '氛围', ar: 'أجواء', hi: 'वाइब' },
  drop: { es: 'lanzamiento', fr: 'sortie', de: 'Release', pt: 'lançamento', it: 'uscita', ja: 'リリース', ko: '발매', zh: '发行', ar: 'إصدار', hi: 'रिलीज' },
  playlist: { es: 'playlist', fr: 'playlist', de: 'Playlist', pt: 'playlist', it: 'playlist', ja: 'プレイリスト', ko: '플레이리스트', zh: '播放列表', ar: 'قائمة تشغيل', hi: 'प्लेलिस्ट' },
  stream: { es: 'escuchar', fr: 'écouter', de: 'streamen', pt: 'ouvir', it: 'ascoltare', ja: 'ストリーミング', ko: '스트리밍', zh: '收听', ar: 'بث', hi: 'स्ट्रीम' },
};
...
class AITranslationService {
  ...
  private async translateToLanguage(
    ...
    translatedContent = this.applyMusicTermTranslations(translatedContent, targetLang);
    ...
    if (headline) {
      ...
      translatedHeadline = this.applyMusicTermTranslations(translatedHeadline, targetLang);
    }

    const translatedHashtags = this.translateHashtags(hashtags, targetLang, langData.hashtagStyle);

    translatedContent = this.addCulturalAdaptations(translatedContent, langData);
    ...
    return {
      ...
      confidence: this.calculateConfidence(content, translatedContent, targetLang),
    };
  }
  ...
  private applyPhraseTranslations(text: string, targetLang: string): string {
    ...
    let result = text;
    for (const [english, translated] of Object.entries(langData.commonPhrases)) {
  ...
  private adaptTone(content: string, langData: typeof LANGUAGE_DATA[string]): string {
    let adapted = content;
  ...
  private translateHashtagContent(tag: string, targetLang: string): string {
    ...
    for (const [english, translations] of Object.entries(MUSIC_TERMS)) {
      if (tag.toLowerCase() === english.toLowerCase() && translations[targetLang]) {
  ...
  private addCulturalAdaptations(content: string, langData: typeof LANGUAGE_DATA[string]): string {
    let adapted = content;

    if (langData.nativeName === 'Español') {
      if (adapted.includes('!') && !adapted.includes('¡')) {
        adapted = adapted.replace(/([A-Za-z])([^!]*!)/g, (match, first, rest) => {
          if (rest.length < 50) {
            return `¡${first}${rest}`;
          }
          return match;
  ...
  private calculateConfidence(original: string, translated: string, targetLang: string): number {
    let confidence = 70;
    ...
    const termsTranslated = Object.keys(MUSIC_TERMS).filter(
      term => original.toLowerCase().includes(term.toLowerCase())
    ).length;
  ...
  private createFallbackTranslation(
    ...
    return {
      ...
      headline: headline,
      hashtags: hashtags,
      culturalNotes: ['Fallback to original content - manual translation recommended'],
      confidence: 30,
    };
  }
