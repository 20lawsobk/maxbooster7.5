...
def train(n_samples:  int   = 600,
    ...
    for epoch in range(n_epochs):
        ...
        for frame_raw, prompt, scene in dataset:
            ...
            # ema.update disabled: f16 Adam state leaves no room for EMA (8 GB cgroup)
    ...
    _final_loss_v3 = losses[-1] if losses else 0.0
    print(f"[DiffusionTrainer v3] Done in {total_time/60:.1f}min  "
          f"final_loss={_final_loss_v3:.4f}", flush=True)
    ...
    meta = {
        ...
        'epochs':         n_epochs,
        'samples':        n_samples,
        'final_loss':     float(_final_loss_v3),
...
def train_v4(n_epochs: int = 5,
    ...
    # ── Music industry scenario engine ────────────────────────────────────────
    try:
        ...
        print(
            "[DiffusionTrainer v4] MusicScenarioEngine online — "
            f"seeds={scenario_eng.active_seed_count()}  "
            f"total_fired={scenario_eng.status()['total_scenarios_fired']}  "
            f"events={scenario_eng.status()['total_events']}",
            flush=True,
        )
    except Exception as _se_err:
        scenario_eng     = None
        _SCENARIO_INJECT_N = 5
        print(f"[DiffusionTrainer v4] MusicScenarioEngine unavailable: {_se_err}",
              flush=True)
    ...
    for epoch in range(n_epochs):
        ...
        for i, (scene, prompt) in enumerate(sample_pairs):
            _current_spec = None  # reset each step (populated by scenario engine)
            try:
                # ── Music industry scenario injection ──────────────────────────
                # Every _SCENARIO_INJECT_N steps: replace (scene, prompt) with a
                # targeted scenario generated by the compounding scenario engine.
                # The scenario is picked based on job-family exposure balance and
                # gradient health — directing training toward the weakest regions.
                if scenario_eng is not None and step_count % _SCENARIO_INJECT_N == 0:
                    try:
                        _gh   = (adv_mem.gradients.scene_grad_health()
                                 if adv_mem is not None else {})
                        _spec = scenario_eng.roll_scenario(
                            scene_hint=scene, gradient_health=_gh)
                        if _spec is not None:
                            _current_spec = _spec
                            scene         = _spec.scene_category
                            prompt        = _spec.scene_prompt
                            if sim is not None:
                                sim.add_scenario_steps(
                                    1, compound_depth=_spec.compound_depth)
                    except Exception:
                        pass
                ...
                if adv_mem is not None:
                    try:
                        _smeta = (
                            {
                                'job_family':    _current_spec.job_family,
                                'event_type':    _current_spec.event_type,
                                'compound_depth':_current_spec.compound_depth,
                                'chain_id':      _current_spec.chain_id,
                                'ye_weight':     _current_spec.ye_weight,
                            }
                            if _current_spec is not None else None
                        )
                        adv_mem.record(
                            scene=scene,
                            prompt=prompt,
                            ...
                            scenario_meta=_smeta,
                        )
                    except Exception:
                        pass

                # ── Plant scenario seeds after successful step ──────────────
                # The consequence seeds from the just-fired scenario are stored
                # so that subsequent rolls can compound the current chain arc.
                if scenario_eng is not None and _current_spec is not None:
                    try:
                        scenario_eng.plant_seeds(_current_spec, step_count)
                    except Exception:
                        pass
                    ...
                    try:
                        ...
                        if partner is not None:
                            ...
                            for interp_frame, interp_prompt in interp_pairs:
                                ...
                                opt.step(all_pairs)
                                # ema.update disabled: 8 GB cgroup
                    except Exception:
                        pass
    ...
    # ── Scenario engine session close ─────────────────────────────────────────
    if scenario_eng is not None:
        try:
            scenario_eng.save()
            _se_st = scenario_eng.status()
            print(
                f"[DiffusionTrainer v4] ScenarioEngine — "
                f"fired={_se_st['total_scenarios_fired']}  "
                f"chains_active={_se_st['active_chains']}  "
                f"avg_depth={_se_st['avg_compound_depth']:.2f}  "
                f"top_job={list(_se_st['top_job_families'].keys())[:1]}",
                flush=True,
            )
        except Exception:
            pass
    ...
    print(f"[DiffusionTrainer v4] Complete. Total time: {total_time:.0f}s  "
          f"Final loss: {losses[-1]:.4f}" if losses else
          f"[DiffusionTrainer v4] Complete. Total time: {total_time:.0f}s  "
          f"Final loss: N/A (no successful steps)", flush=True)
    return meta
