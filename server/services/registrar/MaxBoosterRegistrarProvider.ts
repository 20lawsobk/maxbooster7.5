...
import type {
  RegistrarProvider, AvailabilityResult,
  RegisterParams, RegisterResult,
  RenewResult, DomainInfo,
...
export function buildRdapResponse(row: Record<string, unknown>, privacyRedact = true): RdapDomain {
  ...
  if (row.createdAt) events.push({ eventAction: 'registration', eventDate: new Date(row.createdAt).toISOString() });
  if (row.updatedAt) events.push({ eventAction: 'last changed',  eventDate: new Date(row.updatedAt).toISOString() });
  if (row.expiresAt) events.push({ eventAction: 'expiration',    eventDate: new Date(row.expiresAt).toISOString() });
  ...
  const registrant = privacyRedact && row.privacyEnabled
    ? {
        ...
        remarks: [{ title: 'REDACTED FOR PRIVACY', description: ['Contact privacy enabled via Max Booster registrar.'] }],
      }
    : {
        objectClassName: 'entity',
        roles: ['registrant'],
  ...
  return {
    ...
    nameservers: ns.map((n: string) => ({ objectClassName: 'nameserver', ldhName: n.toUpperCase() })),
    status,
    events,
    entities: [
      registrant,
...
export class MaxBoosterRegistrarProvider implements RegistrarProvider {
  ...
  async renewDomain(fqdn: string, years: number): Promise<RenewResult> {
    ...
    logger.info({ fqdn: domain, years, expiresAt, provider: this.name }, '[MaxBoosterRegistrar] Domain renewed');
  ...
  async setNameservers(fqdn: string, nameservers: string[]): Promise<void> {
    ...
    try {
      const { rows } = await pool.query('SELECT id FROM dns_zones WHERE domain = $1 LIMIT 1', [domain]);
      if (rows.length) {
        await pool.query(`
    ...
    logger.info({ fqdn: domain, ns1, ns2, provider: this.name }, '[MaxBoosterRegistrar] Nameservers updated');
  }
  ...
  async getDomainInfo(fqdn: string): Promise<DomainInfo> {
    ...
    return {
      ...
      nameservers: [row.nameserver1 ?? NS1, row.nameserver2 ?? NS2, NS3].filter(Boolean),
  ...
  async getRdapResponse(fqdn: string, isPublic = true): Promise<RdapDomain | null> {
    const domain = fqdn.toLowerCase();
    try {
      const { rows } = await pool.query(
        'SELECT * FROM claimed_domains WHERE domain = $1 LIMIT 1', [domain]
      );
  ...
  private async _ensureDnsZone(
    ...
    try {
      const existing = await pool.query('SELECT id FROM dns_zones WHERE domain = $1 LIMIT 1', [domain]);
