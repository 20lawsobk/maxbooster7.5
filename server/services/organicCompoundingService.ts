...
class OrganicCompoundingService {
    ...
    try {
      ...
      if (candidates?.length < 5) {
        ...
        const defaultTypes = [
          ...
          {
            type: "instagram_reel",
            topic: "New Music Preview — Hook Reveal",
            intent: "emotional",
            hours: 2,
          },
          {
            type: "youtube_short",
            topic: "Artist Reaction / Production Breakdown",
            intent: "discovery",
            hours: 2,
          },
          {
            type: "playlist_pitch",
            topic: "Independent Playlist Curator Outreach (10 curators)",
            intent: "search",
            hours: 3,
          },
          {
            type: "press_release",
            topic: "New Release Press Release for Music Blogs",
            intent: "search",
            hours: 4,
          },
          {
            type: "seo_article",
            topic:
              "Artist Bio & Story — SEO-Optimized for Google Music Searches",
            intent: "search",
            hours: 5,
          },
          {
            type: "ugc_challenge",
            topic: "Fan Use-My-Sound TikTok Challenge",
            intent: "emotional",
            hours: 2,
          },
          {
            type: "spotify_canvas",
            topic: "Spotify Canvas Loop for Latest Single",
            intent: "discovery",
            hours: 1,
          },
          {
            type: "lyric_video",
            topic: "Official Lyric Video — YouTube + Facebook Native Upload",
            intent: "discovery",
            hours: 6,
          },
          {
            type: "editorial_pitch",
            topic: "Apple Music / Spotify Editorial Pitch for Upcoming Release",
            intent: "search",
            hours: 2,
          },
        ];
  ...
  async createAsset(
    userId: string,
    data: Omit<InsertOrganicAsset, "userId">,
  ): Promise<OrganicAsset> {
    try {
      const [asset] = await db
        .insert(organicAssets)
        .values({
          ...data,
          userId,
        })
  ...
  ): Promise<OrganicAsset | null> {
    try {
      const [updated] = await db
        .update(organicAssets)
        .set({
          performance,
          updatedAt: new Date(),
        })
  ...
  async createChannel(
    userId: string,
    data: Omit<InsertOrganicChannel, "userId">,
  ): Promise<OrganicChannel> {
    try {
      const [channel] = await db
        .insert(organicChannels)
        .values({
          ...data,
          userId,
        })
  ...
  async updateChannelEfficiency(
    channelId: string,
    efficiencyScore: number,
  ): Promise<OrganicChannel | null> {
    try {
      const [updated] = await db
        .update(organicChannels)
        .set({
          efficiencyScore,
          updatedAt: new Date(),
        })
  ...
  async saveRoiSnapshot(
    userId: string,
    assetId: string,
    roiData: RoiData,
  ): Promise<OrganicRoiSnapshot> {
    try {
      const [snapshot] = await db
        .insert(organicRoiSnapshots)
        .values({
          userId,
          assetId,
