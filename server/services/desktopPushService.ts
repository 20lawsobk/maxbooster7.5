...
class DesktopPushService {
  ...
  private initialize() {
    ...
    if (!publicKey || !privateKey) {
      logger.warn('🖥️ Desktop Push Service: VAPID keys not set — desktop push unavailable');
      return;
    }
  ...
  private async getAllUserSubscriptions(userId: string): Promise<SubscriptionRecord[]> {
    try {
      return await db
        .select()
  ...
  async getMobileWebSubscriptions(userId: string): Promise<SubscriptionRecord[]> {
    const all = await this.getAllUserSubscriptions(userId);
  ...
  async sendToDesktop(userId: string, payload: DesktopPushPayload): Promise<{ sent: number; failed: number }> {
    ...
    if (result.sent > 0) {
      logger.info(`🖥️ Desktop push sent to ${result.sent}/${subs.length} browser(s) for user ${userId}`);
    }

    return result;
  }

  async sendRichToDesktop(userId: string, richPayload: RichPushPayload): Promise<{ sent: number; failed: number }> {
