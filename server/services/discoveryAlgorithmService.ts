...
export class DiscoveryAlgorithmService {
    ...
    try {
      ...
      return paginatedResults?.map(({ beat, discoveryScore }) => {
        ...
        return {
          ...
          producer: (beat as any).producerName || "Producer",
          producerId: (beat as any).userId,
          price: ((beat as any)?.priceCents || 0) / 100,
          currency: (beat as any).currency || "usd",
          ...
          isNew: (beat as any).createdAt && (beat as any)?.createdAt >= oneWeekAgo,
          ...
          licenseOptions: metadata.licenses || [
            ...
            {
              type: "premium",
              price: (((beat as any)?.priceCents || 0) / 100) * 2,
              name: "Premium License",
            },
            {
              type: "exclusive",
              price: (((beat as any)?.priceCents || 0) / 100) * 10,
  ...
  private diversifyResults<
    T extends {
      beat: { metadata: Record<string, unknown>; category?: string | null };
  ...
  private async syncStorefrontFollow(
    userId: string,
    producerId: string,
    follow: boolean,
  ) {
