...
class AutopilotPublisher {
  ...
  private startAttemptMapCleanup(): void {
    ...
    setInterval(
      () => {
        const _cutoff = Date?.now() - AutopilotPublisher?.ATTEMPT_TTL_MS;
        ...
        if (
          this?.lastPublishAttempt.size > AutopilotPublisher?.MAX_TRACKED_USERS
        ) {
          const _overflow =
            this?.lastPublishAttempt.size - AutopilotPublisher?.MAX_TRACKED_USERS;
          let evicted = 0;
  ...
  async publishForAllUsers(): Promise<AutoPublishResult[]> {
    ...
    try {
      ...
      const _enabledConfigs = await storage?.getAllEnabledAutopilotConfigs();
      ...
      logger?.info(
        `Found ${enabledConfigs?.length} users with autopilot enabled`,
      );

      // Process each user in parallel (but limit concurrency to avoid overload)
      const results: AutoPublishResult[] = [];
      const _batchSize = 5; // Process 5 users at a time

      for (let i = 0; i < enabledConfigs?.length; i += batchSize) {
        const _batch = enabledConfigs?.slice(i, i + batchSize);
      ...
      const _totalPosts = results?.reduce((sum, r) => sum + r?.socialPosts, 0);
      const _totalCampaigns = results?.reduce((sum, r) => sum + r?.adCampaigns, 0);
      const _totalErrors = results?.reduce((sum, r) => sum + r?.errors.length, 0);
    ...
    try {
      ...
      if (config?.platforms && config?.platforms.length > 0) {
        ...
        if (socialResult?.error) {
          ...
          if (
            !socialResult?.error.includes("threshold") &&
            !socialResult?.error.includes("not trained")
          ) {
            throw new Error(
              `Social media publishing failed: ${socialResult?.error}`,
            );
          }
   ...
   */
  private async pickNextPlatform(
    userId: string,
    platforms: string[],
  ): Promise<string> {
    ...
    try {
      ...
      const _existing = await storage?.getUserSocialPosts(userId);
      const counts: Record<string, number> = {};
      for (const p of platforms) counts[p] = 0;
      for (const post of existing || []) {
        const _pl = post?.platform;
        if (pl && counts[pl] !== undefined) counts[pl]++;
      }
      // Pick the platform with the fewest posts
      const _sorted = platforms?.slice().sort((a, b) => counts[a] - counts[b]);
      logger?.info(
        `[Autopilot] Platform rotation counts: ${JSON?.stringify(counts)} → picking "${sorted[0]}"`,
      );
      return sorted[0];
    } catch {
      // Fallback: rotate by minute so we spread across platforms even without DB data
      const _minuteIndex = Math?.floor(Date?.now() / 60000) % platforms?.length;
  ...
  private async fetchStorefrontContext(userId: string): Promise<{
    ...
    try {
      ...
      const _storefrontUrl = storefront
      ...
      const _recentListings = await db
        .select({
          id: listings?.id,
      ...
      const _beatLines = recentListings?.map((l) => {
        const _meta = (l?.metadata as Record<string, unknown> | null) ?? {};
        const _genre = (meta?.genre as string) || l?.category || "beat";
        const _bpm = meta?.bpm ? `${meta?.bpm} BPM` : "";
        const _key = meta?.key ? `key of ${meta?.key}` : "";
        const _price = l?.priceCents ? `$${(l?.priceCents / 100).toFixed(2)}` : "";
        const _details = [genre, bpm, key, price].filter(Boolean).join(" · ");
      ...
      for (const l of recentListings) {
        ...
        if (hasDiscount) {
          const _origPrice = l?.priceCents
            ? `$${(l?.priceCents / 100).toFixed(2)}`
            : "";
          const _salePrice = l?.discountPriceCents
            ? `$${(l?.discountPriceCents / 100).toFixed(2)}`
            : "";
          const _expiry = l?.discountExpiresAt
      ...
      if (recentListings?.length > 0) {
        ...
        const _bogoTiers = await db
          .select({
            listingId: listingLicenseTiers?.listingId,
            licenseType: listingLicenseTiers?.licenseType,
          })
        ...
        for (const tier of bogoTiers) {
          const _listing = recentListings?.find((l) => l?.id === tier?.listingId);
          if (listing) {
            promoLines?.push(
              `Buy one ${tier?.licenseType} license for "${listing?.title}", get one free`,
            );
          }
        }
      }

      const _promotionContext =
        promoLines?.length > 0
  ...
  private async publishSocialContent(
    config: Record<string, unknown>,
  ): Promise<{ posts: number; error?: string }> {
    try {
      const _userId = config?.userId;
      ...
      const _socialAI = await aiModelManager?.getSocialAutopilot(userId);
      ...
      if (!socialAI?.getIsTrained()) {
        ...
        logger?.info(
          `[Autopilot] User ${userId}: model untrained — running A/B quality gate (target: 81/100 = 90% Veo)`,
        );

        // Enrich the quality gate topic with real beat data when available
        const _enrichedTopic = sfContext?.beatContext
        ...
        if (!gateResult) {
          logger?.info(
            `[Autopilot] User ${userId}: quality gate rejected all variants — skipping post to protect quality`,
          );
          return {
            posts: 0,
            error:
              "Content quality gate: all variants below minimum threshold (73). Skipping to protect brand quality.",
          };
        }

        // Append storefront URL to the gated content so every post links back
        const _baseText = gateResult?.winner.headline
          ? `${gateResult?.winner.headline}\n\n${gateResult?.winner.content}`
          : gateResult?.winner.content;
        const _gatedText =
          sfContext?.storefrontUrl && !baseText?.includes(sfContext?.storefrontUrl)
        ...
        const _nextOptimalTime = await this?.calculateNextOptimalPostingTime(
          targetPlatform,
          config?.postingFrequency || "daily",
          userId,
        );

        const _scheduledPost = await autoPostingServiceV2?.schedulePost(
          userId,
          [targetPlatform],
          postContent,
          nextOptimalTime,
      ...
      const _contentTypes = config?.contentTypes || ["tips", "insights"];
      const _ctSeed = `${userId}:${contentTypes?.join(",")}:${Math?.floor(Date?.now() / (15 * 60 * 1000))}`;
      const _ctIdx = (() => {
        let h = 2166136261;
      ...
      const _selectedContentType = contentTypes[ctIdx];
      ...
      const _genre = config?.genre || "music";
      ...
      const _mcTopic =
        selectedContentType === "promotions" && sfContext?.promotionContext
      ...
      if (confidence < minThreshold) {
        ...
        return {
          posts: 0,
          error: `Confidence ${confidence?.toFixed(2)} below threshold ${minThreshold}`,
        };
      }
      ...
      const _rawText = advancedContent?.primary.hook
        ? `${advancedContent?.primary.hook}\n\n${advancedContent?.primary.body}`
        : advancedContent?.primary.body;
      const _gateResult = await contentQualityGate?.scoreAndGateExisting(
        userId,
        rawText,
        targetPlatform,
        {
      ...
      if (!gateResult) {
        logger?.warn(
          `[Autopilot] User ${userId}: MaxCore content below Veo pressure floor — skipping post to protect quality`,
        );
        return {
          posts: 0,
          error:
            "Veo quality gate: MaxCore content below minimum floor (73). Skipping to protect brand quality.",
        };
      }

      const _finalText = gateResult?.winner.headline
        ? `${gateResult?.winner.headline}\n\n${gateResult?.winner.content}`
      ...
      const _mcMediaRec = advancedContent?.mediaGuidance?.recommendedType;
      ...
      if (resolvedMediaType !== "text") {
        const _generatedAsset = await aiContentService?.generateContent({
          prompt: finalText,
        ...
        if (!generatedAsset?.url) {
          throw new Error(
            `Media generation returned no URL for ${resolvedMediaType}`,
          );
        }
        mediaUrl = generatedAsset?.url;
        logger?.info(
          `✅ Generated ${resolvedMediaType} asset for user ${userId}: ${mediaUrl}`,
        );
      }

      // Create post content with MaxCore-sourced, quality-gated text
      const _mcHashtags = advancedContent?.primary.hashtags || [];
      const postContent: PostContent = {
        text: finalText,
        hashtags:
          gateResult?.winner.hashtags?.length > 0
   ...
   */
  private async publishAdvertisingCampaigns(
    config: Record<string, unknown>,
    ...
    try {
      ...
      const _sfContext = await this?.fetchStorefrontContext(userId);
      ...
      const _advertisingAI =
        await aiModelManager?.getAdvertisingAutopilot(userId);
      const _isTrained = advertisingAI?.getIsTrained();
      logger?.info(
      ...
      if (config?.useMultimodalAnalysis) {
        const _recentAnalysis = await storage?.getRecentAnalyzedContent(
          userId,
          1,
        );
      ...
      const _objective = "brand_awareness"; // Could be configurable
      const _recommendations =
        await advertisingAI?.generateCampaignRecommendations(
          objective,
          enrichedFeatures,
        );
      ...
      const _bestCampaign = recommendations?.reduce(
      ...
      if (bestCampaign?.mediaType !== "text") {
        ...
        if (!generatedAsset?.url) {
          throw new Error(
            `Ad media generation returned no URL for ${bestCampaign?.mediaType}`,
          );
        }
        mediaUrl = generatedAsset?.url;
        logger?.info(
      ...
      const _primaryPlatform = bestCampaign?.platforms?.[0] || "facebook";
      const _nextOptimalTime = await this?.calculateNextOptimalPostingTime(
        primaryPlatform,
        config?.postingFrequency || "daily",
        userId,
      ...
      if (
        sfContext?.storefrontUrl &&
        !adContent?.includes(sfContext?.storefrontUrl)
      ) {
        adContent = `${adContent}\n🔗 ${sfContext?.storefrontUrl}`.trim();
      }

      // Create campaign via storage with AI-selected media type, timing, and generated assets
      const _campaign = await storage?.createAdCampaign({
        userId,
        name:
          bestCampaign?.name ||
          `Auto ${bestCampaign?.mediaType} Campaign - ${primaryPlatform}`,
  ...
  private shouldPostNow(config: Record<string, unknown>): boolean {
    const _frequency = config?.postingFrequency || "daily";
    const _userId = config?.userId;
    const _now = new Date();
    ...
    const _minInterval = intervalMs[frequency] ?? intervalMs["daily"];
    const _lastAttempt = this?.lastPublishAttempt.get(userId);

    if (lastAttempt) {
      const _elapsed = now?.getTime() - lastAttempt?.getTime();
      if (elapsed < minInterval) {
        const _nextAttempt = new Date(lastAttempt?.getTime() + minInterval);
        logger?.info(
  ...
  private async calculateNextOptimalPostingTime(
    ...
    if (userId) {
      try {
        ...
        if (learned && learned?.length > 0) {
          const _hours = Array?.from(new Set(learned?.map((r) => r?.hour))).sort(
            (a, b) => a - b,
          );
    ...
    const _sameHourSlot = platformHours?.find((h) => h === currentHour);
    ...
    if (sameHourSlot !== undefined && currentMinute <= 55) {
      const _scheduledTime = new Date(now);
...
export const _autopilotPublisher = new AutopilotPublisher();
