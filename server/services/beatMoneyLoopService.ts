...
class BeatMoneyLoopService {
  ...
  private async _maxcoreAudio(
    ...
    // Beat duration. Default 30 s (shareable clip, reliably generated by MaxCore).
    // Override via BEAT_DURATION_SECONDS env var (max 120 s for reliability).
    const durationSec = Math.min(
      120,
      Math.max(20, Number(process.env.BEAT_DURATION_SECONDS) || 30),
    );
    ...
    const res = await fetch(`${base}/api/generate/audio`, {
      ...
      // Mode C returns a job_id quickly; Mode B may synchronously render.
      // MaxCore (Replit-hosted) has a cold-start that can exceed 3 min — give
      // up to 10 min for the initial POST so the service has time to wake up.
    ...
    if (data.job_id) {
      // Poll budget: at least 10 min — MaxCore audio rendering takes several
      // minutes for real music generation. Each poll has a 20 s per-request
      // timeout; individual timeouts do NOT terminate the budget.
      const pollBudgetMs = Math.max(600_000, durationSec * 5_000);
