...
const RSS_FEEDS: Array<{ url: string; name: string; followRedirect: boolean }> = [
  { url: 'https://www.musicbusinessworldwide.com/feed/', name: 'Music Business Worldwide', followRedirect: false },
  { url: 'https://www.digitalmusicnews.com/feed/', name: 'Digital Music News', followRedirect: false },
  { url: 'https://musically.com/feed/', name: 'Music Ally', followRedirect: false },
...
class IndustryMonitorService {
  ...
  async fetchLiveChanges(): Promise<LiveIndustryChange[]> {
    ...
    logger.info(`[IndustryMonitor] Fetched ${unique.length} total, ${fresh.length} new changes`);
    return fresh;
  }
  ...
  private async fetchRssFeed(url: string, feedName: string): Promise<LiveIndustryChange[]> {
  ...
  private parseRss(xml: string, feedName: string, feedUrl: string): LiveIndustryChange[] {
    ...
    try {
      ...
      for (const item of items.slice(0, 20)) {
        ...
        const description = this.stripHtml(String(item.description || '')).trim().slice(0, 500);
      ...
      logger.warn(`[IndustryMonitor] Failed to parse RSS from ${feedName}:`, (e as Error).message);
    }
  ...
  private async tavilySearch(query: string, apiKey: string): Promise<LiveIndustryChange[]> {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), FETCH_TIMEOUT_MS);
  ...
  private async exaSearch(query: string, apiKey: string): Promise<LiveIndustryChange[]> {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), FETCH_TIMEOUT_MS);
  ...
  private classifyArticle(title: string, description: string): {
    ...
    if (/copyright|dmca|gdpr|ccpa|royalt|compulsory license|mechanical license|neighbouring rights|eu.?copyright|digital services act|legislation|collect.?society|mma|music modernization/.test(text)) {
    ...
    if (/spotify|apple music|amazon music|youtube music|tidal|deezer|soundcloud|pandora|audiomack|boomplay|napster|qobuz/.test(text)) {
      ...
      if (/loudness|lufs|normali[sz]|codec|format|bitrate|lossless|dolby|spatial audio|atmos/.test(text)) {
        return { source: 'streaming_platform', category: 'standard', urgency: 'high', modules: ['studio', 'distribution'], impact: 75, complexity: 'simple', hours: 8 };
      }
      if (/algorithm|discovery|playlist|recommend|stream count|royalt|payout|per.?stream/.test(text)) {
      ...
      if (/metadata|isrc|upc|artwork|release|label|distributor/.test(text)) {
        return { source: 'streaming_platform', category: 'standard', urgency: 'medium', modules: ['distribution'], impact: 60, complexity: 'trivial', hours: 4 };
      }
    ...
    if (/tiktok|instagram|reels|shorts|youtube|twitter|x\.com|facebook|threads|snapchat|creator|influencer/.test(text)) {
      ...
      if (/algorithm|reach|organic|viral|engagement|views|impressions/.test(text)) {
        return { source: 'social_media', category: 'optimization', urgency: 'high', modules: ['social', 'advertising'], impact: 85, complexity: 'simple', hours: 8 };
      }
    ...
    if (/fl studio|ableton|logic pro|pro tools|studio one|cubase|reaper|garageband|bitwig|reason|bandlab|soundtrap|splice|landr/.test(text)) {
    ...
    if (/stem separat|vocal remov|ai master|ai mix|ai composi|neural audio|music generat|ai produc|plugin|vst|audio codec|midi 2|spatial audio|dolby atmos|sony 360/.test(text)) {
    ...
    if (/royalt|blockchain|nft|web3|smart contract|publish|sync licens|revenue|payout|collect|distributor|aggregator/.test(text)) {
      return { source: 'technology', category: 'feature', urgency: 'medium', modules: ['monetization', 'distribution'], impact: 65, complexity: 'moderate', hours: 30 };
    }

    // General AI in music
    if (/ai |artificial intelligence|machine learning|generative|gpt|llm/.test(text) && /music|audio|song|track|artist|production|sound/.test(text)) {
  ...
  private deduplicateByHash(changes: LiveIndustryChange[]): LiveIndustryChange[] {
