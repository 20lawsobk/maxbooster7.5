...
class SmartDefaultsService {
  private artistTypeSettings: Record<ArtistType, Partial<RecommendedSettings>> = {
    solo: {
      ...
      social: {
        ...
        contentTypes: ['behind-the-scenes', 'music-clips', 'personal-updates'],
        engagementStrategy: 'personal-connection',
      },
      dashboard: {
        priorityWidgets: ['streams', 'social-reach', 'next-release', 'ai-coach'],
    ...
    band: {
      ...
      social: {
        ...
        contentTypes: ['rehearsal-clips', 'live-performances', 'member-spotlights'],
        engagementStrategy: 'community-building',
      },
      dashboard: {
        priorityWidgets: ['collaboration', 'tour-dates', 'merch-sales', 'streams'],
        quickActions: ['share-with-band', 'schedule-rehearsal', 'upload-track'],
        hiddenFeatures: [],
      },
    },
    producer: {
      ...
      dashboard: {
        priorityWidgets: ['beat-sales', 'licensing', 'studio', 'collaborations'],
    ...
    label: {
      distribution: {
        primaryPlatforms: ['spotify', 'apple-music', 'youtube-music', 'deezer', 'tidal'],
      ...
      social: {
        ...
        postingFrequency: 'daily',
        contentTypes: ['artist-spotlights', 'release-announcements', 'industry-news'],
        engagementStrategy: 'brand-authority',
      },
      dashboard: {
        priorityWidgets: ['roster-overview', 'revenue', 'distribution-status', 'contracts'],
    ...
    dj: {
      ...
      dashboard: {
        priorityWidgets: ['upcoming-gigs', 'track-library', 'social-reach', 'booking-requests'],
    ...
    songwriter: {
      ...
      social: {
        ...
        postingFrequency: '3-per-week',
        contentTypes: ['songwriting-process', 'acoustic-versions', 'lyrics-breakdown'],
        engagementStrategy: 'storytelling',
      },
      dashboard: {
        priorityWidgets: ['royalties', 'sync-opportunities', 'collaborations', 'publishing'],
        quickActions: ['submit-to-sync', 'track-royalties', 'find-collaborators'],
        hiddenFeatures: ['beat-marketplace'],
      },
    },
  };

  private careerStageMultipliers: Record<CareerStage, { budget: number; complexity: number }> = {
  ...
  async getRecommendedSettings(userId: string, artistType: ArtistType): Promise<RecommendedSettings> {
    const user = await storage.getUser(userId);
    const onboardingData = (user?.onboardingData as Record<string, any>) || {};
    const careerStage = (onboardingData.careerStage as CareerStage) || 'emerging';
    const genres = (onboardingData.genres as string[]) || ['pop'];
    
    const baseSettings = this.artistTypeSettings[artistType] || this.artistTypeSettings.solo;
    ...
    return {
      distribution: {
        primaryPlatforms: baseSettings.distribution?.primaryPlatforms || ['spotify', 'apple-music'],
      ...
      social: {
        suggestedPlatforms: baseSettings.social?.suggestedPlatforms || ['instagram', 'tiktok'],
        postingFrequency: baseSettings.social?.postingFrequency || 'daily',
        contentTypes: baseSettings.social?.contentTypes || ['music-clips'],
        engagementStrategy: baseSettings.social?.engagementStrategy || 'personal-connection',
      },
      studio: {
      ...
      marketing: {
        budgetRange: [50 * stageMultiplier.budget, 500 * stageMultiplier.budget],
        targetAudience: this.getTargetAudienceByGenre(genres),
        campaignTypes: this.getCampaignTypesByStage(careerStage),
      },
      dashboard: {
        priorityWidgets: baseSettings.dashboard?.priorityWidgets || ['streams', 'revenue'],
  ...
  async analyzeUserBehavior(userId: string): Promise<UserBehaviorAnalysis> {
    ...
    const totalStreams = recentAnalytics.reduce((sum, a) => sum + (a.streams || 0), 0);
    const totalRevenue = recentAnalytics.reduce((sum, a) => sum + (a.revenue || 0), 0);
    const totalFollowers = recentAnalytics.reduce((sum, a) => sum + (a.followers || 0), 0);

    return {
      mostUsedFeatures,
      preferredWorkingHours: this.generatePreferredHours(sessionData),
      contentPatterns: {
        avgTracksPerMonth: userProjects.length / Math.max(1, this.getAccountAgeMonths(user)),
        preferredGenres,
      ...
      growthTrends: {
        followerGrowthRate: this.calculateGrowthRate(recentAnalytics, 'followers'),
  ...
  async predictOptimalSchedule(userId: string): Promise<OptimalSchedule> {
    ...
    return {
      ...
      releases: {
        ...
        reasoning: 'Friday releases align with New Music Friday playlists on major platforms.',
      },
      engagement: {
  ...
  async getSuggestions(userId: string): Promise<ActionSuggestion[]> {
    ...
    if (!onboardingData.hasConnectedSocial) {
      suggestions.push({
        ...
        description: 'Link Instagram, TikTok, and Twitter to manage all platforms from one place',
    ...
    suggestions.push(...this.getCareerStageSuggestions(careerStage, artistType));
    ...
    if (behavior.growthTrends.streamGrowthRate < 0) {
      suggestions.push({
        ...
        description: 'Your streaming numbers are declining. Launch a promotional campaign.',
  ...
  async getDashboardLayout(userId: string): Promise<DashboardLayout> {
    ...
    const savedLayout = preferences.dashboardLayout as DashboardLayout | undefined;
    if (savedLayout) {
      return savedLayout;
    }
    ...
    const allWidgets = [
      'streams', 'revenue', 'social-reach', 'next-release',
      'ai-coach', 'quick-actions', 'recent-activity', 'analytics-chart',
      'collaborations', 'notifications', 'achievements', 'goals',
    ];
    ...
    const widgets = allWidgets.map((id, index) => ({
      id,
      position: priorityWidgets.includes(id) ? priorityWidgets.indexOf(id) : index + 100,
      visible: !hiddenFeatures.includes(id),
      size: priorityWidgets.includes(id) ? 'large' as const : 
            frequentlyUsed.includes(id) ? 'medium' as const : 'small' as const,
  ...
  async updatePreferences(userId: string, preferences: Partial<PersonalizationPreferences>): Promise<void> {
    const user = await storage.getUser(userId);
    ...
    await storage.updateUser(userId, {
      preferences: {
        ...
        dashboardLayout: preferences.dashboardLayout || currentPrefs.dashboardLayout,
      },
    });
  }
  ...
  private getGenreSpecificSettings(genre: string): { bpm: number; key: string } {
  ...
  private getTargetAudienceByGenre(genres: string[]): string[] {
    ...
    genres.forEach(g => {
      (audienceMap[g.toLowerCase()] || audienceMap.pop).forEach(a => audiences.add(a));
    });
  ...
  private getCampaignTypesByStage(stage: CareerStage): string[] {
    const campaigns: Record<CareerStage, string[]> = {
      ...
      developing: ['playlist-pitching', 'social-ads', 'pr-campaign', 'blog-outreach'],
      established: ['radio-promotion', 'tour-support', 'sync-licensing', 'brand-partnerships'],
      professional: ['global-campaigns', 'major-label-partnerships', 'arena-tours', 'merchandise'],
    };
  ...
  private calculateLoginFrequency(sessionData: Record<string, any>): 'daily' | 'weekly' | 'occasional' {
  ...
  private calculateGrowthRate(analyticsData: unknown[], field: string): number {
    ...
    const recentAvg = recent.reduce((sum, a) => sum + (a[field] || 0), 0) / recent.length;
    const olderAvg = older.reduce((sum, a) => sum + (a[field] || 0), 0) / older.length;
    
    if (olderAvg === 0) return recentAvg > 0 ? 100 : 0;
    return Math.round(((recentAvg - olderAvg) / olderAvg) * 100);
  }

  private generatePreferredHours(sessionData: Record<string, any>): { hour: number; activityLevel: number }[] {
    const hourlyActivity = (sessionData.hourlyActivity as number[]) || 
      Array.from({ length: 24 }, (_, i) => (i >= 9 && i <= 22) ? Math.random() * 0.5 + 0.5 : Math.random() * 0.3);
    
    return hourlyActivity.map((level, hour) => ({ hour, activityLevel: level }));
  }

  private getPlatformOptimalTimes(artistType: ArtistType, genres: string[]): OptimalSchedule['posting'] {
    const baseTimes: Record<string, { bestDays: string[]; bestTimes: string[] }> = {
      instagram: { bestDays: ['Tuesday', 'Wednesday', 'Thursday'], bestTimes: ['11:00 AM', '2:00 PM', '7:00 PM'] },
      tiktok: { bestDays: ['Tuesday', 'Thursday', 'Friday'], bestTimes: ['9:00 AM', '12:00 PM', '7:00 PM'] },
      twitter: { bestDays: ['Wednesday', 'Thursday', 'Friday'], bestTimes: ['9:00 AM', '12:00 PM', '5:00 PM'] },
      youtube: { bestDays: ['Thursday', 'Friday', 'Saturday'], bestTimes: ['2:00 PM', '5:00 PM'] },
      facebook: { bestDays: ['Wednesday', 'Thursday', 'Friday'], bestTimes: ['1:00 PM', '3:00 PM'] },
    };
  ...
  private getCareerStageSuggestions(stage: CareerStage, artistType: ArtistType): ActionSuggestion[] {
    const suggestions: ActionSuggestion[] = [];
    ...
    if (stage === 'emerging') {
      suggestions.push({
        ...
        description: 'Focus on growing your social media following with consistent content',
    ...
    if (stage === 'developing') {
      suggestions.push({
        ...
        description: 'Your streams are growing! Time to pitch to editorial playlists',
        action: 'Submit Pitch',
