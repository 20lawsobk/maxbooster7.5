...
class DSPAnalyticsService {
  private platformConfigs: Map<
    DSPPlatform,
    { apiBaseUrl: string; rateLimitPerMinute: number }
  > = new Map([
    [
      "spotify",
  ...
  normalizeAppleMusicData(
    data: AppleMusicAnalytics,
    startDate: Date,
    endDate: Date,
  ): NormalizedDSPAnalytics {
  ...
  private async storeDSPAnalytics(
    userId: string,
  ...
  async getDemographics(userId: string): Promise<{
    ...
    const countryMap: { [key: string]: { name: string; listeners: number } } =
      {};
    const cityMap: {
      [key: string]: { name: string; country: string; listeners: number };
    } = {};
