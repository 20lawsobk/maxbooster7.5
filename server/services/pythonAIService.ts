...
async function callAIModel<T>(
  ...
  try {
    const _response = await fetchWithTimeout(`${AI_MODEL_URL}${endpoint}`, {
      method: "POST",
...
export class PythonAIService {
  ...
  async getBoostSheet(
    ...
    try {
      ...
      const _data = (await response?.json()) as BoostSheetResult;
      return { success: true, data };
    } catch (err) {
      return { success: false, error: "AI Model service unavailable" };
    }
  ...
  async checkHealth(): Promise<AIModelResponse<HealthResult>> {
    try {
      ...
      const _data = (await response?.json()) as HealthResult;
      return { success: true, data };
    } catch {
      return { success: false, error: "AI Model service unavailable" };
    }
...
export const _pythonAIService = PythonAIService?.getInstance();
