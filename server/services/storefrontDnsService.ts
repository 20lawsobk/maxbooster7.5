 ...
 */
async function dohResolve(name: string, type: DoHType, timeoutMs = 5000): Promise<string[]> {
  ...
  async function query(baseUrl: string): Promise<string[]> {
    ...
    try {
      ...
      clearTimeout(tid);
      if (!resp.ok) return [];
      const data = await resp.json() as { Answer?: DoHAnswer[]; Status: number };
...
async function checkVerificationMethods(
  ...
  try {
    ...
    const hasOurNs = nsRecords.some(ns => ourNs.includes(ns.toLowerCase().replace(/\.$/, '')));
    if (hasOurNs) {
      logger.info(`[storefrontDns] Domain ${domain} verified via NS delegation`);
      return 'ns';
    }
  } catch { /* continue */ }
  ...
  try {
    ...
    const hasOurCname = cnameRecords.some(c =>
...
export async function checkDomainHealth(domain: string): Promise<boolean> {
  ...
  try {
    ...
    if (nsRecords.some(ns => ourNs.includes(ns.toLowerCase().replace(/\.$/, '')))) return true;
  } catch { /* continue */ }

  // Check CNAME for www
  try {
    const cnameRecords = await dohResolve(`www.${domain}`, 'CNAME');
    if (cnameRecords.some(c => c.toLowerCase().replace(/\.$/, '').endsWith(`.${BASE_DOMAIN}`))) return true;
  } catch { /* continue */ }
 ...
 */
async function provisionCaaRecords(zoneId: string, userId: string, domain: string): Promise<void> {
  const caaRecords = [
    ...
    { value: `0 iodef "mailto:admin@${BASE_DOMAIN}"`, note: 'CAA violation reports' },
  ];
...
export async function attachDomainToStorefront(
  ...
  try {
    ...
    // Bump zone serial
    await client.query(`UPDATE dns_zones SET updated_at = now() WHERE id = $1`, [zoneId]);
    ...
    const instructions: DomainSetupInstructions = {
      method_ns: {
        label: 'Recommended — Full DNS delegation (fastest, enables wildcard SSL)',
        ns1: NS_ALT1,
        ns2: NS_ALT2,
...
export async function verifyStorefrontDomain(
  ...
  const { domain, verification_token, verification_failures, status, dns_zone_id, storefront_id } = rows[0];

  if (status === 'active') return 'verified';

  const method = await checkVerificationMethods(domain, verification_token, storefront_id);

  if (method) {
    await activateStorefrontDomain(storefrontDomainId, domain, dns_zone_id, storefront_id, method);
    return 'verified';
  }
...
async function activateStorefrontDomain(
  ...
  try {
    ...
    logger.info({ storefrontDomainId, domain, method }, '[storefrontDns] domain activated');

    // Post-activation: provision CAA records (async, non-blocking)
    if (dnsZoneId) {
      const userId = (await pool.query<{ user_id: string }>(
        `SELECT user_id FROM dns_zones WHERE id = $1`, [dnsZoneId],
      )).rows[0]?.user_id;
      if (userId) {
        provisionCaaRecords(dnsZoneId, userId, domain).catch(err => {
          logger.warn({ err, domain }, '[storefrontDns] CAA provisioning failed (non-fatal)');
        });
      }
    }
    ...
    for (const host of hosts) {
      provisionCertificateForHost(host).catch(err => {
        logger.warn({ err, host }, '[storefrontDns] cert provisioning failed (non-fatal)');
      });
    }
  } catch (err) {
    await client.query('ROLLBACK');
...
export async function getDomainStatus(storefrontDomainId: string): Promise<{
  ...
  const { domain, status, storefront_id, verification_token, dns_zone_id } = rows[0];

  const certRow = (await pool.query<{ cert_status: string }>(
    `SELECT cert_status FROM storefront_hosts WHERE host = $1`, [domain],
  )).rows[0];
 ...
 */
export async function runDomainHealthSweep(): Promise<{ checked: number; degraded: number }> {
  const { rows } = await pool.query<{ id: string; domain: string; health_failures: number }>(
  ...
  for (const row of rows) {
    ...
    if (failures !== row.health_failures) {
      await pool.query(
        ...
        logger.warn({ err, domain: row.domain }, '[storefrontDns] health sweep update failed');
      });
    }
    if (newStatus === 'health_degraded') {
      degraded++;
      logger.warn(`[storefrontDns] Domain ${row.domain} health degraded (${failures} consecutive failures)`);
    }
  }
  logger.info(`[storefrontDns] Health sweep complete — ${checked} checked, ${degraded} degraded`);
