...
export async function checkDiffusionAvailable(): Promise<boolean> {
  ...
  if (available) {
    logger.info('[DiffusionScene] PyTorch diffusion API is ready — Tier 1 active');
  } else {
    logger.debug('[DiffusionScene] PyTorch diffusion API not reachable — Tier 1 bypassed, using Ken Burns');
  }
  return available;
}
...
export async function renderDiffusionScene(
  ...
  try {
    ...
    const genreKey = (opts.genre || 'default').toLowerCase().replace(/[^a-z&-]/g, '');
    const aesthetic = GENRE_TO_AESTHETIC[genreKey] || GENRE_TO_AESTHETIC.default;
    const gradeLabel = opts.colorGrade && opts.colorGrade !== 'none' ? `${opts.colorGrade} color grade` : '';
    const artistCtx = opts.artistName ? `${opts.artistName} music video, ` : '';
    const prompt = [artistCtx, aesthetic, gradeLabel, 'professional music video, 4K quality']
      .filter(Boolean)
      .join(', ');
    ...
    const sceneProgress = opts.totalScenes > 1 ? opts.sceneIndex / (opts.totalScenes - 1) : 0.5;
    ...
    const sceneStartSec  = opts.sceneIndex * (ba?.durationSeconds ?? opts.durationSec) / totalSc;
    const sceneEndSec    = sceneStartSec + opts.durationSec;
    const isDrop = ba?.peakPositions?.some(
      t => t >= sceneStartSec && t < sceneEndSec,
    ) ?? false;

    // Emotional arc: curiosity → excitement → inspiration → anthemic
    const emotionalGoal = sceneProgress < 0.25 ? 'curiosity'
      : sceneProgress < 0.55 ? 'excitement'
      : sceneProgress < 0.80 ? 'inspiration'
      : 'anthemic';
    ...
    const diffRequest: PyTorchDiffusionRequest = {
      ...
      bpm,
      energy,
      energy_peak:     isDrop ? 0.95 : energy * 0.90 + 0.05,
    ...
    await execFileAsync(ffmpegPath, [
      '-y',
      '-stream_loop', '-1',          // loop the short diffusion clip if durationSec > clip length
      '-i', rawPath,
      '-vf', vfParts.join(','),
