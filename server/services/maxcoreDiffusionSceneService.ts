...
const _CACHE_TTL_MS = 30_000; // 30 s — recheck if idle
...
export async function checkDiffusionAvailable(): Promise<boolean> {
  ...
  const _available = await isPyTorchDiffusionReady();
...
export async function renderDiffusionScene(
  ...
  try {
    ...
    const _genreKey = (opts?.genre || "default")
      .toLowerCase()
      .replace(/[^a-z&-]/g, "");
    const _aesthetic =
    ...
    const _gradeLabel =
      opts?.colorGrade && opts?.colorGrade !== "none"
        ...
        : "";
    const _artistCtx = opts?.artistName ? `${opts?.artistName} music video, ` : "";
    const _prompt = [
      artistCtx,
      aesthetic,
      gradeLabel,
    ...
    const _sceneStartSec =
      (opts?.sceneIndex * (ba?.durationSeconds ?? opts?.durationSec)) / totalSc;
    const _sceneEndSec = sceneStartSec + opts?.durationSec;
    const _isDrop =
    ...
    const _fps = 30;
    const _frames = Math?.min(Math?.ceil(opts?.durationSec * fps), 48);

    // Cap resolution — diffusion API works best at ≤512 on each axis.
    const _H = Math?.min(opts?.height, 512);
    const _W = Math?.min(opts?.width, 512);
    ...
    // ── Call diffusion API ───────────────────────────────────────────────────
    const _result = await generatePyTorchDiffusionVideo(diffRequest);
    ...
    const _rawPath = opts?.outputPath.replace(/\.mp4$/, "_diffraw?.mp4");
