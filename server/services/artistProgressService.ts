...
class ArtistProgressService {
  ...
  async captureSnapshot(userId: string): Promise<void> {
    try {
      ...
      logger.warn({ err: error }, `Error capturing snapshot for user ${userId}:`);
      throw error;
    }
  }
  ...
  async getProgressHistory(userId: string, days: number = 30): Promise<any[]> {
    try {
      ...
      logger.warn({ err: error }, `Error getting progress history for user ${userId}:`);
      return [];
    }
  }
  ...
  async calculateGrowthMetrics(userId: string): Promise<GrowthMetrics> {
    try {
      ...
      logger.warn({ err: error }, `Error calculating growth metrics for user ${userId}:`);
  ...
  async getCareerMilestones(userId: string): Promise<CareerMilestone[]> {
    try {
      ...
      logger.warn({ err: error }, `Error getting career milestones for user ${userId}:`);
      return this.getDefaultMilestones();
    }
  }
  ...
  async getDashboardData(userId: string): Promise<DashboardData> {
    try {
      ...
      logger.warn({ err: error }, `Error getting dashboard data for user ${userId}:`);
