...
class ViralScoringService {
  ...
  private readonly hookPatterns = [
    ...
    {
      pattern:
        /^(the #1 (mistake|secret|reason)|nobody (told|shows|talks) you|they don't want you to know)/i,
      score: 95,
    },
    {
      pattern:
        /^(you won't believe|this changed (everything|my career|my life)|why (everyone|artists|rappers))/i,
      score: 90,
    },
    {
      pattern:
        /^(i gained \d+k|how i got \d+k|from \d+ to \d+k|how i (blew up|went viral|made \$))/i,
      score: 92,
    },
    // Music-industry specific hooks — high relevance to Max Booster users
    {
      pattern:
        /^(drop everything|if you're (an artist|a producer|making music)|(music|label|deal|stream|royalt))/i,
      score: 87,
    },
    {
      pattern:
        /^(breaking|urgent|finally|here's why|unpopular opinion|hot take|real talk)/i,
      score: 83,
    },
    {
      pattern:
        /^(i was today years old|nobody talks about|the truth about|stop doing this|please stop)/i,
      score: 80,
    },
    // Relatability / POV — strong on TikTok and Reels
    {
      pattern: /^(pov:|when you|that feeling when|imagine|tell me why|not me)/i,
      score: 78,
    },
    ...
    {
      pattern:
        /^(it'?s (finally|officially) here|out now|new music|new single|just dropped)/i,
      score: 85,
    },
    {
      pattern: /^(wait (for|till)|watch what happens|the drop at|rate this)/i,
      score: 79,
    },
    {
      pattern: /^(bar for bar|lyricism|wordplay|flow|cadence|bars)/i,
      score: 74,
    },
    {
      pattern:
        /^(i almost didn't release|been sitting on this|kept this quiet|nobody expected)/i,
      score: 81,
    },
    {
      pattern: /^(day one|first 24|first week|this week only|only \d+ left)/i,
      score: 77,
    },
    // Transformation / journey hooks
    {
      pattern:
        /^(from (bedroom|zero|broke|nothing) to|my (journey|story|struggle)|i started with)/i,
      score: 84,
    },
    // Achievement / milestone hooks
    {
      pattern:
        /(\d+k?m? (streams?|plays?|followers?|listeners?)|playlist (added|placement)|charted|trending)/i,
      score: 82,
    },
  ];
  ...
  private readonly platformOptimalHashtags: Record<
    string,
    { min: number; max: number; niches: number; musicMin: number }
  > = {
  ...
  private readonly optimalDurations: Record<
    string,
    { min: number; max: number; peak: number }
  > = {
    ...
    this.trendingTopics = [
      {
        ...
        score: 95,
        category: "promotion",
        hashtags: ["#newmusic", "#outnow", "#newsingle", "#newrelease"],
      },
      {
      ...
      },
      {
        ...
        score: 85,
        category: "content",
        hashtags: ["#bts", "#studiolife", "#makingof", "#inthelab"],
      },
      {
      ...
      },
      {
        ...
        score: 84,
        category: "music",
        hashtags: ["#freestyle", "#bars", "#rap", "#hiphop"],
      },
      {
        topic: "listening party",
        score: 78,
        category: "promotion",
        hashtags: ["#listeningparty", "#albumdrop", "#newalbum"],
      },
      {
        topic: "music video",
        score: 90,
        category: "visual",
        hashtags: ["#musicvideo", "#mv", "#officialvideo", "#visualizer"],
      },
      {
        topic: "live performance",
        score: 86,
        category: "performance",
        hashtags: ["#live", "#concert", "#performance", "#stage"],
      },
      {
        topic: "producer challenge",
        score: 83,
        category: "trend",
        hashtags: ["#beatbattle", "#producerchallenge", "#flippedthesample"],
      },
      {
        topic: "reaction",
        score: 79,
        category: "engagement",
        hashtags: ["#reactionvideo", "#beatreaction", "#musicreaction"],
      },
      {
        topic: "viral sound",
        score: 92,
        category: "trend",
        hashtags: ["#viralsound", "#trending", "#fyp", "#foryoupage"],
      },
      {
        topic: "day in my life",
        score: 76,
        category: "lifestyle",
        hashtags: ["#dayinmylife", "#artistlifestyle", "#musicianlife"],
      },
      {
        topic: "songwriter",
        score: 78,
  ...
  private analyzeHookStrength(caption: string, platform: string): number {
    ...
    const platformNativePatterns: Record<string, RegExp[]> = {
      tiktok: [
        /^(pov:|tell me why|not me |the way |okay but)/i,
        /stitch|duet|fyp/i,
      ],
      instagram: [
        /this one hits different/i,
        /okay, real talk/i,
        /late night thoughts/i,
      ],
      twitter: [
        /^(hot take:|unpopular opinion:|nobody talks|thread 🧵)/i,
        /okay hear me out/i,
      ],
  ...
  private analyzeHashtagOptimization(
    ...
    const discoveryHashtags = hashtags.filter((h) => {
      ...
      if (platform === "instagram")
        return ["#explore", "#reels", "#reelsinstagram", "#viral"].includes(
          lowH,
        );
      if (platform === "youtube")
