...
function tempPath(tag: string): string {
  return path.join(TEMP_DIR, `img2v_${tag}_${randomBytes(4).toString('hex')}.mp4`);
}
...
async function renderImageWithKenBurns(
  ...
  const intensityScale = intensity === 'subtle' ? 0.5 : intensity === 'dramatic' ? 1.6 : 1.0;
  ...
  await execFileAsync(FFMPEG, [
    '-y',
    '-loop', '1',                          // loop still image as video source
    '-i', imagePath,
    '-vf', filterParts.join(','),
...
function buildColorGrade(grade: string): string {
  switch (grade) {
    case 'warm':     return 'curves=r=0/0.1 1/1:g=0/0 1/0.95:b=0/0 1/0.88,eq=brightness=0.05:saturation=1.2';
    case 'cool':     return 'curves=r=0/0 1/0.90:g=0/0 1/0.95:b=0/0.1 1/1,eq=brightness=0.02:saturation=1.1';
    case 'cinematic': return 'curves=r=0/0.05 1/0.95:g=0/0.02 1/0.93:b=0/0.08 1/0.92,eq=contrast=1.12:saturation=0.92,vignette=angle=PI/4:mode=forward:eval=init';
    case 'neon':     return 'curves=r=0/0 1/1:g=0/0 0.5/0.7 1/1:b=0/0 0.5/0.8 1/1,eq=saturation=1.4:contrast=1.05';
    default:         return '';
  }
}
...
async function applyAudioToVideo(
  ...
  if (hasLogo)  { logoIdx  = nextIdx++; inputs.push('-i', logoPath!); }
  if (hasAudio) { audioIdx = nextIdx++; inputs.push('-i', audioPath!); }
  if (hasVoice) { voiceIdx = nextIdx++; inputs.push('-i', voiceSynthPath!); }
  ...
  if (hasAudio && hasVoice) {
    ...
    parts.push(`[user_a][voice_a][synth]amix=inputs=3:normalize=0:weights=1.0 1.1 0.15[afinal]`);
  } else if (hasAudio) {
    ...
    parts.push(`[user_a][synth]amix=inputs=2:normalize=0:weights=1.0 0.08[afinal]`);
  } else if (hasVoice) {
    ...
    parts.push(`[voice_a][synth]amix=inputs=2:normalize=0:weights=1.2 0.22[afinal]`);
  } else {
  ...
  const ffmpegArgs = [
    ...
    ...(hasLogo ? ['-preset', 'veryfast', '-crf', '18', '-pix_fmt', 'yuv420p'] : []),
    '-c:a', 'aac', '-b:a', '192k',
    '-movflags', '+faststart',
    '-t', String(totalDur),
    outputPath,
...
function buildTextOverlays(
  ...
  // Accent bars
  overlays.push(`drawbox=x=0:y=0:w=${width}:h=${barH}:color=${style.ac}@0.28:t=fill`);
  overlays.push(`drawbox=x=0:y=${height - barH}:w=${width}:h=${barH}:color=${style.ac}@0.28:t=fill`);
  ...
  if (sceneType === 'body' || sceneType === 'all') {
    ...
    const yTime = sceneType === 'all' ? `(h-text_h)/2` : `(h-text_h)/2+25*(1-min(1\\,t*3))`;
    overlays.push(
  ...
  if (sceneType === 'cta' || sceneType === 'all') {
    ...
    const boxH  = cs + 44;
    overlays.push(`drawbox=x=${boxX}:y=${boxY}:w=${boxW}:h=${boxH}:color=${style.cta_bg}@0.94:t=fill:enable='gte(t\\,0.2)'`);
    overlays.push(`drawbox=x=${boxX}:y=${boxY}:w=${boxW}:h=4:color=${style.ac}:t=fill:enable='gte(t\\,0.2)'`);
    overlays.push(`drawbox=x=${boxX}:y=${boxY + boxH - 4}:w=${boxW}:h=4:color=${style.ac}@0.55:t=fill:enable='gte(t\\,0.2)'`);
    overlays.push(
...
const PLATFORM_RATIOS: Record<string, string> = {
  tiktok: '9:16', instagram: '1:1', instagram_reels: '9:16',
  youtube: '16:9', facebook: '1:1', twitter: '16:9', linkedin: '16:9',
};

export async function imageToMusicVideo(opts: ImageToVideoOptions): Promise<VideoGenResult> {
  ...
  const colorGrade   = opts.colorGrade || (style.bgType === 'solid' ? 'cinematic' : 'none');
  ...
  try {
    ...
    if (opts.audioPath && existsSync(opts.audioPath) && opts.beatSync !== false) {
      try {
    ...
    if (scenePaths.length === 1) {
      ...
      await combineImageScenes(scenePaths, sceneDurations, videoPath, transition, transitionDur);
    }

    // ── Apply audio ─────────────────────────────────────────────────────────
    const combinedDur = sceneDurations.reduce((a, b) => a + b, 0) -
    ...
    await applyAudioToVideo(
      videoPath, finalPath, combinedDur,
      genre, opts.audioPath, opts.voiceSynthPath, opts.logoPath,
    );
 ...
 */
export async function singleImageToVideo(opts: Omit<ImageToVideoOptions, 'imagePaths'> & { imagePath: string }): Promise<VideoGenResult> {
