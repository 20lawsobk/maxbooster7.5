...
const PLATFORM_NATIVE_DNA: Record<
  string,
  {
    openers: string[];
    transitions: string[];
...
> = {
  tiktok: {
    ...
    closers: [
      ...
      "Stitch with your take",
      'Comment "yes" if you need this',
  ...
  instagram: {
    openers: [
      ...
      "Saved this feeling for the right moment —",
  ...
  facebook: {
    openers: [
      "I don't usually share things like this, but —",
      "Had to share this with my community:",
      "Something meaningful happened:",
      "For anyone who needs to hear this today:",
      "This community means everything, so —",
    ],
    transitions: [
      "Here's the story:",
...
const EMOTIONAL_ARC_TEMPLATES: Record<
  string,
  {
    hook: string;
    context: string;
  ...
  announcement: [
    {
      ...
      context:
        "The {genre} energy behind this was personal — too personal. I wasn't sure the world was ready.",
      tension:
        "Months went by. I kept second-guessing it. Three full versions scrapped. Every time I was about to drop it, something stopped me.",
      resolution:
        "Then I listened back one night at 3am. And I realized — this is exactly why I make music. It's done. It's honest. It's out.",
    ...
    },
    {
      ...
      context:
        "This {genre} record represents everything {artist} has been building toward.",
      tension:
        "Nothing about this was easy. The production took months. The lyrics were rewritten more times than I can count.",
      resolution:
        "But the version you're hearing right now? That's the right one. I know it.",
  ...
  storytelling: [
    {
      ...
      context:
        "No label, no manager, no plan. Just a {genre} record that felt too personal to share.",
      tension:
        "There were nights I genuinely thought about walking away from music entirely. I had a session where I left mid-record and didn't come back for two weeks.",
      resolution:
        "What brought me back was finishing this song. And now, releasing it — I finally understand why I had to write it.",
      cta: "If this hits you, let me know in the comments. Stream {topic} — link in bio. 💙",
    },
    {
      hook: "This song started as a voice memo at 2am.",
      context:
        "I was in the studio alone, trying to finish {topic}, when something clicked.",
      tension:
        "I'd been stuck for three months. The chord progression was wrong. The hook wasn't landing. I was about to scrap the whole thing.",
      resolution:
        "Then I hit record — no plan, no filter — and what came out was exactly the song it needed to be.",
  ...
  engagement: [
    {
      ...
      context:
        "I made this {genre} track specifically for late-night drives, headphones in, world turned off.",
      tension:
        "The question I keep asking myself — does the music actually reach people, or is it just algorithm numbers on a screen?",
      resolution:
        "Then I read your comments. And I know — it's reaching people. That's all that matters.",
  ...
  behind_scenes: [
    {
      ...
      tension:
        "We scrapped three full versions. At one point we had a complete track that sounded amazing — and I deleted it because it wasn't honest enough.",
      resolution:
        "The version you're hearing now came from starting over. And I'm glad it did.",
...
class AdvancedSocialAIService {
  ...
  private generateHook(
    ...
    const genreHookAddons: Record<string, string[]> = {
      "hip-hop": [
        `Bar for bar, ${topic} is the one 🎤`,
        `Lyricism is not dead. ${topic} is the proof 🔥`,
        `The wordplay on ${topic} goes three layers deep 💯`,
      ],
      trap: [
        `The 808 on ${topic} hits at max volume 🔊`,
        `Built this from scratch — no shortcuts 🔥`,
        `When the 808 drops, you feel it in your chest ⚡`,
      ],
      "r&b": [
        `This melody was a dream I woke up to ✨`,
        `${topic} — late night, mood lighting, repeat 💜`,
        `The harmonies on this bridge will break you ✨`,
      ],
      pop: [
        `${topic} is that song you add to every playlist 🎵`,
        `One play and ${topic} is stuck in your head all week 🎵`,
        `The hook was designed to get in your head 🎯`,
      ],
      afrobeats: [
        `The groove on ${topic} doesn't stop 🔥`,
        `You cannot listen to ${topic} and not move your body 🎶`,
        `Lagos energy built for the world 🔥`,
      ],
      electronic: [
        `The drop on ${topic} was engineered to break speakers ⚡`,
        `Four to the floor and a bassline that won't quit 🎧`,
        `Built for peak hour, hits just as hard at home 🔊`,
      ],
      drill: [
        `The samples on ${topic} are dark and cinematic 🖤`,
        `Authentic drill energy. No imitation 🔥`,
        `Every line is a fact. Listen carefully 💯`,
      ],
      country: [
        `Wrote this on a porch in the middle of nowhere 🎸`,
        `Three chords and the truth — ${topic} ✨`,
    ...
    const hookSeed = seededIndex(
      `${request.artistName}:${request.topic}:${contentType}:hook`,
      templates.length,
    );
    ...
    if (
      ...
      seededGate(
        `${request.artistName}:${request.topic}:${contentType}:curiosity`,
        0.3,
      )
    ) {
    ...
    const platformDNA =
      PLATFORM_NATIVE_DNA[request.platforms[0]?.toLowerCase()] || null;
    if (
      platformDNA &&
      seededGate(
        `${request.artistName}:${request.topic}:${contentType}:native`,
        0.25,
      )
    ) {
      const nativeOpener =
        platformDNA.openers[
          seededIndex(
            `${request.artistName}:${request.topic}:${contentType}:opener`,
            platformDNA.openers.length,
          )
        ];
      // Only prepend if hook doesn't already start with a platform-native opener
      const alreadyNative = platformDNA.openers.some((o) =>
        hook.toLowerCase().startsWith(o.toLowerCase().substring(0, 6)),
      );
  ...
  private generateBody(
    ...
    if (mbContext.isMaxBoosterTopic && mbContext.relevantFeature) {
      const feature =
        MAX_BOOSTER_PLATFORM_KNOWLEDGE.coreFeatures[mbContext.relevantFeature];
      ...
      const contentType = request.contentType || "announcement";
      const bodyVariantList =
        contentBodyVariants[contentType] || contentBodyVariants.announcement;
      const selectedBodyVariant =
        bodyVariantList[
          seededIndex(
            `${request.artistName}:${topic}:${contentType}:body`,
            bodyVariantList.length,
          )
        ];
    ...
    if (tone.emotionality > 0.7) {
      ...
      bodyParts.push(
        emotionalClosers[
          seededIndex(
            `${request.artistName}:${topic}:emotional`,
            emotionalClosers.length,
          )
        ],
      );
    }
    ...
    if (request.objective === "conversions") {
      ...
      bodyParts.push(
        urgencyLines[
          seededIndex(
            `${request.artistName}:${topic}:urgency`,
            urgencyLines.length,
          )
        ],
      );
    ...
    const targetLength =
      audience.preferredLength === "short"
        ? 100
        : audience.preferredLength === "long"
          ? 400
          : 200;
  ...
  private generateCTA(
    ...
    const ctas =
      ctasByObjective[request.objective] || ctasByObjective.engagement;
    let cta =
      ctas[
        seededIndex(
          `${request.artistName}:${request.topic}:${request.objective}:cta`,
          ctas.length,
        )
      ];
