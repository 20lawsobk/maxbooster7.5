...
import { industryMonitor, type LiveIndustryChange } from './industryMonitorService.js';
...
const PRODUCTION_KW: KW<string>[] = [
  ...
  { pattern: /\bhigh[.\s-]?fidelity\b|\bhi[.\s-]?fi\b/i, value: 'hi-fi', weight: 1.5 },
];
...
const PLATFORM_ENTRIES: PlatformEntry[] = [
  ...
  },
  {
    ...
    trendSignals: [
      { pattern: /\batmos\b|\bspatial\b/i,  trend: 'Spatial Audio / Atmos push' },
...
class MusicIndustryContextFilterService {
  ...
  async getContextForMode(mode: GenerationMode): Promise<MusicIndustryContext> {
    try {
      ...
      logger.warn('[IndustryFilter] Context unavailable — returning empty context:', (err as Error).message);
      return this.empty();
    }
  }
  ...
  private build(signals: LiveIndustryChange[]): MusicIndustryContext {
    ...
    for (const sig of signals) {
      ...
      const boost = sig.urgency === 'critical' ? 1.4
                  : sig.urgency === 'high'     ? 1.2
                  : sig.urgency === 'medium'   ? 1.0
                  : 0.8;

      for (const kw of GENRE_KW) {
        if (kw.pattern.test(text)) {
          genreScores.set(kw.value, (genreScores.get(kw.value) ?? 0) + kw.weight * age * boost);
        }
      }
      for (const kw of MOOD_KW) {
        if (kw.pattern.test(text)) {
          moodScores.set(kw.value, (moodScores.get(kw.value) ?? 0) + kw.weight * age * boost);
        }
      }
      for (const kw of PRODUCTION_KW) {
        if (kw.pattern.test(text)) {
          prodScores.set(kw.value, (prodScores.get(kw.value) ?? 0) + kw.weight * age * boost);
        }
      }
    ...
    for (const [platform, trends] of platMap) {
      ...
      for (const trend of trends) {
        ...
        platformSignals.push({ platform, trend, strength: strength as PlatformTrendSignal['strength'] });
        i++;
      }
    }

    const confidence = Math.min(1, signals.length / 40);
    const hints = this.buildHints(trendingGenres, trendingMoods, productionStyles, platformSignals, viralHookPatterns);
  ...
  private applyMode(ctx: MusicIndustryContext, mode: GenerationMode): MusicIndustryContext {
    ...
    switch (mode) {
      ...
      case 'advertising':
        ...
        if (ctx.platformSignals.length) {
          const top = ctx.platformSignals.slice(0,3)
      ...
      case 'music':
        ...
        if (ctx.trendingGenres.length)
          parts.push(`Trending genres: ${ctx.trendingGenres.slice(0,3).join(', ')}`);
        if (ctx.trendingMoods.length)
          parts.push(`Resonant moods: ${ctx.trendingMoods.slice(0,3).join(', ')}`);
        if (ctx.productionStyles.length)
          parts.push(`Production: ${ctx.productionStyles.slice(0,2).join(', ')}`);
      ...
      case 'songwriting':
        ...
        if (ctx.lyricThemes.length)
          parts.push(`Resonant themes: ${ctx.lyricThemes.slice(0,3).join(', ')}`);
        if (ctx.trendingGenres.length)
          parts.push(`Trending genres: ${ctx.trendingGenres.slice(0,3).join(', ')}`);
        if (ctx.viralHookPatterns.length)
          parts.push(`Hook patterns: ${ctx.viralHookPatterns.slice(0,2).join(', ')}`);
        if (ctx.trendingMoods.length)
          parts.push(`Audience mood: ${ctx.trendingMoods.slice(0,2).join(', ')}`);
        break;
    }
  ...
  private empty(): MusicIndustryContext {
    return {
      ...
      viralHookPatterns: [],
      lyricThemes:      [],
      generationHints:  { tempoBias: 'neutral', productionKeywords: [], contentAngles: [], hashtagContext: '' },
      contextString:    '',
      signalCount:      0,
      confidence:       0,
      freshness:        new Date(),
...
export const musicIndustryContextFilter = new MusicIndustryContextFilterService();
