...
function buildReleaseCaption(
  ctx: ArtistContext,
  stage: "out_now" | "pre_save",
): string {
  const _base =
    stage === "out_now"
  ...
  const _body = `${ctx?.mood.charAt(0).toUpperCase() + ctx?.mood.slice(1)} ${ctx?.genre} energy — this one hits different.\n\n`;

  const _links = ctx?.streamingLinks
...
export function generateArtistContent(
  ...
  const _generator = VERTICAL_GENERATORS[vertical];
...
export function generateAllArtistContent(
  ...
  const activeVerticals: ArtistContentVertical[] = [
    "fan_engagement",
    "brand_story",
    "catalog_discovery",
  ];
  ...
  if (ctx?.releaseDate) {
    const _releaseTime = new Date(ctx?.releaseDate).getTime();
    const _now = Date?.now();
    if (releaseTime > now) {
  ...
  // Deduplicate
  const _deduped = [...new Set(activeVerticals)];

  for (const vertical of deduped) {
    for (const platform of platforms) {
      pieces?.push({
        ...generateArtistContent(ctx, vertical, platform),
        source: "MaxCoreAI",
      });
    }
  }
