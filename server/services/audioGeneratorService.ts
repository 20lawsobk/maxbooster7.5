...
async function tryGenerateTTS(text: string, maxDur: number): Promise<string | null> {
  const outPath = path.join(os.tmpdir(), `tts_${randomBytes(4).toString('hex')}.wav`);
  const clean   = text.replace(/['"\\]/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 500);

  const espeakArgs = ['-v', 'en', '-s', '150', '-p', '45', '-a', '160', '-w', outPath, clean];
  ...
  try {
    await execFileAsync(FFMPEG, [
      '-y', '-f', 'lavfi',
      '-i', `flite=text='${fliteText}':voice=kal`,
      '-t', String(maxDur),
      '-ar', '44100', '-ac', '2',
      outPath,
    ], { timeout: 20_000 });
...
export async function generateAudio(opts: AudioGenOptions = {}): Promise<AudioGenResult> {
  ...
  const ttsText = [opts.text, opts.topic, opts.artistName].filter(Boolean).join('. ');
  ...
  const build = (withVoice: boolean) => [
    '-y',
    ...inputs,
    '-filter_complex', buildFilter(withVoice),
    '-map', '[afinal]',
  ...
  logger.info(`[AudioGen] ✅ ${filename} — ${genre} | ${duration}s | TTS: ${!!voPath}`);
  return { success: true, url: `/uploads/audio/${filename}`, filename, durationSec: duration };
}
