...
> = {
  ...
  newsletter:     { content_type: "newsletter",    platform_category: "news"     },
  lyrics:         { content_type: "lyrics",        platform_category: "music"    },
...
export async function analyzeUrl(url: string): Promise<UrlAnalysis> {
  ...
  return {
    ...
    // author: article/newsletter author, or music artist as fallback
    author: (parsed.isMusic ? parsed.artist : parsed.author) ?? parsed.artist ?? "",
    published: parsed.datePublished ?? "",
    modified: parsed.dateModified ?? "",
    og_image: image,
    thumbnail_url: image,
    canonical: parsed.canonical ?? parsed.finalUrl,
    ...
    track_count: parsed.trackCount ?? undefined,
    ...
    view_count: parsed.viewCount ?? null,
    like_count: null,
    comment_count: null,
    play_count: null,
    share_count: null,
    ...
    reading_time_minutes: parsed.wordCount ? Math.ceil(parsed.wordCount / 238) : null,
...
export function urlToContentSeed(a: UrlAnalysis) {
  ...
  return {
    ...
    track_count: a.track_count ?? undefined,
    members: a.members || [],
    ...
    ticket_url: "",           // parsed.ticketUrl not yet in UrlAnalysis; safe default
    // Product-specific
    price: a.price || "",
    currency: a.currency || "",
    brand: a.brand || "",
    ...
    apple_music_type: a.apple_music_type || "",
