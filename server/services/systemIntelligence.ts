...
interface InferenceRule {
  ...
  explain(signals: Signals): Omit<ErrorUnderstanding, 'errorClass' | 'confidence' | 'enrichedAt'>;
}
...
interface SecurityRule {
  ...
  explain(indicators: string[], threatLevel: number, requestCount: number): Omit<SecurityUnderstanding, 'threatIntent' | 'confidence' | 'enrichedAt'>;
}
...
const INFERENCE_RULES: InferenceRule[] = [
  ...
  // ── Rule 1: PDIM Cold-Start Cascade ────────────────────────────────────────
  {
    ...
    matches: (s) => s.pdim5xxCount > 0 && !s.pdimHadFirstSuccess && s.uptimeMs < 120_000,
    confidence: (s) => {
      let c = 0.70;
      if (s.uptimeMs < 60_000) c += 0.15;
      if (!s.pdimCircuitOpen) c += 0.10; // slow-lane, not open = expected
    ...
    explain: (s) => ({
      ...
      severity: 'negligible',
      expectedResolution: 'PDIM will complete initialisation within 3–8 minutes of the first request. The circuit-breaker slow-lane absorbs failures silently. All queued operations will resume automatically.',
      estimatedResolutionMs: Math.max(0, 480_000 - s.uptimeMs),
      recommendedActions: [{
        priority: 'low',
        action: 'Monitor PDIM first-success log line',
        rationale: 'Once "PDIM first success" appears the cascade resolves on its own.',
        automated: true,
      }],
      correlatedSignals: [
        `${s.pdim5xxCount} PDIM HTTP 5xx in window`,
        `Uptime: ${Math.round(s.uptimeMs / 1000)}s (within startup grace)`,
        ...(s.seedingFailCount > 0 ? [`${s.seedingFailCount} seeding deferrals`] : []),
        ...(s.luaSaturationCount > 0 ? [`LuaExecutor saturation ×${s.luaSaturationCount}`] : []),
      ],
      isRoutineNoise: true,
  ...
  // ── Rule 2: PDIM Slow-Lane Pressure ────────────────────────────────────────
  {
    ...
    matches: (s) => s.pdim5xxCount > 0 && !s.pdimCircuitOpen && s.uptimeMs >= 120_000 && s.uptimeMs < 780_000,
    confidence: (s) => {
      let c = 0.65;
      if (s.luaSaturationCount > 0) c += 0.15;
      if (s.seedingFailCount > 0) c += 0.10;
    ...
    explain: (s) => ({
      ...
      severity: 'low',
      expectedResolution: 'Will fully resolve when PDIM\'s first successful response triggers the "warm" transition. Typically 3–6 minutes after the grace window ends.',
      estimatedResolutionMs: Math.max(0, 780_000 - s.uptimeMs),
      recommendedActions: [{
        priority: 'low',
        action: 'No action required',
        rationale: 'Circuit-breaker slow-lane is designed for exactly this scenario. ChainFixer monitors LuaExecutor saturation automatically.',
        automated: true,
      }],
      correlatedSignals: [
        `${s.pdim5xxCount} PDIM 5xx absorbed by slow-lane`,
        `Uptime: ${Math.round(s.uptimeMs / 1000)}s (slow-lane window)`,
        ...(s.luaSaturationCount > 0 ? [`LuaExecutor saturation ×${s.luaSaturationCount} (max queue: ${s.luaMaxQueued})`] : []),
        ...(s.chainFixerFires > 0 ? [`ChainFixer fired ×${s.chainFixerFires}: ${s.chainFixerPatterns.join(', ')}`] : []),
      ],
      isRoutineNoise: true,
      requiresHumanAttention: false,
    }),
  ...
  // ── Rule 3: PDIM Genuine Outage ────────────────────────────────────────────
  {
    ...
    matches: (s) => s.pdim5xxCount > 5 && s.pdimCircuitOpen && s.uptimeMs > 600_000,
    confidence: (s) => {
      let c = 0.60;
      if (s.uptimeMs > 900_000) c += 0.15;  // well past warm-up
      if (s.pdim5xxCount > 20) c += 0.10;
      if (s.incidentCount > 0) c += 0.10;
    ...
    explain: (s) => ({
      ...
      impact: 'All BullMQ job processing paused; real-time features relying on PDIM (WebSocket pub-sub, rate limiting, session store) are degraded. The system is operating in graceful-degradation mode.',
      severity: 'high',
      expectedResolution: 'The circuit breaker probes PDIM every 60–120 seconds and will auto-recover when PDIM responds. No code change needed.',
      estimatedResolutionMs: null,
      recommendedActions: [
        {
          priority: 'high',
          action: 'Check pocketdimensionstorage.replit.app deployment status',
          rationale: 'PDIM is a separate Replit deployment; it may have been stopped, is restarting, or has an error.',
          automated: false,
        },
        {
          priority: 'medium',
          action: 'Monitor circuit breaker HALF-OPEN probe results',
          rationale: 'The circuit automatically probes for recovery; watch for "Circuit CLOSED" in logs.',
          automated: true,
        },
      ],
      correlatedSignals: [
        ...
        ...(s.incidentCount > 0 ? [`${s.incidentCount} open platform incident(s): ${s.incidentTitles.slice(0, 2).join(', ')}`] : []),
      ],
  ...
  // ── Rule 4: LuaExecutor Saturation ─────────────────────────────────────────
  {
    ...
    confidence: (s) => {
      let c = 0.75;
      if (s.pdim5xxCount > 0) c += 0.10; // common concurrent cause
      if (s.luaMaxQueued >= 5) c += 0.10;
      return Math.min(c, 0.92);
    },
    explain: (s) => ({
      ...
      impact: 'BullMQ job polling is delayed; stale-job detection runs late; some lock timeouts may occur causing BullMQ to re-queue jobs (no data loss).',
      severity: s.luaMaxQueued >= 5 ? 'medium' : 'low',
      expectedResolution: 'ChainFixer proactively resets the semaphore when saturation is detected. Queue drains automatically as PDIM warms up and response times improve.',
      estimatedResolutionMs: s.pdimHadFirstSuccess ? 60_000 : null,
      recommendedActions: [{
        priority: 'low',
        action: 'ChainFixer OFFENSIVE mode is already handling this',
        rationale: 'The auto-fixer detects "approaching saturation" and resets slots pre-emptively before timeout cascades.',
        automated: true,
      }],
      correlatedSignals: [
        `LuaExecutor saturation ×${s.luaSaturationCount} (peak queue: ${s.luaMaxQueued})`,
        ...(s.pdim5xxCount > 0 ? [`${s.pdim5xxCount} PDIM 5xx (common trigger)`] : []),
        ...(s.bullmqLockCount > 0 ? [`${s.bullmqLockCount} BullMQ lock race(s) (downstream effect)`] : []),
      ],
  ...
  // ── Rule 5: BullMQ Lock Race ────────────────────────────────────────────────
  {
    ...
    confidence: (s) => {
      let c = 0.80;
      if (s.luaSaturationCount > 0) c += 0.10;
    ...
    explain: (s) => ({
      ...
      impact: 'Some jobs may run twice (at-least-once delivery). If job processors are idempotent, there is no data loss. Job processing resumes on re-queue.',
      severity: 'low',
      expectedResolution: 'Resolves automatically as PDIM warms up and LuaExecutor latency drops below the lock timeout.',
      estimatedResolutionMs: s.uptimeMs < 480_000 ? Math.max(0, 480_000 - s.uptimeMs) : 120_000,
      recommendedActions: [{
        priority: 'low',
        action: 'Verify job processors are idempotent',
        rationale: 'BullMQ re-queues stalled jobs; if processors are not idempotent, duplicate processing could cause data inconsistency.',
        automated: false,
      }],
      correlatedSignals: [
        `${s.bullmqLockCount} lock race(s) detected`,
        ...(s.luaSaturationCount > 0 ? ['LuaExecutor saturation (root cause)'] : []),
      ],
      isRoutineNoise: true,
      requiresHumanAttention: false,
    }),
  ...
  // ── Rule 6: Memory Pressure ─────────────────────────────────────────────────
  {
    ...
    explain: (s) => ({
      ...
      severity: s.memHeapPct > 92 ? 'high' : s.memHeapPct > 85 ? 'medium' : 'low',
      expectedResolution: 'Platform auto-fixer runs GC and trims internal buffers. If heap stays above 85% for > 10 minutes, investigate for memory leaks.',
      estimatedResolutionMs: null,
      recommendedActions: [
        {
          priority: s.memHeapPct > 90 ? 'high' : 'medium',
          action: 'Monitor heap trend over next 5 minutes',
          rationale: 'If heap is growing linearly (not sawtooth from GC), a leak is likely.',
          automated: true,
        },
        ...(s.memHeapPct > 90 ? [{
          priority: 'high' as const,
          action: 'Check for TensorFlow model weights held in memory',
          rationale: 'AI model weights (audio analysis, advertising) are large and may accumulate if not freed after inference.',
          automated: false,
        }] : []),
      ],
      correlatedSignals: [
        `Heap: ${s.memHeapPct}%`,
        ...(s.memPressureEvents > 0 ? [`${s.memPressureEvents} heap warning event(s) in window`] : []),
      ],
  ...
  // ── Rule 7: Database Slow ───────────────────────────────────────────────────
  {
    ...
    confidence: (s) => Math.min(0.70 + (s.dbSlowCount * 0.03), 0.90),
    explain: (s) => ({
      ...
      impact: 'API endpoints that read from the database are slower than normal. HyperLearning analytics queries may time out and return empty results.',
      severity: s.dbSlowCount > 10 ? 'medium' : 'low',
      expectedResolution: 'Neon PostgreSQL cold-starts self-resolve within 10–30 seconds. Persistent slowness requires query analysis.',
      estimatedResolutionMs: 30_000,
      recommendedActions: [
        {
          priority: 'medium',
          action: 'Check for long-running transactions in PostgreSQL',
          rationale: 'A stuck transaction can block the entire table and cause cascading slowness.',
          automated: false,
        },
      ],
  ...
  // ── Rule 8: Self-Healing Active ─────────────────────────────────────────────
  {
    ...
    matches: (s) => s.chainFixerFires > 0 && s.pdim5xxCount === 0 && !s.pdimCircuitOpen,
    confidence: (_s) => 0.90,
    explain: (s) => ({
      ...
      impact: 'The affected subsystem(s) were corrected without service interruption.',
      severity: 'negligible',
      expectedResolution: 'Already resolved — auto-fixer has applied the patch.',
      estimatedResolutionMs: 0,
      recommendedActions: [{
        priority: 'low',
        action: 'Review ChainFixer history if patterns repeat > 10 times',
        rationale: 'High repetition of the same fix suggests a deeper root cause that may warrant a permanent code-level fix.',
        automated: true,
      }],
      correlatedSignals: [`ChainFixer: ${s.chainFixerPatterns.join(', ')} (×${s.chainFixerFires})`],
      isRoutineNoise: true,
      requiresHumanAttention: false,
    }),
  },
  ...
  // ── Rule 9: Unknown Pattern ─────────────────────────────────────────────────
  {
    ...
    explain: (s) => ({
      ...
      impact: 'Unknown — the blast radius cannot be assessed without more context.',
      severity: 'medium',
      expectedResolution: 'Manual investigation required. If the pattern repeats, add a ChainFixer rule with an appropriate auto-fix.',
      estimatedResolutionMs: null,
      recommendedActions: [
        {
          priority: 'medium',
          action: 'Review novel error messages and determine if they are actionable',
          rationale: 'Novel errors that repeat may indicate a regression or new failure mode.',
          automated: false,
        },
      ],
...
const SECURITY_RULES: SecurityRule[] = [
  {
    ...
    indicators: [/auth fail/i, /login fail/i, /invalid.*password/i, /password.*invalid/i],
    explain: (indicators, threatLevel, requestCount) => ({
      intentLabel: 'Automated credential stuffing — testing stolen username/password pairs at scale.',
      attackStage: 'delivery',
      sophistication: requestCount > 100 ? 'automated' : 'semi-targeted',
      sophisticationLabel: requestCount > 100
      ...
      predictedNextMove: 'Will iterate through more username/password variants. If not blocked, may pivot to password-reset flow or OAuth.',
      falsePositiveLikelihood: threatLevel < 0.7 ? 0.20 : 0.05,
      ...
      countermeasures: [
        { priority: 'immediate', action: 'IP blocked by self-healing engine', rationale: 'Automated block on high threat score.', automated: true },
        { priority: 'medium', action: 'Enable CAPTCHA on login endpoint', rationale: 'Increases cost for automated attacks.', automated: false },
      ],
    }),
  },
  {
    ...
    indicators: [/admin/i, /\.env/i, /phpinfo/i, /config/i, /wp-/i, /\.git/i, /backup/i],
    explain: (indicators, threatLevel, requestCount) => ({
      intentLabel: 'Attack surface reconnaissance — mapping endpoints, technologies, and vulnerabilities.',
      attackStage: 'reconnaissance',
      sophistication: 'automated',
      sophisticationLabel: 'Automated scanner probing for common web vulnerabilities and misconfigurations.',
      predictedNextMove: 'Having mapped the attack surface, attacker will likely pivot to exploitation of discovered endpoints or attempt injection attacks.',
      falsePositiveLikelihood: 0.10,
      ...
      countermeasures: [
        { priority: 'immediate', action: 'Rate limiting applied by self-healing engine', rationale: 'Slows scan rate below useful threshold.', automated: true },
        { priority: 'low', action: 'Consider Cloudflare bot-fight mode', rationale: 'Blocks scanner fingerprints at the edge.', automated: false },
      ],
    }),
  },
  {
    ...
    indicators: [/union.*select/i, /drop.*table/i, /or.*1=1/i, /sleep\(/i, /benchmark\(/i],
    explain: (indicators, threatLevel, _requestCount) => ({
      intentLabel: 'SQL injection — attempting to extract data or execute database commands.',
      attackStage: 'exploitation',
      sophistication: indicators.some(i => /sleep|benchmark|waitfor/i.test(i.source)) ? 'targeted' : 'automated',
      sophisticationLabel: indicators.some(i => /sleep|benchmark|waitfor/i.test(i.source))
      ...
      predictedNextMove: 'If injection is possible, attacker will enumerate tables, extract user credentials, and potentially attempt privilege escalation.',
      ...
      countermeasures: [
        { priority: 'immediate', action: 'Request blocked and IP flagged by self-healing engine', rationale: 'SQL injection is always blocked at the WAF layer.', automated: true },
        { priority: 'low', action: 'Verify all raw SQL usage uses $1/$2 parameterisation', rationale: 'ORM does not protect against raw sql`...` calls with interpolation.', automated: false },
      ],
    }),
  },
  {
    ...
    explain: (_indicators, threatLevel, _requestCount) => ({
      intentLabel: 'Cross-site scripting — attempting to inject client-side code into responses.',
      attackStage: 'exploitation',
      sophistication: 'automated',
      sophisticationLabel: 'Automated XSS probe — likely a scanner testing for reflected or stored XSS.',
      predictedNextMove: 'If successful, would attempt to steal session cookies, redirect users, or deliver malware. CSP headers prevent execution.',
      ...
      countermeasures: [
        { priority: 'immediate', action: 'Payload blocked by security middleware', rationale: 'Input sanitisation and CSP provide defence-in-depth.', automated: true },
        { priority: 'low', action: 'Verify CSP does not include unsafe-inline in script-src', rationale: 'Strict CSP is the primary XSS mitigation.', automated: false },
      ],
    }),
  },
  {
    ...
    explain: (_indicators, threatLevel, _requestCount) => ({
      intentLabel: 'Path traversal — attempting to read files outside the web root.',
      attackStage: 'exploitation',
      sophistication: 'automated',
      sophisticationLabel: 'Automated scanner testing for directory traversal vulnerabilities.',
      predictedNextMove: 'If successful, would attempt to read /etc/passwd, .env files, or application secrets.',
      ...
      countermeasures: [
        { priority: 'immediate', action: 'Request blocked by security middleware', rationale: 'Path normalisation prevents traversal.', automated: true },
      ],
    }),
  },
  {
    ...
    explain: (_indicators, threatLevel, requestCount) => ({
      intentLabel: 'Volumetric denial of service — flooding with requests to exhaust server capacity.',
      attackStage: 'exploitation',
      ...
      predictedNextMove: 'Will likely continue to increase request rate or rotate IPs if current IP is blocked.',
      ...
      countermeasures: [
        { priority: 'immediate', action: 'Rate limiting active — 429 responses applied', rationale: 'Distributed rate limiter absorbs excess load.', automated: true },
        { priority: 'medium', action: 'Consider Cloudflare DDoS protection at edge', rationale: 'Edge-level protection reduces load before it reaches the origin.', automated: false },
      ],
    }),
  },
  {
    ...
    indicators: [],
    explain: (_indicators, threatLevel, _requestCount) => ({
      intentLabel: 'Unclassified threat — does not match known attack pattern taxonomy.',
      attackStage: 'reconnaissance',
      sophistication: 'semi-targeted',
      sophisticationLabel: 'Behaviour is anomalous but does not fit known attack signatures.',
      ...
      countermeasures: [
        { priority: 'low', action: 'Monitor source IP for escalating behaviour', rationale: 'Unclassified threats require observation before hard blocking.', automated: true },
      ],
    }),
  },
];
...
class SystemIntelligenceEngine extends EventEmitter {
  ...
  private recentUnderstandings: Array<ErrorUnderstanding & { triggeredBy: string }> = [];
  private recentSecurityUnderstandings: Array<SecurityUnderstanding & { ip: string; threatId: string }> = [];
  private insights: Insight[] = [];
  private _startedAt: number = Date.now();
  private _firstPdimSuccessAt: number = 0;
  private _chainFixerFires: Array<{ ts: number; patternId: string; patternName: string }> = [];
  ...
  initialize(): void {
    ...
    this._subscribeToSystems().catch(() => { /* non-fatal */ });

    // Periodic insight refresh
    const insightTimer = setInterval(() => this._refreshInsights(), 60_000);
    insightTimer.unref();

    logger.info('[SystemIntelligence] Reasoning engine initialized — watching all error and security signals');
  }
  ...
  private async _subscribeToSystems(): Promise<void> {
    ...
    } catch { /* non-fatal — cbIsOpen defaults to false */ }

    try {
      const { chainErrorAutoFixer } = await import('./chainErrorAutoFixer.js');
      chainErrorAutoFixer.on('fixed', (ev: { patternId: string; attempt: number }) => {
        this._chainFixerFires.push({ ts: Date.now(), patternId: ev.patternId, patternName: ev.patternId });
    ...
    try {
      ...
      platformAutoFixer.on('incident:opened', (inc: { severity: string; title: string }) => {
        this._addInsight({
          id: `incident_${Date.now()}`,
          priority: inc.severity as Insight['priority'],
          title: `Platform Incident: ${inc.title}`,
          detail: 'The platform auto-fixer has opened a new incident. Check the Incidents dashboard for details.',
          action: 'Review incident details and applied patches in the Platform Fixer admin panel.',
          automated: true,
          since: Date.now(),
        });
      });
    ...
    try {
      const { selfHealingEngine } = await import('./selfHealingSecurityEngine.js');
      selfHealingEngine.on('threat_detected', (assessment: {
        id: string; threatType: string; threatLevel: number; confidence: number;
        indicators: string[]; detectionTime: number;
      }) => {
        this._threatCount++;
        ...
        if (this.recentSecurityUnderstandings.length > 100) this.recentSecurityUnderstandings.pop();

        if (understanding.falsePositiveLikelihood < 0.30 && assessment.threatLevel > 0.70) {
          this._addInsight({
            id: `threat_${assessment.id}`,
            priority: assessment.threatLevel > 0.90 ? 'critical' : 'high',
            title: `Security: ${understanding.intentLabel.split('—')[0].trim()}`,
            detail: understanding.reasoning,
            action: understanding.countermeasures[0]?.action ?? 'Review threat assessment.',
  ...
  private _ingestLogEntry(entry: LogEntry): void {
    ...
    if (this._firstPdimSuccessAt === 0 &&
      (entry.message.includes('PDIM first success') || entry.message.includes('[PDIM] Circuit CLOSED'))) {
  ...
  private _extractSignals(): Signals {
    ...
    for (const ev of this.eventWindow) {
      ...
      if (/LuaExecutor busy/i.test(m)) {
        ...
        if (match) luaMaxQueued = Math.max(luaMaxQueued, parseInt(match[1], 10));
      }
      ...
      if (/slow.*query|query.*timeout|db.*timeout|connection.*timeout.*pg/i.test(m)) dbSlowCount++;
    ...
    const chainFixerFires = this._chainFixerFires.filter(f => f.ts > windowCutoff);
  ...
  analyzeCurrentState(): ErrorUnderstanding[] {
    ...
    const SEV_ORDER = { critical: 5, high: 4, medium: 3, low: 2, negligible: 1 };
    results.sort((a, b) => {
  ...
  private _classifySecurityThreat(assessment: {
    ...
    for (const rule of SECURITY_RULES) {
      const typeMatch = rule.threatTypes.some(t =>
        assessment.threatType.toLowerCase().includes(t) || t.includes(assessment.threatType.toLowerCase())
      );
      ...
      if (typeMatch || indicatorMatch) {
        return {
          ...
          enrichedAt: Date.now(),
          ...rule.explain(indicators.map(i => i), assessment.threatLevel, requestCount),
        };
      }
    }
  ...
  narrateSystemState(): SystemNarrative {
    ...
    else if (signals.pdim5xxCount > 0) healthScore -= Math.min(15, signals.pdim5xxCount);
    ...
    const recentPdim = this.eventWindow.filter(e =>
      e.ts > Date.now() - 2 * 60_000 && /PDIM HTTP 5/i.test(e.message)
    ).length;
    const olderPdim = this.eventWindow.filter(e =>
      e.ts <= Date.now() - 2 * 60_000 && e.ts > Date.now() - 5 * 60_000 &&
    ...
    if (systemPhase === 'cold_starting') {
      headline = 'System is starting up — initial PDIM errors are expected and being absorbed.';
    } else if (systemPhase === 'warming') {
      headline = 'System is warming up — PDIM slow-lane is absorbing expected failures.';
    ...
    const actionableCount = understandings.filter(u => u.requiresHumanAttention).length;
    ...
    if (noiseCount > 0) {
      bodyParts.push(
        ...
        `these are expected patterns (${understandings.filter(u => u.isRoutineNoise).map(u => u.errorClass.replace(/_/g, ' ')).join(', ')}).`
      );
    }
    ...
    if (actionableCount > 0) {
      bodyParts.push(
        ...
        understandings.filter(u => u.requiresHumanAttention).map(u => u.what).join('; ')
      );
    } else if (systemPhase === 'operational') {
      bodyParts.push('No immediate action is required. Self-healing systems are operating normally.');
    }
    ...
    if (signals.pdim5xxCount > 0 && !signals.pdimCircuitOpen) {
      keyInsights.push(`PDIM: ${signals.pdim5xxCount} 5xx errors absorbed (circuit closed — slow-lane handling).`);
    }
    if (signals.pdimCircuitOpen) {
      keyInsights.push('PDIM circuit breaker is OPEN — queued operations paused.');
    }
    if (signals.luaMaxQueued > 0) {
      keyInsights.push(`LuaExecutor peak queue depth: ${signals.luaMaxQueued} (auto-reset by ChainFixer).`);
    }
    if (signals.memHeapPct > 75) {
      keyInsights.push(`Heap at ${signals.memHeapPct}% of limit — ${signals.memHeapPct > 90 ? 'GC forced' : 'monitoring'}.`);
    }
    if (signals.seedingFailCount > 0) {
      keyInsights.push(`${signals.seedingFailCount} AI data seed(s) deferred — will retry when PDIM is warm.`);
    }
    if (this._threatCount > 0) {
      keyInsights.push(`Security: ${this._threatCount} threat(s) detected, ${this._healedThreats} auto-healed.`);
    }
    if (keyInsights.length === 0) keyInsights.push('All subsystems operating within normal parameters.');

    // Next expected event
    let nextExpectedEvent: string;
    if (systemPhase === 'cold_starting') {
      nextExpectedEvent = 'PDIM first-success response will transition system to slow-lane phase.';
    } else if (systemPhase === 'warming') {
      nextExpectedEvent = 'PDIM will complete warm-up and circuit will transition to CLOSED-stable.';
    } else if (signals.pdimCircuitOpen) {
      nextExpectedEvent = 'Circuit breaker will probe PDIM in the next 60–120 seconds.';
    } else {
      nextExpectedEvent = 'Next HyperLearning cycle in ~5 minutes; MaxCore weight sync in ~10 minutes.';
    }
  ...
  private _addInsight(insight: Insight): void {
    ...
    if (existing !== -1) { this.insights[existing] = insight; return; }
    ...
    for (const u of understandings) {
      if (u.requiresHumanAttention && !u.isRoutineNoise) {
        this._addInsight({
          ...
          priority: u.severity === 'critical' ? 'critical'
            : u.severity === 'high' ? 'high'
            : u.severity === 'medium' ? 'medium' : 'low',
          title: u.errorClass.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase()),
          detail: u.what + ' ' + u.why,
