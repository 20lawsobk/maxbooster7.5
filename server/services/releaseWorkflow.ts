...
export class ReleaseWorkflowService {
  ...
  async requestTakedown(
    ...
    try {
      ...
      if (!["live", "updated", "update_pending"].includes(currentState)) {
        return {
          success: false,
          error: `Cannot takedown release in "${currentState}" state. Must be live.`,
        };
      }
  ...
  async requestUpdate(
    ...
    try {
      ...
      if (!["live", "updated"].includes(currentState)) {
        return {
          success: false,
          error: `Cannot update release in "${currentState}" state. Must be live.`,
        };
      }
  ...
  createChangeRecord(
    ...
    oldValue: Record<string, unknown>,
    newValue: Record<string, unknown>,
    changeType: ReleaseChange["changeType"],
  ): ReleaseChange {
    return {
      field,
      oldValue,
  ...
  diffReleases(
    oldRelease: Record<string, any>,
    newRelease: Record<string, any>,
  ): ReleaseChange[] {
