...
async function getStoredToken(userId: string, platform: string): Promise<string | null> {
  try {
    ...
    if (parts.length === 3) {
      ...
      const key = Buffer.from((process.env.TOKEN_ENCRYPTION_KEY || '').substring(0, 32).padEnd(32, '0'));
...
export class SocialService {
  ...
  async publishPost(
    ...
    const [post] = await db
      ...
      .where(and(eq(socialCampaigns.userId, userId), eq(socialCampaigns.id, postId)))
      .limit(1);
    ...
    if (!token) {
      throw new Error(`No active OAuth token for platform ${platform} — please reconnect in Settings`);
    }
    ...
    try {
      ...
      if (platform === 'twitter' || platform === 'x') {
        ...
        if (!res.ok) {
          ...
          throw new Error(`Twitter publish failed (${res.status}): ${err.slice(0, 200)}`);
        }
        ...
        const res = await timedFetch(`https://graph.facebook.com/v18.0/${pageId}/feed`, {
          method: 'POST',
        ...
        if (!res.ok) {
          const err = await res.text();
          throw new Error(`Facebook publish failed (${res.status}): ${err.slice(0, 200)}`);
        }
        ...
        const createRes = await timedFetch(`https://graph.instagram.com/v18.0/${igId}/media`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (createRes.ok) {
          ...
          if (container.id) {
            await timedFetch(`https://graph.instagram.com/v18.0/${igId}/media_publish`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
        ...
        const res = await timedFetch('https://open.tiktokapis.com/v2/post/publish/content/init/', {
          method: 'POST',
          headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json; charset=UTF-8' },
          body: JSON.stringify({
            post_info: { title: content.slice(0, 150), privacy_level: 'PUBLIC_TO_EVERYONE', disable_comment: false },
            source_info: { source: 'PULL_FROM_URL', video_url: mediaUrls[0] || '' },
          }),
        });
        if (!res.ok) {
          const err = await res.text();
          logger.warn(`TikTok publish failed (${res.status}): ${err.slice(0, 200)}`);
        }

      } else if (platform === 'linkedin') {
        const personIdRes = await timedFetch('https://api.linkedin.com/v2/userinfo', {
        ...
        logger.warn(`publishPost: unsupported platform ${platform} — post queued but not dispatched`);
      }
  ...
  async trackMetrics(postId: string): Promise<{
    ...
    try {
      const [post] = await db
        .select({ engagement: socialCampaigns.engagement, platform: socialCampaigns.platform, userId: socialCampaigns.userId })
        .from(socialCampaigns)
  ...
  async getEngagementAnalytics(userId: string): Promise<{
    ...
    try {
      ...
      const posts = await db
        .select({ engagement: socialCampaigns.engagement, platform: socialCampaigns.platform })
        .from(socialCampaigns)
        .where(
          and(
      ...
      return { totalFollowers: 0, totalReach: 0, engagementRate: 0, topPosts: [] };
    }
  }
  ...
  async getCampaignMetrics(
    ...
    platforms: Record<string, { reach: number; engagement: number; posts: number }>;
    timeline: unknown[];
  }> {
    try {
      ...
      const startDate = campaign.startDate ? new Date(campaign.startDate) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
      const endDate   = campaign.endDate   ? new Date(campaign.endDate)   : new Date();

      const posts = await db
        .select({ engagement: socialCampaigns.engagement, platform: socialCampaigns.platform, publishedAt: socialCampaigns.publishedAt })
        .from(socialCampaigns)
        .where(
          and(
            eq(socialCampaigns.userId, userId),
      ...
      const platforms: Record<string, { reach: number; engagement: number; posts: number }> = {};
      ...
      for (const post of posts) {
        ...
        if (!platforms[platform]) platforms[platform] = { reach: 0, engagement: 0, posts: 0 };
        platforms[platform].reach      += reach;
        platforms[platform].engagement += engSum;
        platforms[platform].posts++;

        const dateKey = post.publishedAt ? new Date(post.publishedAt).toISOString().split('T')[0] : 'unknown';
