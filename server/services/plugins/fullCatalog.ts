import { BUILT_IN_INSTRUMENTS, BUILT_IN_EFFECTS, type PluginDefinition } from './definitions.js';
...
const builtInInstruments = BUILT_IN_INSTRUMENTS.filter((p: PluginDefinition) => !mbIds.has(p.id));
const builtInEffects = BUILT_IN_EFFECTS.filter((p: PluginDefinition) => !mbIds.has(p.id));
...
export const ALL_PLUGINS: PluginDefinition[] = [...EXPANDED_INSTRUMENTS, ...EXPANDED_EFFECTS];
