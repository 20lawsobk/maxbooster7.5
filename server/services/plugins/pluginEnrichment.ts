...
const f = (
  ...
  unit?: string,
  step?: number,
): PluginParameter => ({ id, name, type: 'float', defaultValue, minValue, maxValue, unit, step, automatable: true });
...
const i = (
  ...
  maxValue: number,
  unit?: string,
): PluginParameter => ({ id, name, type: 'int', defaultValue, minValue, maxValue, unit, automatable: false });

const b = (id: string, name: string, defaultValue: boolean): PluginParameter =>
  ({ id, name, type: 'bool', defaultValue, automatable: false });
...
const c = (
  ...
  defaultValue: string,
  choices: string[],
): PluginParameter => ({ id, name, type: 'choice', defaultValue, choices, automatable: false });
...
const eqBands = (count: number): PluginParameter[] => {
  ...
  for (let n = 1; n <= count; n++) {
    ...
    out.push(c(`band${n}Type`, `Band ${n} Type`, n === 1 ? 'high-pass' : n === count ? 'low-pass' : 'bell',
      ['high-pass', 'low-shelf', 'bell', 'notch', 'high-shelf', 'low-pass', 'band-pass', 'tilt']));
    out.push(f(`band${n}Freq`, `Band ${n} Freq`, [80, 250, 800, 2500, 5000, 8000, 12000, 16000][n - 1] || 1000, 20, 22000, 'Hz'));
...
const mbcBands = (count: number, xovers: number[]): PluginParameter[] => {
  const out: PluginParameter[] = [];
  for (let n = 1; n <= count; n++) {
    if (n < count) out.push(f(`xover${n}`, `Crossover ${n}`, xovers[n - 1] || 1000, 20, 20000, 'Hz'));
...
const REFERENCE: Partial<Record<EffectType | InstrumentType | string, ParamFactory>> = {
  // ---------------- EQs ----------------
  'eq': () => [
    ...eqBands(8),
    f('inputGain', 'Input Gain', 0, -24, 24, 'dB'),
    f('outputGain', 'Output Gain', 0, -24, 24, 'dB'),
    c('processing', 'Processing', 'zero-latency', ['zero-latency', 'natural-phase', 'linear-phase']),
    c('channel', 'Channel Mode', 'stereo', ['stereo', 'mid-side', 'left-right']),
  ...
  'mastering': () => [
    ...
    f('inputGain', 'Input Gain', 0, -24, 24, 'dB'),
    f('outputGain', 'Output Gain', 0, -24, 24, 'dB'),
    c('processing', 'Processing', 'linear-phase', ['zero-latency', 'natural-phase', 'linear-phase']),
    c('channel', 'Channel Mode', 'mid-side', ['stereo', 'mid-side', 'left-right']),
  ],
  ...
  'compressor': () => [
    ...
    c('character', 'Character', 'clean', ['clean', 'vca', 'fet', 'opto', 'tube', 'vintage']),
  ...
  'limiter': () => [
    ...
    c('character', 'Character', 'transparent', ['transparent', 'modern', 'aggressive', 'vintage', 'allround']),
    b('truePeak', 'True Peak', true),
    b('isr', 'Inter-Sample Detection', true),
    b('dithering', 'Dither', false),
    c('ditherType', 'Dither Type', 'tpdf', ['tpdf', 'rectangular', 'noise-shaped']),
  ...
  'maximizer': () => [
    ...
    c('algorithm', 'Algorithm', 'IRC-III', ['IRC-I', 'IRC-II', 'IRC-III', 'IRC-IV', 'IRC-LL']),
    b('truePeak', 'True Peak', true),
    f('lookahead', 'Lookahead', 5, 0, 30, 'ms'),
  ],
  ...
  'de-esser': () => [
    ...
    c('character', 'Character', 'modern', ['modern', 'vintage', 'allround', 'female', 'male']),
  ],
  ...
  'transient-shaper': () => [
    ...
    c('detection', 'Detection', 'broadband', ['broadband', 'low', 'mid', 'high']),
  ...
  'reverb': () => [
    ...
    c('algorithm', 'Algorithm', 'hall', ['room', 'hall', 'plate', 'spring', 'chamber', 'shimmer', 'ambient', 'reverse']),
  ],
  'plate': () => [
  ...
  'delay': () => [
    ...
    c('divisionL', 'Left Division', '1/8', ['1/64', '1/32', '1/16T', '1/16', '1/16D', '1/8T', '1/8', '1/8D', '1/4T', '1/4', '1/4D', '1/2', '1']),
    c('divisionR', 'Right Division', '1/4', ['1/64', '1/32', '1/16T', '1/16', '1/16D', '1/8T', '1/8', '1/8D', '1/4T', '1/4', '1/4D', '1/2', '1']),
    f('feedback', 'Feedback', 35, 0, 110, '%'),
    f('crossFeedback', 'Cross Feedback', 0, 0, 100, '%'),
    c('mode', 'Mode', 'digital', ['digital', 'analog', 'tape', 'lo-fi', 'bbd', 'reverse', 'pitch']),
  ...
  'chorus': () => [
    ...
    f('lowCut', 'Low Cut', 100, 20, 1000, 'Hz'),
    f('highCut', 'High Cut', 8000, 1000, 22000, 'Hz'),
    c('mode', 'Mode', 'classic', ['classic', 'vintage', 'ensemble', 'dimension', 'tri-chorus']),
    f('mix', 'Mix', 50, 0, 100, '%'),
  ],
  'phaser': () => [
    f('rate', 'Rate', 0.5, 0.01, 20, 'Hz'),
  ...
  'tremolo': () => [
    ...
    f('depth', 'Depth', 50, 0, 100, '%'),
    c('shape', 'LFO Shape', 'sine', ['sine', 'triangle', 'square', 'saw', 'ramp']),
  ...
  'auto-pan': () => [
    ...
    c('shape', 'LFO Shape', 'sine', ['sine', 'triangle', 'square', 'saw', 'random']),
  ...
  'distortion': () => [
    ...
    f('lowCut', 'Low Cut', 80, 20, 1000, 'Hz'),
    f('highCut', 'High Cut', 8000, 1000, 22000, 'Hz'),
    c('algorithm', 'Algorithm', 'tube', ['tube', 'tape', 'transistor', 'fuzz', 'overdrive', 'fold', 'rectifier', 'bit']),
    f('mix', 'Mix', 100, 0, 100, '%'),
  ],
  'tube': () => [
    f('drive', 'Drive', 30, 0, 100, '%'),
    ...
    f('tone', 'Tone', 50, 0, 100, '%'),
    f('output', 'Output', 0, -24, 24, 'dB'),
    c('tubeType', 'Tube Type', '12AX7', ['12AX7', 'EL34', 'EL84', '6L6', 'KT88']),
    f('mix', 'Mix', 100, 0, 100, '%'),
  ],
  'tape': () => [
  ...
  'waveshape': () => [
    ...
    c('shape', 'Shape', 'tanh', ['tanh', 'sin', 'fold', 'bit', 'sigmoid', 'asymmetric']),
  ...
  'filter': () => [
    ...
    c('mode', 'Mode', 'low-pass', ['low-pass', 'high-pass', 'band-pass', 'notch', 'all-pass']),
  ...
  'auto-tune': () => [
    c('key', 'Key', 'C', ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B']),
    c('scale', 'Scale', 'major', ['chromatic', 'major', 'minor', 'harmonic-minor', 'melodic-minor', 'pentatonic', 'blues']),
  ...
  // ---------------- Microphones ----------------
  'microphone': () => [
    c('model', 'Mic Model', 'large-condenser', ['large-condenser', 'small-condenser', 'dynamic', 'ribbon', 'tube']),
    c('pattern', 'Polar Pattern', 'cardioid', ['omni', 'cardioid', 'super-cardioid', 'figure-8', 'wide-cardioid']),
  ...
  'restore': () => [
    ...
    f('frequency', 'Frequency', 1000, 20, 22000, 'Hz'),
    f('q', 'Q', 1, 0.1, 100),
    c('mode', 'Mode', 'denoise', ['denoise', 'declick', 'dehum', 'decrackle', 'dereverb']),
    b('listen', 'Listen', false),
    f('mix', 'Mix', 100, 0, 100, '%'),
  ],
};
...
const fillForAll = (base: Preset, perGenre: Partial<Record<Genre, Preset>>): GenrePresets => {
...
const GENRE_PRESETS: Partial<Record<string, GenrePresets>> = {
  ...
  compressor: fillForAll(
    { threshold: -18, ratio: 4, attack: 10, release: 100, knee: 6, makeup: 0, mix: 100, character: 'clean', detection: 'peak' },
    {
      'hip-hop':              { threshold: -14, ratio: 3,    attack: 8,   release: 80,  character: 'vca',     mix: 100 },
      'trap':                 { threshold: -10, ratio: 6,    attack: 3,   release: 60,  character: 'fet',     mix: 100 },
      'lofi':                 { threshold: -16, ratio: 3,    attack: 20,  release: 200, character: 'opto',    mix: 80,  scHpf: 80 },
      'rnb':                  { threshold: -16, ratio: 3,    attack: 15,  release: 150, character: 'opto',    mix: 100 },
      'pop':                  { threshold: -14, ratio: 4,    attack: 5,   release: 100, character: 'fet',     mix: 100 },
      'rock':                 { threshold: -12, ratio: 4,    attack: 8,   release: 80,  character: 'vca',     mix: 100 },
      'metal':                { threshold: -10, ratio: 6,    attack: 3,   release: 50,  character: 'fet',     mix: 100, scHpf: 100 },
      'indie':                { threshold: -16, ratio: 3,    attack: 12,  release: 120, character: 'vca',     mix: 100 },
      'country':              { threshold: -18, ratio: 3,    attack: 15,  release: 150, character: 'opto',    mix: 100 },
      'jazz':                 { threshold: -20, ratio: 2,    attack: 25,  release: 250, character: 'tube',    mix: 80 },
      'funk-soul':            { threshold: -14, ratio: 4,    attack: 5,   release: 80,  character: 'vca',     mix: 100 },
      'reggae':               { threshold: -16, ratio: 3,    attack: 12,  release: 180, character: 'opto',    mix: 100 },
      'latin':                { threshold: -14, ratio: 4,    attack: 8,   release: 100, character: 'vca',     mix: 100 },
      'afrobeats':            { threshold: -14, ratio: 4,    attack: 6,   release: 80,  character: 'vca',     mix: 100 },
      'edm-house':            { threshold: -10, ratio: 6,    attack: 1,   release: 50,  character: 'clean',   mix: 100, scHpf: 80 },
      'techno':               { threshold: -12, ratio: 6,    attack: 1,   release: 40,  character: 'clean',   mix: 100, scHpf: 100 },
      'dnb':                  { threshold: -8,  ratio: 8,    attack: 0.5, release: 30,  character: 'fet',     mix: 100, scHpf: 100 },
      'dubstep':              { threshold: -8,  ratio: 8,    attack: 0.3, release: 30,  character: 'fet',     mix: 100, scHpf: 120 },
      'ambient-cinematic':    { threshold: -22, ratio: 2,    attack: 30,  release: 400, character: 'opto',    mix: 60 },
      'classical-orchestral': { threshold: -24, ratio: 1.5,  attack: 30,  release: 500, character: 'tube',    mix: 40 },
    },
  ),

  // ---- Limiter / Maximizer (master-bus aware) ----
  limiter: fillForAll(
    { threshold: -1, ceiling: -0.3, release: 50, lookahead: 5, character: 'transparent', truePeak: true },
    {
      'hip-hop':              { threshold: -6, ceiling: -0.8, release: 30, character: 'aggressive' },
      'trap':                 { threshold: -8, ceiling: -1.0, release: 20, character: 'aggressive' },
      ...
      'metal':                { threshold: -8, ceiling: -1.0, release: 30, character: 'aggressive' },
      'indie':                { threshold: -4, ceiling: -1.0, release: 60, character: 'vintage' },
      'country':              { threshold: -3, ceiling: -1.0, release: 80, character: 'vintage' },
      'jazz':                 { threshold: -2, ceiling: -1.0, release: 100, character: 'transparent' },
      'funk-soul':            { threshold: -4, ceiling: -1.0, release: 50, character: 'vintage' },
      'reggae':               { threshold: -4, ceiling: -1.0, release: 70, character: 'vintage' },
      'latin':                { threshold: -5, ceiling: -1.0, release: 50, character: 'modern' },
      'afrobeats':            { threshold: -6, ceiling: -1.0, release: 40, character: 'modern' },
      'edm-house':            { threshold: -8, ceiling: -1.0, release: 20, character: 'modern' },
      'techno':               { threshold: -7, ceiling: -1.0, release: 25, character: 'modern' },
      'dnb':                  { threshold: -8, ceiling: -1.0, release: 15, character: 'aggressive' },
      'dubstep':              { threshold: -9, ceiling: -1.0, release: 15, character: 'aggressive' },
      'ambient-cinematic':    { threshold: -1, ceiling: -1.0, release: 200, character: 'transparent' },
      'classical-orchestral': { threshold: -1, ceiling: -1.0, release: 300, character: 'transparent' },
    },
  ),

  // ---- Parametric EQ — minimal per-genre tilt (band1 HPF, band 5 air boost, band 6 presence cut/boost) ----
  eq: fillForAll(
    { band1On: true, band1Type: 'high-pass', band1Freq: 30, band1Gain: 0, band1Q: 0.7,
      band5On: true, band5Type: 'high-shelf', band5Freq: 12000, band5Gain: 0,
      processing: 'zero-latency', channel: 'stereo' },
    {
      'hip-hop':              { band1Freq: 30,  band5Gain: 2,   band3On: true, band3Type: 'bell', band3Freq: 200, band3Gain: -2, band3Q: 1.2 },
      'trap':                 { band1Freq: 25,  band5Gain: 3,   band3On: true, band3Type: 'bell', band3Freq: 250, band3Gain: -3, band3Q: 1.5 },
      'lofi':                 { band1Freq: 80,  band5Gain: -4,  band5Freq: 8000 },
      'rnb':                  { band1Freq: 35,  band5Gain: 2.5, band4On: true, band4Type: 'bell', band4Freq: 3000, band4Gain: 1 },
      'pop':                  { band1Freq: 40,  band5Gain: 3,   band4On: true, band4Type: 'bell', band4Freq: 4000, band4Gain: 2 },
      'rock':                 { band1Freq: 60,  band5Gain: 2,   band3On: true, band3Type: 'bell', band3Freq: 500, band3Gain: -2 },
      'metal':                { band1Freq: 80,  band5Gain: 3,   band3On: true, band3Type: 'bell', band3Freq: 400, band3Gain: -4, band3Q: 2 },
      'indie':                { band1Freq: 50,  band5Gain: 1 },
      'country':              { band1Freq: 60,  band5Gain: 2,   band4On: true, band4Type: 'bell', band4Freq: 5000, band4Gain: 1.5 },
      'jazz':                 { band1Freq: 30,  band5Gain: 1 },
      'funk-soul':            { band1Freq: 40,  band5Gain: 2,   band4On: true, band4Type: 'bell', band4Freq: 3500, band4Gain: 1.5 },
      'reggae':               { band1Freq: 30,  band5Gain: 0,   band3On: true, band3Type: 'bell', band3Freq: 100, band3Gain: 2 },
      'latin':                { band1Freq: 50,  band5Gain: 2 },
      'afrobeats':            { band1Freq: 40,  band5Gain: 3 },
      'edm-house':            { band1Freq: 30,  band5Gain: 3,   band3On: true, band3Type: 'bell', band3Freq: 300, band3Gain: -2 },
      'techno':               { band1Freq: 30,  band5Gain: 2,   band3On: true, band3Type: 'bell', band3Freq: 400, band3Gain: -2 },
      'dnb':                  { band1Freq: 30,  band5Gain: 3,   band3On: true, band3Type: 'bell', band3Freq: 250, band3Gain: -3 },
      'dubstep':              { band1Freq: 25,  band5Gain: 3,   band3On: true, band3Type: 'bell', band3Freq: 250, band3Gain: -3 },
  ...
  // ---- Reverb ----
  reverb: fillForAll(
    { predelay: 20, size: 50, decay: 2.0, density: 70, lowDamp: 50, highDamp: 50, lowCut: 80, highCut: 12000, mix: 25, algorithm: 'hall' },
    {
      'hip-hop':              { algorithm: 'plate',  decay: 1.6, predelay: 25, mix: 15 },
      'trap':                 { algorithm: 'plate',  decay: 1.2, predelay: 20, mix: 10 },
      'lofi':                 { algorithm: 'spring', decay: 1.4, predelay: 10, mix: 30, highDamp: 70 },
      ...
      'ambient-cinematic':    { algorithm: 'shimmer',decay: 8.0, predelay: 40, mix: 45 },
      'classical-orchestral': { algorithm: 'hall',   decay: 3.2, predelay: 35, mix: 28 },
    },
  ),

  // ---- Delay ----
  delay: fillForAll(
    { syncL: true, syncR: true, divisionL: '1/8', divisionR: '1/4', feedback: 35, mode: 'digital', mix: 25 },
    {
      'hip-hop':              { divisionL: '1/8',  divisionR: '1/4',  feedback: 30, mode: 'analog', mix: 18 },
      'trap':                 { divisionL: '1/16', divisionR: '1/8',  feedback: 25, mode: 'digital', mix: 12 },
      'lofi':                 { divisionL: '1/8',  divisionR: '1/4',  feedback: 40, mode: 'tape',    mix: 30, wow: 25, flutter: 20 },
      'rnb':                  { divisionL: '1/8',  divisionR: '1/4',  feedback: 40, mode: 'analog',  mix: 20 },
      'pop':                  { divisionL: '1/8D', divisionR: '1/4',  feedback: 35, mode: 'digital', mix: 18 },
      'rock':                 { divisionL: '1/8D', divisionR: '1/4D', feedback: 40, mode: 'analog',  mix: 25 },
      'metal':                { divisionL: '1/16', divisionR: '1/8',  feedback: 25, mode: 'digital', mix: 15 },
      'indie':                { divisionL: '1/8D', divisionR: '1/4',  feedback: 45, mode: 'tape',    mix: 30 },
      'country':              { divisionL: '1/8',  divisionR: '1/4',  feedback: 30, mode: 'analog',  mix: 18 },
      'jazz':                 { divisionL: '1/8',  divisionR: '1/4',  feedback: 25, mode: 'tape',    mix: 12 },
      'funk-soul':            { divisionL: '1/16', divisionR: '1/8',  feedback: 30, mode: 'analog',  mix: 18 },
      'reggae':               { divisionL: '1/8D', divisionR: '1/4D', feedback: 60, mode: 'tape',    mix: 35, wow: 30, flutter: 20 },
      'latin':                { divisionL: '1/8',  divisionR: '1/4',  feedback: 30, mode: 'digital', mix: 18 },
      'afrobeats':            { divisionL: '1/16', divisionR: '1/8',  feedback: 30, mode: 'digital', mix: 18 },
      'edm-house':            { divisionL: '1/8D', divisionR: '1/4',  feedback: 45, mode: 'digital', mix: 25 },
      'techno':               { divisionL: '1/16D',divisionR: '1/8D', feedback: 55, mode: 'digital', mix: 25 },
      'dnb':                  { divisionL: '1/16', divisionR: '1/8',  feedback: 35, mode: 'digital', mix: 20 },
      'dubstep':              { divisionL: '1/8',  divisionR: '1/4',  feedback: 40, mode: 'digital', mix: 18 },
      'ambient-cinematic':    { divisionL: '1/4D', divisionR: '1/2',  feedback: 65, mode: 'tape',    mix: 40 },
      'classical-orchestral': { divisionL: '1/4',  divisionR: '1/2',  feedback: 20, mode: 'digital', mix: 10 },
    },
  ),

  // ---- Gate ----
  gate: fillForAll(
    { threshold: -40, range: -60, attack: 1, hold: 10, release: 100, hysteresis: 6 },
    {
      'hip-hop':              { threshold: -32, range: -40, attack: 0.5, hold: 5,  release: 60,  scHpf: 100 },
      'trap':                 { threshold: -28, range: -50, attack: 0.3, hold: 3,  release: 40,  scHpf: 150 },
      'rock':                 { threshold: -38, range: -50, attack: 1,   hold: 8,  release: 120 },
      'metal':                { threshold: -30, range: -60, attack: 0.3, hold: 5,  release: 80, scHpf: 100 },
      'edm-house':            { threshold: -36, range: -50, attack: 0.5, hold: 5,  release: 60 },
      'techno':               { threshold: -36, range: -50, attack: 0.5, hold: 5,  release: 50 },
      'dnb':                  { threshold: -34, range: -60, attack: 0.2, hold: 3,  release: 40 },
      'dubstep':              { threshold: -34, range: -60, attack: 0.2, hold: 3,  release: 40 },
      'jazz':                 { threshold: -50, range: -30, attack: 2,   hold: 20, release: 200 },
      'classical-orchestral': { threshold: -55, range: -25, attack: 3,   hold: 30, release: 300 },
    },
  ),

  // ---- De-esser ----
  'de-esser': fillForAll(
    { threshold: -24, frequency: 6500, range: -8, q: 1.5, mode: 'split', character: 'modern' },
    {
      'hip-hop':              { frequency: 6000, range: -8,  mode: 'split',     character: 'modern' },
      ...
      'rock':                 { frequency: 7500, range: -6,  mode: 'wide-band', character: 'allround' },
      'metal':                { frequency: 8000, range: -10, mode: 'wide-band', character: 'allround' },
      'indie':                { frequency: 7000, range: -5,  mode: 'split',     character: 'vintage' },
      'country':              { frequency: 6800, range: -5,  mode: 'split',     character: 'female' },
  ...
  'transient-shaper': fillForAll(
    { attack: 0, sustain: 0, output: 0, mode: 'classic', detection: 'broadband' },
    {
  ...
  // ---- Chorus ----
  chorus: fillForAll(
    { rate: 0.5, depth: 30, delay: 15, feedback: 0, voices: 2, spread: 100, mode: 'classic', mix: 50 },
    {
  ...
  // ---- Tape ----
  tape: fillForAll(
    { drive: 30, bias: 50, wow: 10, flutter: 10, hiss: 0, crosstalk: 20, speed: '15ips', formula: 'modern', mix: 100 },
    {
      'lofi':                 { drive: 50, wow: 40, flutter: 35, hiss: 25, speed: '7.5ips', formula: 'vintage' },
      'hip-hop':              { drive: 30, wow: 15, flutter: 12, hiss: 10, speed: '15ips',  formula: 'vintage' },
      'rnb':                  { drive: 25, wow: 12, flutter: 10, hiss: 5,  speed: '15ips',  formula: 'modern' },
      'jazz':                 { drive: 20, wow: 8,  flutter: 8,  hiss: 5,  speed: '15ips',  formula: 'vintage' },
      'rock':                 { drive: 40, wow: 12, flutter: 10, hiss: 5,  speed: '15ips',  formula: 'modern' },
      'indie':                { drive: 45, wow: 25, flutter: 20, hiss: 15, speed: '7.5ips', formula: 'vintage' },
      'ambient-cinematic':    { drive: 20, wow: 15, flutter: 10, hiss: 8,  speed: '15ips',  formula: 'vintage' },
    },
  ),

  // ---- Stereo / imager ----
  stereo: fillForAll(
    { width: 100, balance: 0, midGain: 0, sideGain: 0, lowMonoFreq: 120, monoMaker: true, mode: 'mid-side' },
    {
      ...
      'classical-orchestral': { width: 100, sideGain: 0, lowMonoFreq: 60,  monoMaker: false },
  ...
  plate: fillForAll(
    { predelay: 10, decay: 1.8, size: 60, damping: 50, lowCut: 100, highCut: 10000, mix: 25 },
    {
  ...
  hall: fillForAll(
    { predelay: 40, decay: 2.8, size: 80, diffusion: 80, lowDamp: 30, highDamp: 60, lowCut: 80, highCut: 11000, mix: 25 },
    {
  ...
  spring: fillForAll(
    ...
    { 'reggae': { boing: 60, mix: 40 }, 'lofi': { boing: 40, mix: 35 }, 'rock': { boing: 35, mix: 30 } },
  ),
  shimmer: fillForAll(
    { decay: 4.0, size: 80, shimmer: 50, pitch1: 12, pitch2: 19, feedback: 40, mix: 30 },
    {
  ...
  filter: fillForAll(
    ...
    {
      'edm-house':            { cutoff: 2500, resonance: 0.6, mode: 'low-pass',  lfoRate: 0.5, lfoDepth: 50 },
      'techno':               { cutoff: 1800, resonance: 0.7, mode: 'low-pass',  lfoRate: 0.25, lfoDepth: 60 },
      'dnb':                  { cutoff: 3000, resonance: 0.6, mode: 'band-pass', lfoRate: 1, lfoDepth: 40 },
      'dubstep':              { cutoff: 1500, resonance: 0.8, mode: 'low-pass',  lfoRate: 0.5, lfoDepth: 70 },
      'funk-soul':            { cutoff: 2000, resonance: 0.6, mode: 'band-pass', envFollow: 60 },
...
export function enrichPlugin(plugin: PluginDefinition): PluginDefinition {
  ...
  const mergedDefault: Record<string, number | boolean | string> = { ...plugin.defaultPreset };
  ...
  let genrePresets: Record<string, Record<string, number | boolean | string>> | undefined;
  if (typePresets) {
...
export function enrichmentCoverage(plugins: PluginDefinition[]): {
  ...
  for (const p of plugins) {
    if (REFERENCE[p.type]) { typesEnriched.add(p.type); enriched++; }
    else typesPassThrough.add(p.type);
    if (p.genrePresets && Object.keys(p.genrePresets).length > 0) withGenrePresets++;
  }
  return {
