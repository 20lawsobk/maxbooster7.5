  ...
  process(
    ...
    for (let i = 0; i < input.samples[0].length; i++) {
      ...
      let leftSum = 0,
        rightSum = 0;
      for (let t = 0; t < 8; t++) {
        const tankOut = this.damping[t].process(
      ...
      const wetL = this.outputFilterL.process(leftSum * (1 + modAmount));
      const wetR = this.outputFilterR.process(rightSum * (1 - modAmount));
  ...
  process(
    ...
    for (let i = 0; i < input.samples[0].length; i++) {
      ...
      for (let e = 0; e < 12; e++) {
        ...
        const erSample = this.earlyReflections[e].read(
      ...
      let lateL = 0,
        lateR = 0;
      for (let c = 0; c < 8; c++) {
        const combOut = this.lpFilters[c].process(
...
export class RoomReverbProcessor implements DSPProcessor {
  ...
  process(
    ...
    const reflectionGains = [0.8, 0.7, 0.6, 0.5, 0.4, 0.3].map(
      (g) => g * (1 - absorption),
    );
  ...
  process(
    ...
    for (let i = 0; i < input.samples[0].length; i++) {
      ...
      for (let t = 0; t < 6; t++) {
        ...
        const rawTankOut = this.tanks[t].process(tankInput * 0.167);
        if (t % 2 === 0) {
          const tankOut = this.lpFilterL.process(rawTankOut);
          wetL += tankOut * (1 + mod);
        } else {
          const tankOut = this.lpFilterR.process(rawTankOut);
          wetR += tankOut * (1 - mod);
        }
      }
  ...
  process(
    ...
    for (let i = 0; i < input.samples[0].length; i++) {
      ...
      let wetL = 0,
        wetR = 0;
      for (let t = 0; t < 8; t++) {
        const tankOut = this.lpFilters[t].process(
  ...
  process(
    ...
    for (let i = 0; i < input.samples[0].length; i++) {
      ...
      for (let t = 0; t < 6; t++) {
        ...
        if (t % 2 === 0) {
          const colored = this.outputFilterL.process(tankOut);
          wetL += colored;
        } else {
          const colored = this.outputFilterR.process(tankOut);
          wetR += colored;
        }
      }
