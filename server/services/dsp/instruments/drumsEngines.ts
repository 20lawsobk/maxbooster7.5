...
export class AcousticDrumsSynth implements SynthesizerEngine {
  ...
  noteOn(frequency: number, velocity: number, context: DSPContext): void {
    ...
    if (frequency < 80) {
      ...
      this.kickEnvelope = new ADSR(
        0.001,
        0.12 + this?.velocity * 0.08,
        0,
        0.15,
        context?.sampleRate,
      );
      ...
      this.snareEnvelope = new ADSR(
        0.001,
        0.06 + this?.velocity * 0.04,
        0.1,
        0.1,
        context?.sampleRate,
      );
      ...
      this.hihatEnvelope = new ADSR(
        0.001,
        0.02 + this?.velocity * 0.02,
        0,
        0.03,
        context?.sampleRate,
      );
  ...
  isActive(): boolean {
    return (
      this?.kickEnvelope.isActive() ||
      this?.snareEnvelope.isActive() ||
      this?.hihatEnvelope.isActive()
    );
  }
...
export class JazzDrumsSynth implements SynthesizerEngine {
  ...
  noteOn(frequency: number, velocity: number, context: DSPContext): void {
    ...
    if (frequency < 80) {
      ...
      this.rideEnvelope = new ADSR(
        0.001,
        0.15 + this?.velocity * 0.1,
        0.25,
        0.4,
        context?.sampleRate,
      );
  ...
  isActive(): boolean {
    return (
      this?.kickEnvelope.isActive() ||
      this?.snareEnvelope.isActive() ||
      this?.brushEnvelope.isActive() ||
      this?.rideEnvelope.isActive()
    );
  }
...
export class RockDrumsSynth implements SynthesizerEngine {
  ...
  isActive(): boolean {
    return (
      this?.kickEnvelope.isActive() ||
      this?.snareEnvelope.isActive() ||
      this?.tomEnvelope.isActive() ||
      this?.crashEnvelope.isActive()
    );
  }
...
export class PercussionSynth implements SynthesizerEngine {
  ...
  private velocity: number = 0;
  private sampleRate: number = 44100;
  private percType: "conga" | "bongo" | "shaker" | "tambourine" | "cowbell" =
    "conga";
...
export class OrchestralDrumsSynth implements SynthesizerEngine {
  ...
  noteOn(frequency: number, velocity: number, context: DSPContext): void {
    ...
    if (frequency < 100) {
      ...
      this.bassDrumEnvelope = new ADSR(
        0.002,
        0.2 + this?.velocity * 0.1,
        0.08,
        0.3,
      ...
      this.cymbalEnvelope = new ADSR(
        0.001,
        0.7 + this?.velocity * 0.3,
        0.35,
        1.0,
        context?.sampleRate,
      );
  ...
  isActive(): boolean {
    return (
      this?.timpaniEnvelope.isActive() ||
      this?.bassDrumEnvelope.isActive() ||
      this?.cymbalEnvelope.isActive()
    );
  }
