...
const httpServer = http.createServer((req, res) => {
  ...
  const originalHost: string =
    tlsSocket.servername || (req.headers.host as string) || BACKEND_HOST;
...
export async function startProxy(): Promise<void> {
  ...
  tlsServer.on("tlsClientError", (err, _socket) => {
    ...
    if (
      !err.message.includes("no shared cipher") &&
  ...
  await Promise.all([
    ...
    new Promise<void>((resolve) => {
      ...
      httpRedirectServer.on("error", (err) => {
        ...
        console.warn(
          `[redirect] Could not bind :${HTTP_REDIRECT_PORT}: ${err.message}`,
        );
        resolve();
      });
    }),
    ...
    new Promise<void>((resolve, reject) => {
      tlsServer.listen(PROXY_PORT, PROXY_HOST, () => {
        ...
        console.log(
          `[tls]      Backend: ${BACKEND_USE_TLS ? "https" : "http"}://${BACKEND_HOST}:${BACKEND_PORT}`,
        );
        resolve();
      });
