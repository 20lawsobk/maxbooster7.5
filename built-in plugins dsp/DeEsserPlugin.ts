...
export class DeEsserPlugin extends BasePlugin {
  ...
  setBandwidth(bandwidth: number): void {
    ...
    this.bandPass.Q.setValueAtTime(
      this.bandShape === "narrow" ? q * 2 : q,
      currentTime,
    );
  }
  ...
  applyPreset(
    preset: "subtle" | "moderate" | "aggressive" | "broadcast",
  ): void {
    switch (preset) {
      case "subtle":
  ...
  setParameters(params: Record<string, any>): void {
    if (params.centerFrequency !== undefined)
      this.setCenterFrequency(params.centerFrequency);
    ...
    if (params.sMonEnabled !== undefined)
      this.setSMonEnabled(params.sMonEnabled);
