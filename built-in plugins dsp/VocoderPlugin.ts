...
export class VocoderPlugin extends BasePlugin {
    ...
    const updateEnvelopes = () => {
      ...
      for (let i = 0; i < this.numBands; i++) {
        ...
        if (average > currentValue) {
          this.bandEnvelopeValues[i] =
            currentValue +
            (average - currentValue) * (1 - Math.exp(-1 / (bandAttack * 60)));
        } else {
          this.bandEnvelopeValues[i] =
            currentValue +
            (average - currentValue) * (1 - Math.exp(-1 / (bandRelease * 60)));
        }
  ...
  setFormantShift(semitones: number): void {
    ...
    for (let i = 0; i < this.modulatorBandFilters.length; i++) {
      ...
      const shiftedFreq = Math.min(
        20000,
        Math.max(20, originalFreq * shiftRatio),
      );
      this.carrierBandFilters[i].frequency.setValueAtTime(
        shiftedFreq,
        this.context.currentTime,
      );
    }
  ...
  detectVoiced(): { voiced: number; unvoiced: number } {
    const lowBands = this.bandEnvelopeValues.slice(
      0,
      Math.floor(this.numBands / 3),
    );
    const highBands = this.bandEnvelopeValues.slice(
  ...
  setParameters(params: Record<string, any>): void {
    ...
    if (params.formantShift !== undefined)
      this.setFormantShift(params.formantShift);
    if (params.voicedMix !== undefined) this.setVoicedMix(params.voicedMix);
    if (params.unvoicedMix !== undefined)
      this.setUnvoicedMix(params.unvoicedMix);
    if (params.attack !== undefined) this.setAttack(params.attack);
    if (params.release !== undefined) this.setRelease(params.release);
    if (params.bandwidthFactor !== undefined)
      this.setBandwidthFactor(params.bandwidthFactor);
    if (params.carrierType !== undefined)
      this.setCarrierType(params.carrierType);
    if (params.oscillatorFrequency !== undefined)
      this.setOscillatorFrequency(params.oscillatorFrequency);
    if (params.bandLevels !== undefined)
      this.setAllBandLevels(params.bandLevels);
    if (params.bandAttacks !== undefined)
      this.setAllBandAttacks(params.bandAttacks);
    if (params.bandReleases !== undefined)
      this.setAllBandReleases(params.bandReleases);
