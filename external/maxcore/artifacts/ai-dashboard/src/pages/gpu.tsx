...
export default function GpuStatus() {
  ...
  const { data: silicon, isLoading: siliconLoading } =
    useGetSiliconGpuStatus();
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Live metrics from the digital, silicon SIMT, and hyper GPU
          backends.
        </p>
      </div>

      {/* Hyper GPU Card - primary compute path */}
        ...
        <Card className="glass-panel border-white/10 overflow-hidden">
          <div className="p-6 border-b border-white/5 bg-white/5 flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  RTL-Derived Lockstep Backend
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              {silicon?.is_maxcore_default && (
                <div className="px-2 py-1 rounded-full text-[10px] font-medium border bg-violet-500/10 text-violet-400 border-violet-500/20">
                  DEFAULT
                </div>
              )}
              <div
                className={`px-3 py-1 rounded-full text-xs font-medium border ${silicon?.available ? "bg-green-500/10 text-green-400 border-green-500/20" : "bg-red-500/10 text-red-400 border-red-500/20"}`}
              >
                {silicon?.available ? "Online" : "Offline"}
              </div>
            </div>
          </div>
          ...
          <div className="p-6 space-y-6">
            {siliconLoading ? (
              ...
              <>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="bg-black/30 p-4 rounded-xl">
                    ...
                    <p className="text-xl font-mono text-white mt-1">
                      {formatOps(silicon?.instructions_retired ?? 0)}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 pt-2 border-t border-white/5">
                  <div className="bg-black/20 p-3 rounded-xl">
                    ...
                    <p className="text-lg font-mono text-white mt-1">
                      {formatOps(silicon?.cycles_issued ?? 0)}
                    </p>
                  </div>
                  <div className="bg-black/20 p-3 rounded-xl">
                    <p className="text-xs text-muted-foreground">
                    ...
                    <p className="text-lg font-mono text-white mt-1">
                      {formatOps(silicon?.lanes_committed ?? 0)}
                    </p>
                  </div>
                </div>
              ...
              <div className="text-sm text-red-400/90 bg-red-500/5 border border-red-500/20 rounded-xl p-4">
                Engine unavailable
                {silicon?.engine_load_error || silicon?.error
                  ? `: ${silicon?.engine_load_error || silicon?.error}`
                  : "."}
              </div>
            )}
