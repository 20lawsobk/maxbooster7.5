...
class CohortAnalyticsService {
  ...
  async getCohortAnalysis(
    ...
    const initialSize = (cohort as any)?.initialSize || 1;
    const retention: RetentionData = {
      day1: (((cohort as any)?.day1Retained || 0) / initialSize) * 100,
      day7: (((cohort as any)?.day7Retained || 0) / initialSize) * 100,
      day30: (((cohort as any)?.day30Retained || 0) / initialSize) * 100,
      day90: (((cohort as any)?.day90Retained || 0) / initialSize) * 100,
    };
    ...
    return {
      cohortDate: (cohort as any).cohortDate,
      cohortLabel: this.formatCohortLabel((cohort as any)?.cohortDate),
      initialSize,
      retention,
      avgStreamsPerUser: (cohort as any).avgStreamsPerUser || 0,
      ltv: Number((cohort as any)?.ltv || 0),
      predictedChurn: (cohort as any).predictedChurn || 0,
      loyaltyDistribution,
    };
  }
  ...
  async getRetentionCurves(
    ...
    const cohortsWithCurves = cohorts?.map((cohort) => {
      ...
      return {
        cohortDate: (cohort as any).cohortDate,
        label: this.formatCohortLabel((cohort as any)?.cohortDate),
        curve: [
          { day: 0, retained: initialSize, percentage: 100 },
          {
            day: 1,
            retained: (cohort as any).day1Retained || 0,
            percentage: (((cohort as any)?.day1Retained || 0) / initialSize) * 100,
          },
          {
            day: 7,
            retained: (cohort as any).day7Retained || 0,
            percentage: (((cohort as any)?.day7Retained || 0) / initialSize) * 100,
          },
          {
            day: 30,
            retained: (cohort as any).day30Retained || 0,
            percentage: (((cohort as any)?.day30Retained || 0) / initialSize) * 100,
          },
          {
            day: 90,
            retained: (cohort as any).day90Retained || 0,
            percentage: (((cohort as any)?.day90Retained || 0) / initialSize) * 100,
          },
        ],
      };
    });
  ...
  async calculateLTV(
    ...
    return cohorts?.map((cohort) => {
      const totalRevenue = Number((cohort as any)?.totalRevenue || 0);
      const avgStreamsPerUser = (cohort as any)?.avgStreamsPerUser || 1;
      const initialSize = (cohort as any)?.initialSize || 1;
      const retention90 = (((cohort as any)?.day90Retained || 0) / initialSize) * 100;
  ...
  async predictChurn(
    ...
    const avgChurn =
      cohorts?.reduce((sum, c) => sum + ((c as any)?.predictedChurn || 0), 0) /
        cohorts?.length || 0;
    const totalListeners = cohorts?.reduce(
      (sum, c) => sum + ((c as any)?.initialSize || 0),
      0,
    );
    ...
    const predictions: ChurnPrediction[] = cohorts?.map((c) => {
      const churnProb = Math.min(
        0.99,
        Math.max(0.01, (c as any)?.predictedChurn || avgChurn),
      );
      ...
      const day7 = (c as any)?.day7Retained ?? (c as any)?.initialSize;
      const day30 = (c as any)?.day30Retained ?? (c as any)?.initialSize;
      const retentionDrop = day7 > 0 ? 1 - day30 / day7 : 0;
      const daysSinceLastStream = Math.round(retentionDrop * 45); // 0–45 days
      const totalStreams = (c as any)?.totalStreams ?? 0;
      const historicalEngagement =
        (c as any)?.initialSize > 0
          ? Math.round((totalStreams / (c as any)?.initialSize) * 10) / 10
          : 0;
      return {
        userId: (c as any).cohortMonth || `cohort_${c?.id}`,
  ...
  async getFanLoyaltyTiers(
    ...
    const avgRetention90 =
      cohorts?.reduce((sum, c) => {
        const init = (c as any)?.initialSize || 1;
        return sum + (((c as any)?.day90Retained || 0) / init) * 100;
      }, 0) / cohorts?.length || 0;
  ...
  async generateCohortReport(
    ...
    for (const cohort of cohorts) {
      const analysis = await this.getCohortAnalysis(userId, (cohort as any)?.cohortDate);
      if (analysis) {
        ...
        const monthLabel = this.formatMonthLabel((cohort as any)?.cohortDate);
        retentionTrend?.push({
          month: monthLabel,
