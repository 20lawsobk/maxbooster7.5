...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const pocket = await fabricStorage?.getPocket((req.params.pocketId as string));
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (pocket?.ownerId !== req.user!.id)
        return res.status(403).json({ error: "Forbidden" });
      const volumes = await fabricStorage?.listVolumes((req.params.pocketId as string));
      res.json(volumes);
    } catch (err) {
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (pocket?.ownerId !== req.user!.id)
        return res.status(403).json({ error: "Forbidden" });
      const objects = await fabricStorage?.listObjects((req.params.volumeId as string));
      res.json(objects);
    } catch (err) {
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const result = await fabricStorage?.getObject((req.params.objectId as string));
...
router.delete(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (pocket?.ownerId !== req.user!.id)
        return res.status(403).json({ error: "Forbidden" });
      await fabricStorage?.deleteObject((req.params.objectId as string));
      res.status(204).end();
    } catch (err) {
